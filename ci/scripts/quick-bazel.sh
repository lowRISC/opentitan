...
bazel query 'rdeps(//..., //hw:verilator)' | sed -e 's/^/-/' > bazel-results/slow_tests.txt

run_tests () {
    cat bazel-results/slow_tests.txt \
        | xargs bazel test --nobuild_tests_only --test_tag_filters=-broken,-cw310,-verilator -- //...
}

build_cw310_targets () {
    bazel build //... --build_tag_filters=cw310
}
