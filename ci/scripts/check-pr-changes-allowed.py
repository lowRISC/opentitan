...
def check_changes_against_blocklist(
    changes: list[str], blocklist: list[str]
) -> dict[str, list[str]]:
    ...
    for change in changes:
        ...
        blocked = list(
            filter(
                lambda x: x is not None, map(lambda b: b if fnmatch(change, b) else None, blocklist)
            )
        )
...
def fetch_pr_comments(gh_token: str, repo_name: str, pr_number: int) -> list[tuple[str, str]]:
...
def get_authorized_changes(
    comments: list[tuple[str, str]], committers: dict[str, str]
) -> dict[str, list[str]]:
...
def main() -> int:
    ...
    arg_parser.add_argument("--pr-number", type=int, help="ID of the PR requesting the change")

    arg_parser.add_argument(
        "--gh-token",
        help="""A github access token used to read PR comments
    ...
    if args.gh_token is not None and args.pr_number is not None:
        ...
        pr_comments = fetch_pr_comments(args.gh_token, args.gh_repo, args.pr_number)
        ...
        for change in block_changes_files:
            ...
            # authorized change
            if change in authorized_changes and len(authorized_changes[change]) >= NUM_AUTHS_REQD:
                # Change is authorized so delete it from the blocked_changes
                # dict
                del blocked_changes[change]

                authorizers = ", ".join(
                    [f"{committers[handle]} ({handle})" for handle in authorized_changes[change]]
                )
