...
def main():
    ...
    for p in os.scandir(hw_ip_dir):
        ...
        if "one_line_desc" not in data:
            log.warning(f"Skipping `{name}` because no `one_line_desc` found in its data file.")
            continue
    ...
    with open(hw_ip_dir / "README.md", "r") as f:
        ...
        replace_section_in_this_text(
            "Summary Table", table.to_markdown(shortcut_ref_link=True).rstrip()
        )
        replace_section_in_this_text(
            "README Link Reference Definitions", table.readme_link_references_markdown()
        )
...
class HwSummaryTable:
    ...
    def to_markdown(self, shortcut_ref_link: bool) -> str:
        ...
        data = [
            [e.name_linked_to_readme_markdown(shortcut_ref_link), e.one_line_desc]
...
def replace_section_in_text(text: str, start_marker: str, end_marker: str, new_section: str) -> str:
    ...
    assert start_pos <= end_pos, (
        "text seems to contain end_marker but not start_marker, which makes this function undefined"
    )
...
class TestReplaceSectionInText(unittest.TestCase):
    def test_simple(self):
        self.assertEqual(
            replace_section_in_text("beforeSTARTfooENDafter", "START", "END", "bar"),
            "beforeSTARTbarENDafter",
        )

    def test_neither_end_nor_start(self):
        self.assertEqual(
            replace_section_in_text("before", "START", "END", "bar"), "beforeSTARTbarEND"
        )

    def test_no_end(self):
        self.assertEqual(
            replace_section_in_text("beforeSTARTfoo", "START", "END", "bar"), "beforeSTARTbarEND"
        )
