...
class RegBlock:
    ...
    @staticmethod
    def build_blocks(
        block: "RegBlock", raw: object, bus: BusInterfaces, clocks: Clocking, is_alias: bool
    ) -> Dict[Optional[str], "RegBlock"]:
        ...
        if isinstance(raw, list):
            ...
            block.add_raw_registers(
                raw, "registers field at top-level", clocks, bus.device_async.get(None), is_alias
            )
            block.validate()
            return {None: block}
        ...
        if not isinstance(raw, dict):
            raise ValueError("registers field at top-level is neither a list or a dictionary.")
        ...
        for idx, (r_key, r_val) in enumerate(raw.items()):
            ...
            rb_key = check_str(
                r_key,
                "the key for item {} of the registers dictionary at top-level".format(idx + 1),
            )
            rb_val = check_list(
                r_val,
                "the value for item {} of the registers dictionary at top-level".format(idx + 1),
            )
    ...
    def add_raw_registers(
        self, raw: object, what: str, clocks: Clocking, async_if: Optional[str], is_alias: bool
    ) -> None:
    ...
    def add_raw(self, where: str, raw: object, clocks: Clocking, is_alias: bool) -> None:
            ...
            if t_body is not None:
                ...
                if len(entry) != 1:
                    ...
                    raise ValueError(
                        ...
                        "other keys too: {}.".format(self.offset, where, t, ", ".join(other_keys))
                    )
                entry_type = t
                entry_body = t_body

        entry_where = "At offset {:#x}, {}, type {!r}".format(self.offset, where, entry_type)
    ...
    def _handle_register(self, where: str, body: object, clocks: Clocking, is_alias: bool) -> None:
        reg = Register.from_raw(self._reg_width, self.offset, self._params, body, clocks, is_alias)
    ...
    def _handle_reserved(
        self, where: str, body: object, clocks: Optional[Clocking], is_alias: bool
    ) -> None:
        if is_alias:
        ...
        if nreserved <= 0:
            raise ValueError(
                "Reserved count in {} is {}, which is not positive.".format(where, nreserved)
            )
    ...
    def _handle_skipto(
        ...
        if is_alias:
            raise ValueError("The skipto command is not supported in alias register definitions")

        skipto = check_int(body, "body of " + where)
        if skipto < self.offset:
            raise ValueError(
                "Destination of skipto in {} is {:#x}, "
                "is less than the current offset, {:#x}.".format(where, skipto, self.offset)
            )
        if skipto % self._addrsep:
            raise ValueError(
                "Destination of skipto in {} is {:#x}, not a multiple of addrsep, {:#x}.".format(
    ...
    def _handle_multireg(self, where: str, body: object, clocks: Clocking, is_alias: bool) -> None:
        mr = MultiRegister(
            self.offset, self._addrsep, self._reg_width, self._params, body, clocks, is_alias
        )
        ...
        for reg in mr.regs:
            ...
            if lname in self.name_to_offset:
                raise ValueError(
                    ...
                    "offset {:#x}.".format(
                        mr.reg.name, mr.reg.offset, reg.name, reg.offset, self.name_to_offset[lname]
                    )
                )
    ...
    def validate(self) -> None:
        ...
        for wenname in self.wennames:
            ...
            if re.fullmatch(r"(.+_)*REGWEN(_[0-9]+)?", wenname) is None:
                raise ValueError("Regwen name {} must have the suffix '_REGWEN'".format(wenname))
    ...
    def make_intr_regs(self, interrupts: Sequence[Interrupt]) -> None:
        ...
        swaccesses_obj = list(map(lambda x: SWAccess("RegBlock.make_intr_regs()", x), swaccesses))
        swaccess_dict = {
            IntrType.Event: (swaccesses_obj[0], swaccesses_obj[1], swaccesses_obj[3]),
            IntrType.Status: (swaccesses_obj[2], swaccesses_obj[1], swaccesses_obj[3]),
        }
    ...
    def apply_alias(self, alias_block: "RegBlock", where: str) -> None:
        ...
        intersection = alias_block.name_to_flat_reg.keys() & self.name_to_flat_reg.keys()
        if intersection:
            raise ValueError(
                "Alias register names {} are not unique in alias  {}".format(
                    list(intersection), where
                )
            )
        ...
        for name in self.wennames:
            alias_wennames.update({name: self.name_to_flat_reg[name.lower()].alias_target})
        ...
        for alias_reg in alias_block.registers:
            ...
            if alias_reg.alias_target is None:
                raise ValueError(
                    "No alias target register defined for alias name {} in {}".format(
            ...
            if target not in self.name_to_flat_reg:
                raise ValueError(
                    "Aliased target register {} with alias "
                    "name {} does not exist in reg "
                    "block {} ({}).".format(target, alias_reg.name, self.name, where)
                )
            ...
            if alias_reg.regwen is not None and reg.regwen is not None:
                alias_wenreg = alias_block.name_to_flat_reg[alias_reg.regwen.lower()]
                ...
                raise ValueError(
                    "Missing regwen define in aliased target "
        ...
        for alias_mr in alias_block.multiregs:
            ...
            if alias_mr.alias_target is None:
                raise ValueError(
                    "No alias target multiregister defined for alias name {} in {}".format(
                        alias_mr.name, where
                    )
                )
            ...
            if target not in name_to_multiregs:
                raise ValueError(
                    "Aliased target multiregister {} with alias "
                    "name {} does not exist in reg "
                    "block {} ({}).".format(target, alias_mr.name, self.name, where)
                )
