...
class RegBlock:
    ...
    def make_intr_regs(self, interrupts: Sequence[Interrupt]) -> None:
        ...
        reg = Register(
            ...
            hwre=False,
            regwen=None,
            # Some POR routines have the potential to unpredictably
            # set some `intr_state` fields for various IPs, so we
            # exclude all `intr_state` accesses from CSR checks to
            # prevent this from occurring.
            #
            # An example of an `intr_state` mismatch error occurring
            # due to a POR routine can be seen in issue #6888.
