...
class BusInterfaces:
    ...
    def from_raw(raw: object, where: str) -> "BusInterfaces":
        ...
        for idx, raw_entry in enumerate(check_list(raw, where)):
            ...
            ed = check_keys(
                ...
                entry_what,
                ["protocol", "direction"],
                ["name", "async", "hier_path", "racl_support", "static_racl_support"],
            )

            protocol = check_str(ed["protocol"], "protocol field of " + entry_what)
            if protocol != "tlul":
                raise ValueError("Unknown protocol {!r} at {}".format(protocol, entry_what))

            direction = check_str(ed["direction"], "direction field of " + entry_what)
            if direction not in ["device", "host"]:
                raise ValueError(
                    "Unknown interface direction {!r} at {}".format(direction, entry_what)
                )

            name = check_optional_str(ed.get("name"), "name field of " + entry_what)

            async_clk = check_optional_str(ed.get("async"), "async field of " + entry_what)

            hier_path = check_optional_str(ed.get("hier_path"), "hier_path field of " + entry_what)
            ...
            static_racl_support = check_optional_bool(
                ed.get("static_racl_support"), "static_racl_support field of " + entry_what
            )

            if direction == "host":
                if name is None:
                    if has_unnamed_host:
                        raise ValueError("Multiple un-named host interfaces at {}".format(where))
                    has_unnamed_host = True
                else:
                    if name in named_hosts:
                        raise ValueError(
                            "Duplicate host interface with name {!r} at {}".format(name, where)
                        )
                ...
                if hier_path is not None:
                    raise ValueError(
                        "Hier path is not supported for hostinterface with name {!r} at {}".format(
                            name, where
                        )
                    )
            else:
                if name is None:
                    if has_unnamed_device:
                        raise ValueError("Multiple un-named device interfaces at {}".format(where))
                    has_unnamed_device = True
                else:
                    if name in named_devices:
                        raise ValueError(
                            "Duplicate device interface with name {!r} at {}".format(name, where)
                        )
    ...
    def as_dicts(self) -> List[Dict[str, object]]:
        return [BusInterfaces._if_dict(is_host, name) for is_host, name in self._interfaces()]
    ...
    def _if_inter_signal(self, is_host: bool, name: Optional[str]) -> InterSignal:
        ...
        return InterSignal(
            self.get_port_name(is_host, name), None, "tl", "tlul_pkg", "req_rsp", act, 1, None
        )

    def inter_signals(self) -> List[InterSignal]:
        return [self._if_inter_signal(is_host, name) for is_host, name in self._interfaces()]
    ...
    def find_port_name(self, is_host: bool, name: Optional[str]) -> str:
        ...
        if not self.has_interface(is_host, name):
            ...
            raise KeyError(
                "There is no {} bus interface {}.".format("host" if is_host else "device", called)
            )
