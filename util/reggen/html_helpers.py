...
def expand_paras(s: str, rnames: Set[str]) -> List[str]:
    ...
    paras = [
        _expand_paragraph(paragraph.strip(), rnames) for paragraph in re.split(r"\n(?:\s*\n)+", s)
    ]
    ...
    def fieldsub(match: Match[str]) -> str:
        ...
        if base in rnames or len(rnames & mr_names) == 2:
            if match.group(1)[-1] == ".":
                ...
                return '<a href="#' + base + '"><code class="reg">' + match.group(1) + "</code></a>"
        log.warn("!!" + match.group(1).partition(".")[0] + " not found in register list.")
