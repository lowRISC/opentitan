...
def check_int(
    x: Union[int, str], err_prefix: str, suppress_err_msg: bool = False
) -> Tuple[int, bool]:
    if isinstance(x, int):
        return x, False
    if x[0] == "0" and len(x) > 2:
        if x[1] in "bB":
            ...
            if not suppress_err_msg:
                log.error(err_prefix + ": int must start digit, 0b, 0B, 0o, 0O, 0x or 0X")
            return 0, True
        for c in x[2:]:
            if c not in validch:
                if not suppress_err_msg:
...
def check_ln(obj: Dict[str, object], x: str, withwidth: bool, err_prefix: str) -> int:
    error = 0
    entry = obj[x]
    ...
    for y in entry:
        error += check_keys(
            y, ln_required, ln_optional if withwidth else {}, {}, err_prefix + " element " + x
        )
        if withwidth:
            if "width" in y:
...
val_types = {
    ...
    "b": ["bitrange", "bit number as decimal integer, or bit-range as decimal integers msb:lsb"],
    "l": ["list", "comma separated list enclosed in `[]`"],
    "ln": [
        "name list",
        "comma separated list enclosed in `[]` of "
...
list_optone = {
    ...
    "window": [
        "g",
        "group defining an address range for something other than standard registers",
    ],
    "multireg": ["g", "group defining registers generated from a base instance."],
}
