...
REQUIRED_FIELDS = {
    ...
    "cname": [
        "s",
        "base name for each instance, mostly useful for referring to instance in messages.",
    ],
    "fields": [
        "l",
...
OPTIONAL_FIELDS.update(
    {
        ...
        "compact": ["pb", "If true, allow multireg compacting.If false, do not compact."],
        "cdc": [
            "s",
...
class MultiRegister(RegBase):
    def __init__(
        ...
        rd = check_keys(raw, "multireg", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys()))
        ...
        self.reg = Register.from_raw(reg_width, offset, params, reg_rd, clocks, is_alias)
        ...
        self.cname = check_name(rd["cname"], "cname field of multireg {}".format(self.reg.name))
        ...
        if is_alias:
            if "alias_target" in rd:
                ...
                raise ValueError(
                    "alias multiregister {} does not define the alias_target key.".format(self.name)
                )
        else:
            if "alias_target" in rd:
                if rd["alias_target"] is not None:
                    raise ValueError(
                        "Illegal alias_target key in multiregister {} "
                        "(this is not an alias register block).".format(self.name)
                    )

        self.regwen_multi = check_bool(
            rd.get("regwen_multi", False), "regwen_multi field of multireg {}".format(self.reg.name)
        )

        default_compact = True if len(self.reg.fields) == 1 and not self.regwen_multi else False
        self.compact = check_bool(
            rd.get("compact", default_compact), "compact field of multireg {}".format(self.reg.name)
        )
        if self.compact and len(self.reg.fields) > 1:
            raise ValueError(
                "Multireg {} sets the compact flag but has multiple fields.".format(self.reg.name)
            )

        if self.regwen_multi and self.compact:
            raise ValueError(
                "Multireg {} sets the compact flag but has regwen_multi set.".format(self.reg.name)
            )

        count_str = check_str(rd["count"], "count field of multireg {}".format(self.reg.name))
        self.count = params.expand(count_str, "count field of multireg " + self.reg.name)
        if self.count <= 0:
            raise ValueError(
        ...
        for attr in attrs:
            if getattr(self, attr) != getattr(alias_reg, attr):
                raise ValueError(
                    ...
                    "alias multireg {} and multireg {} in {}.".format(
