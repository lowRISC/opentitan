...
def gen_cfg_html(cfgs: IpBlock, outfile: TextIO) -> None:
    ...
    # clocks
    gen_kv(outfile, "Primary Clock", "<b><code>{}</code></b>".format(cfgs.clocking.primary.clock))
    other_clocks = cfgs.clocking.other_clocks()
    if other_clocks:
        other_clocks_str = ["<b><code>{}</code></b>".format(clk) for clk in other_clocks]
    ...
    if not cfgs.interrupts:
        ...
        for x in cfgs.interrupts:
            genout(
                outfile,
                "<tr><td>{}</td><td>{}</td>{}</tr>".format(
                    name_width(x), x.intr_type.name, render_td(x.desc, rnames, None)
                ),
            )
        genout(outfile, "</table>\n")
    ...
    if not cfgs.alerts:
        ...
        genout(
            outfile,
            '<table class="cfgtable"><tr><th>Alert Name</th>' + "<th>Description</th></tr>\n",
        )
        for x in cfgs.alerts:
            genout(
                outfile, "<tr><td>{}</td>{}</tr>".format(x.name, render_td(x.desc, rnames, None))
            )
        genout(outfile, "</table>\n")
    ...
    if not cfgs.countermeasures:
        ...
        for cm in cfgs.countermeasures:
            genout(
                outfile,
                "<tr><td>{}</td>{}</tr>".format(
                    cfgs.name.upper() + "." + str(cm), render_td(cm.desc, rnames, None)
                ),
            )
        genout(outfile, "</table>\n")
