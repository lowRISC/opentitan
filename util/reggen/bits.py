...
class Bits:
    ...
    @staticmethod
    def from_raw(where: str, reg_width: int, params: ReggenParams, raw: object) -> "Bits":
        ...
        if msb < lsb:
            raise ValueError("msb for {} is {}: less than {}, the msb.".format(where, msb, lsb))
        if lsb < 0:
            raise ValueError("lsb for {} is {}, which is negative.".format(where, lsb))
        if msb >= reg_width:
            raise ValueError(
                "msb for {} is {}, which doesn't fit in {} bits.".format(where, msb, reg_width)
            )
    ...
    def _parse_str(where: str, params: ReggenParams, str_val: str) -> Tuple[int, int]:
        ...
        if len(parts) != 2:
            raise ValueError(
                "bits field for {} is not an integer or of the form msb:lsb. Saw {!r}.".format(
                    where, str_val
                )
            )
        return (
