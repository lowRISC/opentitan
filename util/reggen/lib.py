...
def check_str_dict(obj: object, what: str) -> Dict[str, object]:
    if not isinstance(obj, dict):
        raise ValueError(
            "{} is expected to be a dict, but was actually a {}.".format(what, type(obj).__name__)
        )

    for key in obj:
        if not isinstance(key, str):
            raise ValueError("{} has a key {!r}, which is not a string.".format(what, key))

    return cast(Dict[str, object], obj)


def check_keys(
    obj: object, what: str, required_keys: List[str], optional_keys: List[str]
) -> Dict[str, object]:
    ...
    if missing or unexpected:
        mstr = (
            "The following required fields were missing: {}.".format(", ".join(missing))
            if missing
            else ""
        )
        ustr = (
            "The following unexpected fields were found: {}.".format(", ".join(unexpected))
            if unexpected
            else ""
        )
        raise ValueError(
            "{} doesn't have the right keys. {}{}{}".format(
                what, mstr, " " if mstr and ustr else "", ustr
            )
        )
...
def check_str(obj: object, what: str) -> str:
    ...
    if not isinstance(obj, str):
        raise ValueError("{} is of type {}, not a string.".format(what, type(obj).__name__))
    return obj
...
def check_name(obj: object, what: str) -> str:
    ...
    if not re.match(r"[a-zA-Z_][a-zA-Z_0-9]*$", as_str):
        raise ValueError(
            "{} is {!r}, which isn't a valid symbol in "
            "C / Verilog, so isn't allowed as a name.".format(what, as_str)
        )

    # Also check that this isn't a reserved word.
    if as_str in _VERILOG_KEYWORDS:
        raise ValueError(
            "{} is {!r}, which is a reserved word in "
            "SystemVerilog, so isn't allowed as a name.".format(what, as_str)
        )
...
def check_bool(obj: object, what: str) -> bool:
    ...
    if isinstance(obj, str):
        as_bool = {"true": True, "false": False, "1": True, "0": False}.get(obj.lower())
        if as_bool is None:
            raise ValueError("{} is {!r}, which cannot be parsed as a bool.".format(what, obj))
        return as_bool

    if obj is True or obj is False:
        return obj

    raise ValueError("{} is of type {}, not a bool.".format(what, type(obj).__name__))
...
def check_list(obj: object, what: str) -> List[object]:
    ...
    if not isinstance(obj, list):
        raise ValueError("{} is of type {}, not a list.".format(what, type(obj).__name__))
    return obj
...
def check_str_list(obj: object, what: str) -> List[str]:
    ...
    for idx, elt in enumerate(lst):
        if not isinstance(elt, str):
            raise ValueError(
                "Element {} of {} is of type {}, not a string.".format(
                    idx, what, type(elt).__name__
                )
            )
...
def check_int(obj: object, what: str) -> int:
    ...
    if isinstance(obj, str):
        try:
            ...
            raise ValueError(
                "{} is {!r}, which cannot be parsed as an int.".format(what, obj)
            ) from None

    raise ValueError("{} is of type {}, not an integer.".format(what, type(obj).__name__))
...
def check_xint(obj: object, what: str) -> Optional[int]:
    ...
    if isinstance(obj, str):
        ...
        try:
            ...
            raise ValueError(
                '{} is {!r}, which is not "x", nor can it be parsed as an int.'.format(what, obj)
            ) from None
