...
def gen_define(
    name: str, args: List[str], body: str, existing_defines: Set[str], indent: str = "  "
) -> str:
...
def gen_cdefine_register(
    ...
    genout(
        outstr, gen_define(defname + "_REG_RESVAL", [], uint_literal(reg.resval), existing_defines)
    )
    ...
    for field in reg.fields:
        ...
        if field_width == 1:
            # single bit
            genout(outstr, gen_define(dname + "_BIT", [], str(field.bits.lsb), existing_defines))
        else:
            # multiple bits (unless it is the whole register)
            if field_width != width:
                mask = field.bits.bitmask() >> field.bits.lsb
                genout(
                    outstr, gen_define(dname + "_MASK", [], uint_literal(mask), existing_defines)
                )
                genout(
                    outstr, gen_define(dname + "_OFFSET", [], str(field.bits.lsb), existing_defines)
                )
                genout(
                    outstr,
                    gen_define(
        ...
        if field.enum is not None:
            for enum in field.enum:
                ...
                genout(
                    outstr,
                    gen_define(
                        defname + "_" + as_define(field.name) + "_VALUE_" + ename,
                        [],
                        value,
                        existing_defines,
                    ),
...
def gen_cdefines_module_param(
    outstr: TextIO, param: LocalParam, module_name: str, existing_defines: Set[str]
) -> None:
    ...
    if param.param_type not in known_types:
        warnings.warn("Cannot generate a module define of type {}".format(param.param_type))
        return
...
def gen_cdefines_module_params(
    outstr: TextIO,
    module_data: IpBlock,
    module_name: str,
    register_width: int,
...
def gen_multireg_field_defines(
    outstr: TextIO,
    regname: str,
    fields: List[Field],
    subreg_num: int,
...
def gen_cdefine_multireg(
    ...
    if len(multireg.reg.fields) >= 1:
        ...
        gen_multireg_field_defines(
            outstr, regname, multireg.reg.fields, len(multireg.regs), regwidth, existing_defines
        )
    else:
        log.warn(
            "Fieldless multireg " + multireg.reg.name + " skip multireg specific data generation."
        )

    for subreg in multireg.regs:
        gen_cdefine_register(outstr, subreg, component, regwidth, rnames, existing_defines)


def gen_cdefines_interrupt_field(
    outstr: TextIO, interrupt: Signal, component: str, regwidth: int, existing_defines: Set[str]
) -> None:
    ...
    if interrupt.bits.width() == 1:
        # single bit
        genout(outstr, gen_define(defname + "_BIT", [], str(fieldlsb), existing_defines))
    else:
        # multiple bits (unless it is the whole register)
        if interrupt.bits.width() != regwidth:
            mask = interrupt.bits.msb >> fieldlsb
            genout(outstr, gen_define(defname + "_MASK", [], hex(mask), existing_defines))
            genout(outstr, gen_define(defname + "_OFFSET", [], str(fieldlsb), existing_defines))
            genout(
                outstr,
                gen_define(
                    defname + "_FIELD",
...
def gen_cdefines_interrupts(
    outstr: TextIO, block: IpBlock, component: str, regwidth: int, existing_defines: Set[str]
) -> None:
...
def gen_cdefines(block: IpBlock, outfile: TextIO, src_lic: Optional[str], src_copy: str) -> int:
    ...
    gen_cdefines_module_params(outstr, block, block.name, block.regwidth, existing_defines)

    gen_cdefines_interrupts(outstr, block, block.name, block.regwidth, existing_defines)

    for rb in block.reg_blocks.values():
        for x in rb.entries:
            if isinstance(x, Register):
                gen_cdefine_register(
                    outstr, x, block.name, block.regwidth, rnames, existing_defines
                )
                continue

            if isinstance(x, MultiRegister):
                gen_cdefine_multireg(
                    outstr, x, block.name, block.regwidth, rnames, existing_defines
                )
                continue

            if isinstance(x, Window):
                gen_cdefine_window(outstr, x, block.name, block.regwidth, rnames, existing_defines)
                continue
...
def test_gen_define() -> None:
    ...
    assert (
        gen_define("MACRO_NAME", ["arg1", "arg2"], "arg1 + arg2", set()) == basic_oneline_with_args
    )
    ...
    assert (
        gen_define(long_macro_name, [], "a_fairly_long_body + something_else + 10", set())
        == multiline
    )
