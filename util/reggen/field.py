...
class Field:
    ...
    def from_raw(
        ...
        if is_mubi:
            ...
            if not prim_mubi.is_width_valid(bits.width()):
                raise ValueError(f"mubi field for {name} does not support width of {bits.width()}")
        ...
        if raw_enum is None:
            ...
            for idx, raw_entry in enumerate(raw_entries):
                ...
                if entry.value in enum_val_to_name:
                    raise ValueError(
                        "In {}, duplicate enum entries for value {} ({} and {}).".format(
                            where, entry.value, enum_val_to_name[entry.value], entry.name
                        )
                    )
                enum.append(entry)
    ...
    def make_multi(
        ...
        for reg_idx in range(min_reg_idx, max_reg_idx + 1):
            ...
            bits = self.bits if bit_offset == 0 else self.bits.make_translated(bit_offset)
    ...
    def make_suffixed(self, suffix: str, cname: str, creg_idx: int, stripped: bool) -> "Field":
        desc = "For {}{}".format(cname, creg_idx) if stripped else self.desc
        enum = None if stripped else self.enum
        ...
        for attr in attrs:
            if getattr(self, attr) != getattr(alias_field, attr):
                raise ValueError(
                    ...
                    "alias field {} and field {} in {}.".format(
                        attr, self.name, alias_field.name, where
                    )
                )
