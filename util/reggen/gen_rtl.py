...
def get_type_name_pfx(block: IpBlock, iface_name: Optional[str]) -> str:
    return block.name.lower() + ("" if iface_name is None else "_{}".format(iface_name.lower()))
...
def get_iface_tx_type(block: IpBlock, iface_name: Optional[str], hw2reg: bool) -> str:
...
def gen_rtl(block: IpBlock, outdir: str) -> int:
    ...
    reg_top_tpl = Template(filename=str(importlib.resources.files("reggen") / "reg_top.sv.tpl"))
    reg_pkg_tpl = Template(filename=str(importlib.resources.files("reggen") / "reg_pkg.sv.tpl"))
    ...
    reg_pkg_path = os.path.join(outdir, block.name.lower() + alias_impl + "_reg_pkg.sv")
    ...
    for if_name, rb in block.reg_blocks.items():
        ...
        with open(reg_top_path, "w", encoding="UTF-8") as fout:
            try:
                fout.write(
                    reg_top_tpl.render(
                        block=block, mod_base=mod_base, mod_name=mod_name, if_name=if_name, rb=rb
                    )
                )
            except:  # noqa F722 for template Exception handling
                log.error(exceptions.text_error_template().render())
