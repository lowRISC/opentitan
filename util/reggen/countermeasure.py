...
class CounterMeasure:
    ...
    def from_raw(what: str, raw: object) -> "CounterMeasure":
        ...
        try:
            ...
            raise ValueError(f"Invalid countermeasure ID format: {name} ({what}).")
        if not re.fullmatch(r"^([a-zA-Z0-9_]*)", instance):
            raise ValueError(f"Invalid countermeasure instance name: {instance} ({what}).")
        if asset not in CM_ASSETS:
            raise ValueError(f"Invalid countermeasure asset: {asset} ({what}).")
        if cm_type not in CM_TYPES:
            raise ValueError(f"Invalid countermeasure type: {cm_type} ({what}).")
    ...
    @staticmethod
    def search_rtl_files(paths: Sequence[str]) -> Dict[str, List[Tuple[str, int]]]:
                ...
                for idx, line in enumerate(fd):
                    ...
                    for entry in entries:
                        ...
                        if not good_name.match(entry):
                            raise ValueError(
                                "Malformed countermeasure name, {!r}, at {}, line {}.".format(
                                    entry, path, idx + 1
                                )
                            )
    ...
    @staticmethod
    def check_annotation_list(
        block_name: str,
        hjson_path: str,
        rtl_names: Dict[str, List[Tuple[str, int]]],
        hjson_list: List["CounterMeasure"],
    ) -> bool:
        ...
        for name in rtl_only_countermeasures:
            for rtl_path, line in rtl_names[name]:
                log.error(
                    f"No Hjson countermeasure {name} exists for RTL "
        ...
        for name in missing_countermeasures:
            log.error(
                f"Hjson countermeasure {name} declared in Hjson file "
        ...
        if len(rtl_only_countermeasures) > 0:
            if len(missing_countermeasures) > 0:
                log.error(
                    f"Block {block_name}: Some unknown and some missing countermeasures in RTL"
                )
            else:
                log.error(f"Block {block_name}: Some unknown countermeasures referenced in RTL.")
            return False
        elif len(missing_countermeasures) > 0:
            log.error(f"Block {block_name}: Some countermeasures in Hjson don't appear in RTL.")
            return False
        return True
