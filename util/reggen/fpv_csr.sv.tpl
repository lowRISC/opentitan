...
// TODO: This automation does not support: shadow reg, regwen reg, RO reg
...
%>\
  ...
  // reading a WO register will return the reset value
  property r_wo_P(bit [${addr_msb}:0] addr, bit [DWidth-1:0] exp_data, bit [DWidth-1:0] mask,
      int lsb);
    logic [DWidth-1:0] id;
...
%>\
      % if field_access == "WO":
  `ASSERT(${assert_name}_rd_A, r_wo_P(${reg_offset}, 'h${format(f.resval, 'x')}, 'h${field_wr_mask_h}, ${lsb}))
      % elif not r.ishomog:
