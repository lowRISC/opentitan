...
REQUIRED_FIELDS = {
    ...
    "desc": ["t", "description of the register. This field supports the markdown syntax."],
    "fields": ["l", "list of register field description groups"],
}

OPTIONAL_FIELDS = {
    "alias_target": ["s", "name of the register to apply the alias definition to."],
    "async": [
        "s",
    ...
    "swaccess": [
        "s",
        "software access permission to use for fields that don't specify swaccess",
    ],
    "hwaccess": [
        "s",
        "hardware access permission to use for fields that don't specify hwaccess",
    ],
    "hwext": ["s", "'true' if the register is stored outside of the register module"],
    "hwqe": [
        "s",
    ...
    "tags": ["s", "tags for the register, following the format 'tag_name:item1:item2...'"],
    "shadowed": ["s", "'true' if the register is shadowed"],
    "update_err_alert": [
        "s",
        "alert that will be triggered if this shadowed register has update error",
    ],
    "storage_err_alert": [
        "s",
        "alert that will be triggered if this shadowed register has storage error",
    ],
...
class Register(RegBase):
    ...
    def __init__(
        ...
        if self.hwre and not self.hwext:
            raise ValueError("The {} register specifies hwre but not hwext.".format(self.name))
        ...
        if self.shadowed and not sounds_shadowy:
            raise ValueError(
                "Register {} has the shadowed flag but its name "
                "doesn't end with the _shadowed suffix.".format(self.name)
            )
        elif sounds_shadowy and not self.shadowed:
            raise ValueError(
                "Register {} has a name ending in _shadowed, but "
                "the shadowed flag is not set.".format(self.name)
            )
        ...
        for field in self.fields:
            if field.name in self.name_to_field:
                raise ValueError(
                    "Register {} has duplicate fields called {}.".format(self.name, field.name)
                )
        ...
        if self.hwext:
            for field in self.fields:
                if field.hwaccess.key == "hro" and field.sw_readable():
                    raise ValueError(
                        "The {} register has hwext set, but "
                        "field {} has hro hwaccess and the "
                        "field value is readable by software "
                        "(mode {}).".format(self.name, field.name, field.swaccess.key)
                    )
                if not field.hwqe and field.sw_writable():
                    raise ValueError(
                        "The {} register has hwext set and field "
                        "{} is writable by software (mode {}), "
                        "so the register must also enable hwqe.".format(
                            self.name, field.name, field.swaccess.key
                        )
                    )
        ...
        if self.shadowed:
            for field in self.fields:
                if field.swaccess.key not in ["rw", "ro", "wo", "rw1s", "rw0c"]:
                    raise ValueError(
                        "Shadowed register {} has a field ({}) with incompatible type '{}'.".format(
                            self.name, field.name, field.swaccess.key
                        )
                    )
        ...
        if rc_fields and we_fields:
            raise ValueError(
                "Register {} has both software writable fields "
                "({}) and read-clear fields ({}), meaning it "
                "doesn't have a single write-enable signal.".format(
                    self.name, ", ".join(rc_fields), ", ".join(we_fields)
                )
            )
        ...
        for field in self.fields:
            ...
            if bits_used & field_mask:
                raise ValueError(
                    "Register {} has non-disjoint fields: "
                    "{} uses bits {:#x} used by other fields.".format(
                        self.name, field.name, bits_used & field_mask
                    )
                )
        ...
        if resval is not None and self.resval != resval:
            raise ValueError(
                "Register {} specifies a reset value of {:#x} but "
                "collecting reset values across its fields yields "
                "{:#x}.".format(self.name, resval, self.resval)
            )
    ...
    def from_raw(
        ...
        rd = check_keys(raw, "register", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys()))
        ...
        if rd.get("alias_target") is not None:
            if is_alias:
                alias_target = check_name(rd["alias_target"], "name of alias target register")
            else:
                raise ValueError("Field {} may not have an alias_target key".format(name))
        elif is_alias:
            raise ValueError("alias register {} does not define the alias_target key.".format(name))
        ...
        if async_name:
            ...
            if async_name not in valid_clocks:
                raise ValueError(
                    "async clock {} defined for {} does not exist "
                    "in valid module clocks {}.".format(async_name, name, valid_clocks)
                )
            else:
                async_clk = clocks.get_by_clock(async_name)

        sync_name = check_str(
            rd.get("sync", ""), "different sync clock for {} register".format(name)
        )
        sync_clk = None
        ...
        if sync_name:
            ...
            if sync_name not in valid_clocks:
                raise ValueError(
                    "sync clock {} defined for {} does not exist in valid module clocks {}.".format(
                        sync_name, name, valid_clocks
                    )
                )
            else:
                sync_clk = clocks.get_by_clock(sync_name)

        swaccess = SWAccess("{} register".format(name), rd.get("swaccess", "none"))
        hwaccess = HWAccess("{} register".format(name), rd.get("hwaccess", "hro"))

        hwext = check_bool(rd.get("hwext", False), "hwext flag for {} register".format(name))

        hwqe = check_bool(rd.get("hwqe", False), "hwqe flag for {} register".format(name))

        hwre = check_bool(rd.get("hwre", False), "hwre flag for {} register".format(name))

        raw_regwen = rd.get("regwen", "")
        if not raw_regwen:
            regwen = None
        else:
            regwen = check_name(raw_regwen, "regwen for {} register".format(name))

        tags = check_str_list(rd.get("tags", []), "tags for {} register".format(name))

        raw_resval = rd.get("resval")
        if raw_resval is None:
            resval = None
        else:
            resval = check_int(raw_resval, "resval for {} register".format(name))
            if not 0 <= resval < (1 << reg_width):
                raise ValueError(
                    "resval for {} register is {}, not an unsigned {}-bit number.".format(
                        name, resval, reg_width
                    )
                )

        shadowed = check_bool(
            rd.get("shadowed", False), "shadowed flag for {} register".format(name)
        )

        raw_fields = check_list(rd["fields"], "fields for {} register".format(name))
        if not raw_fields:
        ...
        for idx, rf in enumerate(raw_fields):
            ...
            if overlap_bits:
                raise ValueError(
                    f"Field {field.name} uses bits {overlap_bits:#x} that appear in other fields."
                )
        ...
        if raw_uea is None:
            ...
            update_err_alert = check_name(raw_uea, "update_err_alert for {} register".format(name))
        ...
        if raw_sea is None:
            ...
            storage_err_alert = check_name(
                raw_sea, "storage_err_alert for {} register".format(name)
            )

        writes_ignore_errors = check_bool(
            rd.get("writes_ignore_errors", False),
            "writes_ignore_errors flag for {} register".format(name),
        )
    ...
    def get_n_bits(self, bittype: List[str]) -> int:
        return sum(field.get_n_bits(self.hwext, self.hwre, bittype) for field in self.fields)
    ...
    def make_multi(
        ...
        new_name = "{}_{}".format(self.name, creg_idx) if creg_count > 1 else self.name

        new_alias_target = None
        if self.alias_target is not None:
            new_alias_target = (
                "{}_{}".format(self.alias_target, creg_idx) if creg_count > 1 else self.alias_target
            )
        ...
        if compact:
            ...
            new_fields = self.fields[0].make_multi(
                reg_width, min_reg_idx, max_reg_idx, cname, creg_idx, strip_field
            )
        else:
            ...
            new_fields = [
                field.make_suffixed("_{}".format(creg_idx), cname, creg_idx, strip_field)
                for field in self.fields
            ]
        ...
        return Register(
            ...
            self.shadowed,
            new_fields,
            self.update_err_alert,
            self.storage_err_alert,
    ...
    def check_valid_regwen(self) -> None:
        ...
        if self.async_clk is not None:
            raise ValueError("Regwen {} cannot be declared as async.".format(self.name))
        ...
        if len(self.fields) != 1:
            raise ValueError(
                ...
                "field. It actually has {}.".format(self.name, len(self.fields))
            )
        ...
        if wen_fld.mubi:
            if wen_fld.bits.width() % 4:
                raise ValueError(
                    "One or more registers use {} as a multi-bit "
                    "write-enable so its field bit-width should "
                    "be a multiple of 4, not {}.".format(self.name, wen_fld.bits.width())
                )
        else:
            if wen_fld.bits.width() != 1:
                raise ValueError(
                    ...
                    "not {}.".format(self.name, wen_fld.bits.width())
                )
        if wen_fld.bits.lsb != 0:
            raise ValueError(
                "One or more registers use {} as a "
                "write-enable so its field should have LSB 0, "
                "not {}.".format(self.name, wen_fld.bits.lsb)
            )
            ...
            if wen_fld.mubi:
                if self.resval != prim_mubi.mubi_value_as_int(True, wen_fld.bits.width()):
                    raise ValueError(
                        ...
                        "it should have a multi-bit TRUE reset value.".format(self.name)
                    )
            else:
                if not self.resval:
                    raise ValueError(
                        ...
                        "it should have a nonzero reset value.".format(self.name)
                    )
        ...
        raise ValueError(
            "One or more registers use {} as a write-enable. "
            "However, its field has invalid access permissions "
            "({} / {}). It should either have swaccess=RW0C "
            "or have swaccess=RO and hwaccess=HWO.".format(
                self.name, wen_fld.swaccess.key, wen_fld.hwaccess.key
            )
        )
    ...
    def apply_alias(self, alias_reg: "Register", where: str) -> None:
        ...
        for attr in attrs:
            if getattr(self, attr) != getattr(alias_reg, attr):
                raise ValueError(
                    "Value mismatch for attribute {} between "
                    "alias register {} and register {} in {}.".format(
                        attr, self.name, alias_reg.name, where
                    )
                )
        ...
        if len(alias_reg.fields) != len(self.fields):
            raise ValueError(
                "The number of fields does not match for "
                "alias register {} and register {} in {}.".format(alias_reg.name, self.name, where)
            )
        ...
        for k, (alias_field, field) in enumerate(fields):
            ...
            # Otherwise the names need to match
                ...
                raise ValueError(
                    "Inconsistent aliased field name {} in alias register {} in {}.".format(
                        alias_field.alias_target, alias_reg.alias_target, where
                    )
                )
        ...
        for k, field in enumerate(self.fields):
            if field.alias_target is not None:
                ...
                if m and field.alias_target != f"field{k}":
                    raise ValueError(
                        "Alias field alias_target {} is "
                        "incorrectly numbered "
                        "in alias register {} in {}.".format(field.alias_target, self.name, where)
                    )
                elif field.alias_target in known_field_names:
                    raise ValueError(
                        "Alias field alias_target {} is not "
                        "unique in alias register {} in {}.".format(
                            field.alias_target, self.name, where
                        )
                    )
                else:
