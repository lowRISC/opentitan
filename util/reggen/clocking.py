...
class ClockingItem:
    ...
    def from_raw(raw: object, only_item: bool, where: str) -> "ClockingItem":
        ...
        primary = check_bool(rd.get("primary", only_item), "primary field of " + what)
        internal = check_bool(rd.get("internal", False), "internal field of " + what)
        ...
        if not clock or clock in ["clk_i", "scan_clk_i"]:
            ...
            raise ValueError(
                f"clock name must be of the form clk_*_i or clk_i. {clock} is illegal."
            )

        if primary:
            if clock is None:
                raise ValueError(f"No clock signal for primary clocking item at {what}.")
            if reset is None:
                raise ValueError(f"No reset signal for primary clocking item at {what}.")
...
class Clocking:
    ...
    def from_raw(raw: object, where: str) -> "Clocking":
        ...
        if len(primaries) != 1:
            raise ValueError(
                "There should be exactly one primary clocking "
