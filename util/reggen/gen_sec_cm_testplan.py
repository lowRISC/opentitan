...
def gen_sec_cm_testplan(block: IpBlock, outdir: str) -> int:
    ...
    if outfile.exists():
        ...
        names_from_spec = [
            "sec_cm_{}".format(str(cm).lower().replace(".", "_")) for cm in block.countermeasures
        ]

        if sorted(names_from_spec) != sorted(names_from_testplan):
            log.error(
                "The generated security countermeasures testplan "
    ...
    with open(outfile, "w", encoding="UTF-8") as f:
        try:
            f.write(sec_cm_testplan_tpl.render(block=block, block_name=block.name.lower()))
