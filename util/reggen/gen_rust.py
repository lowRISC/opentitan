...
def format_comment(s: str) -> str:
    ...
    """
    comment = textwrap.wrap(s, width=77, initial_indent="// ", subsequent_indent="// ")
    return "\n".join(comment) + "\n"
...
def gen_const_multireg(
    ...
    if len(multireg.reg.fields) == 1:
        ...
        gen_multireg_field_defines(
            outstr, regname, multireg.reg.fields[0], len(multireg.regs), regwidth, existing_defines
        )
    else:
        log.warn(
            "Non-homogeneous multireg "
            + multireg.reg.name
            + " skip multireg specific data generation."
        )

    for subreg in multireg.regs:
        gen_const_register(outstr, subreg, component, regwidth, rnames, existing_defines)


def gen_interrupt_field(
...
def gen_rust(block: IpBlock, outfile: TextIO, src_lic: Optional[str], src_copy: str) -> int:
    ...
    gen_const_module_params(outstr, block, block.name, block.regwidth, existing_defines)

    gen_const_interrupts(outstr, block, block.name, block.regwidth, existing_defines)

    for rb in block.reg_blocks.values():
        for x in rb.entries:
            if isinstance(x, Register):
                gen_const_register(outstr, x, block.name, block.regwidth, rnames, existing_defines)
                continue

            if isinstance(x, MultiRegister):
                gen_const_multireg(outstr, x, block.name, block.regwidth, rnames, existing_defines)
                continue

            if isinstance(x, Window):
                gen_const_window(outstr, x, block.name, block.regwidth, rnames, existing_defines)
                continue
