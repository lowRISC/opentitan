...
def gen_cfg_md(cfgs: IpBlock, output: TextIO, register_file: Optional[str] = None) -> None:
    ...
    list_items.append(
        "Bus Host Interfaces (TL-UL): " + (", ".join(host_ports) if host_ports else italic("none"))
    )

    # IO
    ios = (
        [("input", x) for x in cfgs.xputs[1]]
    ...
    if not cfgs.alerts:
        ...
        rows = [[x.name, regref_to_link(x.desc, register_file)] for x in cfgs.alerts]
        tables.append(
            (
                "Security Alerts",
