...
OPTIONAL_FIELDS = {
    "data-intg-passthru": [
        "s",
        "True if the window has data integrity pass through. Defaults to false if not present.",
    ],
    "byte-write": ["s", "True if byte writes are supported. Defaults to false if not present."],
    "validbits": [
        "d",
        "Number of valid data bits within "
        "regwidth sized word. "
    ...
    "unusual": [
        "s",
        "True if window has unusual parameters "
...
class Window:
    ...
    @staticmethod
    def from_raw(offset: int, reg_width: int, params: ReggenParams, raw: object) -> "Window":
        rd = check_keys(raw, "window", list(REQUIRED_FIELDS.keys()), list(OPTIONAL_FIELDS.keys()))
        ...
        unusual = check_bool(rd.get("unusual", False), "unusual field for " + wind_desc)
        byte_write = check_bool(rd.get("byte-write", False), "byte-write field for " + wind_desc)
        data_intg_passthru = check_bool(
            rd.get("data-intg-passthru", False), "data-intg-passthru field for " + wind_desc
        )

        validbits = check_int(rd.get("validbits", reg_width), "validbits field for " + wind_desc)
        if validbits <= 0:
            raise ValueError("validbits field for {} is not positive.".format(wind_desc))
        if validbits > reg_width:
            raise ValueError(
                "validbits field for {} is {}, "
                "which is greater than {}, the register width.".format(
                    wind_desc, validbits, reg_width
                )
            )
        ...
        if items <= 0:
            raise ValueError(
                "Items field for {} is {}, which isn't positive.".format(wind_desc, items)
            )
        ...
        if po2_size != size_in_bytes and not unusual:
            raise ValueError(
                "Items field for {} is {}, which gives a size of "
                "{} bytes. This is not a power of 2 (next power "
                "of 2 is {}). If you want to do this even so, "
                'set the "unusual" flag.'.format(wind_desc, items, size_in_bytes, po2_size)
            )
        ...
        if not (swaccess.value[4] or unusual):
            raise ValueError(
                "swaccess field for {} is {}, which is an "
                "unusual access type for a window. If you want "
                'to do this, set the "unusual" flag.'.format(wind_desc, swaccess.key)
            )
