...
def format_comment(s: str) -> str:
    ...
    """
    comment = textwrap.wrap(s, width=97, initial_indent="/// ", subsequent_indent="/// ")
    return "\n".join(comment) + "\n"
...
def access(reg: Any) -> str:
    ...
    if swaccess == "ro":
        ...
        warnings.warn("{}: Unknown swaccess type {}".format(reg.name, swaccess))
        return "ReadWrite"
        ...
        if field.auto_split:
            for sub_field_id in range(field.bits.lsb, field.bits.width()):
                genout(
                    fieldout,
                    "\n{} OFFSET({}) NUMBITS({}) [],",
                    "{}_{}".format(field.name.upper(), sub_field_id),
                    sub_field_id,
                    1,
                )
        else:
...
def gen_const_interrupts(
    fieldout: TextIO, block: IpBlock, component: str, regwidth: int, existing_defines: Set[str]
) -> None:
...
def gen_tock(
    ...
    gen_const_module_params(paramout, block, block.name, block.regwidth, existing_defines)

    gen_const_interrupts(fieldout, block, block.name, block.regwidth, existing_defines)
    ...
    for rb in block.reg_blocks.values():
        for x in rb.entries:
            ...
            if isinstance(x, Window):
                gen_const_window(
                    regout, x, block, rnames, existing_defines, access_type, highest_address
                )
                continue
