...
""",
    "required": {
        ...
        "size_byte": ["d", "Memory space of the device. It is required for the device"],
    },
    "optional": {},
    "added": {},
}
...
""",
    ...
    "optional": {
        ...
        "req_fifo_pass": ["pb", "If true, pipeline fifo has passthrough behavior on req"],
        "rsp_fifo_pass": ["pb", "If true, pipeline fifo has passthrough behavior on rsp"],
...
""",
    "required": {
        ...
        "clock": [
            "s",
            "Main clock. Internal components use this clock."
            " If not specified, it is assumed to be in main clock domain",
        ],
        "reset": ["s", "Main reset"],
        "connections": ["g", "List of edge. Key is host, entry in value list is device"],
    ...
    "added": {
        "reset_connections": [
            "g",
            "Generated by topgen. Key is the reset signal inside IP"
            " and value is the top reset signal",
        ],
        "addr_spaces": [
            "l",
            "Generated by topgen. List of address spaces used across all hosts",
        ],
        "inter_signal_list": ["l", "Generated by topgen. List of inter signals for this xbar"],
    },
}
...
def check_keys(obj: Dict[Any, Any], control: Dict[Any, Any], prefix: str = "") -> int:
    ...
    for k, v in obj.items():
        ...
        if checker[0] is not None and checker[0] not in val_types:
            log.error(
                prefix + " field {} is undefined type. Check val_types {}".format(k, checker[0])
            )
        if checker[0] is None:
            pass
        elif checker[0] == "lg":
            # List of subgroup
            error += sum(map(partial(check_keys, control=checker[1], prefix=prefix_name), obj[k]))
        elif checker[0] == "g":
            # if second entry isn't string type, call recursively
            if isinstance(checker[1], str):
                log.info("Skipping {} as no further control group is given".format(prefix_name))
                continue

            error += check_keys(obj=obj[k], control=checker[1], prefix=prefix_name)
            ...
            log.error(prefix_name + " is not supported in this configuration format")
...
def isNotMinSpacing(range1: Tuple[int, int], range2: Tuple[int, int]) -> bool:
    return not (
        range2[0] < range1[0] - MIN_DEVICE_SPACING or range2[0] >= range1[0] + MIN_DEVICE_SPACING
    )
...
def checkAddressOverlap(addr: Tuple[int, int], ranges: List[Tuple[int, int]]) -> bool:
    result = [(hex(x[0]), hex(x[1])) for x in ranges if isOverlap(x, addr)]
    return len(result) != 0


def checkAddressSpacing(addr: Tuple[int, int], ranges: List[Tuple[int, int]]) -> bool:
...
def validate(obj: Dict[Any, Any]) -> Optional[Xbar]:
    ...
    for nodeobj in obj["nodes"]:
        ...
        clock = nodeobj["clock"].lower() if "clock" in nodeobj.keys() else xbar.clock

        reset = nodeobj["reset"].lower() if "reset" in nodeobj.keys() else xbar.reset
        ...
        node = mk_node(
            typ=nodeobj["type"].lower(), name=nodeobj["name"].lower(), clock=clock, reset=reset
        )
                ...
                for asid, base_addr in addr["base_addrs"].items():
                    ...
                    if checkBaseSizeOverlap(address_from, size):
                        log.error(
                            ...
                            " Check the config. Addr(0x%x - 0x%x)" % (addr_entry[0], addr_entry[1])
                        )
