...
def simplify_addr(dev, xbar, asid):
    ...
    @param addrs List of Dict[Addr] : {'base_addr':Dict[ASID,Addr],'size_byte':}
    """
    ...
    for e in newlist:
        ...
        if no_device_in_range(xbar, dev["name"], result[-1], e, asid):
            ...
            smallest_addr_gte = get_next_base_addr(e["base_addrs"], xbar,
                                                   dev["name"], asid)
            ...
            if smallest_addr_gte == -1:
                ...
                next_value = int(smallest_addr_gte["base_addrs"][asid], 0)
...
def no_device_in_range(xbar, name, f, t, asid):
    ...
    for node in [
        ...
        for addr in node["addr_range"]:
            b_addr = int(addr["base_addrs"][asid], 0)
...
def get_next_base_addr(addr, xbar, name, asid):
    ...
    gte_list = [x for x in sorted_list if int(x["base_addrs"][asid], 0) > value]
