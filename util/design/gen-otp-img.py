...
def _override_seed(args, seed_name, config):
    ...
    if arg_seed:
        log.warning("Commandline override of {} with {}.".format(seed_name, arg_seed))
    ...
    # randomly generate a new seed if not.
        ...
        if config.setdefault("seed", new_seed) == new_seed:
            log.warning("No {} specified, setting to {}.".format(seed_name, new_seed))
...
def main():
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="""Don't print out progress messages."""
    )
    parser.add_argument(
        "-stamp",
        action="store_true",
    ...
    parser.add_argument(
        "--seed", type=int, metavar="<seed>", help="Custom seed used for randomization."
    )
    parser.add_argument(
        "--img-seed",
        type=int,
    ...
    try:
        ...
        for f in args.add_cfg:
            log.info("Processing additional image configuration file {}".format(f))
            log.info("")
