...
def _render_template(template: str, output_file: str, lc_st_enc: LcStEnc):
    with open(template, "r") as tplfile:
        ...
        if output_file:
            ...
            with open(
                Path(template).parent.joinpath(Path(template).stem), "wb", buffering=2097152
            ) as outfile:
                outfile.write(tpl.render(lc_st_enc=lc_st_enc).encode("utf-8"))
...
def main():
    ...
    with open(args.lc_state_def_file, "r") as infile:
        ...
        if args.seed:
            log.warning("Commandline override of seed with {}.".format(args.seed))
        ...
        if args.raw_unlock_rs_template:
            _render_template(args.raw_unlock_rs_template, args.raw_unlock_rs_output, lc_st_enc)
        else:
