...
from lib.common import inverse_permute_bits, validate_data_perm_option, vmem_permutation_string
...
OTP_FLASH_DATA_DEFAULT_CFG_RE = re.compile(r"CREATOR_SW_CFG: CREATOR_SW_CFG_FLASH_DATA_DEFAULT_CFG")
...
OTP_SECRET1_FLASH_ADDR_KEY_SEED_STOP = OTP_FLASH_ADDR_KEY_SEED_SIZE // OTP_SECRET1_BLOCK_SIZE
OTP_SECRET1_FLASH_DATA_KEY_SEED_START = OTP_FLASH_ADDR_KEY_SEED_SIZE // OTP_SECRET1_BLOCK_SIZE
...
FLASH_GF_OPERAND_A_MASK = (
    FLASH_GF_OPERAND_B_MASK & ~(0xFFFF << (FLASH_WORD_SIZE - FLASH_ADDR_SIZE))
) << FLASH_WORD_SIZE
# Create GF(2^64) with irreducible_polynomial = x^64 + x^4 + x^3 + x + 1
FLASH_GF_2_64 = ffield.FField(64, gen=((0x1 << 64) | (0x1 << 4) | (0x1 << 3) | (0x1 << 1) | 0x1))

# Format string for generating new VMEM file.
FLASH_VMEM_WORD_SIZE = FLASH_WORD_SIZE + FLASH_INTEGRITY_ECC_SIZE + FLASH_RELIABILITY_ECC_SIZE
...
def _xex_scramble(data: int, word_addr: int, flash_addr_key: int, flash_data_key: int) -> int:
    operand_a = (
        (flash_addr_key & FLASH_GF_OPERAND_A_MASK) >> (FLASH_WORD_SIZE - FLASH_ADDR_SIZE)
    ) | word_addr
    operand_b = flash_addr_key & FLASH_GF_OPERAND_B_MASK
    mask = FLASH_GF_2_64.Multiply(operand_a, operand_b)
    masked_data = data ^ mask
    return prince.prince(masked_data, flash_data_key, FLASH_PRINCE_NUM_HALF_ROUNDS) ^ mask


def _convert_array_2_int(data_array: List[int], data_size: int, little_endian=True) -> int:
...
def _get_otp_ctrl_netlist_consts(
    otp_mmap_file: str, otp_seed: int, scrambling_configs: FlashScramblingConfigs
):
...
def _get_flash_scrambling_configs(
    otp_vmem_file: str, otp_data_perm: list, configs: FlashScramblingConfigs
) -> None:
    ...
    for line in otp_vmem_lines:
        if OTP_FLASH_DATA_DEFAULT_CFG_RE.search(line) or OTP_SECRET1_RE.search(line):
            ...
            if otp_data_perm:
                ...
                otp_data_word_w_ecc = int(
                    inverse_permute_bits(otp_data_word_as_str, otp_data_perm), 2
                )
            # Drop ECC bits.
            ...
            if OTP_FLASH_DATA_DEFAULT_CFG_RE.search(line):
                if idx == (OTP_FLASH_DATA_DEFAULT_CFG_BLOCK_SIZE // OTP_WORD_SIZE):
    ...
    if flash_data_default_cfg is None:
        raise RuntimeError("Cannot read flash scrambling enablement state from OTP.")
    ...
    otp_secret1_present_cipher = Present(
        configs.otp_secret1_key,
        rounds=OTP_SECRET1_PRESENT_NUM_ROUNDS,
        keylen=OTP_SECRET1_PRESENT_KEY_LENGTH,
    )
    descrambled_secret1_blocks = list(map(otp_secret1_present_cipher.decrypt, secret1_data_blocks))
    configs.addr_key_seed = _convert_array_2_int(
        descrambled_secret1_blocks[
            OTP_SECRET1_FLASH_ADDR_KEY_SEED_START:OTP_SECRET1_FLASH_ADDR_KEY_SEED_STOP
        ],
        OTP_SECRET1_BLOCK_SIZE,
    )
    configs.data_key_seed = _convert_array_2_int(
        descrambled_secret1_blocks[
            OTP_SECRET1_FLASH_DATA_KEY_SEED_START:OTP_SECRET1_FLASH_DATA_KEY_SEED_STOP
        ],
        OTP_SECRET1_BLOCK_SIZE,
    )


def _compute_flash_scrambling_key(
    scrambling_configs: FlashScramblingConfigs, key_type: FlashScramblingKeyType
) -> int:
    if key_type == FlashScramblingKeyType.ADDRESS:
    ...
    for i in range(2):
        round_1_present_key = (
            key_seed >> (FLASH_KEY_COMPUTATION_KEY_SIZE * i)
        ) & FLASH_KEY_COMPUTATION_KEY_MASK
        key_half = 0
        for j in range(2):
            if j == 0:
                cipher = Present(round_1_present_key)
                key_half = cipher.encrypt(
                    scrambling_configs.get_iv(key_type)
                ) ^ scrambling_configs.get_iv(key_type)
            else:
...
def _compute_flash_scrambling_keys(scrambling_configs: FlashScramblingConfigs) -> None:
...
def _reformat_flash_vmem(
    flash_vmem_file: str, scrambling_configs: FlashScramblingConfigs
) -> List[str]:
    ...
    for line in flash_vmem_lines:
        ...
        for item in line_items:
            ...
            # Process the data words.
                ...
                reformatted_line += str.format(VMEM_FORMAT_STR, data_w_full_ecc)
                address_offset += 1
...
def main(argv: List[str]):
    ...
    parser.add_argument("--in-flash-vmem", type=str, help="Input VMEM file to reformat.")
    parser.add_argument("--in-otp-mmap", type=str, help="OTP memory map HJSON file.")
    parser.add_argument(
        "--in-otp-vmem", type=str, help="Input OTP (VMEM) file to retrieve data from."
    )
    parser.add_argument(
        "--otp-seed",
        type=int,
        help="Configuration override seed used to randomize OTP netlist constants.",
    )
    ...
    if args.in_otp_vmem:
        _get_otp_ctrl_netlist_consts(args.in_otp_mmap, args.otp_seed, scrambling_configs)
        _get_flash_scrambling_configs(args.in_otp_vmem, args.otp_data_perm, scrambling_configs)
    ...
    reformatted_vmem_lines = _reformat_flash_vmem(args.in_flash_vmem, scrambling_configs)
