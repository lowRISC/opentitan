...
def _present_64bit_encrypt(plain, key):
    ...
    assert (plain >= 0) and (plain < 2**64), "Data block is out of 64bit range"

    # Make sure key is within 128bit range
    assert (key >= 0) and (key < 2**128), "Key is out of 128bit range"

    # Make sure inputs are integers
    assert isinstance(plain, int) and isinstance(key, int), "Data and key need to be of type int"
...
def _to_memfile_with_ecc(data, annotation, config, data_perm) -> Tuple[str, int]:
    ...
    assert data_width % 8 == 0, "OTP data width must be a multiple of 8"
    assert data_width <= 64, "OTP data width cannot be larger than 64"
    ...
    log.info("Memory layout is {} x {}bit (with ECC)".format(num_words, bitness))
    ...
    for i_word in range(num_words):
        ...
        line = "@{:06x} {} // {}".format(i_word, word_hex, ", ".join(sorted(word_annotations)))
...
class OtpMemImg(OtpMemMap):
    def __init__(self, lc_state_config, otp_mmap_config, img_config, data_perm):
        ...
        assert enc_test == 0x0E9D28685E671DD6, "Encryption module test failed"
    ...
    def merge_part_data(self, part):
        ...
        if mmap_part is None:
            raise RuntimeError("Partition {} does not exist".format(part["name"]))
        ...
        if part["lock"] and not mmap_part["hw_digest"]:
            raise RuntimeError(
                "Partition {} does not contain a hardware digest".format(part["name"])
            )
        ...
        if part["name"] == "LIFE_CYCLE":
            ...
            if len(part["items"]) > 0:
                raise RuntimeError("Life cycle items cannot directly be overridden")
            if part["lock"]:
            ...
            part["items"] = [
                {"name": "LC_STATE", "value": "0x{:X}".format(state)},
                {"name": "LC_TRANSITION_CNT", "value": "0x{:X}".format(count)},
            ]
    ...
    def merge_item_data(self, part, item):
        ...
        log.info(
            "> Adding {}item {} with size {}B and value{}:".format(
    ...
    def streamout_partition(self, part):
        ...
        if part["secret"]:
            ...
            for key in self.config["scrambling"]["keys"]:
                ...
                raise RuntimeError("Scrambling key cannot be found {}".format(key_sel))

            for k in range(len(data_blocks)):
                if data_block_defined[k]:
                    data_blocks[k] = _present_64bit_encrypt(data_blocks[k], key["value"])
        ...
        if part["hw_digest"]:
            ...
            if data_blocks[-1] != 0:
                raise RuntimeError(
                    "Digest of partition {} cannot be overridden manually".format(part_name)
                )
            ...
            if part.setdefault("lock", False):
                ...
                data_blocks[-1] = _present_64bit_digest(data_blocks[0:-1], iv, const)
            else:
    ...
    def streamout_memfile(self) -> Tuple[str, int]:
        ...
        for part in self.config["partitions"]:
            ...
            assert part["offset"] <= otp_size, "Partition offset out of bounds"
            idx_low = part["offset"]
        ...
        return _to_memfile_with_ecc(data, annotation, self.lc_state.config, self.data_perm)
        ...
        for part in self.config["partitions"]:
            ...
            for item in part["items"]:
                ...
                if item["size"] < alignment:
                    continue
                assert (item["size"] % alignment) == 0, "invalid field alignment"
