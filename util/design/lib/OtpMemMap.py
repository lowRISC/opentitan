...
def _validate_part(part: Dict, key_names: List[str], is_last: bool):
    ...
    if "size" not in part:
        ...
        raise RuntimeError(
            f"{part['name']} declared partition size {part['size']} can't "
            "fit all items, needs {calc_size}")
    elif int(part["size"]) > calc_size:
        log.warning(
            f"{part['name']} declared partition size {part['size']} exceeds "
            f"calculated size {calc_size}")
        log.warning(
