...
def _calc_size(part: Dict, size: int) -> int:
    size = SCRAMBLE_BLOCK_WIDTH * int((size + SCRAMBLE_BLOCK_WIDTH - 1) / SCRAMBLE_BLOCK_WIDTH)
...
def _validate_part(part: Dict, key_names: List[str], is_last: bool):
    ...
    if (part["variant"] == "LifeCycle") != is_last:
        raise RuntimeError("The last partition must always be the life cyclepartition")
    ...
    if check_bool(part["secret"]) and part["key_sel"] == "NoKey":
        raise RuntimeError("A secret partition needs a key select value other than NoKey")
    ...
    if part["sw_digest"] and part["hw_digest"]:
        raise RuntimeError("Partition cannot support both a SW and a HW digest at the same time.")
    ...
    if part["variant"] == "Buffered" and part["read_lock"].lower() == "csr":
        raise RuntimeError("CSR read lock is only supported for SW partitions.")

    if not part["sw_digest"] and not part["hw_digest"]:
        if part["write_lock"].lower() == "digest" or part["read_lock"].lower() == "digest":
            raise RuntimeError(
    ...
    if part["size"] % SCRAMBLE_BLOCK_WIDTH:
        raise RuntimeError(f"Partition size must be {SCRAMBLE_BLOCK_WIDTH * 8}-bit aligned")
...
def _validate_item(item: Dict, buffered: bool, secret: bool):
    ...
    if item["ismubi"]:
        if not is_width_valid(item_width):
            raise RuntimeError("Mubi value {} has invalid width".format(item["name"]))
...
def _validate_mmap(config: Dict) -> Dict:
    ...
    for j, part in enumerate(config["partitions"]):
        if part["name"] in part_dict:
            raise RuntimeError("Partition name {} is not unique".format(part["name"]))

        part["offset"] = offset
        if check_int(part["offset"]) % SCRAMBLE_BLOCK_WIDTH:
            raise RuntimeError(
                f"Partition {part['name']} offset must be {SCRAMBLE_BLOCK_WIDTH * 8}-bit aligned"
            )

        log.info(
            "Partition {} at offset {} size {}".format(part["name"], part["offset"], part["size"])
        )
        ...
        for k, item in enumerate(part["items"]):
            if item["name"] in item_dict:
                raise RuntimeError("Item name {} is not unique".format(item["name"]))
            item["offset"] = offset
            log.info(
                "> Item {} at offset {} with size {}".format(item["name"], offset, item["size"])
            )
        ...
        if part["sw_digest"] or part["hw_digest"]:
            ...
            if digest_name in item_dict:
                raise RuntimeError("Digest name {} is not unique".format(digest_name))
            item_dict[digest_name] = len(part["items"])
            part["items"].append(
                {
                    "name": digest_name,
                    "size": DIGEST_SIZE,
                    "offset": check_int(part["offset"]) + check_int(part["size"]) - DIGEST_SIZE,
                    "ismubi": False,
                    "isdigest": True,
                    "inv_default": "<random>",
            ...
            # of a partition.
            canonical_offset = check_int(part["offset"]) + check_int(part["size"]) - DIGEST_SIZE
...
class OtpMemMap:
    ...
    def create_partitions_table(self) -> str:
        ...
        for part in self.config["partitions"]:
            ...
            if part["integrity"]:
                integrity = "yes"
            desc = part["desc"]
            row = [part["name"], is_secret, is_buffered, integrity, wr_lockable, rd_lockable, desc]
            table.append(row)
    ...
    def create_mmap_table(self) -> str:
        ...
        for k, part in enumerate(self.config["partitions"]):
            for j, item in enumerate(part["items"]):
                ...
                row.extend([name, "0x{:03X}".format(check_int(item["offset"])), str(item["size"])])
    ...
    def create_description_table(self) -> str:
        ...
        for k, part in enumerate(self.config["partitions"]):
            for j, item in enumerate(part["items"]):
                if part["secret"] or part["name"] in {"VENDOR_TEST", "LIFE_CYCLE"}:
                    continue
        ...
        for part in self.config["partitions"]:
            if check_bool(part["hw_digest"]) or check_bool(part["sw_digest"]):
                ...
                for item in part["items"]:
                    ...
                    raise RuntimeError("Partition with digest does not contain a digest item")
    ...
    def get_part(self, part_name) -> str:
        ...
        return None if entry is None else self.config["partitions"][entry["index"]]
    ...
    def get_item(self, part_name, item_name) -> str:
        ...
        if entry is not None:
            ...
            return None if idx is None else self.config["partitions"][entry["index"]]["items"][idx]
        else:
            return None
