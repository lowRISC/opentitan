...
def _get_incremental_codewords(config, base_ecc, existing_words):
    ...
    for k in range(1, 2**free_bits):
        ...
        incr_cand = scatter_bits(base_data, format(k, "0" + str(free_bits) + "b"))
    ...
    while 1:
        ...
        if pop_cnt >= config["min_hw"] and pop_cnt <= config["max_hw"]:
            ...
            for w in existing_words:
                ...
                incr_cands_ecc = _get_incremental_codewords(config, base_cand_ecc, existing_words)
...
def _validate_words(config, words):
    ...
    for k, w in enumerate(words):
        ...
        if not is_valid_codeword(config, w):
            raise RuntimeError("Codeword {} at index {} is not valid".format(w, k))
        # Check that word fulfills the Hamming weight constraints.
        pop_cnt = w.count("1")
        if pop_cnt < config["min_hw"] or pop_cnt > config["max_hw"]:
            raise RuntimeError("Codeword {} at index {} has wrong Hamming weight".format(w, k))
            ...
            for k2, w2 in enumerate(words[k + 1 :]):
                if get_hd(w, w2) < config["min_hd"]:
                    raise RuntimeError(
                        ...
                        "and codeword {} at index {} is too low.".format(w, k, w2, k + 1 + k2)
                    )
...
def _validate_constraints(config):
    ...
    if (
        config["min_hw"] >= total_width
        or config["max_hw"] > total_width
        or config["min_hw"] >= config["max_hw"]
    ):
...
def _validate_tokens(config):
    ...
    if config["token_size"] % 8:
        raise ValueError("Size of token {} must be byte aligned".format(config["token_size"]))
    ...
    for token in config["tokens"]:
        ...
        hashed_token["value"] = int.from_bytes(hashobj.read(num_bytes), byteorder="little")
        ...
        for k, state in enumerate(config[typ].keys()):
            ...
            for j, entry in enumerate(config[typ][state]):
                ...
                if entry not in legal_values:
                    raise RuntimeError(
                        'Illegal entry "{}" found in {} of {}'.format(entry, state, typ)
                    )
...
class LcStEnc:
    ...
    def encode(self, name, state):
        ...
        if state not in self.config[name]:
            raise RuntimeError("Unknown state {} of type {}".format(state, name))

        # Assemble list of state words
        words = []
        for j, entry in enumerate(self.config[name][state]):
            # This creates an index lookup table
            val_idx = {fmt.format(j): i for i, fmt in enumerate(LC_STATE_TYPES[name])}
            idx = val_idx[entry]
            if idx == 0:
                words.append(0)
            else:
