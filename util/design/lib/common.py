...
def validate_data_perm_option(word_bit_length, data_perm):
    ...
    if len(data_perm) != word_bit_length:
        raise RuntimeError(
            ...
            "it does not have the same length ({}) as the data.".format(data_perm, word_bit_length)
        )
    for k in data_perm:
        if k >= word_bit_length:
            raise RuntimeError(
                'Data permutation "{}" is not bijective, '
                "since the index {} is out of bounds.".format(data_perm, k)
            )
    if len(set(data_perm)) != word_bit_length:
        raise RuntimeError(
            'Data permutation "{}" is not bijective, since it contains duplicated indices.'.format(
                data_perm
            )
        )
    ...
    # Check that the range is correct.
        ...
        try:
            ...
            if dict_obj[key] >= 2**num_bits:
                raise RuntimeError('Value "{}" is out of range.'.format(dict_obj[key]))
        except ValueError:
            raise RuntimeError(
                'Invalid value "{}". Must be hex or "<random>".'.format(dict_obj[key])
            )
