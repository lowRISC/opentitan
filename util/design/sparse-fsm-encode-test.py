...
class TestCheckCandidate(unittest.TestCase):
    def setUp(self):
        self.generator = enc.EncodingGenerator(
            min_hd=3, num_states=8, encoding_len=8, seed=1234567890, language="c", avoid_zero=True
        )
    ...
    def test_no_avoid_zero(self):
        noAvoidZeroGenerator = enc.EncodingGenerator(
            min_hd=3, num_states=8, encoding_len=8, seed=1234567890, language="c", avoid_zero=False
        )
...
class TestArgsParsing(unittest.TestCase):
    def test_disallowed_encoding_lengths(self):
        ...
        with unittest.mock.patch(
            ...
            with self.assertRaises(SystemExit) as e, self.assertLogs("root", level="ERROR") as logs:
                enc.main()
            self.assertEqual(e.exception.code, 1)
            self.assertTrue(
                any(
                    "When using C or Rust, widths must be a power-of-two" in log
    ...
    def test_too_few_states(self):
        with unittest.mock.patch(
            ...
            self.assertEqual(e.exception.code, 1)
            self.assertTrue(
                any("Number of states (m) must be at least 2" in log for log in logs.output)
            )
    ...
    def test_encoding_too_small(self):
        with unittest.mock.patch(
            ...
            self.assertEqual(e.exception.code, 1)
            r = (
                r".*State is only [0-9]+ bits wide, which is not enough to "
...
class TestBackwardsCompatibility(unittest.TestCase):
    def test_backwards_compatibility(self):
        ...
        generator = enc.EncodingGenerator(
            min_hd=6, num_states=10, encoding_len=32, seed=3775359077, language="c", avoid_zero=True
        )
        generator.generate()
        self.assertEqual(
            generator.encodings,
