 ...
 */
typedef struct ${ip.name_camel}_reg_info {
  ptrdiff_t offset;
  bitfield_bit32_index_t bit_index;
} ${ip.name_camel}_reg_info_t;
 ...
 */
static ptrdiff_t ${ip.name_camel}_offset_from_reg0(dif_${ip.name_snake}_irq_id_t irq) {
 ...
 * be bit 0).
 */
static uint8_t plic_main_irq_bit_index(dif_${ip.name_snake}_irq_id_t irq) {
 ...
 */
static ptrdiff_t plic_main_irq_enable_base_for_target(
 ...
 */
static ptrdiff_t plic_main_software_irq_base_for_target(
    dif_${ip.name_snake}_target_t target) {
 ...
 */
static ptrdiff_t plic_main_threshold_base_for_target(
    dif_${ip.name_snake}_target_t target) {
 ...
 */
static plic_main_reg_info_t plic_main_irq_enable_reg_info(
    dif_${ip.name_snake}_irq_id_t irq, dif_${ip.name_snake}_target_t target) {
  ptrdiff_t offset = plic_main_offset_from_reg0(irq);
  return (plic_main_reg_info_t){
      .offset = plic_main_irq_enable_base_for_target(target) + offset,
      .bit_index = plic_main_irq_bit_index(irq),
  };
}
 ...
 */
static plic_main_reg_info_t plic_main_irq_pending_reg_info(
 ...
 */
static ptrdiff_t plic_main_priority_reg_offset(dif_${ip.name_snake}_irq_id_t irq) {
...
dif_result_t dif_${ip.name_snake}_reset(const dif_${ip.name_snake}_t *plic) {
  ...
  for (uint32_t i = 0; i < ${ip.name_upper}_PARAM_NUM_SRC; ++i) {
    ptrdiff_t offset = plic_main_priority_reg_offset(i);
  ...
  for (dif_${ip.name_snake}_target_t target = 0;
    ...
    ptrdiff_t offset = plic_main_irq_enable_base_for_target(target);
    ...
    offset = plic_main_threshold_base_for_target(target);
    mmio_region_write32(plic->base_addr, offset, 0);

    // Clear software interrupt registers.
    offset = plic_main_software_irq_base_for_target(target);
    mmio_region_write32(plic->base_addr, offset, 0);
  }
...
dif_result_t dif_${ip.name_snake}_irq_get_enabled(const dif_${ip.name_snake}_t *plic,
  ...
  plic_main_reg_info_t reg_info = plic_main_irq_enable_reg_info(irq, target);
...
dif_result_t dif_${ip.name_snake}_irq_set_priority(const dif_${ip.name_snake}_t *plic,
  ...
  ptrdiff_t offset = plic_main_priority_reg_offset(irq);
...
dif_result_t dif_${ip.name_snake}_target_set_threshold(
  ...
  ptrdiff_t threshold_offset = plic_main_threshold_base_for_target(target);
...
dif_result_t dif_${ip.name_snake}_irq_is_pending(const dif_${ip.name_snake}_t *plic,
  ...
  plic_main_reg_info_t reg_info = plic_main_irq_pending_reg_info(irq);
  uint32_t reg = mmio_region_read32(plic->base_addr, reg_info.offset);
...
dif_result_t dif_${ip.name_snake}_software_irq_force(const dif_${ip.name_snake}_t *plic,
  ...
  ptrdiff_t msip_offset = plic_main_software_irq_base_for_target(target);
