...
% for ip in ips_with_difs:
  % if ip.irqs:
    ...
    % endif
      ...
      // Get the IRQ type. We rely on this function returning an error if
      // the irq is invalid (ie was not triggered by this peripheral).
      dif_irq_type_t type;
      dif_result_t res = dif_${ip.name_snake}_irq_get_type(
        ${ip.name_snake}_ctx.${ip.name_snake}, irq, &type);
