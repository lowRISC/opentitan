...
It will instantiate:
- `sw/ip/<ip>/dif/dif_template.h.tpl` as the DIF Header boilerplate
  (into `dif_<ip>.h`), which should be manually edited/enhanced.
- `sw/ip/<ip>/dif/autogen/dif_autogen.h.tpl` as the autogenerated DIF
  DIF Header (into `dif_<ip>_autogen.h`).
- `sw/ip/<ip>/dif/autogen/dif_autogen.c.tpl` as the autogenerated DIF
  implementation (into `dif_<ip>_autogen.c`).
- `sw/ip/<ip>/dif/autogen/dif_autogen_unittest.cc.tpl` as the
  autogenerated DIF unit tests (into `dif_<ip>_autogen_unittest.cc`).
- `doc/project_governance/checklist/sw_checklist.md.tpl` as the DIF Checklist (into dif_<ip>.md),
...
def main():
    ...
    if args.mode == "regen":
        ...
        for autogen_src_filename in glob.iglob(
                str(REPO_TOP / "sw/ip/*/dif/autogen/*.c")):
    ...
    for ip in ips:
        ip_dir = REPO_TOP / f"sw/ip/{ip.name_snake}"
        dif_dir = ip_dir / "dif"
        autogen_dif_dir = dif_dir / "autogen"
        testutils_dir = ip_dir / "test" / "utils"

        # Create output directories if needed.
        if len(args.only) > 0:
            dif_dir.mkdir(exist_ok=True, parents=True)
            autogen_dif_dir.mkdir(exist_ok=True)
            if ip.name_snake.startswith("rv_plic"):
                testutils_dir.mkdir(exist_ok=True, parents=True)

        if "header" in args.only:
            if ip.name_snake.startswith("rv_plic"):
                header_template_file = (
                    REPO_TOP / "util/make_new_dif/templates/dif_rv_plic.h.tpl")
            else:
                header_template_file = (
        ...
        if "autogen" in args.only:
            ...
            if ip.name_snake.startswith("rv_plic"):
                templates = [
                    (REPO_TOP / "util/make_new_dif/templates/dif_rv_plic.c.tpl"),
                    (REPO_TOP / "util/make_new_dif/templates/BUILD_dif.tpl"),
        ...
        try:
            filtered_paths = [p for p in out_files if p.suffix in ['.h', '.c', '.cc']]
            subprocess.check_call(
                ["./bazelisk.sh", "run", "//quality:clang_format_fix", "--"] + filtered_paths
            )
