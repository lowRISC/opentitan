...
class BazelAqueryResults:
    ...
    def __init__(self, output: str):
        ...
        self.actions = [BazelAqueryAction(action) for action in parsed["actions"]]
...
class PathBuilder:
    ...
    def __init__(self, script_path):
        ...
        self.bazel_exec_root = os.path.join(self.top_dir, f"bazel-{os.path.basename(self.top_dir)}")


def build_compile_commands(paths: PathBuilder, device_build: bool) -> Tuple[List[Dict], List[Dict]]:
    ...
    try:
        completed_process = subprocess.run(
            bazel_aquery_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True
        )
    ...
    aquery_results = BazelAqueryResults(completed_process.stdout.decode("utf-8"))
    ...
    for action in aquery_results.actions:
        ...
        for artifact in aquery_results.iter_artifacts_for_dep_sets(action.input_dep_set_ids):
            command = {
...
def main(args):
    ...
    if not args.audit_device_cc_flags:
        device_commands, device_unittest_commands = build_compile_commands(paths, device_build=True)
        host_commands, host_unittest_commands = build_compile_commands(paths, device_build=False)
    else:
        all_device_commands, _ = build_compile_commands(paths, device_build=True)
        ...
        logger.warning(
            "Audit found %d sources that are missing required flags", len(set(troubled_sources))
        )
    ...
    all_compile_commands = (
        device_commands + host_commands + host_unittest_commands + device_unittest_commands
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", default="//...", help='Bazel target. Default is "//...".')
    parser.add_argument(
        "--audit_device_cc_flags",
        action="store_true",
        help="When specified, only non-test device sources missing required flags will be emitted.",
    )
    parser.add_argument("--out", help="Path of output file for compilation DB. Defaults to stdout.")
