...
def delete_rule(lines, rule_name, target_name, file_name):
    try:
        ...
        assert False, f"did not find beginning of rule {rule_name} (target name {target_name}) in {file_name}"
    try:
        end_idx = lines.index(')\n', start_idx + 1)
    except ValueError:
        assert False, f"did not find end of rule {rule_name} (target name {target_name}) in {file_name}"
...
def main():
    ...
    parser.add_argument('-t', '--top', action='append', default=[], help='Path to the top to edit (optional,c an be specified several times)')
    ...
    parser.add_argument('--no-regs', action='store_true', help='Do not generate headers for this block')
    ...
    assert ip_dir.exists() and ip_dir.is_dir(), "IP directory invalid (must exists and be a directory)"
    ip_name = ip_dir.name
    project_root = Path(args.root if args.root is not None else Path.cwd()).resolve()
    assert project_root.exists() and project_root.is_dir() and (project_root / 'WORKSPACE').exists(), "project root directory invalid"
    ...
    if hjson_path is not None:
        assert hjson_path.exists() and hjson_path.is_file(), f"HSJON path {hjson_path} invalid (must exists and be a file)"
        # Find the BUILD file for the hsjon.
        ...
        hjson_build, all_files_idx, _ = delete_rule(hjson_build, "filegroup", 'name = "all_files"', hjson_build_path)
        # Add hsjon filegroup instead
        ...
        hjson_bazel_target = "//{}:{}".format(hjson_build_path.parent.relative_to(project_root),
    ...
    try:
        ...
        assert False, f"did not find visibility statement in {ip_build_path}, not touching this file"
    # Insert new load.
    ip_build.insert(vis_index, '\n')
    ip_build.insert(vis_index, 'load("//rules/opentitan:hw.bzl", "opentitan_ip")\n')
    # Find and delete the 'all_files' filegroup.
    ip_build, all_files_idx, _ = delete_rule(ip_build, "filegroup", 'name = "all_files"', ip_build_path)
    ...
    ip_build = ip_build[:all_files_idx] + \
        [
            ...
            '    name = "{}",\n'.format(ip_name),
            '    doc = glob(["doc/**"]),\n',
            '    hjson = "{}",\n'.format(hjson_bazel_target) if hjson_bazel_target is not None else '',
            '    rtl = glob([\n',
            '        "dv/**",\n',
            '        "rtl/**",\n',
            '        "lint/**",\n',
    ...
    if args.git:
        subprocess.run(
            ["git", "commit" ,"-vas", "-m", f"[bazel,{ip_name}] Use new rules to describe IP",
             "-m", commit_msg],
            check=True,
            cwd = project_root
        )
    ...
    try:
        ...
        assert False, f"did not find end of IP list in {hw_top_build_path}, not touching this file"
    # Add IP.
    ...
    for top in args.top:
        ...
        try:
            ...
            assert False, f"did not find end of opentitan_top in {top_build_path}, not touching this file"
        # Find 'ips' list inside.
        try:
            ot_top_ips_index = top_build.index('    ips = [\n', ot_top_index, ot_top_end_index)
        except ValueError:
            assert False, f"did not find ip list in opentitan_top in {top_build_path}, not touching this file"
    ...
    if not args.no_regs:
        ...
        hjson_build, _, _ = delete_rule(hjson_build, 'load', '"//rules:autogen.bzl"', hjson_build_path)
        # Remove autogen_hjson_c_header and autogen_hjson_rust_header
        hjson_build, _, _ = delete_rule(hjson_build, 'autogen_hjson_c_header', None, hjson_build_path)
        hjson_build, _, _ = delete_rule(hjson_build, 'autogen_hjson_rust_header', None, hjson_build_path)
        hjson_build_path.write_text(''.join(hjson_build))
    ...
    if args.git:
        subprocess.run(
            ["git", "commit", "-vas", "-m", f"[bazel] Replace old {ip_name} targets by new one",
             "-m", commit_msg],
            check=True,
            cwd = project_root
        )
