...
class FlowCfg:
    ...
    def _load_child_cfg(self, entry, mk_config):
        ...
        if type(entry) is str:
            ...
            log.error(
                'Type of entry "%s" in the "use_cfgs" key is invalid: %s', entry, str(type(entry))
            )
            sys.exit(1)
    ...
    def _process_overrides(self):
        ...
        if hasattr(self, "overrides"):
            ...
            if type(overrides) is not list:
                log.error(
                    'The type of key "overrides" is %s - it should be a list', type(overrides)
                )
                sys.exit(1)
    ...
    def prune_selected_cfgs(self):
        ...
        if not self.is_primary_cfg:
            log.error(
                "The configuration file at {!r} is not a primary "
    ...
    def _publish_results(self, results_server: ResultsServer):
        ...
        try:
            ...
            log.error(
                "Failed to list {} with gsutil. Acting as if there was nothing.".format(
                    self.rel_path
                )
            )
            existing_paths = []
        ...
        for existing_basename in existing_basenames[:max_old_page_links]:
            relative_url = "../{}/{}".format(existing_basename, self.results_html_name)
            history_txt += "- [{}]({})\n".format(existing_basename, relative_url)
        ...
        dst_no_suffix = "{}/latest/{}".format(self.rel_path, html_name_no_suffix)
    ...
    def publish_results(self):
        ...
        try:
            ...
            log.error("Google Cloud SDK not installed. Cannot access the results server")
            return
