...
def read_max_parallel(arg):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Bad argument for --max-parallel ({!r}): must be a positive integer.".format(arg)
        )
...
def copy_repo(src, dest):
    ...
    try:
        ...
        log.error("Failed to copy over %s to %s: %s", src, dest, e.stderr.decode("utf-8").strip())
    log.info("Done.")
...
def parse_reseed_multiplier(as_str: str) -> float:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Invalid reseed multiplier: {!r}. Must be a float.".format(as_str)
        )
    if ret <= 0:
...
def parse_args():
    ...
    disg.add_argument(
        "--remote", action="store_true", help=("Trigger copying of the repo to scratch area.")
    )
    ...
    pathg.add_argument(
        "--purge", action="store_true", help="Clean the scratch directory before running."
    )
    ...
    pubg.add_argument(
        "--publish", action="store_true", help="Publish results to reports.opentitan.org."
    )
