...
class ResultsServer:
    ...
    def get_creation_time(self, path: str) -> Optional[datetime.datetime]:
        ...
        try:
            ...
            log.error("Could not parse creation time ({}) from GCP".format(timestamp))
            return None
    ...
    def mv(self, from_path: str, to_path: str) -> None:
        ...
        try:
            ...
            # place of safety!
            log.error("Failed to use gsutil to move {} to {}".format(from_path, to_path))
            raise
    ...
    def upload(self, local_path: str, dst_path: str, recursive: bool = False) -> None:
        ...
        try:
            ...
            if recursive:
                sub_cmd.append("-r")
            subprocess.run(
                ["gsutil"] + sub_cmd + [local_path, self._path_in_bucket(dst_path)], check=True
            )
        except subprocess.CalledProcessError:
            # If we failed to copy the file, print an error message but
            # otherwise keep going. We don't want our failed upload to kill the
            # rest of the job.
            log.error("Failed to use gsutil to copy {} to {}".format(local_path, dst_path))
