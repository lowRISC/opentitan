...
def _stringify_wildcard_value(value):
    ...
    try:
        ...
        raise ValueError("Wildcard had value {!r} which is not of a supported type.".format(value))
...
def _subst_wildcards(var, mdict, ignored, ignore_error, seen):
    ...
    while True:
        ...
        if name in seen:
            raise ValueError(
                "String contains circular expansion of wildcard {!r}.".format(match.group(0))
            )
        ...
        if value is None:
            ...
            raise ValueError(
                "String to be expanded contains unknown wildcard, {!r}.".format(match.group(0))
            )
...
def find_and_substitute_wildcards(sub_dict, full_dict, ignored_wildcards=[], ignore_error=False):
    """
            ...
            for i in range(len(sub_dict_key_values)):
                if type(sub_dict_key_values[i]) in [dict, OrderedDict]:
                    ...
                    sub_dict_key_values[i] = find_and_substitute_wildcards(
                        sub_dict_key_values[i], full_dict, ignored_wildcards, ignore_error
                    )
...
def md_results_to_html(title, css_file, md_text):
    ...
    html_text = transform(html_text, external_styles=css_file, cssutils_logging_level=log.ERROR)
    return html_text
