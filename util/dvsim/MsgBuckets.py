...
class MsgBuckets:
    ...
    def get_labels(self, severity_filter: List[str] = []) -> List[str]:
        """
    ...
    def get_keys(self, severity_filter: List[str] = []) -> List[str]:
        """
    ...
    def get_counts(self, keys: List[str] = None, severity_filter: List[str] = []) -> List[int]:
        """
    ...
    def get_counts_md(
        self, keys: List[str] = None, severity_filter: List[str] = [], colmap: bool = True
    ) -> List[str]:
        """
    ...
    def has_signatures(self, severity_filter: List[str] = []) -> bool:
        """
    ...
    def print_signatures_md(self, severity_filter: List[str] = [], max_per_bucket: int = -1) -> str:
        """
        ...
        for k in keys:
            msgs += print_msg_list(
                f"#### {self.buckets[k].label}", self.buckets[k].signatures, max_per_bucket
            )
        return msgs
    ...
    def load_hjson(self, result_path: Path) -> None:
        ...
        for k, signatures in results_dict.items():
            if not isinstance(signatures, List):
                raise RuntimeError(f"Signatures in {k} must be a list of strings")
