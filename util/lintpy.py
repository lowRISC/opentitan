...
def run_linter(tool, dofix, verbose, files):
    ...
    subprocess.check_call(fix + files, stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL)
    return (True, False)
...
def main():
    ...
    parser.add_argument("--fix", action="store_true", help="Fix files detected with problems")
    parser.add_argument(
        "--hook", action="store_true", help="Install as ../.git/hooks/pre-commit and exit"
    )
    parser.add_argument(
        "-f",
        "--file",
