...
def cmdgen_rewrite_md(filepath: Path, dry_run: bool, update: bool) -> bool:
    ...
    while True:
        ...
        res = subprocess.run(cmd, shell=True, capture_output=True, cwd=REPO_ROOT)
        if res.stderr:
            logger.info(
        ...
        if new_content != old_content:
            if update:
                logger.info(f"{rel_path}:L{match_start_linum}: Updating generated content.")
                # replace content
                content = (
                    content[0 : match_start.end(0)] + new_content + content[match_end.start(0) :]
                )
                modified = True
                # update position to account for new content size
                pos += len(new_content) - len(old_content)
            else:
                logger.info(f"{rel_path}:L{match_start_linum}: Generated content needs updating.")
                needs_updating = True
...
def main() -> int:
    ...
    parser.add_argument(
        "-u",
        "--update",
        help="Update out of date content.",
        action="store_true",
    )
