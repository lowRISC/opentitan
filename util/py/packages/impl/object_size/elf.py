...
def get_segment_of_section(elf: elffile.ELFFile, section):
    for segment in elf.iter_segments():
        if segment.section_in_segment(section):
            return segment
    raise RuntimeError(f"Could not find the segment that {section.name} belongs to")
    return None


def parse_section(section: elf.sections.Section, segment: elf.segments.Segment) -> Section:
    vma = section["sh_addr"]
...
def parse_symbol(sym, elf, sections):
    ...
    if not section:
        log.info(f"Skipping symbol {sym.name} from skipped section {section_name}")
        return None
    size = sym["st_size"]
    if size == 0:
...
def parse_elf_file(path: Path, memories: dict[str, Memory]) -> dict[str, Section]:
    sections = {}
    with path.open("rb") as f:
        elf = elffile.ELFFile(f)
        for section in elf.iter_sections():
            if (
                section["sh_type"] not in ["SHT_PROGBITS", "SHT_NOBITS"]
