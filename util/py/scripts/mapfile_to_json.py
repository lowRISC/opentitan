...
parser = argparse.ArgumentParser(description="Mapfile converter and visualizer")
parser.add_argument("--json", default="", help="Emit the mapfile as json to the named file")
parser.add_argument("--html", default="", help="Emit a mapfile visualization to the named file")
parser.add_argument(
    "--regions", default="rom,ram_main,eflash", help="Which memory regions to analyze"
)
parser.add_argument("mapfile", metavar="MAPFILE", type=str, help="Mapfile to process")
parser.add_argument(
    "-v",
    "--view",
    default=False,
...
class Mapfile(object):
    ...
    def parse_sections_gnu(self, f):
        ...
        sub_re = re.compile("[ ]{16}0x(?P<offset>[0-9a-f]+)[ ]+(?P<function>.+)\n+", re.I)
        f = f.read()
        pos = 0
        sections = []
        while True:
                ...
                if section.startswith(" "):
                    ...
                    while True:
                        if m := sub_re.match(f, pos):
                            ...
                            if "=" in func or "ASSERT" in func or "ALIGN" in func:
                                continue
                            if sections:
    ...
    def parse(self, filename):
        ...
        with open(filename) as f:
            ...
            if linker == self.GNU_LD:
                ...
                raise Exception("Failed to detect type of mapfile (clang or gnu)")
    ...
    def by_filename(self):
        ...
        for region in top["children"]:
            for section in region["children"]:
                ...
                section["children"] = [
                    dict(name="[{}]".format(name), children=info) for name, info in files.items()
                ]
        return top
...
def main(args):
    m = Mapfile(regions=args.regions.split(","), ignore_debug=args.ignore_debug)
    ...
    if args.html or args.view:
        filename = (
            args.html if args.html else os.path.basename(args.mapfile).replace(".map", "") + ".html"
        )
        width, height = args.dimensions.split("x")
        with open(filename, "wt") as f:
            f.write(
                D3_TEMPLATE.replace("@DATA@", text)
                .replace("@WIDTH@", width)
