...
parser = argparse.ArgumentParser(description="Bitstream Cache Entry Generator")
parser.add_argument("fragment", nargs="+", help="Path to manifest fragment file")
parser.add_argument("--schema", required=True, help="Path to validation schema")
parser.add_argument(
    "--stamp-file", required=True, help="A file supplying the stamp in BUILD_SCM_REVISION"
)
...
def create_archive(outfile: str, manifest: Dict, file_map: Dict):
    ...
    tar.add(manifest, arcname="manifest.json", filter=reset_info)
    for file_path, short_path in file_map.items():
        tar.add(file_path, arcname=short_path, filter=reset_info)
    tar.close()
