...
A `clocks.srcs` entry normally describes a clock coming into the top from outside.
A clock can instead be declared as generated by an IP instance inside the top by adding `module`,
`signal` and `partition` keys.
For example, the following clock would be generated by the primary partition of the `ast` module:

```hjson
{ name: "main", aon: "no", freq: "100000000",
  module: "ast", signal: "clk_src_sys", partition: "primary" },
```

- `module` is the name of the instance that produces the clock.
- `signal` is the name of that instance's inter-signal carrying the clock.
- `partition` is the partition of `module` that owns the signal (`primary` or `secondary`, see
  [Split IPs](#split-ips)).
  - It may be omitted for a non-split IP, in which case `primary` is assumed.

The referenced signal must resolve to exactly one inter-signal on `module` (qualified by `partition` where needed).
That inter-signal must be a `uni`/`req`/`logic` inter-signal.
