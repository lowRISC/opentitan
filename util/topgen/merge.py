...
def elaborate_instance(instance, block: IpBlock):
    ...
    for param in block.params.by_name.values():
        ...
        if param.name.lower().startswith("sec") and not param_expose:
            log.warning(
                "{} has security-critical parameter {} not exposed to top".format(
                    mod_name, param.name
                )
            )
        ...
        for prefix in param_prefixes:
            if not param.name.startswith(prefix):
                ...
                if param.name == prefix:
                    raise ValueError(
                        f"Module instance {mod_name} has a "
        ...
        if isinstance(param, RandParameter):
            ...
            if key in instance["memory"]:
                ...
                log.error(
                    "Missing memory configuration for memory {} in instance {}".format(
                        key, instance["name"]
                    )
                )
    ...
    for decl in param_decl_accounting:
        log.error(
            "{} is not a valid parameter of {} that can be set from top level".format(
                decl, block.name
            )
        )
        ...
        if isinstance(s["width"], Parameter):
            for p in instance["param_list"]:
                if p["name"] == s["width"].name:
                    ...
                    # single mangled parameter. Since parameters are instance dependent, that would
    ...
    if base_addrs is None:
        if "base_addr" not in instance:
            raise ValueError(
                "Instance {!r} has neither a base_addr nor a base_addrs field.".format(
                    instance["name"]
                )
            )
        else:
            ...
            if len(block.reg_blocks) != 1:
                raise ValueError(
                    "Instance {!r} has a base_addr field but it "
                    "instantiates the block {!r}, which has {} "
                    "device interfaces.".format(instance["name"], block.name, len(block.reg_blocks))
                )
            else:
        ...
        if "base_addr" in instance:
            log.error(
                "Instance {!r} has both a base_addr and a base_addrs field.".format(
                    instance["name"]
                )
            )
        ...
        if block_if_names != inst_if_names:
            log.error(
                "Instance {!r} has a base_addrs field with keys {} "
                "but the block it instantiates ({!r}) has device "
                "interfaces {}.".format(instance["name"], inst_if_names, block.name, block_if_names)
            )
...
def xbar_addhost(top, xbar, host):
    ...
    if len(obj) == 0:
        log.warning("host %s doesn't exist in the node list. Using default values" % host)
        obj = OrderedDict(
            [
                ("name", host),
                ("clock", xbar["clock"]),
    ...
    obj[0]["req_fifo_pass"] = (
        obj[0]["req_fifo_pass"] if obj[0]["pipeline"] and "req_fifo_pass" in obj[0] else True
    )
    obj[0]["rsp_fifo_pass"] = (
        obj[0]["rsp_fifo_pass"] if obj[0]["pipeline"] and "rsp_fifo_pass" in obj[0] else True
    )
...
def process_pipeline_var(node):
    ...
    node["req_fifo_pass"] = node["req_fifo_pass"] if "req_fifo_pass" in node else True
    node["req_fifo_pass"] = node["req_fifo_pass"] if "req_fifo_pass" in node else True
...
def xbar_adddevice(
    ...
    instances = [node for node in top["module"] + top["memory"] if node["name"] == device_base]
    ...
    log.info(
        "Handling xbar device {} (matches instance? {}; matches node? {})".format(
            device, inst is not None, node is not None
        )
    )

    # case 1: another xbar --> check in xbar list
    if node is None and device in other_xbars:
        log.error("Another crossbar %s needs to be specified in the 'nodes' list" % device)
        return
    ...
    if inst is None:
        ...
        if device in other_xbars:
            log.info(
                "device {} in Xbar {} is connected to another Xbar".format(device, xbar["name"])
            )
    ...
    base_addrs, size_byte = lib.get_base_and_size(name_to_block, inst, device_ifname)
    addr_range = {
        "base_addrs": {asid: hex(base_addr) for (asid, base_addr) in base_addrs.items()},
    ...
    if node is None:
        log.error(
            "Cannot connect to {!r} because the crossbar defines no node for {!r}.".format(
                device, device_base
            )
        )
        return
...
def amend_xbar(top: Dict[str, object], name_to_block: Dict[str, IpBlock], xbar: Dict[str, object]):
    ...
    addr_spaces = {x["addr_space"] for x in topxbar["nodes"] if "addr_space" in x}
...
def xbar_cross(xbar, xbars):
    ...
    addr = []
    for node in [
        x for x in xbar["nodes"] if x["type"] == "device" and "xbar" in x and x["xbar"] is False
    ]:
        addr.extend(node["addr_range"])

    # Step 2: visit xbar device ports
    xbar_nodes = [
        x for x in xbar["nodes"] if x["type"] == "device" and "xbar" in x and x["xbar"] is True
    ]
    ...
    for node in host_xbar["nodes"]:
        ...
        if "xbar" in node and node["xbar"] is True:
            if "addr_range" not in node:
                ...
                (_asid, xbar_addr) = xbar_cross_node(node["name"], host_xbar, xbars, visited, asid)
...
def _get_clock_group_name(clk: Union[str, OrderedDict], default_ep_grp: str) -> Tuple[str, str]:
...
def connect_clocks(top, name_to_block: Dict[str, IpBlock]):
    ...
    for clk_name in typed_clocks.hint_names().keys():
        ...
        if len(ep_names) != 1:
            raise ValueError(
                f"There are {len(ep_names)} end-points connected "
        ...
        if ep_inst is None:
            raise ValueError(
                f"No module instance with name {ep_name}: only "
        ...
        for ep_name, ep_port in sig.endpoints:
            ...
            for item in ip_block.clocking.items:
                ...
                if item.idle is None:
                    raise ValueError(
                        f"Cannot connect the {sig.name} clock to "
        ...
        if not idle_signal.endswith("_o"):
            raise ValueError(
                f"Idle signal for {ep_port} of {ep_name} is "
...
def amend_resets(top, name_to_block, allow_missing_blocks=False):
    ...
    top_resets = (
        top["resets"] if isinstance(top["resets"], Resets) else Resets(top["resets"], top["clocks"])
    )
    ...
    if rstmgr_name is None and allow_missing_blocks:
        ...
        for intf in top["exported_rsts"]:
            top["inter_module"]["external"][f"{rstmgr_name}.resets_{intf}"] = "rsts_{}".format(intf)
...
def get_interrupt_modules(
    top: OrderedDict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False
):
...
def commit_interrupt_modules(top: OrderedDict, name_to_block: Dict[str, IpBlock]):
...
def commit_outgoing_alert_modules(top: OrderedDict, name_to_block: Dict[str, IpBlock]):
    """Ensure top['outgoing_alert_module'] is populated in the final config."""
    top["outgoing_alert_module"] = get_outgoing_alert_modules(top, name_to_block)


def amend_alert(top: OrderedDict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False):
    ...
    outgoing_modules = get_outgoing_alert_modules(top, name_to_block, allow_missing_blocks)
    ...
    for m in alert_modules + list(chain(*outgoing_modules.values())):
        ...
        for alert in block.alerts:
            ...
            alert_dict["async"] = "1"
            qual_sig = lib.add_module_prefix_to_signal(alert_dict, module=m.lower())
    ...
    if missing_ips:
        raise SystemExit(
            "The following IPs contributing alerts cannot be found: , ".join(missing_ips)
        )
...
def amend_wkup(topcfg: OrderedDict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False):
    ...
    for m in topcfg["module"]:
        ...
        for signal in block.wakeups:
            log.info("Adding wakeup signal %s from module %s", signal.name, m["name"])
            wakeups.append(
                {"name": signal.name, "width": str(signal.bits.width()), "module": m["name"]}
            )
    ...
    if pwrmgr_name:
        ...
        signal_names = [f"{s['module'].lower()}.{s['name'].lower()}" for s in topcfg["wakeups"]]
        topcfg["inter_module"]["connect"][f"{pwrmgr_name}.wakeups"] = signal_names
        log.info("Intermodule signals: {}".format(topcfg["inter_module"]["connect"]))


# Handle reset requests from modules
def amend_reset_request(
    topcfg: OrderedDict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False
):
    ...
    if pwrmgr_name:
        ...
        topcfg["inter_module"]["connect"][f"{pwrmgr_name}.rstreqs"] = signal_names
    log.info("Intermodule signals: {}".format(topcfg["inter_module"]["connect"]))
...
def amend_pinmux_io(top: Dict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False):
    ...
    for sig in pinmux["signals"]:
        ...
        if sig["port"] != "":
            ...
            if idx >= 0 and idx >= sig_inst["width"]:
                raise SystemExit(
                    "Index {} is out of bounds for signal {} with width {}.".format(
    ...
    pinmux["io_counts"] = {
        "dedicated": {"inouts": 0, "inputs": 0, "outputs": 0, "pads": 0},
    ...
    for sig in pinmux["ios"]:
        ...
        if sig["pad"] in known_mapped_dio_pads:
            raise SystemExit(
                "Cannot have multiple IOs mapped to the same DIO pad {}".format(sig["pad"])
            )
    ...
    for target in targets:
        for entry in target["pinmux"]["special_signals"]:
            ...
            # there needs to exist a DIO connection.
                ...
                idx = (
                    known_mapped_dio_pads[entry["pad"]]["glob_idx"]
...
def amend_racl(top_cfg: OrderedDict, name_to_block: Dict[str, IpBlock], allow_missing_blocks=False):
    ...
    for m in top_cfg["module"]:
        ...
        for if_name, mapping_path in racl_mappings.items():
            ...
            parsed_register_mapping, parsed_window_mapping, parsed_range_mapping, racl_group, _ = (
                parse_racl_mapping(
                    top_cfg["racl"], top_cfg["cfg_path"] / mapping_path, if_name, block
                )
            )
...
def merge_top(
    topcfg: OrderedDict, name_to_block: Dict[str, IpBlock], xbarobjs: OrderedDict
) -> OrderedDict:
