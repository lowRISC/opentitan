...
def create_pinout_table(top, c_helper, target):
    ...
    for sig in special_signals:
        special_pads.update({sig["pad"]: {"name": sig["name"], "desc": sig["desc"]}})
    ...
    ret_str = tabulate(table_rows, headers="firstrow", tablefmt="pipe", colalign=colalign)
...
def create_pinmux_table(top, c_helper):
    ...
    for sig in top["pinmux"]["ios"]:
        ...
        if sig["connection"] == "muxed" and sig["type"] in ["inout", "input"]:
            ...
            periph_in = "-"
        row = [port, sig["connection"], pad, outsel + " / " + periph_in, sig["desc"]]
        table_rows.append(set_md_table_font_size(row, "smaller"))

    return tabulate(table_rows, headers="firstrow", tablefmt="pipe", colalign=colalign)
...
def gen_pinmux_docs(top, c_helper, out_path):
    ...
    gencmd = (
        "util/topgen.py -t hw/top_{topname}/data/top_{topname}.hjson -o hw/top_{topname}/".format(
            topname=top["name"]
        )
    )
    ...
    summary_table += tabulate(table_rows, headers="firstrow", tablefmt="pipe", colalign=colalign)
    summary_table += "\n"
