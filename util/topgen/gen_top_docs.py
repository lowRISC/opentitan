...
def gen_address_map_docs(top, c_helper, out_path):
    """Generate address map documentation for all address spaces.
    ...
    template_path = Path(__file__).parent / 'templates' / 'address_map.md.tpl'
    template = Template(filename=str(template_path))

    address_map_content = template.render(
        top=top,
        helper=c_helper,
        gencmd=gencmd
    )

    address_map_path = doc_path / 'address_map.md'
    address_map_path.write_text(address_map_content)


def gen_top_docs(top, c_helper, out_path):
    gen_address_map_docs(top, c_helper, out_path)
