...
from .resets import UnmanagedResets, Resets
...
def _check_equivalent(actual: object, loaded: object, path: str) -> (bool, str):
    ...
    assert type(actual) == type(loaded) or type_ok, f"objects at {path} do not have " + \
        "the same type: expected {}, got {}".format(type(actual), type(loaded))  # noqa: E721

    if isinstance(actual, list):
        assert len(actual) == len(loaded), f"lists at {path} do not have the same length: " + \
            "expected {}, got {}".format(len(actual), len(loaded))
        for i in range(len(actual)):
            _check_equivalent(actual[i], loaded[i], f"{path}[{i}]")

    elif isinstance(actual, dict) or isinstance(actual, OrderedDict):
        if actual.keys() != loaded.keys():
            msg = f"objects at {path}.keys() are not equal: "
            expected = [key for key in actual.keys() if key not in loaded.keys()]
            if len(expected) > 0:
                msg += f"expected keys {expected} "
            extra = [key for key in loaded.keys() if key not in actual.keys()]
            if len(extra) > 0:
                msg += f"did not expect keys {extra} "
            assert False, msg
        ...
        assert actual == loaded, f"objects at {path} are not equal: " + \
            "expected {}, got {}".format(actual, loaded)
...
class CompleteTopCfg:
    ...
    def from_hjson(topcfg: Dict[str, object]) -> object:
        ...
        topcfg['unmanaged_resets'] = UnmanagedResets.fromdict(topcfg['unmanaged_resets'])
