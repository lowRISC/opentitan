...
top_required = {
    ...
    "memory": ["l", "list of memories. At least one memory is needed to run the software"],
...
top_added = {
    ...
    "wakeups": ["l", "list of wakeup requests each holding name, width, and module"],
...
pinmux_sig_required = {
    ...
    "connection": ["s", "Specification of connection type, can be direct, manual or muxed"],
}
pinmux_sig_optional = {
...
pinout_required = {"banks": ["l", "List of IO power banks"], "pads": ["l", "List of pads"]}
pinout_optional = {}
pinout_added = {
...
module_optional = {
    ...
    "clock_reset_export": [
        "l",
        "optional list with prefixes for exported clocks and resets at the chip level",
    ],
    "attr": [
        "s",
        'optional attribute indicating whether the IP is "ipgen", "reggen_top", or "reggen_only"',
    ],
    "base_addr": [
        "g",
        "dict of address space mapped to the corresponding "
        "hex start address of the peripheral "
        "(if the IP has only a single TL-UL interface)",
    ],
    "base_addrs": [
        "g",
        "hex start addresses of the peripheral  (if the IP has multiple TL-UL interfaces)",
    ],
    ...
    "generate_dif": [
        "pb",
        "optional bool to indicate if a DIF should be generated for that module",
    ],
    "outgoing_alert": [
        "s",
        "optional string to indicate alerts are routed externally to the named group",
    ],
    "incoming_alert": [
        "l",
        "optional list of paths to incoming alert configurations for the alert_handler",
    ],
    "ipgen_param": ["g", "Optional ipgen parameters for that instance"],
    "template_type": ["s", "Base template type of ipgen IPs"],
    "racl_group": [
        "s",
        "Only valid for racl_ctrl IPs. Defines the RACL group this control IP is associated to",
    ],
    "racl_mappings": ["g", "dict that maps an interface to its associated RACL mapping"],
    "racl_mapping": [
        "s",
        "A special case of racl_mappings. If specified, this is taken to "
...
memory_optional = {
    "size": [
        "d",
        "memory region size in bytes for the linker script, xbar and RTL parameterisations",
    ],
    "config": ["d", "Extra configuration for a particular memory"],
    "data_intg_passthru": ["pb", "Integrity bits are passed through directly from the memory"],
}
...
reset_requests_optional = {
    "int": ["s", "internal request list, for example, escalation reset and power glitches"],
    "debug": ["s", "debug request list, since a different set of resets becomes active"],
    "peripheral": [
        "s",
        "peripheral request list, where the reset requests are explicit in the top config",
    ],
}
...
inter_sig_required = {
    ...
    "type": ["s", "whether the signal is unidirectional or part of a request-response pair"],
...
class Flash:
    ...
    def check_values(self):
        ...
        if not pow2_check:
            raise ValueError(
                "flash power of 2 check failed. A supplied parameter is not power of 2"
            )
...
def check_pad(
    top: Dict, pad: Dict, known_pad_names: Dict, valid_connections: List[str], prefix: str
) -> int:
    error = 0
    ...
    if pad["connection"] not in valid_connections:
        log.warning(
            "Connection type {} of pad {} is invalid".format(pad["connection"], pad["name"])
        )
        error += 1
...
def check_alerts(top: Dict, prefix: str) -> int:
    ...
    for alert in top["alert"]:
        error += check_keys(alert, alert_required, alert_optional, alert_added, prefix + " Alert")
    return error
...
def check_pinout(top: Dict, prefix: str) -> int:
    error = check_keys(
        top["pinout"], pinout_required, pinout_optional, pinout_added, prefix + " Pinout"
    )

    known_names = {}
    for pad in top["pinout"]["pads"]:
        error += check_keys(pad, pad_required, pad_optional, pad_added, prefix + " Pinout")

        error += check_pad(top, pad, known_names, ["direct", "manual", "muxed"], prefix + " Pad")
...
def check_pinmux(top: Dict, prefix: str) -> int:
    error = check_keys(
        top["pinmux"], pinmux_required, pinmux_optional, pinmux_added, prefix + " Pinmux"
    )
    ...
    for sig in top["pinmux"]["signals"]:
        ...
        if padname != "":
            if padname in known_direct_pads:
                if known_direct_pads[padname] == 1:
                    ...
                    log.warning("Warning, direct pad {} is already connected".format(padname))
                    error += 1
            else:
        ...
        if sig["connection"] == "direct":
            ...
            if padname != "":
                log.warning(
                    "Muxed connection of instance {} port {} "
                    "must not have an associated pad".format(sig["instance"], sig["port"])
                )
                error += 1
    ...
    for io in top.get("ios", []):
        error += check_keys(
            io, pinmux_io_required, pinmux_io_optional, pinmux_io_added, f"{prefix} Pinmux ios"
        )
...
def check_implementation_targets(top: Dict, prefix: str) -> int:
    ...
    for target in top["targets"]:
        ...
        for pad in target["pinout"]["add_pads"]:
            error += check_pad(top, pad, known_pad_names, ["manual"], prefix + " Additional Pad")
...
def check_clocks_resets(top, ip_name_to_block, xbar_name_to_block):
    ...
    if unmanaged_resets:
        if isinstance(unmanaged_resets, UnmanagedResets):
            ...
            unmanaged_reset_nets = [net for reset in unmanaged_resets for net in reset.values()]
    ...
    for ipcfg in top["module"]:
        ...
        error += validate_reset(
            ipcfg, ip_name_to_block[ipcfg_name], reset_nets, unmanaged_reset_nets
        )
        error += validate_clock(
            ipcfg, ip_name_to_block[ipcfg_name], clock_srcs, unmanaged_clock_srcs
        )
    ...
    for xbarcfg in top["xbar"]:
        ...
        error += validate_reset(
            xbarcfg, xbar_name_to_block[xbarcfg_name], reset_nets, unmanaged_reset_nets, "xbar"
        )
        error += validate_clock(
            xbarcfg, xbar_name_to_block[xbarcfg_name], clock_srcs, unmanaged_clock_srcs, "xbar"
        )
...
def check_reset_requests(top: Dict, component: str) -> int:
    ...
    for key, resets in top.get("reset_requests", {}).items():
        all_keys = [
            k
            for d in [reset_requests_required, reset_requests_optional, reset_requests_added]
            for k in d.keys()
        ]
...
def check_wakeups(top: Dict, component: str) -> int:
    ...
    for wakeup in top.get("wakeups", []):
        error += check_keys(
            wakeup, wakeup_required, wakeup_optional, wakeup_added, f"{component} Wakeups"
        )
    return error
...
def validate_reset(top, inst, reset_nets, unmanaged_reset_nets, prefix=""):
    ...
    if isinstance(inst, IpBlock):
        ...
        name = inst["name"]
        reset_signals = [inst.get("reset_primary", "rst_ni")] + inst.get("other_reset_list", [])
    ...
    for port, reset in top["reset_connections"].items():
        if isinstance(reset, str):
            ...
            if len(top["domain"]) > 1:
                raise ValueError(f"{top['name']} reset connection {reset} has no assigned domain")
            else:
    ...
    missing_port = [port for port in top["reset_connections"].keys() if port not in reset_signals]
    ...
    missing_net = [
        net["name"]
        for net in top["reset_connections"].values()
...
def validate_clock(top, inst, clock_srcs, unmanaged_clock_srcs, prefix=""):
    ...
    if isinstance(inst, IpBlock):
        ...
        name = inst["name"]
        clock_signals = [inst.get("clock_primary", "rst_ni")] + inst.get("other_clock_list", [])

    if len(top["clock_srcs"]) != len(clock_signals):
        error += 1
        log.error("%s %s mismatched number of clock ports and nets" % (prefix, name))

    missing_port = [port for port in top["clock_srcs"].keys() if port not in clock_signals]
...
def check_power_domains(top):
    ...
    if top["power"]["default"] not in top["power"]["domains"]:
        raise ValueError(f"Default power domain {top['power']['default']} is not a valid domain")
...
def check_modules(top, prefix):
    ...
    for m in top["module"]:
        ...
        error += check_keys(
            m, module_required, module_optional, module_added, prefix + " " + modname
        )

        # these fields are mutually exclusive
        if "base_addr" in m and "base_addrs" in m:
            log.error(
                "{} {} a module cannot define both the 'base_addr' "
                "and 'base_addrs' keys at the same time".format(prefix, modname)
            )
            error += 1
            ...
            for intf, value in m["memory"].items():
                ...
                if "size" not in value:
                    ...
                    if mem_type == "flash":
                        ...
                        raise ValueError(
                            f"{m['name']} memory config declaration does not have a valid type"
                        )
                ...
                if attr not in ["ro", "rw"]:
                    log.error(
                        "{} {} swaccess attribute {} of memory region {} is not valid".format(
                            prefix, modname, attr, intf
                        )
                    )
                    error += 1
