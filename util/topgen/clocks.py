...
def _bool_from_yn(val: str, what: str) -> bool:
    if val == 'yes':
        return True
    if val == 'no':
        return False

    raise ValueError('{} is not a boolean: {} (expected "yes" or "no")'
                     .format(what, val))
...
class SourceClock:
    ...
    def fromdict(cls, src: Dict[str, object]) -> object:
        src['aon'] = _bool_from_yn(src['aon'], f"SourceClock from {src}: aon field")
...
class Group:
    ...
    @classmethod
    def fromdict(cls, src: Dict[str, object], clocks: Dict[str, SourceClock]) -> object:
        src['unique'] = _bool_from_yn(src['unique'], f"Group from {src}: unique field")
        src['clocks'] = {
