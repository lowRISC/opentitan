...
class ModuleClockRef(NamedTuple):
    '''A deferred reference to a clock generated by an IP inside the top.

    This is a placeholder for a `clock_connections` entry when the source clock's net cannot be
    resolved yet because the producing module's IpBlock isn't yet in the topgen datastructure. A
    later pass replaces these with the real net name.
    '''
    module: str
    signal: str
    partition: object
...
class SourceClock:
    ...
    This is either an input to the top-level or a clock generated by an IP inside the top.'''
    ...
    def __init__(self, raw: Dict[str, object]):
        ...
        self.signal = raw.get('signal', None)
        self.partition = raw.get('partition', None)
