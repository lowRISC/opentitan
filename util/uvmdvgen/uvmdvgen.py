...
def main():
    ...
    if re.search(r"\W", args.name):
        log.error("The block name '%s' contains non-alphanumeric characters.", args.name)
        sys.exit(1)
    ...
    if not args.has_ral and (args.is_cip or args.has_interrupts):
        args.has_ral = True
        print("NOTE: --has_ral switch is enabled since either --is_cip or --has_interrupts is set.")

    if args.gen_agent:
        gen_agent.gen_agent(
            args.name, args.has_separate_host_device_driver, args.agent_outdir, args.vendor
        )
