...
typedef struct dt_${device_name} {
  struct {
    ...
    uint32_t base_addrs[${helper.reg_block_enum.name.as_c_enum()}Count];
% if helper.has_irqs():
    // This can be kDtPlicIrqIdNone if the block is not connected to the PLIC.
    dt_plic_irq_id_t first_irq;
% endif
    dt_clock_t clocks[${helper.clock_enum.name.as_c_enum()}Count];
% if helper.has_pins():
    dt_pin_t pins[${helper.pin_enum.name.as_c_enum()}Count];
% endif
  } __internal;
} dt_${device_name}_t;
