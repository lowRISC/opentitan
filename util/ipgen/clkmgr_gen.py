...
def _clock_src_vars(obj) -> ConfigT:
    """Return a SourceClock's attributes as a dict.

    Omits fields only relevant to clocks generated by an IP instance inside the top.
    This ensures that tops that don't have internal clock sources see no change to their generated
    ipconfig.
    """
    attrs = vars(obj).copy()
    if attrs.get('module') is None:
        del attrs['module']
        del attrs['signal']
        del attrs['partition']
    return attrs
...
def get_clkmgr_params(top: ConfigT) -> ParamsT:
    ...
    ipgen_params.update({
        "src_clks":
        OrderedDict({name: _clock_src_vars(obj)
                     for name, obj in clocks.srcs.items()}),
        "derived_clks":
        OrderedDict(
            {name: _clock_src_vars(obj)
