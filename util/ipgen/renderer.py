...
from .lib import IpConfig, IpTemplate, TemplateParseError
...
class IpTemplateRendererBase:
    ...
    def _render_mako_template_to_str(self, template_filepath: Path) -> str:
        ...
        helper_funcs = {}
        helper_funcs['unique_vlnv'] = self._tplfunc_unique_vlnv
        ...
        tpl_args = {
            "instance_name": self.ip_config.instance_name,
            **helper_funcs,
...
class IpBlockRenderer(IpTemplateRendererBase):
    ...
    - Copy everything but templates ending with '.tpl' into the destination
      directory.
    - Process all Mako templates (files ending in '.tpl') and place them into
      the output directory. The '.tpl' suffix is stripped off.
    ...
    def render(self, output_dir: Path, overwrite_output_dir: bool) -> None:
        ...
        # Prepare the IP directory in a staging area to later atomically
        ...
        try:
            ...
            if do_overwrite:
                shutil.rmtree(output_dir_existing_bak, ignore_errors=True)
        finally:
            # Ensure that the staging directory is removed at the end.
