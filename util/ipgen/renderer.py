...
class IpTemplateRendererBase:
        ...
        for name, template_param in self.ip_template.params.items():
            ...
            try:
                ...
                raise TemplateRenderError(
                    ...
                    f"to {template_param.param_type}."
                ) from None
    ...
    def _get_mako_template_lookup(self) -> MakoTemplateLookup:
        ...
        if self._lookup is None:
            ...
            self._lookup = MakoTemplateLookup(
                directories=[str(self.ip_template.template_path)], strict_undefined=True
            )
        return self._lookup
    ...
    def _tplfunc_instance_vlnv(self, template_vlnv_str: str, prefix: str = None) -> str:
        ...
        template_core_version = template_vlnv[3] if len(template_vlnv) == 4 else None
    ...
    def _render_mako_template_to_str(self, template_filepath: Path) -> str:
        ...
        template_filepath_rel = template_filepath.relative_to(self.ip_template.template_path)
        ...
        try:
            ...
            raise TemplateRenderError(
                "Unable to render template: " + mako_exceptions.text_error_template().render(),
                self.get_template_parameter_values(),
            ) from None

    def _render_mako_template_to_file(self, template_filepath: Path, outdir_path: Path) -> None:
        ...
        try:
            ...
            raise TemplateRenderError(f"Cannot write to {outdir_path / outfile_name!s}: {e!s}")
    ...
    def _filename_without_tpl_suffix(self, filepath: Path) -> str:
        ...
        # Only render the module instance name if the original template file starts with
        # the template name. Otherwise, leave the filename untouched
        if (
            filename.startswith(self.ip_template.name)
            and "module_instance_name" in self.ip_config.param_values
        ):
            filename = (
                self.ip_config.param_values["module_instance_name"]
                + filename[len(self.ip_template.name) :]
            )
        return filename
...
class IpDescriptionOnlyRenderer(IpTemplateRendererBase):
    ...
    def render(self) -> str:
        ...
        """
        hjson_tpl_path = (
            self.ip_template.template_path / "data" / f"{self.ip_template.name}.hjson.tpl"
        )
        if hjson_tpl_path.is_file():
            return self._render_mako_template_to_str(hjson_tpl_path)

        raise TemplateRenderError(f"No IP description template at {hjson_tpl_path}")
...
class IpBlockRenderer(IpTemplateRendererBase):
    ...
    def _refresh_directory(
        self, staging_dir: Path, output_dir: Path, overwrite_output_dir: bool
    ) -> None:
        ...
        output_dir_existing_bak = output_dir.with_suffix(".bak~" + str(int(time.time())))
        if do_overwrite:
            try:
                os.rename(output_dir, output_dir_existing_bak)
            except OSError as e:
                msg = f"Cannot move existing directory {output_dir} to {output_dir_existing_bak}."
                raise TemplateRenderError(msg).with_traceback(e.__traceback__)

        try:
            os.rename(staging_dir, output_dir)
        except OSError as e:
            msg = f"Cannot move staging directory {staging_dir} to {output_dir}."
            raise TemplateRenderError(msg).with_traceback(e.__traceback__)
    ...
    def render(self, output_dir: Path, overwrite_output_dir: bool) -> None:
        ...
        if not overwrite_output_dir and output_dir.exists():
            raise TemplateRenderError(
                "Output directory '{}' exists and should not be overwritten.".format(output_dir)
            )

        staging_dir = output_dir.parents[0] / f".~{output_dir.stem}.staging"
        if staging_dir.is_dir():
            raise TemplateRenderError(
                f"Output staging directory '{staging_dir}' already exists: remove it and try again."
            )
        ...
        try:
            for template_filepath in template_path.glob("**/*.tpl"):
                template_filepath_rel = template_filepath.relative_to(template_path)
                ...
                self._render_mako_template_to_file(template_filepath, outdir_path)
        except Exception:
        ...
        self.ip_config.to_file(
            staging_dir / "data" / f"{self.ip_config.instance_name}.ipconfig.hjson",
