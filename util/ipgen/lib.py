...
def _parse_template_parameter(where: str, raw: object) -> TemplateParameter:
    ...
    if not param_type in TemplateParameter.VALID_PARAM_TYPES:
...
class IpConfig:
    def __init__(self,
                 ...
                 instance_name: str,
                 param_values: Dict[str, Union[str, int, object]] = {}):
        self.template_params = template_params
        self.instance_name = instance_name
        # TODO: Check for valid data types when setting param_value (as we do
        # in the from_raw() case below.)
    ...
    def from_raw(cls, template_params: TemplateParams, raw: object, where: str) -> 'IpConfig':
        ...
        if not isinstance(raw, dict):
            raise ValueError(
                f"The IP configuration is expected to be a dict, but was "
                "actually a {type(raw).__name__}.")

        param_values = {}
        for key, value in rd['param_values'].items():
            ...
            if template_params[key].param_type == 'string':
                param_value_typed = check_str(value,
                    f"the key {key} of the IP configuration")
            elif template_params[key].param_type == 'int':
                param_value_typed = check_int(value,
                    f"the key {key} of the IP configuration")
            elif template_params[key].param_type == 'object':
                param_value_typed = value
            else:
                assert True, "Unexpeced parameter type found, expand this check"

            param_values[key] = param_value_typed
