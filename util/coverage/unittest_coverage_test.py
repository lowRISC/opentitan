...
class TestUnit(unittest.TestCase):
    ...
    def test_handle_libs(self, mock_run):
        ...
        self.assertEqual(unittest_coverage.handle_libs(device_libs_all), device_libs)
    ...
    def test_handle_objs(self, mock_run):
        ...
        mock_run.assert_called_once_with(
            "<lld_host>", "--shared", "-o", str(merged_library), "//foo/a.o", "//foo/d.o"
        )
    ...
    def test_handle_test_log_dirs(self):
        ...
        profiles = [Path("/testlogs/foo/coverage.dat"), Path("/testlogs/bar/coverage.dat")]
        self.assertEqual(unittest_coverage.handle_test_log_dirs(test_log_dirs), profiles)

    def test_params(self):
        self.assertEqual(unittest_coverage.PARAMS.bazel_test_type, AnyStringWith("cc_test"))
        self.assertEqual(unittest_coverage.PARAMS.config, "ot_coverage_off_target")
        self.assertEqual(unittest_coverage.PARAMS.libs_fn, unittest_coverage.handle_libs)
        self.assertEqual(unittest_coverage.PARAMS.objs_fn, unittest_coverage.handle_objs)
        self.assertEqual(
            unittest_coverage.PARAMS.test_targets_fn, unittest_coverage.handle_test_targets
        )
        self.assertEqual(
            unittest_coverage.PARAMS.test_log_dirs_fn, unittest_coverage.handle_test_log_dirs
        )
        self.assertEqual(unittest_coverage.PARAMS.report_title, AnyStringWith("Unit Test"))
