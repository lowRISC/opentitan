...
from common import LLVM_COV, LLVM_PROFDATA, CoverageType, LogLevel, artifacts_relpath, run
...
def add_test_type_to_combined_report(out_root_dir: Path = Path("coverage")) -> None:
    ...
    def covered_rows(path: Path):
        ...
        def second_col_non_zero(row: Any) -> bool:
            ...
            if row.name == "tr":
                ...
                return len(cols) > 1 and cols[1].getText().strip().lower() not in ["0", "", "count"]
            return False

        return BeautifulSoup(path.read_text(), "html.parser").find_all(second_col_non_zero)
    ...
    def add_back_link(target_row, combined_rows, string: str, relpath) -> None:
        ...
        pre.append(string)
        link_to_target = combined_html.new_tag("a", href=relpath + link_in_target["href"])
    ...
    for combined_file in combined.rglob("*.html"):
        ...
        combined_rows = combined_html.find("table", recursive=True).contents[1:]
        ...
        for row in combined_rows[1:]:
            row.insert(1, combined_html.new_tag("td", style="text-align: center;"))
...
def combined_report(
    ...
    run(LLVM_PROFDATA, "merge", "--sparse", "--output", str(merged_profile), *profiles)
