...
class TestCommon(unittest.TestCase):
    ...
    def test_artifacts_relpath(self, mock_run):
        ...
        self.assertEqual(calls_run[1][0], ("git", "show", "-s", "--format=%ct", "HEAD"))
    ...
    def test_instrument_device_libs(self, mock_run):
        ...
        self.assertEqual(common.instrument_device_libs(device_libs, "qux"), obj_files)
        calls_run = mock_run.call_args_list
        self.assertEqual(len(calls_run), 2)
        self.assertEqual(calls_run[0][0], (common.BAZEL, "build", "--config=qux", *device_libs))
    ...
    def test_get_test_log_dirs(self, mock_run):
        ...
        self.assertEqual(
            ...
            [Path("/qux/testlogs/foo/bar"), Path("/qux/testlogs/foo/baz")],
        )
        mock_run.assert_called_once_with(common.BAZEL, "info", "bazel-testlogs")

    @patch("common.print")
    @patch("pathlib.Path.open", new_callable=mock_open)
    @patch("pathlib.Path.__truediv__", side_effect=Path.__truediv__, autospec=True)
    @patch("common.run")
    def test_generate_report(self, mock_run, mock_path_div, mock_path_open, mock_print):
        for print_text_report in (True, False):
            ...
            with self.subTest(f"print_text_report:{print_text_report}"):
                ...
                text_report_content = ["<contents of coverage report in text format>"]
                # (1) html report, (2) text report, (3) print text report (if requested)
                mock_run.side_effect = ([], text_report_content, text_report_content)

                common.generate_report(
                    out_dir, merged_profile, merged_library, report_title, print_text_report
                )
                ...
                mock_path_open().write.assert_called_once_with(*text_report_content)
                ...
                if print_text_report:
                    ...
                    self.assertEqual(calls_print[1][0], AnyStringWith("coverage artifacts in"))
                else:
                    self.assertEqual(len(calls_run), 2)
                    mock_print.assert_called_once_with(AnyStringWith("coverage artifacts in"))
    ...
    def test_measure_coverage(
        ...
        mock_path_mkdir.assert_called_once_with(out_dir, parents=True, exist_ok=True)
        calls_run = mock_run.call_args_list
        self.assertEqual(len(calls_run), 4)
        self.assertEqual(calls_run[0][0], (common.BAZEL, "query", common.DEVICE_LIBS_QUERY))
        mock_libs_fn.assert_called_once_with(device_libs_all)
        mock_instrument_device_libs.assert_called_once_with(device_libs, config)
        mock_objs_fn.assert_called_once_with(out_dir / "merged.so", obj_files)
        self.assertEqual(
            calls_run[1][0], (common.BAZEL, "query", AnyStringWith(bazel_test_type.value))
        )
