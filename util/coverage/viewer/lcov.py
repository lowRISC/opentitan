...
from typing import Dict, List, Iterator, NamedTuple, Optional, Set
...
class FileProfile:
    ...
    sf: str
    fn: Dict[str, Set[int]] = field(default_factory=dict)  # name -> [lineno]
        ...
        if tag == 'SF':
            ...
            if current_profile and ',' in params:
                try:
                    ...
                    if f_name not in current_profile.fn:
                        current_profile.fn[f_name] = set()
                    current_profile.fn[f_name].add(int(f_lineno_str))
                except ValueError:
                    continue
        elif tag == 'FNDA':
            if current_profile and ',' in params:
