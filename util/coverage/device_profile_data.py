...
def extract_profile_data(device_output):
    ...
    """
    device_output = re.sub(r"^.*] (.*)$", r"\1", device_output, flags=re.MULTILINE)
    device_output = device_output.translate(device_output.maketrans("", "", "\r\n"))
    match = re.search(
        r"""
    ...
    if not match:
        raise ValueError("Could not detect LLVM profile data in device output.")
    exp_length = int(match.group("len"))
    exp_checksum = int(match.group("crc"), 0)
    byte_array = int(match.group("data"), 0).to_bytes(
        len(match.group("data")) // 2 - 1, byteorder="little", signed=False
    )
    # Check length
    act_length = len(byte_array)
    if act_length != exp_length:
        raise ValueError(
            ("Length check failed! ", f"Expected: {exp_length}, actual: {act_length}.")
        )
    # Check checksum
    act_checksum = zlib.crc32(byte_array)
    if act_checksum != exp_checksum:
        raise ValueError(
            ("Checksum check failed! ", f"Expected: {exp_checksum}, actual: {act_checksum}.")
        )
    return byte_array
...
def main():
    ...
    output."""
    argparser = argparse.ArgumentParser(description="Extract LLVM profile data from device output.")
    ...
    argparser.add_argument(
        "--input_file", type=argparse.FileType("rb"), default=sys.stdin.buffer, help="device output"
    )
    args = argparser.parse_args()

    args.output_file.write(extract_profile_data(args.input_file.read().decode("ascii", "ignore")))
