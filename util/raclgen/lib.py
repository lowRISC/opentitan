...
mapping_optional = {
    ...
    "ranges": [
        "l",
        "List, specifying the policy for each range."
...
def format_parameter_name_prefix(
    module_name: str, racl_group: str = None, if_name: str = None
) -> str:
...
def parse_racl_config(config_path: str) -> Dict[str, object]:
    ...
    racl_config["nr_ctn_uid_bits"] = (
        racl_config["ctn_uid_bit_msb"] - racl_config["ctn_uid_bit_lsb"] + 1
    )
    ...
    assert (
        racl_config["role_bit_lsb"] > racl_config["ctn_uid_bit_msb"]
        or racl_config["ctn_uid_bit_lsb"] > racl_config["role_bit_msb"]
    )
...
def parse_racl_mapping(
    racl_config: Dict[str, object], mapping_path: str, if_name: Optional[str], ip_block: IpBlock
    ...
    if len(mapping) != 1:
        raise SystemExit(
            "Mapping file must be a single-element dict mapping "
    ...
    def policy_name_to_idx(policy_name: str) -> int:
        try:
            ...
            raise SystemExit(
                f"RACL policy {policy_name} not defined in RACL config for group {racl_group}"
            )
