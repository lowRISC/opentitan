...
def main():
    ...
    if not args.format in SUPPORTED_FORMAT:
        log.error("Format {} is not supported by the tool".format(args.format))
        raise SystemExit()
    ...
    with args.input:
        try:
            log.info("Reading the Hjson file {}.".format(args.input.name))
    ...
    if args.param != '' and len(params) != 0:
        ...
        log.info("  Received parameters: {}".format(params))
        config["unroll"] = True
