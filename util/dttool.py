...
def render_template(template_path: Path, rendered_path: Path, **other_info):
...
def main():
    ...
    parser.add_argument(
        "--topgencfg", "-t", type=Path, required=True, help="`top_{name}.gen.hjson` file."
    )
    parser.add_argument(
        "--ip",
        "-i",
    ...
    parser.add_argument("--gen-ip", action="store_true", help="Generate IP-level files (dt_<ip>.h)")
    parser.add_argument(
        "--default-node",
        action="store",
        default=None,
        ...
        for ipname, ip in name_to_block.items():
            ...
            if default_node is None:
                ...
                if len(ip.reg_blocks) > 1:
                    logging.warning(
                        f"IP {ipname} has more than one register block node "
