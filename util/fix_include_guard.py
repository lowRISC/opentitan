...
def main():
    ...
    parser.add_argument("headers", type=str, nargs="+", help="headers to fix guards for")
    ...
    for header_path in args.headers:
        ...
        if first_line_match is None:
            total_unfixable += 1
            print("No non-comment line found in `{}'.".format(header), file=sys.stderr)
            continue
        ...
        if ifndef_match is None:
            total_unfixable += 1
            print(
                "Unsupported first non-comment line in `{}': {!r}.".format(header, first_line),
                file=sys.stderr,
            )
            continue
    ...
    if total_unfixable:
        print("Failed to fix {} files.".format(total_unfixable), file=sys.stderr)
