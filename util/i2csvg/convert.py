...
I2cOp = namedtuple("I2cOp", "read rcont start stop nackok mvalue adr size fbyte tag")
...
def parse_i2c_fifodata(line):
    ...
    if serr:
        err = True
    return I2cOp(resr, resc, ress, resp, resn, resm, resa, size, resb, None), err
...
def parse_i2c_code(line):
    ...
    if firstval == len(line):
        ...
        if resm:
            ...
            rest = None
            resb, verr = check_int(line[firstval:], "Value in " + line + " " + str(firstval))
            if verr:
                err = True
    ...
    if serr:
        err = True
    return I2cOp(resr, resc, ress, resp, resn, resm, resa, size, resb, rest), err
...
def svg_use(item, x, y):
    return '    <use href="#' + item + '" x="' + str(x) + '" y="' + str(y) + '" />\n'
...
def svg_element(tr, xpos, ypos):
    ...
    if tr.read and not tr.mvalue:
        for n in range(0, 1 if tr.tag else tr.fbyte):
            ...
            t, xpos = svg_rdbyte(xpos, ypos, acktype, tr.tag if tr.tag else "D" + str(n + 1))
            etext += t
        ...
        t, xpos = svg_wrbyte(xpos, ypos, tr.nackok, tr.tag if tr.tag else hex(tr.fbyte))
        etext += t
...
def out_svg(outfile, svg, ypos, svgtext):
    global first_svg
    outfile.write("<svg\n" + sdata.svgtag_consts)
    outfile.write('viewBox="0 0 ' + str(sdata.svgw) + " " + str(ypos + sdata.linesep + 8) + '">\n')
    if first_svg:
