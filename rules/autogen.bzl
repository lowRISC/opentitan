...
def opentitan_top_dt_gen(name, gen_ips = [], gen_top = False, output_groups = {}, target_compatible_with = []):
    opentitan_top_dt_gen_rule(
        ...
        ips_hjson = flatten([
            opentitan_select_ip_attr(ip, "hjson", required = False, default = [], to_list = True)
            for ip in gen_ips
        ]),
        ips_config = flatten([
            opentitan_select_ip_attr(ip, "ipconfig", required = False, default = [], to_list = True)
            for ip in gen_ips
        ]),
        extensions = flatten([
            opentitan_select_ip_attr(ip, "extension", required = False, default = [], to_list = True)
            for ip in gen_ips
        ]),
