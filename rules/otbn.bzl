...
def _otbn_assemble_sources(ctx, additional_srcs = []):
    """Helper function that, in the provided context, adds an action to the
    context that invokes the otbn assember (otbn_as.py), producing a
    corresponding object file by assembling the provided src files.
    Returns an object file that will be generated by these actions.
    """
    ...
    files = ctx.files.srcs + additional_srcs
    paths = [src.path for src in ctx.files.srcs + additional_srcs]
    ...
    ctx.actions.run(
        ...
        arguments = ["-o", obj.path] + paths + ctx.attr.args,
...
def _otbn_library(ctx):
    """Produces an object file used as a dependency for otbn binaries."""
    obj = _otbn_assemble_sources(ctx)

    return [
        DefaultInfo(
            files = depset([obj]),
            data_runfiles = ctx.runfiles(files = [obj]),
        ),
    ]
...
def _otbn_binary(ctx, additional_srcs = []):
    ...
    # Run the otbn assembler on source files to produce an object (.o) file.
    obj = _otbn_assemble_sources(ctx, additional_srcs)
...
def otbn_sim_testgen(name, testgen, number, srcs = [], deps = [], includes = [], type = "expectation", **kwargs):
