...
OpenTitanTopInfo = provider(
    ...
    fields = {
        ...
        "ip_alias": "dictionary of IPs aliases files (dict: string => file)",
    },
)
...
def _describe_top(ctx):
    ...
    for (hjson, ipname) in ctx.attr.ip_hjson.items():
    ...
    for (alias, ipname) in ctx.attr.ip_alias.items():
        if len(alias[DefaultInfo].files.to_list()) != 1:
            fail("IP {} in top {} must provide exactly one alias file".format(ipname, ctx.attr.topname))

        # Extract the file from the Target.
        ip_alias[ipname] = alias[DefaultInfo].files.to_list()[0]
...
describe_top_rule = rule(
    ...
    attrs = {
        ...
        "ip_hjson": attr.label_keyed_string_dict(allow_files = True, doc = "mapping from hjson files to IP name"),
        "ip_alias": attr.label_keyed_string_dict(allow_files = True, doc = "mapping from alias files to IP name"),
...
def describe_top(name, all_tops, top):
    ...
    for _top in all_tops:
        ...
        for ip in _top.ips:
            all_hjson[ip.hjson] = ip.name
            if ip.alias:
                all_aliases[ip.alias] = ip.name
