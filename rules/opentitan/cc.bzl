...
def _opentitan_binary_blob(ctx):
    """
    Creates a static library containing the compiled library as a hashed binary
    blob, resolving function calls via injected ELF symbols.

    The following steps are performed to create the final library:
    - Generate a linker script to pack the required functions and anchor them at address 0.
    - Link the dependencies into an initial ELF executable using the generated script.
    - Extract the raw binary from the executable.
    - Compute the SHA-384 hash of the binary and append it to the end of the blob.
    - Extract the memory offsets of the required public functions from the ELF.
    - Wrap the hashed binary blob into a new object file and inject the function offsets as ELF symbols.
    - Archive the final object file into a static library.
    """
    ...
    # Generate linker script to pack memory and force function retention
    template_file = ctx.file._linker_script_template
    linker_script = ctx.actions.declare_file(ctx.attr.name + "_linker_script.ld")

    # If a config file is provided, append EXTERN(func_name) for each line
    # to the checked-in static template.
    ...
    # Link the initial ELF using the generated script
    ...
    # Extract symbols from the ELF to an arguments file
    ...
    # Create the final object file wrapping the hashed binary and the symbols
    blob_o_file = ctx.actions.declare_file(name_blob_o)
    ctx.actions.run(
        outputs = [blob_o_file],
        inputs = [binary_hash_file, sym_args_file],
        executable = ctx.executable._riscv32_objcopy,
        arguments = [
            "-I",
            "binary",
            ...
            "elf32-littleriscv",
            "--add-section",
            ".text.blob=" + binary_hash_file.path,
            "--set-section-flags",
            ".text.blob=alloc,readonly,code",
            "--add-symbol",
            "cryptolib_blob=.text.blob:0,global,object",
            "@" + sym_args_file.path,
            binary_hash_file.path,
            blob_o_file.path,
        ],
    )
