...
Our exec\_envs include the `sim_verilator`, `sim_dv` and `fpga_cw340` environments (among others).
...
For example, the `fpga_cw340_test_rom` environment is used to execute test programs on the CW340 board with the test program linked in flash memory at the ROM\_EXT stage and executed after the `test_rom`.
There may be several additional environments for the `fpga_cw340` that execute the test program under different conditions (ie: under the real ROM or under the ROM\_EXT).
...
For example,
  `fpga_cw340_binary`, `fpga_cw340_mapfile`, `fpga_cw340_disassembly`, etc.
...
- `uart_smoketest_fpga_cw340_test_rom`
...
This is because tests can specify additional parameters via a params block (ie: `fpga_params(...)`). A dict value of `None` instructs the macro to fetch the params block from the default-named argument for the exec\_env (ie: the `fpga_cw340` exec\_env gets it's param block from the `cw340` parameter).
Any other dict str-value instructs the macro to find a parameter with the same name as the value.
This allows us to dispatch a test to multiple `fpga_cw340_...` exec\_envs with different parameter blocks (as needed).
