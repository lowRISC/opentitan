...
class TestBitstreamCache(unittest.TestCase):
    ...
    def test_get_from_cache(self):
        ...
        MOCKED_OS_WALK_RETURN = [
            ...
            ("cache/abcd", [], [MOCK_BITSTREAM, "manifest.json", "memories.mmi"]),
        ]
        os.walk = unittest.mock.MagicMock(name="os.walk", return_value=MOCKED_OS_WALK_RETURN)

        cache = BitstreamCache("/", "/tmp/cache/opentitan-bitstreams", "latest.txt", offline=True)
    ...
    def test_write_build_file(self):
        ...
        MOCKED_OS_WALK_RETURN = [
            # os.walk() yields tuples of the form (root, dir, files).
            ("cache/abcd", [], [BITSTREAM_ORIG, BITSTREAM_SPLICE, "manifest.json", "memories.mmi"]),
        ]
        os.walk = unittest.mock.MagicMock(name="os.walk", return_value=MOCKED_OS_WALK_RETURN)

        BitstreamCache._GetDateTimeStr = unittest.mock.MagicMock(
            name="BitstreamCache._GetDateTimeStr", return_value="2022-07-14T15:02:54.463801"
        )
...
class TestFetchAvailableBitstreams(unittest.TestCase):
    ...
    def setUp(self):
        self.cache = BitstreamCache("/", "/tmp/cache/opentitan-bitstreams", "latest.txt")
