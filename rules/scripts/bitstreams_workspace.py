...
KNOWN_DESIGNS = {
    "chip_earlgrey_cw340": {
        "bitstream": "@lowrisc_opentitan//hw/bitstream/vivado:fpga_cw340_test_rom",
        "mmi": "@lowrisc_opentitan//hw/bitstream/vivado:cw340_mmi",
    },
}
...
class BitstreamCache(object):
    ...
    def ListLocalCacheEntries(self) -> list[str]:
        """Return the list of local entries in the cache.

        Returns:
            A sorted list of entry keys.
        """
        return [
            entry_name for entry_name in sorted(self.available.keys())
            if self.available[entry_name] == "local"
        ]
    ...
    def _ConstructBazelString(self, default_key: str) -> str:
        ...
        local_cache_entries = self.ListLocalCacheEntries()
        if default_key not in local_cache_entries:
            local_cache_entries.append(default_key)
        ...
        for entry_name in local_cache_entries:
            ...
            if manifest_path is None:
                ...
                manifest_path = os.path.join("cache", entry_name,
        ...
        for design_name in sorted(KNOWN_DESIGNS.keys()):
            for target_ext, alias in KNOWN_DESIGNS[design_name].items():
                ...
                # requesting a non-existent bitstreamd does not silently fallsback to default.
        ...
        # Also create a selecting alias for the manifest. Note that there is no meaningful manifest
        # in the default case because.
        bazel_lines += [
            'alias(',
            '    name = "manifest",',
            '    actual = select({',
        ...
        for entry_name in local_cache_entries:
            bazel_lines += [
                f'            ":bitstream_{entry_name}": ":cache_{entry_name}_manifest",'
            ]
        bazel_lines += [
            f'            "@lowrisc_opentitan//hw/bitstream:bitstream_gcp": ":cache_{default_key}_manifest",',  # noqa: E501
            '        },',
            '        no_match_error = "the requested bitstream was not found in the cache",',  # noqa: E501
            '    ),',
            ')',
    ...
    def WriteBazelFiles(self, build_file: Path, key: str) -> str:
        ...
        with open(build_file, 'wt') as f:
            f.write(self._ConstructBazelString(key))
