...
static void vector8_zero(vector8_t *out) { memset(&out->v, 0, sizeof(out->v)); }

static void vector8_add(vector8_t *out, const vector8_t *lhs,
                        const vector8_t *rhs) {
...
static void vector8_sub(vector8_t *out, const vector8_t *lhs,
                        const vector8_t *rhs) {
  for (size_t i = 0; i < 8; i++) {
...
static void vector8_mul_scalar(vector8_t *out, const vector8_t *lhs,
                               const scalar_t *rhs) {
  for (size_t i = 0; i < 8; i++) {
...
static void vector8_power2_round(vector8_t *t1, vector8_t *t0,
                                 const vector8_t *t) {
  for (size_t i = 0; i < 8; i++) {
...
static void vector8_use_hint(vector8_t *out, const hint_t *h,
                             const vector8_t *r) {
  for (size_t i = 0; i < 8; i++) {
    scalar_use_hint_32(&out->v[i], h->v[i].v, h->v[i].num_hints, &r->v[i]);
  }
}
...
// multiplication.
static void matrix87_expand_mul(vector8_t *out, const uint8_t rho[K_RHO_BYTES],
                                const vector7_t *a) {
...
static void expand_s1(vector7_t *s1, const uint8_t sigma[K_SIGMA_BYTES]) {
  for (size_t i = 0; i < 7; i++) {
    expand_scalar_2(&s1->v[i], sigma, i);
  }
}

static void expand_s2(vector8_t *s2, const uint8_t sigma[K_SIGMA_BYTES]) {
  for (size_t i = 0; i < 8; i++) {
    expand_scalar_2(&s2->v[i], sigma, i + 7);
  }
}
...
static void expand_s2_ntt_mul_scalar(scalar_t *out, size_t i,
                                     const uint8_t sigma[K_SIGMA_BYTES],
                                     const scalar_t *rhs) {
  expand_scalar_2(out, sigma, (uint8_t)i + 7);
  scalar_ntt(out);
  scalar_mul(out, out, rhs);
}
...
static void matrix87_expand_mul_mask(vector8_t *out,
  ...
  for (size_t j = 0; j < 7; j++) {
    scalar_t y_j;
    scalar_expand_mask_19(&y_j, rho_prime, kappa, j);
    scalar_ntt(&y_j);
    for (size_t i = 0; i < 8; i++) {
      scalar_t m_ij;
      derived_seed[K_RHO_BYTES + 1] = (uint8_t)i;
...
static void vector8_encode_10(uint8_t *out, const vector8_t *a) {
  for (size_t i = 0; i < 8; i++) {
...
static int hint_bit_unpack(hint_t *h, const uint8_t in[OMEGA + 8]) {
  int index = 0;
  for (size_t i = 0; i < 8; i++) {
...
static int decode_signature(uint8_t c_tilde[2 * LAMBDA_BYTES], vector7_t *z,
                            hint_t *h,
                            const uint8_t in[MLDSA87_SIGNATURE_BYTES]) {
  memcpy(c_tilde, in, 2 * LAMBDA_BYTES);

  const uint8_t *z_input = &in[2 * LAMBDA_BYTES];
  vector7_decode_signed_20_19(z, z_input);
...
static void generate_key_internal(
    ...
    private_key_t *priv, const uint8_t entropy[MLDSA87_PRIVATE_SEED_BYTES]) {
  ...
  expand_s1(&s.s1_ntt, sigma);
  vector7_ntt(&s.s1_ntt);
  ...
  vector8_t *t = &priv->t0_ntt;
  matrix87_expand_mul(t, rho, &s.s1_ntt);
  vector8_inverse_ntt(t);
  expand_s2(&s.s2, sigma);
  vector8_add(t, t, &s.s2);

  vector8_power2_round(&s.t1, &priv->t0_ntt, t);
  vector8_ntt(&priv->t0_ntt);

  memcpy(out_encoded_public_key, rho, K_RHO_BYTES);
  vector8_encode_10(&out_encoded_public_key[K_RHO_BYTES], &s.t1);
}
...
static void sign_internal_with_mu(
  ...
  SHAKE256_absorb(&shake256_ctxt, randomizer, MLDSA87_RANDOMIZER_BYTES);
  ...
  for (size_t kappa = 0;; kappa += 7) {
    vector8_t *const w = &tmp;
    matrix87_expand_mul_mask(w, priv->rho, rho_prime, kappa);
    ...
    for (size_t i = 0; i < 8; i++) {
      uint8_t w1_encoded_part[128];
      scalar_t temp;
      scalar_high_bits_32(&temp, &w->v[i]);
      scalar_encode_4(w1_encoded_part, &temp);
      SHAKE256_absorb(&shake256_ctxt, w1_encoded_part, 128);
    }
    ...
    for (size_t i = 0; i < 7; i++) {
      ...
      scalar_t z_i;
      scalar_expand_mask_19(&z_i, rho_prime, kappa, i);
      ...
      // Encode z_i.
      scalar_encode_signed_20_19(z_output + i * 20 * K_DEGREE / 8, &z_i);
    }

    uint32_t r0_max = 0, ct0_max = 0;
    size_t h_ones = 0;
    vector8_t *const h = &tmp;
    for (size_t i = 0; i < 8; i++) {
      scalar_t cs_i;
      ...
      scalar_make_hint_32(&h->v[i], ct0_i, &cs_i, &w->v[i]);
    ...
    // Encode hint.
    uint8_t *hint_output = &out_encoded_signature[2 * LAMBDA_BYTES + 640 * 7];
...
static int verify_internal_with_mu(
    const uint8_t encoded_public_key[MLDSA87_PUBLIC_KEY_BYTES],
  ...
  if (!decode_signature(sig_c_tilde, &s.z, &h, encoded_signature))
    return 0;

  // Compute ||z||_\infty and set z = NTT(z).
  uint32_t z_max = vector7_max(&s.z);
  vector7_ntt(&s.z);
  ...
  matrix87_expand_mul(&az_ntt, encoded_public_key, &s.z);

  vector8_t *const t1 = &s.ct1_ntt;
  vector8_decode_10(t1, &encoded_public_key[K_RHO_BYTES]);
  vector8_scale_power2_round(&s.ct1_ntt, t1);
  vector8_ntt(&s.ct1_ntt);

  vector8_mul_scalar(&s.ct1_ntt, &s.ct1_ntt, &c_ntt);

  vector8_t *w1 = &az_ntt;
  vector8_sub(w1, &az_ntt, &s.ct1_ntt);
  vector8_inverse_ntt(w1);

  vector8_use_hint(w1, &h, w1);
  ...
  for (size_t i = 0; i < 8; i++) {
    uint8_t w1_encoded_part[128];
    scalar_encode_4(w1_encoded_part, &w1->v[i]);
    SHAKE256_absorb(&shake256_ctxt, w1_encoded_part, 128);
  }
  SHAKE256_squeeze(&shake256_ctxt, c_tilde, 2 * LAMBDA_BYTES);
  SHAKE256_free(&shake256_ctxt);
...
void mldsa87_tiny_pub_from_seed(
    uint8_t out_encoded_public_key[MLDSA87_PUBLIC_KEY_BYTES],
    const uint8_t private_key_seed[MLDSA87_PRIVATE_SEED_BYTES]) {
  private_key_t priv;
...
void mldsa87_tiny_sign(
    uint8_t out_encoded_signature[MLDSA87_SIGNATURE_BYTES],
    const uint8_t private_key_seed[MLDSA87_PRIVATE_SEED_BYTES],
    const uint8_t randomizer[MLDSA87_RANDOMIZER_BYTES], const uint8_t *msg,
    size_t msg_len) {
  private_key_t priv;
...
void mldsa87_tiny_sign_deterministic(
    uint8_t out_encoded_signature[MLDSA87_SIGNATURE_BYTES],
    const uint8_t private_key_seed[MLDSA87_PRIVATE_SEED_BYTES],
...
void mldsa87_tiny_sign_mu_deterministic(
    uint8_t out_encoded_signature[MLDSA87_SIGNATURE_BYTES],
    const uint8_t private_key_seed[MLDSA87_PRIVATE_SEED_BYTES],
    const uint8_t mu[K_MU_BYTES]) {
  private_key_t priv;
...
int mldsa87_tiny_verify(
    const uint8_t encoded_public_key[MLDSA87_PUBLIC_KEY_BYTES],
    const uint8_t encoded_signature[MLDSA87_SIGNATURE_BYTES],
    const uint8_t *msg, size_t msg_len) {
  uint8_t mu[K_MU_BYTES];
  compute_mu(mu, encoded_public_key, MLDSA87_PUBLIC_KEY_BYTES, NULL, 0, msg,
             msg_len);
  return verify_internal_with_mu(encoded_public_key, encoded_signature, mu);
}

int mldsa87_tiny_verify_mu(
    const uint8_t encoded_public_key[MLDSA87_PUBLIC_KEY_BYTES],
    const uint8_t encoded_signature[MLDSA87_SIGNATURE_BYTES],
    const uint8_t mu[K_MU_BYTES]) {
  return verify_internal_with_mu(encoded_public_key, encoded_signature, mu);
