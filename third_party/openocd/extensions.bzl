...
def _openocd_repos():
    ...
    http_archive(
        ...
        # See Issue(#18087)
        patches = [
            Label("//third_party/openocd/patches:calloc_transpose.patch"),
            Label("//third_party/openocd/patches:reset_on_dmi_op_error.patch"),
            Label("//third_party/openocd/patches:string_truncate_build_error.patch"),
        ],
        patch_args = ["-p1"],
    )
