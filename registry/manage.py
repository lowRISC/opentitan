...
def do_add_module(mod_name, mod_ver, source_json, module_bazel):
    ...
    if not metadata_json_path.exists():
        logging.info(f"Module {mod_name} does not exist yet in the registry, creating an empty entry...")
...
def do_add_module_patch(mod_name, mod_ver, patch_path):
    ...
    (mod_dir / "patches" / patch_name).write_bytes(patch_path.read_bytes())

    # Compute digest
    h = hashlib.sha256()
    h.update(patch_path.read_bytes())
    patch_integrity = "sha256-{}".format(base64.b64encode(h.digest()).decode('utf-8'))

    # Add entry to the json information
    source_json['patches'] = source_json.get('patches') | OrderedDict([(patch_name, patch_integrity)])
    source_json_path.write_text(dump_json(source_json))


def add_module_patch(args):
    do_add_module_patch(args.module_name, args.module_version, Path(args.patch))
