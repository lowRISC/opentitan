...
def opentitan_select_top_attr(attr_name, required = True, default = None, fn = None):
    """
    Return a select expression giving access to a given top attribute. Specifically,
    `fn` will be called for each with this attribute and the returned value will be
    inserted into the select expression. If `fn` is not provided, the attribute itself
    will be returned, as if the identity function was provided.
...
def opentitan_select_ip_attr(ipname, attr_name, required = True, default = None, fn = None):
    """
    Return a select expression giving access to a given IP attribute. Specifically,
    `fn` will be called for each with this attribute and the returned value will be
    inserted into the select expression. If `fn` is not provided, the attribute itself
    will be returned, as if the identity function was provided.
