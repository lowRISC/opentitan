...
The following macros are particularly useful. To see examples of use, see the last section. For more details, look at the documentation in `//hw/top:defs.bzl`
...
Consider the case where we want to add add a new `ast_init` attribute to the Darjeeling top.
...
We start creating a stub implementation in `sw/device/lib/testing/test_rom/`:
```c
