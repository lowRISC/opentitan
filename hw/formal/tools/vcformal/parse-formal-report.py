...
def get_summary(str_buffer):
    ...
    summary["pass_rate"] = format_percentage(
        summary["proven"], summary["cex"] + summary["undetermined"]
    )
        ...
        with Path(logpath).open() as f:
            ...
            for key, item in cov_results.items():
                if len(item) == 1:
                    ...
                    cov_results[key] = "N/A"
                    log.warning(
                        "Parse %s coverage error. Expect one matching value, get %s", key, item
                    )
            return cov_results
...
def main():
    parser = argparse.ArgumentParser(
        description="""This script parses the output log to extract below information.
