...
def _extract_messages(full_file, results, key, args):
    ...
    for severity, pattern in err_warn_patterns:
        results["messages"][severity] += re.findall(pattern, full_file, flags=re.MULTILINE)
        ...
        for match in matches:
            ...
            if name not in instances:
                ...
                if name in args.expand_modules or depth < args.expand_depth:
                    _extract_area_recursive(
                        full_file, results, key, args, depth=depth + 1, prefix=name + "/"
                    )
...
def _check_area(results, key, args):
    ...
    for k, inst in enumerate(instances[:-1]):
        ...
        if do_check:
            checks = [("comb", comb_check), ("reg", reg_check), ("macro", macro_check)]
    ...
    try:
        ...
        for k, c in enumerate(clocks):
            if c[0].strip() not in results[key]:
                results[key].update({c[0].strip(): {"tns": 0.0, "wns": 0.0, "period": float(c[1])}})
    except ValueError as err:
        results["messages"]["flow_errors"] += ["ValueError: %s" % err]
...
def _extract_timing(full_file, results, key, args):
    ...
    slack = re.findall(r"^  slack \(.+\) \s*(" + FP_NUMBER + ")", full_file, flags=re.MULTILINE)
    try:
        # get TNS and WNS in that group
        for k, g in enumerate(groups):
            if g.strip() not in results[key]:
                results[key].update({g.strip(): {"tns": 0.0, "wns": 0.0, "period": float("nan")}})
            ...
            if match:
                if len(match[0]) == 2:
                    if match[0][1].strip() in units:
                        results[key][subkey] = float(match[0][0]) * units[match[0][1].strip()]
        except ValueError as err:
            results["messages"]["flow_errors"] += ["ValueError: %s" % err]
...
def get_results(args):
    ...
    rep_types = [
        (args.log_path, "synthesis.log", "flow", _extract_messages),
...
def main():
    ...
    parser.add_argument(
        "--dut", type=str, help="""Name of the DUT. This is needed to parse the reports."""
    )
    ...
    parser.add_argument(
        "--expand-depth", type=int, default=1, help="""Area Report with hierarchical depth"""
    )
    ...
    with Path(args.out_dir).joinpath("results.hjson").open("w") as results_file:
