...
interface ibex_icache_core_protocol_checker (
  ...
  `OCAH_OT_ASSERT(NoReadyWithoutReq, !req |-> !ready, clk, !rst_n)
  ...
  `OCAH_OT_ASSERT(NoReqInReset, req |-> rst_n, clk, 1'b0)
  ...
  `OCAH_OT_ASSERT(BranchAddrAligned, branch |-> !branch_addr[0], clk, !rst_n)
  ...
  `OCAH_OT_ASSERT(NoFetchWithoutAddr, ready |-> (branch | has_addr), clk, !rst_n)
  ...
  `OCAH_OT_ASSERT(ValidUntilReady, unfinished_valid |=> valid,                clk, !rst_n);
  `OCAH_OT_ASSERT(AddrStable,      unfinished_valid |=> $stable(addr),        clk, !rst_n);
  `OCAH_OT_ASSERT(ErrStable,       unfinished_valid |=> $stable(err),         clk, !rst_n);
  `OCAH_OT_ASSERT(Err2Stable,      unfinished_valid |=> $stable(err_plus2),   clk, !rst_n);

  `OCAH_OT_ASSERT(LoRDataStable,
          unfinished_valid & ~err |=> $stable(rdata[15:0]),
          clk, !rst_n);
  `OCAH_OT_ASSERT(HiRDataStable,
  ...
  `OCAH_OT_ASSERT(ErrPlus2ImpliesUncompressed,
  ...
  `OCAH_OT_ASSERT_KNOWN(BranchKnown,     branch,     clk, !rst_n)
  `OCAH_OT_ASSERT_KNOWN(ReadyKnown,      ready,      clk, !rst_n)
  `OCAH_OT_ASSERT_KNOWN(EnableKnown,     enable,     clk, !rst_n)
  `OCAH_OT_ASSERT_KNOWN(InvalidateKnown, invalidate, clk, !rst_n)

  // The branch_addr value must be known if branch is high
  `OCAH_OT_ASSERT_KNOWN_IF(BranchAddrKnown, branch_addr, branch, clk, !rst_n)
  ...
  `OCAH_OT_ASSERT_KNOWN_IF(ValidKnown, valid, has_addr, clk, !rst_n)
  ...
  `OCAH_OT_ASSERT_KNOWN_IF(RespAddrKnown, addr,      has_addr & valid,       clk, !rst_n)
  `OCAH_OT_ASSERT_KNOWN_IF(ErrKnown,      err,       has_addr & valid,       clk, !rst_n)
  `OCAH_OT_ASSERT_KNOWN_IF(ErrPlus2Known, err_plus2, has_addr & valid & err, clk, !rst_n)

  // The lower 16 bits of a response must be known if valid is high and err is not
  `OCAH_OT_ASSERT_KNOWN_IF(LowRDataKnown, rdata[15:0], has_addr & valid & ~err, clk, !rst_n)

  // The upper 16 bits of a response must be known if valid is high, err is not and the instruction
  // is not compressed.
  `OCAH_OT_ASSERT_KNOWN_IF(HiRDataKnown,
                   rdata[31:16],
                   has_addr & valid & ~err & (rdata[1:0] == 2'b11),
                   clk, !rst_n)

  // The busy signal must always have a known value
  `OCAH_OT_ASSERT_KNOWN(BusyKnown, busy, clk, !rst_n)
