...
(
  ...
  // we use the outouts from both stages of synchroniser to detect edges.
  ...
  // EscClkStopEscTimeout_A expects esc_timeout to be set until rst_lc_ni goes active.
  //
  // The counter is reset when rst_ni is low, io_clk_en is low, clk_esc_edge is high (indicating clk_esc_i is alive)
  // or when clk_esc_dead_cnt reaches ESC_CLK_DEAD_CNT.
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni || !io_clk_en || clk_esc_edge || clk_esc_dead_cnt == ESC_CLK_DEAD_CNT)
      clk_esc_dead_cnt <= 0;
    else
  ...
  // For slow fsm invalid states the fast fsm explicitly transitions to FastPwrStateInvalid
  // For fast fsm invalid states we use the fast_fsm_invalid input.
  ...
  `ASSERT(EscClkStopEscTimeout_A,
          (clk_esc_dead_cnt == ESC_CLK_DEAD_CNT) |=> esc_timeout until !rst_lc_ni,
