...
class tl_monitor extends dv_base_monitor#(
  ...
  // The most recently seen request on the A channel. Used when device_can_rsp_on_same_cycle to
  // ferry the item that we've seen between the sampling time and the following clock edge.
  ...
  task ad_channels_thread();
    ...
    forever begin
      ...
      if (!cfg.device_can_rsp_on_same_cycle) begin
        seen_a_chan_item = check_a_channel(.immediate(1'b0));
      end
      if (seen_a_chan_item) begin
        a_chan_port.write(a_chan_req_item);
        if (cfg.synchronise_ports) channel_dir_port.write(AddrChannel);
        seen_a_chan_item = 1'b0;
      end
      ...
      if (cfg.device_can_rsp_on_same_cycle) begin
        ...
        seen_a_chan_item = check_a_channel(.immediate(1'b1));
        if (seen_a_chan_item) begin
  ...
  // block.
  //
  // In either case, if a transaction is found then it fills in a_chan_req_item and returns 1'b1.
  // Otherwise, it returns 1'b0.
  function bit check_a_channel(bit immediate);
    ...
    if (a_valid && a_ready) begin
      ...
      a_chan_req_item = tl_seq_item::type_id::create("req");
      a_chan_req_item.a_addr   = h2d.a_address;
      a_chan_req_item.a_opcode = h2d.a_opcode;
      a_chan_req_item.a_size   = h2d.a_size;
      a_chan_req_item.a_param  = h2d.a_param;
      a_chan_req_item.a_data   = h2d.a_data;
      a_chan_req_item.a_mask   = h2d.a_mask;
      a_chan_req_item.a_source = h2d.a_source;
      a_chan_req_item.a_user   = h2d.a_user;
      `uvm_info("tl_logging",
                $sformatf("[%0s][a_chan] : %0s", agent_name, a_chan_req_item.convert2string()),
                UVM_HIGH)

      if (cfg.en_cov) sample_outstanding_cov(a_chan_req_item);

      `downcast(cloned_req, a_chan_req_item.clone());
