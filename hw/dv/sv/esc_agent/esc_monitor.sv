...
class esc_monitor extends dv_reactive_monitor #(
  .ITEM_T(esc_seq_item),
  .CFG_T (esc_agent_cfg),
  .COV_T (esc_agent_cov)
);
  ...
  // Track the rst_n signal in the interface and maintain cfg.in_reset
  extern task track_resets();
...
task esc_monitor::esc_thread();
  ...
  forever @(cfg.vif.monitor_cb) begin
    if (!cfg.in_reset && get_esc() === 1'b1) begin
      ...
      req.m_sig_cycle_cnt++;
      if (is_sig_int_err()) req.m_esc_handshake_sta = EscHSIntFail;
      ...
      if (get_esc() === 1'b0) begin
        ...
        if (ping_cnter >= cfg.ping_timeout_cycle) begin
          ...
          while (!cfg.get_esc_en() &&
                 !(req.m_esc_handshake_sta inside {EscHSIntFail,
                                                   EscRespComplete, EscReceived})) begin
            @(cfg.vif.monitor_cb);
            check_esc_resp(.req(req), .is_ping(1), .ping_triggered(1));
          end
        end
      ...
      if (cfg.en_cov) begin
        cov.m_handshake_complete_cg.sample(req.m_esc_handshake_sta);
...
task esc_monitor::check_esc_resp(esc_seq_item req, bit is_ping, bit ping_triggered = 0);
  case (req.m_esc_handshake_sta)
    EscHSIntFail, EscReceived: begin
      if (cfg.vif.monitor_cb.esc_rx.resp_p !== 0) begin
        esc_seq_item req_clone;
        `downcast(req_clone, req.clone());
        req_clone.m_esc_handshake_sta = EscHSIntFail;
        m_esc_port.write(req_clone);
      ...
      if (!cfg.get_esc_en() &&
          (ping_triggered || (req.m_esc_handshake_sta == EscHSIntFail && !is_ping))) begin
    ...
    EscRespHi: begin
      if (is_ping && cfg.get_esc_en()) begin
        ...
        req.m_esc_handshake_sta = EscHSIntFail;
        m_esc_port.write(req);
