...
class esc_receiver_driver extends dv_base_driver#(esc_seq_item, esc_agent_cfg);
...
task esc_receiver_driver::rsp_escalator();
  forever begin
    ...
    `uvm_info(`gfn,
              $sformatf("starting to send receiver item, int_fail=%0b", req.m_int_err_cyc != 0),
              UVM_HIGH)
    fork
      begin : non_blocking_fork
        ...
        disable fork;
        `uvm_info(`gfn,
                  $sformatf("have sent receiver item int_fail=%0b", req.m_int_err_cyc != 0),
                  UVM_HIGH)
