...
class jtag_dmi_reg_frontdoor extends uvm_reg_frontdoor;
  ...
  task read_dmi_over_jtag(jtag_dtm_reg_block jtag_dtm_ral, csr_field_t csr_or_fld);
    ...
    // At this point, we've seen two consecutive reads where op_rsp is not DmiOpInProgress. Use the
    // results of the second read to populate rw_info. If we are doing a UVM_READ, this includes
    // extracting the CSR or field from rdata.
