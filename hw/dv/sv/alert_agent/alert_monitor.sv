...
class alert_monitor extends dv_reactive_monitor #(
...
task alert_monitor::run_phase(uvm_phase phase);
  // Run the base class run_phase task in parallel
  fork
...
task alert_monitor::track_resets();
        ...
        begin
          ...
          forever @(cfg.vif.monitor_cb) begin
            if (cfg.vif.monitor_cb.alert_rx_final.ping_p != last_ping) begin
              ...
              req.m_trans_type = AlertPingTrans;
...
task alert_monitor::monitor_alert_handshake();
  ...
  req.m_trans_type = AlertSigTrans;
  req.m_alert_handshake_sta = AlertReceived;
  ...
  if (cfg.en_cov) begin
    cov.m_handshake_complete_cg.sample(req.m_alert_handshake_sta);
