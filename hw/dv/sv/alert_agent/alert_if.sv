...
interface alert_if(input clk, input rst_n);
  ...
  // if alert sender is async mode, the clock will be driven in alert_agent,
  ...
  assign alert_tx = (if_mode == dv_utils_pkg::Host && is_active)   ? alert_tx_int : 'z;
  assign alert_rx = (if_mode == dv_utils_pkg::Device && is_active) ? alert_rx_int : 'z;
  ...
  `ASSERT_KNOWN(PingKnown_A, alert_rx.ping_p ^ alert_rx.ping_n, clk, !rst_n)
  `ASSERT_KNOWN(AckKnown_A, alert_rx.ack_p ^ alert_rx.ack_n, clk, !rst_n)
  `ASSERT_KNOWN(AlertKnown_A, alert_tx.alert_p ^ alert_tx.alert_n, clk, !rst_n)
