...
// A sequence that repeatedly sends alert_seq_item items that are configured to request pings. These
// ping requests are configured with large ping_delay values. Note that if an alert is raised while
// the driver is waiting at the start of sending a ping request then the driver will complete the
// item immediately (and the sequencer will then be able to enqueue an alert response item)
