...
task kmac_app_host_driver::on_enter_reset();
  cfg.vif.host_cb.valid <= 0;
  cfg.vif.host_cb.data  <= 'x;
  cfg.vif.host_cb.strb  <= 'x;
  cfg.vif.host_cb.last  <= 'x;
endtask
...
task kmac_app_host_driver::send_req();
  ...
  cfg.vif.host_cb.valid <= 1;
  cfg.vif.host_cb.data <= req.m_data;
  cfg.vif.host_cb.strb <= (1 << req.m_num_bytes) - 1;
  cfg.vif.host_cb.last <= req.m_last;
  fork : isolation_fork begin
    fork
      ...
      begin
        do begin
          @(cfg.vif.host_cb);
        end while (!cfg.vif.host_cb.ready);
      end
    join_any
    disable fork;
  end join
