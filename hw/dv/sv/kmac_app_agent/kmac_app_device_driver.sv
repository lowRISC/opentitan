...
task kmac_app_device_driver::get_and_drive();
  forever begin
    ...
    if (!cfg.in_reset) begin
      ...
      cfg.vif.device_cb.digest_s0 <= rsp.m_digest_s0;
      cfg.vif.device_cb.digest_s1 <= rsp.m_digest_s1;
...
task kmac_app_device_driver::invalidate_signals();
  ...
  cfg.vif.device_cb.digest_s0 <= 'x;
  cfg.vif.device_cb.digest_s1 <= 'x;
...
task kmac_app_device_driver::drive_ready_until_req();
  ...
  forever begin
    ...
    if (cfg.vif.mon_cb.req_valid && cfg.vif.mon_cb.req_ready) begin
      ...
      req.m_data_s0 = cfg.vif.mon_cb.req_data_s0;
      req.m_data_s1 = cfg.vif.mon_cb.req_data_s1;

      // Count the bytes asserted in the request
      for (int unsigned i = 0; i < MsgStrbW; i++) begin
    ...
    cfg.vif.device_cb.req_ready <= ready;
