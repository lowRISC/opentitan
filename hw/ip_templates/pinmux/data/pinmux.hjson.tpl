    ...
    { multireg: { name:     "MIO_PAD_ATTR",
                  ...
                  fields: [
                    ...
                    { bits: "2",
                      ...
                      // Note: The special index where this has a different resval depends on the
                      // TargetCfg parameter that gets passed in. That, in turn, is configured at
                      // chip level and it would be nicer if we could resolve it as we expanded this
                      // template. Instead, we expect the top that renders the template to give
                      // matching parameters and we include a basic check in pinmux.sv.tpl to make
                      // sure everything is in sync.
                      resval: (multireg_idx == ${tap_strap0_idx})
                    },
                    { bits: "3",
                      name: "pull_select",
