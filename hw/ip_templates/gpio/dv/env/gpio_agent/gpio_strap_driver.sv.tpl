...
class ${module_instance_name}_strap_driver extends dv_base_driver #(.ITEM_T(uvm_sequence_item),
                                                 .CFG_T(${module_instance_name}_strap_agent_cfg));

  `uvm_component_utils(${module_instance_name}_strap_driver)
  ...
  // Strap virtual sequencer used to get the ${module_instance_name} seq items
  local strap_sequencer    m_seqr;
  // ${module_instance_name} config env object;
  ${module_instance_name}_env_cfg m_${module_instance_name}_env_cfg;

  // Constructor: Creates a new instance of the ${module_instance_name}_driver class
  ...
  virtual task run_phase(uvm_phase phase);
    forever begin
      ${module_instance_name}_seq_item m_item;
      seq_item_port.get_next_item(req);

      if(!$cast(m_item , req)) begin
          `uvm_error("${module_instance_name}_strap_driver", "Failed to cast item to ${module_instance_name}_seq_item")
      end
  ...
  task drive_item(${module_instance_name}_seq_item req);
