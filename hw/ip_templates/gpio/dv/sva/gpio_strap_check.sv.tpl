...
module ${module_instance_name}_strap_check #(parameter int NUM_GPIOS = 32)
  (
  input logic                 clk_i,
  input logic                 rst_ni,
  ...
  `ASSERT(strap_data_assert, strap_en_i && !strap_valid |=> (strap_data == $past(gpio_i)));
  ...
  // must be high one clock cycle before and make sure the strap valid is not high too.
  ...
  // Ensure that strap_valid is high only one clock cycle after strap_en_i
  // and does not rise in any other circumstances.
  `ASSERT(strap_valid_one_cycle_assert, $rose(strap_valid) |=> ($past(strap_en_i) &&
                                                               !$past(strap_valid, 2)));
  // Ensure that strap_valid does not rise in any other circumstances.
  `ASSERT(strap_valid_no_other_rise_assert, !($rose(strap_valid) && !$past(strap_en_i)));
