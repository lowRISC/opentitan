...
This feature aims to provide additional security against fault injection attacks targeting the addresses of flash read requests.
By default, on each read request, the data fetched from the underyling memory is infected with the address used for the memory access.
Before returning the data to the memory request issuer over the bus, the address is removed from the infected data.
