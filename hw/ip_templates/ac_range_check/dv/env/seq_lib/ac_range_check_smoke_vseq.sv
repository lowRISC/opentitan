...
class ac_range_check_smoke_vseq extends ac_range_check_base_vseq;
  ...
  bit [7:0] fixed_deny_cnt_threshold;
...
// Except for the cases where the threshold is high; deny as much as possible
// then
constraint ac_range_check_smoke_vseq::range_attr_c {
  solve deny_cnt_threshold before zero_attr;
  solve zero_attr before dut_cfg.range_attr;
  ...
  if (deny_cnt_threshold inside {[245:255]} && zero_attr) {
    foreach (dut_cfg.range_base[i]) {
      dut_cfg.range_attr[i].execute_access == 0;
      dut_cfg.range_attr[i].write_access == 0;
      dut_cfg.range_attr[i].read_access == 0;
