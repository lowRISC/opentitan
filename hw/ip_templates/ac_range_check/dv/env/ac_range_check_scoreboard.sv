...
task ac_range_check_scoreboard::process_tl_access(tl_seq_item item,
  ...
  case (csr_name)
    ...
    "range_perm": begin
      if (tl_phase == AddrWrite) begin
        dut_cfg.range_perm[csr_idx].log_denied_access = prim_mubi_pkg::mubi4_to_bool(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].log_denied_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].execute_access    = prim_mubi_pkg::mubi4_to_bool(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].execute_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].write_access      = prim_mubi_pkg::mubi4_to_bool(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].write_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].read_access       = prim_mubi_pkg::mubi4_to_bool(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].read_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].enable            = prim_mubi_pkg::mubi4_to_bool(
          prim_mubi_pkg::mubi4_t'(
