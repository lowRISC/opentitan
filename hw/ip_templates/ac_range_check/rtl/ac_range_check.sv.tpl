...
#(
  ...
  for (genvar i = 0; i < NumRanges; i++) begin : gen_range_checks
    ...
    assign deny_mask[i] = addr_hit & ~(perm_read_access | perm_write_access | perm_execute_access);

    // Determine the read, write, and execute mask. Store a hit in their index
    assign read_mask[i]    = addr_hit & perm_read_access;
    assign write_mask[i]   = addr_hit & perm_write_access;
    assign execute_mask[i] = addr_hit & perm_execute_access;
  end
  ...
  assign hw2reg.log_status.denied_no_match.d  = 1'b0;
