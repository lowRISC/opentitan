...
class ${module_instance_name}_scoreboard extends cip_base_scoreboard #(.CFG_T(${module_instance_name}_env_cfg),
  ...
  local ${module_instance_name}_error_arb_predictor arb_predictor;
...
function void ${module_instance_name}_scoreboard::build_phase(uvm_phase phase);
  ...
  arb_predictor = ${module_instance_name}_error_arb_predictor::type_id::create("arb_predictor", this);
...
function void ${module_instance_name}_scoreboard::process_tl_a_channel(tl_seq_item item, string ral_name);
...
function void ${module_instance_name}_scoreboard::process_tl_d_channel(tl_seq_item item, string ral_name);
...
function void ${module_instance_name}_scoreboard::check_policies();
  dv_base_reg  regs[$];
...
task ${module_instance_name}_scoreboard::watch_policies();
  forever begin
...
task ${module_instance_name}_scoreboard::watch_arbitrated_errors();
...
function void ${module_instance_name}_scoreboard::take_error_log_write(uvm_reg_data_t data, bit [3:0] byte_mask);
...
function bit ${module_instance_name}_scoreboard::get_intr_pred(bit require_enable);
  bit enabled = require_enable ? intr_enable_d_pred : 1'b1;
  return (intr_valid_q_pred | intr_test_q_pred) & enabled;
endfunction

task ${module_instance_name}_scoreboard::watch_interrupt();
...
task ${module_instance_name}_scoreboard::mirror_error_log();
  dv_base_reg error_log_reg = cfg.regs.get_error_log_reg();
  forever begin
...
task ${module_instance_name}_scoreboard::mirror_error_log_address();
  dv_base_reg error_log_address_reg = cfg.regs.get_error_log_address_reg();
  forever begin
    // Wait until we are told that the event has been triggered
...
task ${module_instance_name}_scoreboard::mirror_racl_error_field();
...
task ${module_instance_name}_scoreboard::track_resets();
  dv_base_reg error_log_reg         = cfg.regs.get_error_log_reg();
  dv_base_reg error_log_address_reg = cfg.regs.get_error_log_address_reg();
...
task ${module_instance_name}_scoreboard::model_registers();
...
task ${module_instance_name}_scoreboard::process_csr_writes();
  uvm_reg_addr_t csr_addr;
  uvm_reg        csr;
  string         csr_name;
