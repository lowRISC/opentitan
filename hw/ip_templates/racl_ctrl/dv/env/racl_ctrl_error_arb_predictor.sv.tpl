...
class ${module_instance_name}_error_arb_predictor extends uvm_component;
  `uvm_component_utils(${module_instance_name}_error_arb_predictor)
...
function ${module_instance_name}_error_arb_predictor::new(string name="", uvm_component parent=null);
  super.new(name, parent);
endfunction

function void ${module_instance_name}_error_arb_predictor::build_phase(uvm_phase phase);
...
task ${module_instance_name}_error_arb_predictor::run_phase(uvm_phase phase);
  fork
...
task ${module_instance_name}_error_arb_predictor::on_reset();
  internal_errors_fifo.flush();
  external_errors_fifo.flush();
  merged_errors_fifo.flush();
endtask

task ${module_instance_name}_error_arb_predictor::watch_errors();
  forever begin
...
function racl_error_log_vec_item
  ${module_instance_name}_error_arb_predictor::flush_vec_fifo(error_vec_fifo_t fifo);
