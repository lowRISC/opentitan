...
waive -rules ONE_BIT_MEM_WIDTH             -location {ibex_core.sv} -regexp {Memory 'pmp_req_err_raw' has word width which is single bit wide} ${"\\"}
      -comment "For consistency with related signals, we use an unpacked array for this signal."
waive -rules HIER_NET_NOT_READ    -location {ibex_core.sv} -regexp {Net 'instr_first_cycle_id' is not read from} ${"\\"}
      -comment "Only read by the CHERIoT execution block and RVFI, both absent in RV32I-only configurations"
waive -rules NOT_READ             -location {ibex_core.sv} -regexp {Signal 'instr_first_cycle_id' is not read from} ${"\\"}
      -comment "Only read by the CHERIoT execution block and RVFI, both absent in RV32I-only configurations"
