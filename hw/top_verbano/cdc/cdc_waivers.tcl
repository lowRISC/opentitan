...
set_rule_status -rule {W_RECON_GROUPS} -status {Waived}                           \
  -expression {(ControlSignal =~ "u_ast.u_ast_clks_byp.*u_impl_generic.q_o*") && (ReconSignal=~"top_verbano.*.u_reg*")} \
...
set_rule_status -rule {W_RECON_GROUPS} -status {Waived}                           \
  -expression {(ControlSignal =~ "*u_pinmux_aon.u_reg.u_wkup_detector*cdc.u_src_to_dst_req*.u_sync1.*u_impl_generic.q_o*") && (ReconSignal=~"top_verbano.*.u_reg*")} \
  -comment {Reconverged signals coming from AST}

set_rule_status -rule {W_RECON_GROUPS} -status {Waived}                           \
  -expression {(ControlSignal =~ "*u_reg.*cdc*.prim_flop_2sync.u_sync_1.*u_impl_generic.q_o*") && (ReconSignal=~"top_verbano.*.u_reg*")} \
  -comment {Reconverged signals coming from AST}
...
set_rule_status -rule {W_RECON_GROUPS} -status {Waived} -expression {(ReconSignal=~"top_verbano.u_xbar_main.u_asf_35.rspfifo.storage[0]*")} -comment {Intended reconvergence in xbar main}
set_rule_status -rule {W_RECON_GROUPS} -status {Waived} -expression {(ReconSignal=~"top_verbano.u_pwrmgr_aon.u_fsm.u_state_regs.u_state_flop.q_o*")} -comment {Intended reconvergence in pwrmgr fsm}
set_rule_status -rule {W_RECON_GROUPS} -status {Waived} -expression {(ControlSignal=~"top_verbano.u_xbar_main.u_asf_*.rs*fifo.sync_*ptr.u_sync_1.q_o*")} -comment {reconvergence caused by unrecognized qualification in async fifo}
set_rule_status -rule {W_RECON_GROUPS} -status {Waived} -expression {(ControlSignal=~"top_verbano.*.*sync*.*sync_1.q_o*")} -comment {reconvergence caused by unrecognized qualification in async fifo}
set_rule_status -rule {W_RECON_GROUPS} -status {Waived} -expression {(ControlSignal=~"top_verbano.*.*cdc*.*sync_1.q_o*")} -comment {reconvergence caused by unrecognized qualification in async fifo}
...
set_rule_status -rule {W_INTERFACE} -status {Waived} -expression             \
  {(Signal=~"IO*") && \
  (ReceivingFlop=~ "top_verbano.u_spi_host1.u_spi_core.u_shift_reg.*_q*")} -comment {W_INTERFACE issues from PAD to spi_host}
...
set_rule_status -rule {W_INTERFACE} -status {Waived} -expression             \
  {(Signal=~"top_verbano.*.u_reg.*.q*") && \
  (ReceivingFlop=~ "top_verbano.u_pinmux_aon.mio_out_retreg_q*")} -comment {W_INTERFACE issues from u_reg to pinmux}

set_rule_status -rule {W_INTERFACE} -status {Waived} -expression             \
  {(Signal=~"top_verbano.*.u_reg.*_q*") && \
  (ReceivingFlop=~ "top_verbano.u_pinmux_aon.mio_out_retreg_q*")} -comment {W_INTERFACE issues from u_reg to pinmux}

set_rule_status -rule {W_CNTL} -status {Waived}                           \
  -expression {(Signal=~"top_verbano.*.q_o*") && (ReceivingFlop=~"top_verbano.*.u_sync_1.q_o*")} \
  -comment {PAD driving to I2C. PADs are not clock bounded}

set_rule_status -rule {W_CNTL} -status {Waived}                           \
  -expression {(Signal=~"top_verbano.*.u_reg.*_cdc.u_src_to_dst_req.src_level") && (ReceivingFlop=~"top_verbano.*.u_sync_1.q_o*")} \
  -comment {PAD driving to I2C. PADs are not clock bounded}

set_rule_status -rule {W_CNTL} -status {Waived}                           \
  -expression {(Signal=~"top_verbano.*.u_reg.*_cdc.u_arb.*_sync.*.dst_ack_q") && (ReceivingFlop=~"top_verbano.*.u_sync_1.q_o*")} \
  -comment {PAD driving to I2C. PADs are not clock bounded}
