// Copyright lowRISC contributors.
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0
//
// xbar_main module generated by `tlgen.py` tool
// all reset signals should be generated from one reset signal to not make any deadlock
//
// Interconnect
// rv_core_ibex.corei
//   -> s1n_34
//     -> sm1_35
//       -> rom_ctrl0.rom
//     -> sm1_36
//       -> rom_ctrl1.rom
//     -> sm1_37
//       -> rv_dm.mem
//     -> sm1_38
//       -> sram_ctrl_main.ram
//     -> sm1_39
//       -> flash_ctrl.mem
//     -> sm1_40
//       -> soc_proxy.ctn
// rv_core_ibex.cored
//   -> s1n_41
//     -> sm1_35
//       -> rom_ctrl0.rom
//     -> sm1_42
//       -> rom_ctrl0.regs
//     -> sm1_36
//       -> rom_ctrl1.rom
//     -> sm1_43
//       -> rom_ctrl1.regs
//     -> sm1_37
//       -> rv_dm.mem
//     -> sm1_44
//       -> rv_dm.regs
//     -> sm1_38
//       -> sram_ctrl_main.ram
//     -> sm1_46
//       -> asf_45
//         -> peri
//     -> sm1_48
//       -> asf_47
//         -> spi_host1
//     -> sm1_50
//       -> asf_49
//         -> usbdev
//     -> sm1_51
//       -> flash_ctrl.core
//     -> sm1_52
//       -> flash_ctrl.prim
//     -> sm1_39
//       -> flash_ctrl.mem
//     -> sm1_53
//       -> aes
//     -> sm1_54
//       -> entropy_src
//     -> sm1_55
//       -> csrng
//     -> sm1_56
//       -> edn0
//     -> sm1_57
//       -> edn1
//     -> sm1_58
//       -> hmac
//     -> sm1_59
//       -> rv_plic
//     -> sm1_60
//       -> otbn
//     -> sm1_61
//       -> keymgr
//     -> sm1_62
//       -> kmac
//     -> sm1_63
//       -> sram_ctrl_main.regs
//     -> sm1_64
//       -> rv_core_ibex.cfg
//     -> sm1_65
//       -> sram_ctrl_mbox.ram
//     -> sm1_66
//       -> sram_ctrl_mbox.regs
//     -> sm1_40
//       -> soc_proxy.ctn
//     -> sm1_67
//       -> soc_proxy.core
//     -> sm1_68
//       -> dma
// rv_dm.sba
//   -> s1n_69
//     -> sm1_35
//       -> rom_ctrl0.rom
//     -> sm1_42
//       -> rom_ctrl0.regs
//     -> sm1_36
//       -> rom_ctrl1.rom
//     -> sm1_43
//       -> rom_ctrl1.regs
//     -> sm1_37
//       -> rv_dm.mem
//     -> sm1_44
//       -> rv_dm.regs
//     -> sm1_38
//       -> sram_ctrl_main.ram
//     -> sm1_46
//       -> asf_45
//         -> peri
//     -> sm1_48
//       -> asf_47
//         -> spi_host1
//     -> sm1_50
//       -> asf_49
//         -> usbdev
//     -> sm1_51
//       -> flash_ctrl.core
//     -> sm1_52
//       -> flash_ctrl.prim
//     -> sm1_39
//       -> flash_ctrl.mem
//     -> sm1_53
//       -> aes
//     -> sm1_54
//       -> entropy_src
//     -> sm1_55
//       -> csrng
//     -> sm1_56
//       -> edn0
//     -> sm1_57
//       -> edn1
//     -> sm1_58
//       -> hmac
//     -> sm1_59
//       -> rv_plic
//     -> sm1_60
//       -> otbn
//     -> sm1_61
//       -> keymgr
//     -> sm1_62
//       -> kmac
//     -> sm1_63
//       -> sram_ctrl_main.regs
//     -> sm1_64
//       -> rv_core_ibex.cfg
//     -> sm1_65
//       -> sram_ctrl_mbox.ram
//     -> sm1_66
//       -> sram_ctrl_mbox.regs
//     -> sm1_40
//       -> soc_proxy.ctn
//     -> sm1_67
//       -> soc_proxy.core
//     -> sm1_68
//       -> dma
// dma.host
//   -> s1n_70
//     -> sm1_38
//       -> sram_ctrl_main.ram
//     -> sm1_46
//       -> asf_45
//         -> peri

module xbar_main (
  input clk_main_i,
  input clk_fixed_i,
  input clk_usb_i,
  input clk_spi_host1_i,
  input rst_main_ni,
  input rst_fixed_ni,
  input rst_usb_ni,
  input rst_spi_host1_ni,

  // Host interfaces
  input  tlul_pkg::tl_h2d_t tl_rv_core_ibex__corei_i,
  output tlul_pkg::tl_d2h_t tl_rv_core_ibex__corei_o,
  input  tlul_pkg::tl_h2d_t tl_rv_core_ibex__cored_i,
  output tlul_pkg::tl_d2h_t tl_rv_core_ibex__cored_o,
  input  tlul_pkg::tl_h2d_t tl_rv_dm__sba_i,
  output tlul_pkg::tl_d2h_t tl_rv_dm__sba_o,
  input  tlul_pkg::tl_h2d_t tl_dma__host_i,
  output tlul_pkg::tl_d2h_t tl_dma__host_o,

  // Device interfaces
  output tlul_pkg::tl_h2d_t tl_rv_dm__regs_o,
  input  tlul_pkg::tl_d2h_t tl_rv_dm__regs_i,
  output tlul_pkg::tl_h2d_t tl_rv_dm__mem_o,
  input  tlul_pkg::tl_d2h_t tl_rv_dm__mem_i,
  output tlul_pkg::tl_h2d_t tl_rom_ctrl0__rom_o,
  input  tlul_pkg::tl_d2h_t tl_rom_ctrl0__rom_i,
  output tlul_pkg::tl_h2d_t tl_rom_ctrl0__regs_o,
  input  tlul_pkg::tl_d2h_t tl_rom_ctrl0__regs_i,
  output tlul_pkg::tl_h2d_t tl_rom_ctrl1__rom_o,
  input  tlul_pkg::tl_d2h_t tl_rom_ctrl1__rom_i,
  output tlul_pkg::tl_h2d_t tl_rom_ctrl1__regs_o,
  input  tlul_pkg::tl_d2h_t tl_rom_ctrl1__regs_i,
  output tlul_pkg::tl_h2d_t tl_peri_o,
  input  tlul_pkg::tl_d2h_t tl_peri_i,
  output tlul_pkg::tl_h2d_t tl_soc_proxy__core_o,
  input  tlul_pkg::tl_d2h_t tl_soc_proxy__core_i,
  output tlul_pkg::tl_h2d_t tl_soc_proxy__ctn_o,
  input  tlul_pkg::tl_d2h_t tl_soc_proxy__ctn_i,
  output tlul_pkg::tl_h2d_t tl_spi_host1_o,
  input  tlul_pkg::tl_d2h_t tl_spi_host1_i,
  output tlul_pkg::tl_h2d_t tl_usbdev_o,
  input  tlul_pkg::tl_d2h_t tl_usbdev_i,
  output tlul_pkg::tl_h2d_t tl_flash_ctrl__core_o,
  input  tlul_pkg::tl_d2h_t tl_flash_ctrl__core_i,
  output tlul_pkg::tl_h2d_t tl_flash_ctrl__prim_o,
  input  tlul_pkg::tl_d2h_t tl_flash_ctrl__prim_i,
  output tlul_pkg::tl_h2d_t tl_flash_ctrl__mem_o,
  input  tlul_pkg::tl_d2h_t tl_flash_ctrl__mem_i,
  output tlul_pkg::tl_h2d_t tl_hmac_o,
  input  tlul_pkg::tl_d2h_t tl_hmac_i,
  output tlul_pkg::tl_h2d_t tl_kmac_o,
  input  tlul_pkg::tl_d2h_t tl_kmac_i,
  output tlul_pkg::tl_h2d_t tl_aes_o,
  input  tlul_pkg::tl_d2h_t tl_aes_i,
  output tlul_pkg::tl_h2d_t tl_entropy_src_o,
  input  tlul_pkg::tl_d2h_t tl_entropy_src_i,
  output tlul_pkg::tl_h2d_t tl_csrng_o,
  input  tlul_pkg::tl_d2h_t tl_csrng_i,
  output tlul_pkg::tl_h2d_t tl_edn0_o,
  input  tlul_pkg::tl_d2h_t tl_edn0_i,
  output tlul_pkg::tl_h2d_t tl_edn1_o,
  input  tlul_pkg::tl_d2h_t tl_edn1_i,
  output tlul_pkg::tl_h2d_t tl_rv_plic_o,
  input  tlul_pkg::tl_d2h_t tl_rv_plic_i,
  output tlul_pkg::tl_h2d_t tl_otbn_o,
  input  tlul_pkg::tl_d2h_t tl_otbn_i,
  output tlul_pkg::tl_h2d_t tl_keymgr_o,
  input  tlul_pkg::tl_d2h_t tl_keymgr_i,
  output tlul_pkg::tl_h2d_t tl_rv_core_ibex__cfg_o,
  input  tlul_pkg::tl_d2h_t tl_rv_core_ibex__cfg_i,
  output tlul_pkg::tl_h2d_t tl_sram_ctrl_main__regs_o,
  input  tlul_pkg::tl_d2h_t tl_sram_ctrl_main__regs_i,
  output tlul_pkg::tl_h2d_t tl_sram_ctrl_main__ram_o,
  input  tlul_pkg::tl_d2h_t tl_sram_ctrl_main__ram_i,
  output tlul_pkg::tl_h2d_t tl_sram_ctrl_mbox__regs_o,
  input  tlul_pkg::tl_d2h_t tl_sram_ctrl_mbox__regs_i,
  output tlul_pkg::tl_h2d_t tl_sram_ctrl_mbox__ram_o,
  input  tlul_pkg::tl_d2h_t tl_sram_ctrl_mbox__ram_i,
  output tlul_pkg::tl_h2d_t tl_dma_o,
  input  tlul_pkg::tl_d2h_t tl_dma_i,

  input prim_mubi_pkg::mubi4_t scanmode_i
);

  import tlul_pkg::*;
  import tl_main_pkg::*;

  // scanmode_i is currently not used, but provisioned for future use
  // this assignment prevents lint warnings
  logic unused_scanmode;
  assign unused_scanmode = ^scanmode_i;

  tl_h2d_t tl_s1n_34_us_h2d ;
  tl_d2h_t tl_s1n_34_us_d2h ;


  tl_h2d_t tl_s1n_34_ds_h2d [6];
  tl_d2h_t tl_s1n_34_ds_d2h [6];

  // Create steering signal
  logic [2:0] dev_sel_s1n_34;


  tl_h2d_t tl_sm1_35_us_h2d [3];
  tl_d2h_t tl_sm1_35_us_d2h [3];

  tl_h2d_t tl_sm1_35_ds_h2d ;
  tl_d2h_t tl_sm1_35_ds_d2h ;


  tl_h2d_t tl_sm1_36_us_h2d [3];
  tl_d2h_t tl_sm1_36_us_d2h [3];

  tl_h2d_t tl_sm1_36_ds_h2d ;
  tl_d2h_t tl_sm1_36_ds_d2h ;


  tl_h2d_t tl_sm1_37_us_h2d [3];
  tl_d2h_t tl_sm1_37_us_d2h [3];

  tl_h2d_t tl_sm1_37_ds_h2d ;
  tl_d2h_t tl_sm1_37_ds_d2h ;


  tl_h2d_t tl_sm1_38_us_h2d [4];
  tl_d2h_t tl_sm1_38_us_d2h [4];

  tl_h2d_t tl_sm1_38_ds_h2d ;
  tl_d2h_t tl_sm1_38_ds_d2h ;


  tl_h2d_t tl_sm1_39_us_h2d [3];
  tl_d2h_t tl_sm1_39_us_d2h [3];

  tl_h2d_t tl_sm1_39_ds_h2d ;
  tl_d2h_t tl_sm1_39_ds_d2h ;


  tl_h2d_t tl_sm1_40_us_h2d [3];
  tl_d2h_t tl_sm1_40_us_d2h [3];

  tl_h2d_t tl_sm1_40_ds_h2d ;
  tl_d2h_t tl_sm1_40_ds_d2h ;

  tl_h2d_t tl_s1n_41_us_h2d ;
  tl_d2h_t tl_s1n_41_us_d2h ;


  tl_h2d_t tl_s1n_41_ds_h2d [30];
  tl_d2h_t tl_s1n_41_ds_d2h [30];

  // Create steering signal
  logic [4:0] dev_sel_s1n_41;


  tl_h2d_t tl_sm1_42_us_h2d [2];
  tl_d2h_t tl_sm1_42_us_d2h [2];

  tl_h2d_t tl_sm1_42_ds_h2d ;
  tl_d2h_t tl_sm1_42_ds_d2h ;


  tl_h2d_t tl_sm1_43_us_h2d [2];
  tl_d2h_t tl_sm1_43_us_d2h [2];

  tl_h2d_t tl_sm1_43_ds_h2d ;
  tl_d2h_t tl_sm1_43_ds_d2h ;


  tl_h2d_t tl_sm1_44_us_h2d [2];
  tl_d2h_t tl_sm1_44_us_d2h [2];

  tl_h2d_t tl_sm1_44_ds_h2d ;
  tl_d2h_t tl_sm1_44_ds_d2h ;

  tl_h2d_t tl_asf_45_us_h2d ;
  tl_d2h_t tl_asf_45_us_d2h ;
  tl_h2d_t tl_asf_45_ds_h2d ;
  tl_d2h_t tl_asf_45_ds_d2h ;


  tl_h2d_t tl_sm1_46_us_h2d [3];
  tl_d2h_t tl_sm1_46_us_d2h [3];

  tl_h2d_t tl_sm1_46_ds_h2d ;
  tl_d2h_t tl_sm1_46_ds_d2h ;

  tl_h2d_t tl_asf_47_us_h2d ;
  tl_d2h_t tl_asf_47_us_d2h ;
  tl_h2d_t tl_asf_47_ds_h2d ;
  tl_d2h_t tl_asf_47_ds_d2h ;


  tl_h2d_t tl_sm1_48_us_h2d [2];
  tl_d2h_t tl_sm1_48_us_d2h [2];

  tl_h2d_t tl_sm1_48_ds_h2d ;
  tl_d2h_t tl_sm1_48_ds_d2h ;

  tl_h2d_t tl_asf_49_us_h2d ;
  tl_d2h_t tl_asf_49_us_d2h ;
  tl_h2d_t tl_asf_49_ds_h2d ;
  tl_d2h_t tl_asf_49_ds_d2h ;


  tl_h2d_t tl_sm1_50_us_h2d [2];
  tl_d2h_t tl_sm1_50_us_d2h [2];

  tl_h2d_t tl_sm1_50_ds_h2d ;
  tl_d2h_t tl_sm1_50_ds_d2h ;


  tl_h2d_t tl_sm1_51_us_h2d [2];
  tl_d2h_t tl_sm1_51_us_d2h [2];

  tl_h2d_t tl_sm1_51_ds_h2d ;
  tl_d2h_t tl_sm1_51_ds_d2h ;


  tl_h2d_t tl_sm1_52_us_h2d [2];
  tl_d2h_t tl_sm1_52_us_d2h [2];

  tl_h2d_t tl_sm1_52_ds_h2d ;
  tl_d2h_t tl_sm1_52_ds_d2h ;


  tl_h2d_t tl_sm1_53_us_h2d [2];
  tl_d2h_t tl_sm1_53_us_d2h [2];

  tl_h2d_t tl_sm1_53_ds_h2d ;
  tl_d2h_t tl_sm1_53_ds_d2h ;


  tl_h2d_t tl_sm1_54_us_h2d [2];
  tl_d2h_t tl_sm1_54_us_d2h [2];

  tl_h2d_t tl_sm1_54_ds_h2d ;
  tl_d2h_t tl_sm1_54_ds_d2h ;


  tl_h2d_t tl_sm1_55_us_h2d [2];
  tl_d2h_t tl_sm1_55_us_d2h [2];

  tl_h2d_t tl_sm1_55_ds_h2d ;
  tl_d2h_t tl_sm1_55_ds_d2h ;


  tl_h2d_t tl_sm1_56_us_h2d [2];
  tl_d2h_t tl_sm1_56_us_d2h [2];

  tl_h2d_t tl_sm1_56_ds_h2d ;
  tl_d2h_t tl_sm1_56_ds_d2h ;


  tl_h2d_t tl_sm1_57_us_h2d [2];
  tl_d2h_t tl_sm1_57_us_d2h [2];

  tl_h2d_t tl_sm1_57_ds_h2d ;
  tl_d2h_t tl_sm1_57_ds_d2h ;


  tl_h2d_t tl_sm1_58_us_h2d [2];
  tl_d2h_t tl_sm1_58_us_d2h [2];

  tl_h2d_t tl_sm1_58_ds_h2d ;
  tl_d2h_t tl_sm1_58_ds_d2h ;


  tl_h2d_t tl_sm1_59_us_h2d [2];
  tl_d2h_t tl_sm1_59_us_d2h [2];

  tl_h2d_t tl_sm1_59_ds_h2d ;
  tl_d2h_t tl_sm1_59_ds_d2h ;


  tl_h2d_t tl_sm1_60_us_h2d [2];
  tl_d2h_t tl_sm1_60_us_d2h [2];

  tl_h2d_t tl_sm1_60_ds_h2d ;
  tl_d2h_t tl_sm1_60_ds_d2h ;


  tl_h2d_t tl_sm1_61_us_h2d [2];
  tl_d2h_t tl_sm1_61_us_d2h [2];

  tl_h2d_t tl_sm1_61_ds_h2d ;
  tl_d2h_t tl_sm1_61_ds_d2h ;


  tl_h2d_t tl_sm1_62_us_h2d [2];
  tl_d2h_t tl_sm1_62_us_d2h [2];

  tl_h2d_t tl_sm1_62_ds_h2d ;
  tl_d2h_t tl_sm1_62_ds_d2h ;


  tl_h2d_t tl_sm1_63_us_h2d [2];
  tl_d2h_t tl_sm1_63_us_d2h [2];

  tl_h2d_t tl_sm1_63_ds_h2d ;
  tl_d2h_t tl_sm1_63_ds_d2h ;


  tl_h2d_t tl_sm1_64_us_h2d [2];
  tl_d2h_t tl_sm1_64_us_d2h [2];

  tl_h2d_t tl_sm1_64_ds_h2d ;
  tl_d2h_t tl_sm1_64_ds_d2h ;


  tl_h2d_t tl_sm1_65_us_h2d [2];
  tl_d2h_t tl_sm1_65_us_d2h [2];

  tl_h2d_t tl_sm1_65_ds_h2d ;
  tl_d2h_t tl_sm1_65_ds_d2h ;


  tl_h2d_t tl_sm1_66_us_h2d [2];
  tl_d2h_t tl_sm1_66_us_d2h [2];

  tl_h2d_t tl_sm1_66_ds_h2d ;
  tl_d2h_t tl_sm1_66_ds_d2h ;


  tl_h2d_t tl_sm1_67_us_h2d [2];
  tl_d2h_t tl_sm1_67_us_d2h [2];

  tl_h2d_t tl_sm1_67_ds_h2d ;
  tl_d2h_t tl_sm1_67_ds_d2h ;


  tl_h2d_t tl_sm1_68_us_h2d [2];
  tl_d2h_t tl_sm1_68_us_d2h [2];

  tl_h2d_t tl_sm1_68_ds_h2d ;
  tl_d2h_t tl_sm1_68_ds_d2h ;

  tl_h2d_t tl_s1n_69_us_h2d ;
  tl_d2h_t tl_s1n_69_us_d2h ;


  tl_h2d_t tl_s1n_69_ds_h2d [30];
  tl_d2h_t tl_s1n_69_ds_d2h [30];

  // Create steering signal
  logic [4:0] dev_sel_s1n_69;

  tl_h2d_t tl_s1n_70_us_h2d ;
  tl_d2h_t tl_s1n_70_us_d2h ;


  tl_h2d_t tl_s1n_70_ds_h2d [2];
  tl_d2h_t tl_s1n_70_ds_d2h [2];

  // Create steering signal
  logic [1:0] dev_sel_s1n_70;



  assign tl_sm1_35_us_h2d[0] = tl_s1n_34_ds_h2d[0];
  assign tl_s1n_34_ds_d2h[0] = tl_sm1_35_us_d2h[0];

  assign tl_sm1_36_us_h2d[0] = tl_s1n_34_ds_h2d[1];
  assign tl_s1n_34_ds_d2h[1] = tl_sm1_36_us_d2h[0];

  assign tl_sm1_37_us_h2d[0] = tl_s1n_34_ds_h2d[2];
  assign tl_s1n_34_ds_d2h[2] = tl_sm1_37_us_d2h[0];

  assign tl_sm1_38_us_h2d[0] = tl_s1n_34_ds_h2d[3];
  assign tl_s1n_34_ds_d2h[3] = tl_sm1_38_us_d2h[0];

  assign tl_sm1_39_us_h2d[0] = tl_s1n_34_ds_h2d[4];
  assign tl_s1n_34_ds_d2h[4] = tl_sm1_39_us_d2h[0];

  assign tl_sm1_40_us_h2d[0] = tl_s1n_34_ds_h2d[5];
  assign tl_s1n_34_ds_d2h[5] = tl_sm1_40_us_d2h[0];

  assign tl_sm1_35_us_h2d[1] = tl_s1n_41_ds_h2d[0];
  assign tl_s1n_41_ds_d2h[0] = tl_sm1_35_us_d2h[1];

  assign tl_sm1_42_us_h2d[0] = tl_s1n_41_ds_h2d[1];
  assign tl_s1n_41_ds_d2h[1] = tl_sm1_42_us_d2h[0];

  assign tl_sm1_36_us_h2d[1] = tl_s1n_41_ds_h2d[2];
  assign tl_s1n_41_ds_d2h[2] = tl_sm1_36_us_d2h[1];

  assign tl_sm1_43_us_h2d[0] = tl_s1n_41_ds_h2d[3];
  assign tl_s1n_41_ds_d2h[3] = tl_sm1_43_us_d2h[0];

  assign tl_sm1_37_us_h2d[1] = tl_s1n_41_ds_h2d[4];
  assign tl_s1n_41_ds_d2h[4] = tl_sm1_37_us_d2h[1];

  assign tl_sm1_44_us_h2d[0] = tl_s1n_41_ds_h2d[5];
  assign tl_s1n_41_ds_d2h[5] = tl_sm1_44_us_d2h[0];

  assign tl_sm1_38_us_h2d[1] = tl_s1n_41_ds_h2d[6];
  assign tl_s1n_41_ds_d2h[6] = tl_sm1_38_us_d2h[1];

  assign tl_sm1_46_us_h2d[0] = tl_s1n_41_ds_h2d[7];
  assign tl_s1n_41_ds_d2h[7] = tl_sm1_46_us_d2h[0];

  assign tl_sm1_48_us_h2d[0] = tl_s1n_41_ds_h2d[8];
  assign tl_s1n_41_ds_d2h[8] = tl_sm1_48_us_d2h[0];

  assign tl_sm1_50_us_h2d[0] = tl_s1n_41_ds_h2d[9];
  assign tl_s1n_41_ds_d2h[9] = tl_sm1_50_us_d2h[0];

  assign tl_sm1_51_us_h2d[0] = tl_s1n_41_ds_h2d[10];
  assign tl_s1n_41_ds_d2h[10] = tl_sm1_51_us_d2h[0];

  assign tl_sm1_52_us_h2d[0] = tl_s1n_41_ds_h2d[11];
  assign tl_s1n_41_ds_d2h[11] = tl_sm1_52_us_d2h[0];

  assign tl_sm1_39_us_h2d[1] = tl_s1n_41_ds_h2d[12];
  assign tl_s1n_41_ds_d2h[12] = tl_sm1_39_us_d2h[1];

  assign tl_sm1_53_us_h2d[0] = tl_s1n_41_ds_h2d[13];
  assign tl_s1n_41_ds_d2h[13] = tl_sm1_53_us_d2h[0];

  assign tl_sm1_54_us_h2d[0] = tl_s1n_41_ds_h2d[14];
  assign tl_s1n_41_ds_d2h[14] = tl_sm1_54_us_d2h[0];

  assign tl_sm1_55_us_h2d[0] = tl_s1n_41_ds_h2d[15];
  assign tl_s1n_41_ds_d2h[15] = tl_sm1_55_us_d2h[0];

  assign tl_sm1_56_us_h2d[0] = tl_s1n_41_ds_h2d[16];
  assign tl_s1n_41_ds_d2h[16] = tl_sm1_56_us_d2h[0];

  assign tl_sm1_57_us_h2d[0] = tl_s1n_41_ds_h2d[17];
  assign tl_s1n_41_ds_d2h[17] = tl_sm1_57_us_d2h[0];

  assign tl_sm1_58_us_h2d[0] = tl_s1n_41_ds_h2d[18];
  assign tl_s1n_41_ds_d2h[18] = tl_sm1_58_us_d2h[0];

  assign tl_sm1_59_us_h2d[0] = tl_s1n_41_ds_h2d[19];
  assign tl_s1n_41_ds_d2h[19] = tl_sm1_59_us_d2h[0];

  assign tl_sm1_60_us_h2d[0] = tl_s1n_41_ds_h2d[20];
  assign tl_s1n_41_ds_d2h[20] = tl_sm1_60_us_d2h[0];

  assign tl_sm1_61_us_h2d[0] = tl_s1n_41_ds_h2d[21];
  assign tl_s1n_41_ds_d2h[21] = tl_sm1_61_us_d2h[0];

  assign tl_sm1_62_us_h2d[0] = tl_s1n_41_ds_h2d[22];
  assign tl_s1n_41_ds_d2h[22] = tl_sm1_62_us_d2h[0];

  assign tl_sm1_63_us_h2d[0] = tl_s1n_41_ds_h2d[23];
  assign tl_s1n_41_ds_d2h[23] = tl_sm1_63_us_d2h[0];

  assign tl_sm1_64_us_h2d[0] = tl_s1n_41_ds_h2d[24];
  assign tl_s1n_41_ds_d2h[24] = tl_sm1_64_us_d2h[0];

  assign tl_sm1_65_us_h2d[0] = tl_s1n_41_ds_h2d[25];
  assign tl_s1n_41_ds_d2h[25] = tl_sm1_65_us_d2h[0];

  assign tl_sm1_66_us_h2d[0] = tl_s1n_41_ds_h2d[26];
  assign tl_s1n_41_ds_d2h[26] = tl_sm1_66_us_d2h[0];

  assign tl_sm1_40_us_h2d[1] = tl_s1n_41_ds_h2d[27];
  assign tl_s1n_41_ds_d2h[27] = tl_sm1_40_us_d2h[1];

  assign tl_sm1_67_us_h2d[0] = tl_s1n_41_ds_h2d[28];
  assign tl_s1n_41_ds_d2h[28] = tl_sm1_67_us_d2h[0];

  assign tl_sm1_68_us_h2d[0] = tl_s1n_41_ds_h2d[29];
  assign tl_s1n_41_ds_d2h[29] = tl_sm1_68_us_d2h[0];

  assign tl_sm1_35_us_h2d[2] = tl_s1n_69_ds_h2d[0];
  assign tl_s1n_69_ds_d2h[0] = tl_sm1_35_us_d2h[2];

  assign tl_sm1_42_us_h2d[1] = tl_s1n_69_ds_h2d[1];
  assign tl_s1n_69_ds_d2h[1] = tl_sm1_42_us_d2h[1];

  assign tl_sm1_36_us_h2d[2] = tl_s1n_69_ds_h2d[2];
  assign tl_s1n_69_ds_d2h[2] = tl_sm1_36_us_d2h[2];

  assign tl_sm1_43_us_h2d[1] = tl_s1n_69_ds_h2d[3];
  assign tl_s1n_69_ds_d2h[3] = tl_sm1_43_us_d2h[1];

  assign tl_sm1_37_us_h2d[2] = tl_s1n_69_ds_h2d[4];
  assign tl_s1n_69_ds_d2h[4] = tl_sm1_37_us_d2h[2];

  assign tl_sm1_44_us_h2d[1] = tl_s1n_69_ds_h2d[5];
  assign tl_s1n_69_ds_d2h[5] = tl_sm1_44_us_d2h[1];

  assign tl_sm1_38_us_h2d[2] = tl_s1n_69_ds_h2d[6];
  assign tl_s1n_69_ds_d2h[6] = tl_sm1_38_us_d2h[2];

  assign tl_sm1_46_us_h2d[1] = tl_s1n_69_ds_h2d[7];
  assign tl_s1n_69_ds_d2h[7] = tl_sm1_46_us_d2h[1];

  assign tl_sm1_48_us_h2d[1] = tl_s1n_69_ds_h2d[8];
  assign tl_s1n_69_ds_d2h[8] = tl_sm1_48_us_d2h[1];

  assign tl_sm1_50_us_h2d[1] = tl_s1n_69_ds_h2d[9];
  assign tl_s1n_69_ds_d2h[9] = tl_sm1_50_us_d2h[1];

  assign tl_sm1_51_us_h2d[1] = tl_s1n_69_ds_h2d[10];
  assign tl_s1n_69_ds_d2h[10] = tl_sm1_51_us_d2h[1];

  assign tl_sm1_52_us_h2d[1] = tl_s1n_69_ds_h2d[11];
  assign tl_s1n_69_ds_d2h[11] = tl_sm1_52_us_d2h[1];

  assign tl_sm1_39_us_h2d[2] = tl_s1n_69_ds_h2d[12];
  assign tl_s1n_69_ds_d2h[12] = tl_sm1_39_us_d2h[2];

  assign tl_sm1_53_us_h2d[1] = tl_s1n_69_ds_h2d[13];
  assign tl_s1n_69_ds_d2h[13] = tl_sm1_53_us_d2h[1];

  assign tl_sm1_54_us_h2d[1] = tl_s1n_69_ds_h2d[14];
  assign tl_s1n_69_ds_d2h[14] = tl_sm1_54_us_d2h[1];

  assign tl_sm1_55_us_h2d[1] = tl_s1n_69_ds_h2d[15];
  assign tl_s1n_69_ds_d2h[15] = tl_sm1_55_us_d2h[1];

  assign tl_sm1_56_us_h2d[1] = tl_s1n_69_ds_h2d[16];
  assign tl_s1n_69_ds_d2h[16] = tl_sm1_56_us_d2h[1];

  assign tl_sm1_57_us_h2d[1] = tl_s1n_69_ds_h2d[17];
  assign tl_s1n_69_ds_d2h[17] = tl_sm1_57_us_d2h[1];

  assign tl_sm1_58_us_h2d[1] = tl_s1n_69_ds_h2d[18];
  assign tl_s1n_69_ds_d2h[18] = tl_sm1_58_us_d2h[1];

  assign tl_sm1_59_us_h2d[1] = tl_s1n_69_ds_h2d[19];
  assign tl_s1n_69_ds_d2h[19] = tl_sm1_59_us_d2h[1];

  assign tl_sm1_60_us_h2d[1] = tl_s1n_69_ds_h2d[20];
  assign tl_s1n_69_ds_d2h[20] = tl_sm1_60_us_d2h[1];

  assign tl_sm1_61_us_h2d[1] = tl_s1n_69_ds_h2d[21];
  assign tl_s1n_69_ds_d2h[21] = tl_sm1_61_us_d2h[1];

  assign tl_sm1_62_us_h2d[1] = tl_s1n_69_ds_h2d[22];
  assign tl_s1n_69_ds_d2h[22] = tl_sm1_62_us_d2h[1];

  assign tl_sm1_63_us_h2d[1] = tl_s1n_69_ds_h2d[23];
  assign tl_s1n_69_ds_d2h[23] = tl_sm1_63_us_d2h[1];

  assign tl_sm1_64_us_h2d[1] = tl_s1n_69_ds_h2d[24];
  assign tl_s1n_69_ds_d2h[24] = tl_sm1_64_us_d2h[1];

  assign tl_sm1_65_us_h2d[1] = tl_s1n_69_ds_h2d[25];
  assign tl_s1n_69_ds_d2h[25] = tl_sm1_65_us_d2h[1];

  assign tl_sm1_66_us_h2d[1] = tl_s1n_69_ds_h2d[26];
  assign tl_s1n_69_ds_d2h[26] = tl_sm1_66_us_d2h[1];

  assign tl_sm1_40_us_h2d[2] = tl_s1n_69_ds_h2d[27];
  assign tl_s1n_69_ds_d2h[27] = tl_sm1_40_us_d2h[2];

  assign tl_sm1_67_us_h2d[1] = tl_s1n_69_ds_h2d[28];
  assign tl_s1n_69_ds_d2h[28] = tl_sm1_67_us_d2h[1];

  assign tl_sm1_68_us_h2d[1] = tl_s1n_69_ds_h2d[29];
  assign tl_s1n_69_ds_d2h[29] = tl_sm1_68_us_d2h[1];

  assign tl_sm1_38_us_h2d[3] = tl_s1n_70_ds_h2d[0];
  assign tl_s1n_70_ds_d2h[0] = tl_sm1_38_us_d2h[3];

  assign tl_sm1_46_us_h2d[2] = tl_s1n_70_ds_h2d[1];
  assign tl_s1n_70_ds_d2h[1] = tl_sm1_46_us_d2h[2];

  assign tl_s1n_34_us_h2d = tl_rv_core_ibex__corei_i;
  assign tl_rv_core_ibex__corei_o = tl_s1n_34_us_d2h;

  assign tl_rom_ctrl0__rom_o = tl_sm1_35_ds_h2d;
  assign tl_sm1_35_ds_d2h = tl_rom_ctrl0__rom_i;

  assign tl_rom_ctrl1__rom_o = tl_sm1_36_ds_h2d;
  assign tl_sm1_36_ds_d2h = tl_rom_ctrl1__rom_i;

  assign tl_rv_dm__mem_o = tl_sm1_37_ds_h2d;
  assign tl_sm1_37_ds_d2h = tl_rv_dm__mem_i;

  assign tl_sram_ctrl_main__ram_o = tl_sm1_38_ds_h2d;
  assign tl_sm1_38_ds_d2h = tl_sram_ctrl_main__ram_i;

  assign tl_flash_ctrl__mem_o = tl_sm1_39_ds_h2d;
  assign tl_sm1_39_ds_d2h = tl_flash_ctrl__mem_i;

  assign tl_soc_proxy__ctn_o = tl_sm1_40_ds_h2d;
  assign tl_sm1_40_ds_d2h = tl_soc_proxy__ctn_i;

  assign tl_s1n_41_us_h2d = tl_rv_core_ibex__cored_i;
  assign tl_rv_core_ibex__cored_o = tl_s1n_41_us_d2h;

  assign tl_rom_ctrl0__regs_o = tl_sm1_42_ds_h2d;
  assign tl_sm1_42_ds_d2h = tl_rom_ctrl0__regs_i;

  assign tl_rom_ctrl1__regs_o = tl_sm1_43_ds_h2d;
  assign tl_sm1_43_ds_d2h = tl_rom_ctrl1__regs_i;

  assign tl_rv_dm__regs_o = tl_sm1_44_ds_h2d;
  assign tl_sm1_44_ds_d2h = tl_rv_dm__regs_i;

  assign tl_peri_o = tl_asf_45_ds_h2d;
  assign tl_asf_45_ds_d2h = tl_peri_i;

  assign tl_asf_45_us_h2d = tl_sm1_46_ds_h2d;
  assign tl_sm1_46_ds_d2h = tl_asf_45_us_d2h;

  assign tl_spi_host1_o = tl_asf_47_ds_h2d;
  assign tl_asf_47_ds_d2h = tl_spi_host1_i;

  assign tl_asf_47_us_h2d = tl_sm1_48_ds_h2d;
  assign tl_sm1_48_ds_d2h = tl_asf_47_us_d2h;

  assign tl_usbdev_o = tl_asf_49_ds_h2d;
  assign tl_asf_49_ds_d2h = tl_usbdev_i;

  assign tl_asf_49_us_h2d = tl_sm1_50_ds_h2d;
  assign tl_sm1_50_ds_d2h = tl_asf_49_us_d2h;

  assign tl_flash_ctrl__core_o = tl_sm1_51_ds_h2d;
  assign tl_sm1_51_ds_d2h = tl_flash_ctrl__core_i;

  assign tl_flash_ctrl__prim_o = tl_sm1_52_ds_h2d;
  assign tl_sm1_52_ds_d2h = tl_flash_ctrl__prim_i;

  assign tl_aes_o = tl_sm1_53_ds_h2d;
  assign tl_sm1_53_ds_d2h = tl_aes_i;

  assign tl_entropy_src_o = tl_sm1_54_ds_h2d;
  assign tl_sm1_54_ds_d2h = tl_entropy_src_i;

  assign tl_csrng_o = tl_sm1_55_ds_h2d;
  assign tl_sm1_55_ds_d2h = tl_csrng_i;

  assign tl_edn0_o = tl_sm1_56_ds_h2d;
  assign tl_sm1_56_ds_d2h = tl_edn0_i;

  assign tl_edn1_o = tl_sm1_57_ds_h2d;
  assign tl_sm1_57_ds_d2h = tl_edn1_i;

  assign tl_hmac_o = tl_sm1_58_ds_h2d;
  assign tl_sm1_58_ds_d2h = tl_hmac_i;

  assign tl_rv_plic_o = tl_sm1_59_ds_h2d;
  assign tl_sm1_59_ds_d2h = tl_rv_plic_i;

  assign tl_otbn_o = tl_sm1_60_ds_h2d;
  assign tl_sm1_60_ds_d2h = tl_otbn_i;

  assign tl_keymgr_o = tl_sm1_61_ds_h2d;
  assign tl_sm1_61_ds_d2h = tl_keymgr_i;

  assign tl_kmac_o = tl_sm1_62_ds_h2d;
  assign tl_sm1_62_ds_d2h = tl_kmac_i;

  assign tl_sram_ctrl_main__regs_o = tl_sm1_63_ds_h2d;
  assign tl_sm1_63_ds_d2h = tl_sram_ctrl_main__regs_i;

  assign tl_rv_core_ibex__cfg_o = tl_sm1_64_ds_h2d;
  assign tl_sm1_64_ds_d2h = tl_rv_core_ibex__cfg_i;

  assign tl_sram_ctrl_mbox__ram_o = tl_sm1_65_ds_h2d;
  assign tl_sm1_65_ds_d2h = tl_sram_ctrl_mbox__ram_i;

  assign tl_sram_ctrl_mbox__regs_o = tl_sm1_66_ds_h2d;
  assign tl_sm1_66_ds_d2h = tl_sram_ctrl_mbox__regs_i;

  assign tl_soc_proxy__core_o = tl_sm1_67_ds_h2d;
  assign tl_sm1_67_ds_d2h = tl_soc_proxy__core_i;

  assign tl_dma_o = tl_sm1_68_ds_h2d;
  assign tl_sm1_68_ds_d2h = tl_dma_i;

  assign tl_s1n_69_us_h2d = tl_rv_dm__sba_i;
  assign tl_rv_dm__sba_o = tl_s1n_69_us_d2h;

  assign tl_s1n_70_us_h2d = tl_dma__host_i;
  assign tl_dma__host_o = tl_s1n_70_us_d2h;

  always_comb begin
    // default steering to generate error response if address is not within the range
    dev_sel_s1n_34 = 3'd6;
    if ((tl_s1n_34_us_h2d.a_address &
         ~(ADDR_MASK_ROM_CTRL0__ROM)) == ADDR_SPACE_ROM_CTRL0__ROM) begin
      dev_sel_s1n_34 = 3'd0;

    end else if ((tl_s1n_34_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL1__ROM)) == ADDR_SPACE_ROM_CTRL1__ROM) begin
      dev_sel_s1n_34 = 3'd1;

    end else if ((tl_s1n_34_us_h2d.a_address &
                  ~(ADDR_MASK_RV_DM__MEM)) == ADDR_SPACE_RV_DM__MEM) begin
      dev_sel_s1n_34 = 3'd2;

    end else if ((tl_s1n_34_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MAIN__RAM)) == ADDR_SPACE_SRAM_CTRL_MAIN__RAM) begin
      dev_sel_s1n_34 = 3'd3;

    end else if ((tl_s1n_34_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__MEM)) == ADDR_SPACE_FLASH_CTRL__MEM) begin
      dev_sel_s1n_34 = 3'd4;

    end else if ((tl_s1n_34_us_h2d.a_address &
                  ~(ADDR_MASK_SOC_PROXY__CTN)) == ADDR_SPACE_SOC_PROXY__CTN) begin
      dev_sel_s1n_34 = 3'd5;
end
  end

  always_comb begin
    // default steering to generate error response if address is not within the range
    dev_sel_s1n_41 = 5'd30;
    if ((tl_s1n_41_us_h2d.a_address &
         ~(ADDR_MASK_ROM_CTRL0__ROM)) == ADDR_SPACE_ROM_CTRL0__ROM) begin
      dev_sel_s1n_41 = 5'd0;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL0__REGS)) == ADDR_SPACE_ROM_CTRL0__REGS) begin
      dev_sel_s1n_41 = 5'd1;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL1__ROM)) == ADDR_SPACE_ROM_CTRL1__ROM) begin
      dev_sel_s1n_41 = 5'd2;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL1__REGS)) == ADDR_SPACE_ROM_CTRL1__REGS) begin
      dev_sel_s1n_41 = 5'd3;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_RV_DM__MEM)) == ADDR_SPACE_RV_DM__MEM) begin
      dev_sel_s1n_41 = 5'd4;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_RV_DM__REGS)) == ADDR_SPACE_RV_DM__REGS) begin
      dev_sel_s1n_41 = 5'd5;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MAIN__RAM)) == ADDR_SPACE_SRAM_CTRL_MAIN__RAM) begin
      dev_sel_s1n_41 = 5'd6;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_PERI)) == ADDR_SPACE_PERI) begin
      dev_sel_s1n_41 = 5'd7;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SPI_HOST1)) == ADDR_SPACE_SPI_HOST1) begin
      dev_sel_s1n_41 = 5'd8;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_USBDEV)) == ADDR_SPACE_USBDEV) begin
      dev_sel_s1n_41 = 5'd9;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__CORE)) == ADDR_SPACE_FLASH_CTRL__CORE) begin
      dev_sel_s1n_41 = 5'd10;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__PRIM)) == ADDR_SPACE_FLASH_CTRL__PRIM) begin
      dev_sel_s1n_41 = 5'd11;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__MEM)) == ADDR_SPACE_FLASH_CTRL__MEM) begin
      dev_sel_s1n_41 = 5'd12;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_AES)) == ADDR_SPACE_AES) begin
      dev_sel_s1n_41 = 5'd13;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_ENTROPY_SRC)) == ADDR_SPACE_ENTROPY_SRC) begin
      dev_sel_s1n_41 = 5'd14;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_CSRNG)) == ADDR_SPACE_CSRNG) begin
      dev_sel_s1n_41 = 5'd15;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_EDN0)) == ADDR_SPACE_EDN0) begin
      dev_sel_s1n_41 = 5'd16;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_EDN1)) == ADDR_SPACE_EDN1) begin
      dev_sel_s1n_41 = 5'd17;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_HMAC)) == ADDR_SPACE_HMAC) begin
      dev_sel_s1n_41 = 5'd18;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_RV_PLIC)) == ADDR_SPACE_RV_PLIC) begin
      dev_sel_s1n_41 = 5'd19;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_OTBN)) == ADDR_SPACE_OTBN) begin
      dev_sel_s1n_41 = 5'd20;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_KEYMGR)) == ADDR_SPACE_KEYMGR) begin
      dev_sel_s1n_41 = 5'd21;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_KMAC)) == ADDR_SPACE_KMAC) begin
      dev_sel_s1n_41 = 5'd22;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MAIN__REGS)) == ADDR_SPACE_SRAM_CTRL_MAIN__REGS) begin
      dev_sel_s1n_41 = 5'd23;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_RV_CORE_IBEX__CFG)) == ADDR_SPACE_RV_CORE_IBEX__CFG) begin
      dev_sel_s1n_41 = 5'd24;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MBOX__RAM)) == ADDR_SPACE_SRAM_CTRL_MBOX__RAM) begin
      dev_sel_s1n_41 = 5'd25;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MBOX__REGS)) == ADDR_SPACE_SRAM_CTRL_MBOX__REGS) begin
      dev_sel_s1n_41 = 5'd26;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SOC_PROXY__CTN)) == ADDR_SPACE_SOC_PROXY__CTN) begin
      dev_sel_s1n_41 = 5'd27;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_SOC_PROXY__CORE)) == ADDR_SPACE_SOC_PROXY__CORE) begin
      dev_sel_s1n_41 = 5'd28;

    end else if ((tl_s1n_41_us_h2d.a_address &
                  ~(ADDR_MASK_DMA)) == ADDR_SPACE_DMA) begin
      dev_sel_s1n_41 = 5'd29;
end
  end

  always_comb begin
    // default steering to generate error response if address is not within the range
    dev_sel_s1n_69 = 5'd30;
    if ((tl_s1n_69_us_h2d.a_address &
         ~(ADDR_MASK_ROM_CTRL0__ROM)) == ADDR_SPACE_ROM_CTRL0__ROM) begin
      dev_sel_s1n_69 = 5'd0;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL0__REGS)) == ADDR_SPACE_ROM_CTRL0__REGS) begin
      dev_sel_s1n_69 = 5'd1;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL1__ROM)) == ADDR_SPACE_ROM_CTRL1__ROM) begin
      dev_sel_s1n_69 = 5'd2;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_ROM_CTRL1__REGS)) == ADDR_SPACE_ROM_CTRL1__REGS) begin
      dev_sel_s1n_69 = 5'd3;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_RV_DM__MEM)) == ADDR_SPACE_RV_DM__MEM) begin
      dev_sel_s1n_69 = 5'd4;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_RV_DM__REGS)) == ADDR_SPACE_RV_DM__REGS) begin
      dev_sel_s1n_69 = 5'd5;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MAIN__RAM)) == ADDR_SPACE_SRAM_CTRL_MAIN__RAM) begin
      dev_sel_s1n_69 = 5'd6;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_PERI)) == ADDR_SPACE_PERI) begin
      dev_sel_s1n_69 = 5'd7;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SPI_HOST1)) == ADDR_SPACE_SPI_HOST1) begin
      dev_sel_s1n_69 = 5'd8;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_USBDEV)) == ADDR_SPACE_USBDEV) begin
      dev_sel_s1n_69 = 5'd9;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__CORE)) == ADDR_SPACE_FLASH_CTRL__CORE) begin
      dev_sel_s1n_69 = 5'd10;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__PRIM)) == ADDR_SPACE_FLASH_CTRL__PRIM) begin
      dev_sel_s1n_69 = 5'd11;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_FLASH_CTRL__MEM)) == ADDR_SPACE_FLASH_CTRL__MEM) begin
      dev_sel_s1n_69 = 5'd12;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_AES)) == ADDR_SPACE_AES) begin
      dev_sel_s1n_69 = 5'd13;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_ENTROPY_SRC)) == ADDR_SPACE_ENTROPY_SRC) begin
      dev_sel_s1n_69 = 5'd14;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_CSRNG)) == ADDR_SPACE_CSRNG) begin
      dev_sel_s1n_69 = 5'd15;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_EDN0)) == ADDR_SPACE_EDN0) begin
      dev_sel_s1n_69 = 5'd16;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_EDN1)) == ADDR_SPACE_EDN1) begin
      dev_sel_s1n_69 = 5'd17;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_HMAC)) == ADDR_SPACE_HMAC) begin
      dev_sel_s1n_69 = 5'd18;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_RV_PLIC)) == ADDR_SPACE_RV_PLIC) begin
      dev_sel_s1n_69 = 5'd19;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_OTBN)) == ADDR_SPACE_OTBN) begin
      dev_sel_s1n_69 = 5'd20;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_KEYMGR)) == ADDR_SPACE_KEYMGR) begin
      dev_sel_s1n_69 = 5'd21;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_KMAC)) == ADDR_SPACE_KMAC) begin
      dev_sel_s1n_69 = 5'd22;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MAIN__REGS)) == ADDR_SPACE_SRAM_CTRL_MAIN__REGS) begin
      dev_sel_s1n_69 = 5'd23;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_RV_CORE_IBEX__CFG)) == ADDR_SPACE_RV_CORE_IBEX__CFG) begin
      dev_sel_s1n_69 = 5'd24;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MBOX__RAM)) == ADDR_SPACE_SRAM_CTRL_MBOX__RAM) begin
      dev_sel_s1n_69 = 5'd25;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SRAM_CTRL_MBOX__REGS)) == ADDR_SPACE_SRAM_CTRL_MBOX__REGS) begin
      dev_sel_s1n_69 = 5'd26;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SOC_PROXY__CTN)) == ADDR_SPACE_SOC_PROXY__CTN) begin
      dev_sel_s1n_69 = 5'd27;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_SOC_PROXY__CORE)) == ADDR_SPACE_SOC_PROXY__CORE) begin
      dev_sel_s1n_69 = 5'd28;

    end else if ((tl_s1n_69_us_h2d.a_address &
                  ~(ADDR_MASK_DMA)) == ADDR_SPACE_DMA) begin
      dev_sel_s1n_69 = 5'd29;
end
  end

  always_comb begin
    // default steering to generate error response if address is not within the range
    dev_sel_s1n_70 = 2'd2;
    if ((tl_s1n_70_us_h2d.a_address &
         ~(ADDR_MASK_SRAM_CTRL_MAIN__RAM)) == ADDR_SPACE_SRAM_CTRL_MAIN__RAM) begin
      dev_sel_s1n_70 = 2'd0;

    end else if ((tl_s1n_70_us_h2d.a_address &
                  ~(ADDR_MASK_PERI)) == ADDR_SPACE_PERI) begin
      dev_sel_s1n_70 = 2'd1;
end
  end


  // Instantiation phase
  tlul_socket_1n #(
    .HReqDepth (4'h0),
    .HRspDepth (4'h0),
    .DReqDepth (24'h0),
    .DRspDepth (24'h0),
    .N         (6)
  ) u_s1n_34 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_s1n_34_us_h2d),
    .tl_h_o       (tl_s1n_34_us_d2h),
    .tl_d_o       (tl_s1n_34_ds_h2d),
    .tl_d_i       (tl_s1n_34_ds_d2h),
    .dev_select_i (dev_sel_s1n_34)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DRspPass  (1'b0),
    .M         (3)
  ) u_sm1_35 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_35_us_h2d),
    .tl_h_o       (tl_sm1_35_us_d2h),
    .tl_d_o       (tl_sm1_35_ds_h2d),
    .tl_d_i       (tl_sm1_35_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DRspPass  (1'b0),
    .M         (3)
  ) u_sm1_36 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_36_us_h2d),
    .tl_h_o       (tl_sm1_36_us_d2h),
    .tl_d_o       (tl_sm1_36_ds_h2d),
    .tl_d_i       (tl_sm1_36_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (3)
  ) u_sm1_37 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_37_us_h2d),
    .tl_h_o       (tl_sm1_37_us_d2h),
    .tl_d_o       (tl_sm1_37_ds_h2d),
    .tl_d_i       (tl_sm1_37_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (16'h0),
    .HRspDepth (16'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (4)
  ) u_sm1_38 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_38_us_h2d),
    .tl_h_o       (tl_sm1_38_us_d2h),
    .tl_d_o       (tl_sm1_38_ds_h2d),
    .tl_d_i       (tl_sm1_38_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (3)
  ) u_sm1_39 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_39_us_h2d),
    .tl_h_o       (tl_sm1_39_us_d2h),
    .tl_d_o       (tl_sm1_39_ds_h2d),
    .tl_d_i       (tl_sm1_39_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DRspPass  (1'b0),
    .M         (3)
  ) u_sm1_40 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_40_us_h2d),
    .tl_h_o       (tl_sm1_40_us_d2h),
    .tl_d_o       (tl_sm1_40_ds_h2d),
    .tl_d_i       (tl_sm1_40_ds_d2h)
  );
  tlul_socket_1n #(
    .HReqDepth (4'h0),
    .HRspDepth (4'h0),
    .DReqDepth (120'h0),
    .DRspDepth (120'h0),
    .N         (30)
  ) u_s1n_41 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_s1n_41_us_h2d),
    .tl_h_o       (tl_s1n_41_us_d2h),
    .tl_d_o       (tl_s1n_41_ds_h2d),
    .tl_d_i       (tl_s1n_41_ds_d2h),
    .dev_select_i (dev_sel_s1n_41)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_42 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_42_us_h2d),
    .tl_h_o       (tl_sm1_42_us_d2h),
    .tl_d_o       (tl_sm1_42_ds_h2d),
    .tl_d_i       (tl_sm1_42_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_43 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_43_us_h2d),
    .tl_h_o       (tl_sm1_43_us_d2h),
    .tl_d_o       (tl_sm1_43_ds_h2d),
    .tl_d_i       (tl_sm1_43_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_44 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_44_us_h2d),
    .tl_h_o       (tl_sm1_44_us_d2h),
    .tl_d_o       (tl_sm1_44_ds_h2d),
    .tl_d_i       (tl_sm1_44_ds_d2h)
  );
  tlul_fifo_async #(
    .ReqDepth        (1),
    .RspDepth        (1)
  ) u_asf_45 (
    .clk_h_i      (clk_main_i),
    .rst_h_ni     (rst_main_ni),
    .clk_d_i      (clk_fixed_i),
    .rst_d_ni     (rst_fixed_ni),
    .tl_h_i       (tl_asf_45_us_h2d),
    .tl_h_o       (tl_asf_45_us_d2h),
    .tl_d_o       (tl_asf_45_ds_h2d),
    .tl_d_i       (tl_asf_45_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (12'h0),
    .HRspDepth (12'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (3)
  ) u_sm1_46 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_46_us_h2d),
    .tl_h_o       (tl_sm1_46_us_d2h),
    .tl_d_o       (tl_sm1_46_ds_h2d),
    .tl_d_i       (tl_sm1_46_ds_d2h)
  );
  tlul_fifo_async #(
    .ReqDepth        (1),
    .RspDepth        (1)
  ) u_asf_47 (
    .clk_h_i      (clk_main_i),
    .rst_h_ni     (rst_main_ni),
    .clk_d_i      (clk_spi_host1_i),
    .rst_d_ni     (rst_spi_host1_ni),
    .tl_h_i       (tl_asf_47_us_h2d),
    .tl_h_o       (tl_asf_47_us_d2h),
    .tl_d_o       (tl_asf_47_ds_h2d),
    .tl_d_i       (tl_asf_47_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (2)
  ) u_sm1_48 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_48_us_h2d),
    .tl_h_o       (tl_sm1_48_us_d2h),
    .tl_d_o       (tl_sm1_48_ds_h2d),
    .tl_d_i       (tl_sm1_48_ds_d2h)
  );
  tlul_fifo_async #(
    .ReqDepth        (1),
    .RspDepth        (1)
  ) u_asf_49 (
    .clk_h_i      (clk_main_i),
    .rst_h_ni     (rst_main_ni),
    .clk_d_i      (clk_usb_i),
    .rst_d_ni     (rst_usb_ni),
    .tl_h_i       (tl_asf_49_us_h2d),
    .tl_h_o       (tl_asf_49_us_d2h),
    .tl_d_o       (tl_asf_49_ds_h2d),
    .tl_d_i       (tl_asf_49_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (2)
  ) u_sm1_50 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_50_us_h2d),
    .tl_h_o       (tl_sm1_50_us_d2h),
    .tl_d_o       (tl_sm1_50_ds_h2d),
    .tl_d_i       (tl_sm1_50_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_51 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_51_us_h2d),
    .tl_h_o       (tl_sm1_51_us_d2h),
    .tl_d_o       (tl_sm1_51_ds_h2d),
    .tl_d_i       (tl_sm1_51_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_52 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_52_us_h2d),
    .tl_h_o       (tl_sm1_52_us_d2h),
    .tl_d_o       (tl_sm1_52_ds_h2d),
    .tl_d_i       (tl_sm1_52_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_53 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_53_us_h2d),
    .tl_h_o       (tl_sm1_53_us_d2h),
    .tl_d_o       (tl_sm1_53_ds_h2d),
    .tl_d_i       (tl_sm1_53_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_54 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_54_us_h2d),
    .tl_h_o       (tl_sm1_54_us_d2h),
    .tl_d_o       (tl_sm1_54_ds_h2d),
    .tl_d_i       (tl_sm1_54_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_55 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_55_us_h2d),
    .tl_h_o       (tl_sm1_55_us_d2h),
    .tl_d_o       (tl_sm1_55_ds_h2d),
    .tl_d_i       (tl_sm1_55_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_56 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_56_us_h2d),
    .tl_h_o       (tl_sm1_56_us_d2h),
    .tl_d_o       (tl_sm1_56_ds_h2d),
    .tl_d_i       (tl_sm1_56_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_57 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_57_us_h2d),
    .tl_h_o       (tl_sm1_57_us_d2h),
    .tl_d_o       (tl_sm1_57_ds_h2d),
    .tl_d_i       (tl_sm1_57_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_58 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_58_us_h2d),
    .tl_h_o       (tl_sm1_58_us_d2h),
    .tl_d_o       (tl_sm1_58_ds_h2d),
    .tl_d_i       (tl_sm1_58_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_59 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_59_us_h2d),
    .tl_h_o       (tl_sm1_59_us_d2h),
    .tl_d_o       (tl_sm1_59_ds_h2d),
    .tl_d_i       (tl_sm1_59_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_60 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_60_us_h2d),
    .tl_h_o       (tl_sm1_60_us_d2h),
    .tl_d_o       (tl_sm1_60_ds_h2d),
    .tl_d_i       (tl_sm1_60_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_61 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_61_us_h2d),
    .tl_h_o       (tl_sm1_61_us_d2h),
    .tl_d_o       (tl_sm1_61_ds_h2d),
    .tl_d_i       (tl_sm1_61_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_62 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_62_us_h2d),
    .tl_h_o       (tl_sm1_62_us_d2h),
    .tl_d_o       (tl_sm1_62_ds_h2d),
    .tl_d_i       (tl_sm1_62_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_63 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_63_us_h2d),
    .tl_h_o       (tl_sm1_63_us_d2h),
    .tl_d_o       (tl_sm1_63_ds_h2d),
    .tl_d_i       (tl_sm1_63_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_64 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_64_us_h2d),
    .tl_h_o       (tl_sm1_64_us_d2h),
    .tl_d_o       (tl_sm1_64_ds_h2d),
    .tl_d_i       (tl_sm1_64_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (2)
  ) u_sm1_65 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_65_us_h2d),
    .tl_h_o       (tl_sm1_65_us_d2h),
    .tl_d_o       (tl_sm1_65_ds_h2d),
    .tl_d_i       (tl_sm1_65_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqDepth (4'h0),
    .DRspDepth (4'h0),
    .M         (2)
  ) u_sm1_66 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_66_us_h2d),
    .tl_h_o       (tl_sm1_66_us_d2h),
    .tl_d_o       (tl_sm1_66_ds_h2d),
    .tl_d_i       (tl_sm1_66_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_67 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_67_us_h2d),
    .tl_h_o       (tl_sm1_67_us_d2h),
    .tl_d_o       (tl_sm1_67_ds_h2d),
    .tl_d_i       (tl_sm1_67_ds_d2h)
  );
  tlul_socket_m1 #(
    .HReqDepth (8'h0),
    .HRspDepth (8'h0),
    .DReqPass  (1'b0),
    .DRspPass  (1'b0),
    .M         (2)
  ) u_sm1_68 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_sm1_68_us_h2d),
    .tl_h_o       (tl_sm1_68_us_d2h),
    .tl_d_o       (tl_sm1_68_ds_h2d),
    .tl_d_i       (tl_sm1_68_ds_d2h)
  );
  tlul_socket_1n #(
    .HReqPass  (1'b0),
    .HRspPass  (1'b0),
    .DReqDepth (120'h0),
    .DRspDepth (120'h0),
    .N         (30)
  ) u_s1n_69 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_s1n_69_us_h2d),
    .tl_h_o       (tl_s1n_69_us_d2h),
    .tl_d_o       (tl_s1n_69_ds_h2d),
    .tl_d_i       (tl_s1n_69_ds_d2h),
    .dev_select_i (dev_sel_s1n_69)
  );
  tlul_socket_1n #(
    .HReqDepth (4'h0),
    .HRspDepth (4'h0),
    .DReqDepth (8'h0),
    .DRspDepth (8'h0),
    .N         (2)
  ) u_s1n_70 (
    .clk_i        (clk_main_i),
    .rst_ni       (rst_main_ni),
    .tl_h_i       (tl_s1n_70_us_h2d),
    .tl_h_o       (tl_s1n_70_us_d2h),
    .tl_d_o       (tl_s1n_70_ds_h2d),
    .tl_d_i       (tl_s1n_70_ds_d2h),
    .dev_select_i (dev_sel_s1n_70)
  );

endmodule
