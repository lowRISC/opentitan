...
set_rule_status -rule {W_MASYNC} -status {Waived}           \
  -expression {(Driver=~"IO*") &&                           \
    (ReceivingFlop=~"*u_gpio.gen_filter*prim_generic_flop_2sync*")} \
...
set_rule_status -rule {W_RECON_GROUPS} -status {Waived}                           \
  -expression {(ControlSignal =~ "*u_reg.*cdc*.prim_generic_flop_2sync.u_sync_1.*u_impl_generic.q_o*") && (ReconSignal=~"top_darjeeling.*.u_reg*")} \
  -comment {Reconverged signals coming from AST}
