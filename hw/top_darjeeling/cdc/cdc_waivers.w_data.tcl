...
set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"IO*") && \
  (ReceivingFlop =~ "*top_darjeeling.u_lc_ctrl.u_dmi_jtag.i_dmi_jtag_tap.*q*")} -comment {SPI read cmds combined}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.u_mem.gen_generic.u_impl_generic.b_rdata_o*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_fwmode.u_tx_fifo.storage*")} -comment {SPI fwmode mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.u_mem.gen_generic.u_impl_generic.b_rdata_o*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_fwmode.u_txf_ctrl.sram_rdata_q*")} -comment {SPI fwmode mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.u_mem.gen_generic.u_impl_generic.b_rdata_o*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd.p2s_byte_o*")} -comment {SPI readcmd mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.u_mem.gen_generic.u_impl_generic.b_rdata_o*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd.u_readsram.u_fifo.gen_normal_fifo.storage*")} -comment {SPI readcmd mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.u_mem.gen_generic.u_impl_generic.b_rdata_o*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd.u_readsram.u_sram_fifo.gen_normal_fifo.storage*")} -comment {SPI readcmd mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_fwmode.u_rx_fifo.sync_wptr.u_sync_2.gen_generic.u_impl_generic.q_o[3:0]") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_reg.u_reg_if.rdata*")} -comment {SPI reg mux}

set_rule_status -rule {W_DATA} -status {Waived} -expression             \
  {(Signal=~"top_darjeeling.u_spi_device.u_fwmode.u_tx_fifo.storage*") && \
  (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_p2s.out_shift*")} -comment {SPI p2s mux}
...
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"u_ast.dft_scan_md_o*") && (ReceivingFlop =~ "top_darjeeling.u_clkmgr_aon*_scan*")} -comment {W_DATA issues in AST block}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"top_darjeeling.u_spi_device.u_memory_2p.b_rvalid_sram_q*") && (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd*")} -comment {W_DATA from 2 port memory}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"top_darjeeling.u_spi_device.u_reg.u_cmd_info_0_valid_0.q*") && (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_cmdparse.intercept_jedec_o*")} -comment {W_DATA from SPI mux}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(ReceivingFlop=~"top_darjeeling.u_pinmux_aon.dio_oe_retreg_q*")} -comment {multiple source to pinmux output}
...
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"top_darjeeling.u_spi_device.u_reg.u_intercept_en_*") && (ReceivingFlop =~ "top_darjeeling.u_spi_device.u_cmdparse.intercept_jedec_o*")} -comment {W_DATA issues in spi_device jedec}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd.mailbox_assumed_o*")} -comment {Pre-configured, static}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"IO*") && (ReceivingFlop =~ "top_darjeeling.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.*_strap_q*")} -comment {W_DATA issues in pinmux dft strap}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(ReceivingFlop=~"top_darjeeling.u_spi_device.u_readcmd.readbuf_addr*")} -comment {multiple source to spi readcmd readbuf}
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"top_darjeeling.u_spi_device.u_reg.*mailbox*.q*")} -comment {demux output from mailbox in spi device }
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"top_darjeeling.u_spi_device.u_reg.u_intercept_en_*.q*")} -comment {demux output from mailbox in spi device}
...
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.u_rv_dm.dap.*_q*") && (Signal =~ "IOR*") && (Association =~ "None")} -status {Waived} -comment  {W_DATA issues caused by duplicate clocks on PAD}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.u_spi_device.u_spi_tpm.is_*_reg*") && (Signal =~ "top_darjeeling.u_spi_device.u_spi_tpm.sys_clk_tpm_cfg*") && (Association =~ "None")} -status {Waived} -comment {signal is qualified by synchronized pulse}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.u_spi_device.u_upload.*_cmdfifo_set*") && (Signal =~ "top_darjeeling.u_spi_device.u_upload.*_cmdfifo_set*") && (Association =~ "None")} -status {Waived} -comment {tool doesn't recognize multiple spi clocks on the same domain}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.u_spi_device.u_readcmd.u_readbuffer.watermark_crossed*") && (Signal =~ "top_darjeeling.u_spi_device.u_reg.u_read_threshold.q[9:0]*") && (Association =~ "None")} -status {Waived} -comment {tool doesn't recognize multiple spi clocks on the same domain}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.*.i_dmi_jtag_tap.bypass_q*") && (Signal =~ "top_darjeeling.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.u_prim_lc_sender_pinmux_hw_debug_en.gen_flops.u_prim_flop.u_secure_anchor_flop.gen_generic.u_impl_generic.q_o*") && (Association =~ "None")} -status {Waived}  -comment {JTAG mux in quasi-static}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.*.i_dmi_jtag_tap.bypass_q*") && (Signal =~ "top_darjeeling.u_pinmux_aon.mio_pad_attr_q*") && (Association =~ "None")} -status {Waived}  -comment {included in waived paths : start signal and receiving signal (flop) have been reviewed and waived in the same error or other errors} -comment {JTAG mux in quasi-static}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.*.i_dmi_jtag_tap.bypass_q*") && (Signal =~ "top_darjeeling.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.tap_strap_q*") && (Association =~ "None")} -status {Waived}   -comment {JTAG mux in quasi-static}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.*.i_dmi_jtag_tap.bypass_q*") && (Signal =~ "top_darjeeling.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.u_prim_lc_sync_lc_dft_en.gen_flops.u_prim_flop_2sync.u_sync_2.gen_generic.u_impl_generic.q_o*") && (Association =~ "None")} -status {Waived}   -comment {JTAG mux in quasi-static}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "u_ast.rng_en_i*") && (Signal =~ "top_darjeeling.u_entropy_src.*") && (Association =~ "None")} -status {Waived}   -comment {Clarified by Nuvoton : ast liberty model does not have a synchronizer}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "u_ast.rng_fips_i*") && (Signal =~ "top_darjeeling.u_entropy_src.*") && (Association =~ "None")} -status {Waived}   -comment {Clarified by Nuvoton : ast liberty model does not have a synchronizer}
