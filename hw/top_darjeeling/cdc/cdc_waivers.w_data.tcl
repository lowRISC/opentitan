...
set_rule_status -rule {W_DATA} -status {Waived} -expression {(Signal=~"IO*") && (ReceivingFlop =~ "top_darjeeling.u_pinmux_aon.u_pinmux_strap_sampling.*_strap_q*")} -comment {W_DATA issues in pinmux dft strap}
...
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "top_darjeeling.*.i_dmi_jtag_tap.bypass_q*") && (Signal =~ "top_darjeeling.u_pinmux_aon.u_pinmux_strap_sampling.tap_strap_q*") && (Association =~ "None")} -status {Waived}   -comment {JTAG mux in quasi-static}
