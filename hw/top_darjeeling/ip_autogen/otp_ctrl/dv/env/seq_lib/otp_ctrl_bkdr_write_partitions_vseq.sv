...
class otp_ctrl_bkdr_write_partitions_vseq extends otp_ctrl_base_vseq;
  ...
  virtual task otp_ctrl_init();
    ...
    `DV_CHECK_STD_RANDOMIZE_FATAL(soc_dbg_state)
    ...
    `DV_CHECK_STD_RANDOMIZE_FATAL(creator_seed)
    ...
    `DV_CHECK_STD_RANDOMIZE_FATAL(owner_seed)
    otp_ctrl_mem_bkdr_util_pkg::otp_write_secret3_partition(
        .mem_bkdr_util_h(cfg.mem_bkdr_util_h),
  ...
  task body();
    ...
    csr_rd_check(.ptr(ral.partition_status_0.secret3_error), .compare_value(0));
    ...
    digest = cfg.mem_bkdr_util_h.read64(Secret3DigestOffset);
    `DV_CHECK_NE(digest, 0)
    csr_rd_check(.ptr(ral.secret3_digest[0]), .compare_value(digest[31:0]));
    csr_rd_check(.ptr(ral.secret3_digest[1]), .compare_value(digest[63:32]));
  endtask : body
