...
For example, the digest field is never writable through the `WRITE` command in a hardware partition.
    ...
    Although the fuses of a partition can be zeroized in any order, it is recommended to first zeroize the zeroization field at the end of a partition to mark it as zeroized or being zeroized in case the flow is interrupted and needs to be resumed at a later point.
...
2. A successful zeroization of a fuse word results in the number of set bits in the zeroized workd being returned to software in the `DIRECT_ACCESS_RDATA` registers for inspection bypassing the descrambling mechanism if the word belongs to a secret partition.
