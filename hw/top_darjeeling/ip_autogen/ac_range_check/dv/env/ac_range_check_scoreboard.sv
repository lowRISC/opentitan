...
class ac_range_check_scoreboard extends cip_base_scoreboard #(
  ...
  extern function check_access_e check_access(tl_seq_item item);
...
function check_access_e ac_range_check_scoreboard::check_access(tl_seq_item item);
  ...
  `uvm_info(`gfn, $sformatf("Analyzing tl_unfilt_a_chan item #%0d", unfilt_tr_cnt), UVM_LOW)

  // Due to the note above, we should keep this loop starting from index 0
  for (int i=0; i<NUM_RANGES; i++) begin
    // Only consider the enabled ranges
            ...
            if (!dut_cfg.range_perm[i].execute_access) begin
              ...
              if (!dut_cfg.range_racl_policy[i].read_perm) begin
                ...
                `uvm_info(`gfn, $sformatf({"EXECUTE access to address 0x%0h is GRANTED as ",
                          "configured in registers with index #%0d"}, item.a_addr, i), UVM_LOW)
  ...
  `uvm_info(`gfn, $sformatf("No matching access found for %0s access to address 0x%0h ",
                            access_type, item.a_addr), UVM_LOW)
  return AccessDenied;
...
task ac_range_check_scoreboard::process_tl_unfilt_a_chan_fifo();
  ...
  forever begin
    ...
    unfilt_tr_cnt++;
    `uvm_info(`gfn, $sformatf("Received tl_unfilt_a_chan item #%0d:\n%0s",
    ...
    if (check_access(tl_filt.item) == AccessGranted) begin
      ...
      `uvm_info(`gfn, $sformatf("Transaction #%0d on tl_unfilt_a_chan has been filtered",
...
task ac_range_check_scoreboard::compare_tl_a_chan();
  ...
  if (exp_tl_filt_q.size() == 0) begin
    `DV_WAIT(exp_tl_filt_q.size() > 0,
             ...
             " The lastest filtered item was: \n%0s"}, latest_filtered_item.sprint()),
            //  1_000, `gfn, 0)   // TODO see if Rupert understands why uvm_error is not thrown
             1_000, `gfn, 1)
  end
