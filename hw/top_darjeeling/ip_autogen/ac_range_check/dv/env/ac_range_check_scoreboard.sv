...
class ac_range_check_scoreboard extends cip_base_scoreboard #(
  ...
  // Local queues
  tl_filt_t act_tl_a_chan_q[$];               // Actual pending packets to check for A channel
  tl_filt_t exp_tl_d_chan_q[$];               // Expected pending packets to check for D channel
  tl_filt_t act_tl_d_chan_q[$];               // Actual pending packets to check for D channel
...
function cip_tl_seq_item ac_range_check_scoreboard::predict_tl_unfilt_d_chan();
  ...
  `uvm_create_obj(cip_tl_seq_item, tmp_exp)
  ...
  `DV_CHECK_FATAL($cast(tmp_exp, latest_filtered_item));
  ...
  end else if (latest_filtered_item.a_opcode == tlul_pkg::Get) begin
...
// This task predicts the EXPECTED item by using the check_access function. Gets the ACTUAL item
// from its dedicated queue. When both items are available, calls the comparison function.
//
// For D channels, when the check_access called from manage_tl_a_chan return:
...
//     on the tl_filt_d_chan_fifo are forwarded without any change.
// This task predicts the EXPECTED item based on what has been assessed by the manage_tl_a_chan task
// (by calling the check_access function). Gets the ACTUAL item from its dedicated queue. When both
// items are available, calls the comparison function.
task ac_range_check_scoreboard::manage_tl_fifos();
  ...
  forever begin
    ...
    if (tr_access_decision_q[all_unfilt_a_chan_cnt] == AccessGranted) begin
...
task ac_range_check_scoreboard::process_tl_unfilt_a_chan_fifo(output tl_filt_t tl_unfilt);
  ...
  if (tr_access_decision_q[all_unfilt_a_chan_cnt] == AccessGranted) begin
    ...
    latest_filtered_item = tl_unfilt.item;
  end
...
function void ac_range_check_scoreboard::check_phase(uvm_phase phase);
  ...
  if (cfg.en_scb) begin
    ...
    if (d_chan_matching_cnt != a_chan_matching_cnt) begin
      `uvm_error(`gfn, {"The number of matching transactions on the A and on the D channels must",
                        " be equal."})
    end
...
function void ac_range_check_scoreboard::report_phase(uvm_phase phase);
  ...
  if (cfg.en_scb) begin
    `uvm_info(`gfn, $sformatf({"The number of transactions that matched the prediction on a_chan ",
                              "is %0d"}, a_chan_matching_cnt), UVM_MEDIUM)
    `uvm_info(`gfn, $sformatf({"The number of transactions that matched the prediction on d_chan ",
                              "is %0d"}, d_chan_matching_cnt), UVM_MEDIUM)
  end
