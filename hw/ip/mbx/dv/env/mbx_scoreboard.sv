    ...
    fork
     forever begin
       ...
       if(exp_irq == 1) begin
         ...
         `DV_CHECK_EQ(exp_irq, cfg.intr_vif.pins[MbxCoreReady], "Expecting interrupt pin to go high")
       end
       if(exp_irq == 0) begin
...
return;
  ...
  virtual function void process_tl_mbx_soc_access(tl_seq_item item, tl_channels_e channel);
    ...
    bit     do_read_check   = 1'b1;
    bit     write           = item.is_write();
    uvm_reg_addr_t csr_addr = cfg.ral_models[cfg.mbx_soc_ral_name].get_word_aligned_addr(item.a_addr);
    ...
    case(csr.get_name())
      "soc_control" : begin
        ...
        if(data_phase_write) begin
          ...
          end else if((item.a_data[31] & item.a_mask[3]) == 1) begin // if (ctl_reg_h.abort.get() == 1)
