...
covergroup dma_config_cg with function sample(dma_seq_item item);
  ...
  cp_src_asid: coverpoint item.m_src_asid;
  cp_dst_asid: coverpoint item.m_dst_asid;
  cp_data_size: coverpoint item.m_total_transfer_size {
  ...
  cp_transfer_width: coverpoint item.m_per_transfer_width{
    bins one_byte = {0};
    bins two_byte = {1};
    bins fourbyte = {3};
  }
  cp_control_hwen: coverpoint item.m_handshake;
  cp_control_bufinc: coverpoint item.m_auto_inc_buffer;
  cp_control_fifoinc: coverpoint item.m_auto_inc_fifo;
  cp_control_dir: coverpoint item.m_direction;
  cp_range_unlock: coverpoint item.m_mem_range_unlock;
  cp_handshake_intr: coverpoint item.m_handshake_intr_en;
endgroup
