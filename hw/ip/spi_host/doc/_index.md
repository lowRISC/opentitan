...
<tbody>
<tr><td rowspan=2>`byte_starting`</td><td>Entering `WaitLead`</td></tr>
<tr><td>Entering `InternalClkLow` and `bit_cntr` == 0 </td></tr>
<tr><td>`bit_shifting`</td><td>Entering `InternalClkLow` and `bit_cntr` != 0</td></tr>
<tr><td>`byte_ending`</td><td>Exiting `InternalClkHigh` and `bit_cntr` == 0</td></tr>
</tbody>
</table>
...
These delays are likely to cause transient stalls particularly, when constructing commands with many short (less than 4-byte) segments.
...
In practice, this is unlikely to cause a problem, as no Quad-SPI Flash commands require even two back-to-back RX segments.
