...
module spi_host_reg_top (
  ...
  always_comb begin
    ...
    addr_hit[ 4] = (reg_addr == SPI_HOST_CFG_OFFSET);
    addr_hit[ 5] = (reg_addr == SPI_HOST_FIFO_LEVEL_OFFSET);
    addr_hit[ 6] = (reg_addr == SPI_HOST_ASYNC_FIFO_LEVEL_OFFSET);
    addr_hit[ 7] = (reg_addr == SPI_HOST_STATUS_OFFSET);
    addr_hit[ 8] = (reg_addr == SPI_HOST_RXF_PTR_OFFSET);
    addr_hit[ 9] = (reg_addr == SPI_HOST_TXF_PTR_OFFSET);
    addr_hit[10] = (reg_addr == SPI_HOST_RXF_ADDR_OFFSET);
    addr_hit[11] = (reg_addr == SPI_HOST_TXF_ADDR_OFFSET);
    addr_hit[12] = (reg_addr == SPI_HOST_DUMMY_CTRL_OFFSET);
    addr_hit[13] = (reg_addr == SPI_HOST_DUMMY_GATE_OFFSET);
  end
  ...
  always_comb begin
    ...
    if (addr_hit[13] && reg_we && (SPI_HOST_PERMIT[13] != (SPI_HOST_PERMIT[13] & reg_be))) wr_err = 1'b1 ;
  end
