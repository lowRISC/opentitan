...
class spi_host_scoreboard extends cip_base_scoreboard #(
  ...
  // TLM agent fifos
  uvm_tlm_analysis_fifo #(spi_host_item) spi_host_fifo;

  // local queues to hold incoming packets pending comparison
  spi_host_item spi_host_q[$];

  `uvm_component_new

  function void build_phase(uvm_phase phase);
    super.build_phase(phase);
    spi_host_fifo = new("spi_host_fifo", this);
  endfunction
  ...
  task run_phase(uvm_phase phase);
    ...
    fork
      process_spi_host_fifo();
    join_none
  endtask

  virtual task process_spi_host_fifo();
    spi_host_item item;
    forever begin
      spi_host_fifo.get(item);
