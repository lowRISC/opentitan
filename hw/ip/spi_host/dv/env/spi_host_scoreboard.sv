...
class spi_host_scoreboard extends cip_base_scoreboard #(
        ...
        begin
          fork
            ...
            begin
              ...
              // Clock driven in neg-edge if phase != polarity
              if (spi_configopts.cpol[0] == spi_configopts.cpha[0])
  ...
  virtual task check_csaat(bit csaat);
    ...
    int num_times = ((curr_spi_configopts.csntrail[0] + 1 + 1) *
                      (curr_spi_configopts.clkdiv[0] + 1)) + 1;
    spi_host_status_t status;
    `uvm_info(`gfn, $sformatf("%m - carrying out CSAAT (%0d) check", csaat), UVM_DEBUG)

    // Only 1 csb line is currently supported, hence checks reference csb as "vif.CSB[0]"
    fork begin : iso_fork
      fork
