...
class spi_host_base_vseq extends cip_base_vseq #(
  ...
  rand bit [TL_AW:0]    fifo_tx_base_addr;
  rand bit [TL_AW:0]    fifo_rx_base_addr;
  ...
  constraint fifo_base_addr_c {
    fifo_tx_base_addr inside {[SPI_HOST_TX_FIFO_START : SPI_HOST_TX_FIFO_END]};
    fifo_rx_base_addr inside {[SPI_HOST_RX_FIFO_START : SPI_HOST_RX_FIFO_END]};
  }
  ...
  virtual function bit [TL_AW-1:0] get_aligned_tl_addr(spi_host_fifo_e fifo);
    ...
    if (fifo == TxFifo) begin
      `DV_CHECK_MEMBER_RANDOMIZE_FATAL(fifo_tx_base_addr)
      fifo_addr = ral.get_addr_from_offset(fifo_tx_base_addr);
    end else if (fifo == RxFifo) begin
      `DV_CHECK_MEMBER_RANDOMIZE_FATAL(fifo_rx_base_addr)
      fifo_addr = ral.get_addr_from_offset(fifo_rx_base_addr);
    end
    return fifo_addr;
