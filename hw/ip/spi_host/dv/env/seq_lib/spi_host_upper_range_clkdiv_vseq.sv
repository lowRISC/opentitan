...
`ifdef FAST_PRESCALER
  constraint spi_config_regs_faster_clkdiv_c {
    ...
    // VSEQ
    // TODO: disable/adjust csnlead/idle/trail checks when this constraint is available.
    //       currently those checks are part of a PR not yet in master.
    foreach(spi_config_regs.clkdiv[i])
      spi_config_regs.clkdiv[i][3:0] == 0;
  }
`endif
