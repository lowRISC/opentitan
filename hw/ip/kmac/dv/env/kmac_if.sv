...
interface kmac_if(input clk_i, input rst_ni);
  ...
  `OCAH_OT_ASSERT(KmacMaskingO_A, `EN_MASKING == en_masking_o)

  `OCAH_OT_ASSERT(AppKeymgrErrOutputZeros_A, app_rsp[AppKeymgr].error |->
          app_rsp[AppKeymgr].digest_s0 == 0 && app_rsp[AppKeymgr].digest_s1 == 0)

  `OCAH_OT_ASSERT(AppLcErrOutputZeros_A, app_rsp[AppLc].error |->
          app_rsp[AppLc].digest_s0 == 0 && app_rsp[AppLc].digest_s1 == 0)

  `OCAH_OT_ASSERT(AppRomErrOutputZeros_A, app_rsp[AppRom].error |->
