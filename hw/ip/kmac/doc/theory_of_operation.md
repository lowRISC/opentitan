...
#### SwContinueWithoutContext (0x0C)

The Continue command resumes the absorbing stage from the Keccak state instead of starting from a cleared one.
The StateWrite command clears the Keccak state, so the SW must write a context after issuing it.
If the SW issues the Continue command without having written a context into [`STATE`](registers.md#state) after the StateWrite command, the KMAC_ERRCHK module reports the `SwContinueWithoutContext` error.

This error blocks the command, so the KMAC HWIP does not absorb a message on top of an all zero state.
It also catches the case that the SW claimed the KMAC HWIP but its writes to [`STATE`](registers.md#state) were dropped, for example because they were issued before [`STATUS.state_write`](registers.md#status--state_write) was set.
A Continue command without any preceding StateWrite command is reported as `SwCmdSequence` instead, as the KMAC_ERRCHK module does not accept it in the idle state.
The lower 3 bits of [`ERR_CODE`](registers.md#err_code) contain the state of the KMAC_ERRCHK module.
