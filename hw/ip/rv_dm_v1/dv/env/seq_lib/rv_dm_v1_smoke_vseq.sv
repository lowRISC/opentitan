...
// smoke test vseq
class rv_dm_v1_smoke_vseq extends rv_dm_v1_base_vseq;
  `uvm_object_utils(rv_dm_v1_smoke_vseq)
  ...
  task body();
    ...
    repeat ($urandom_range(1, 10)) begin
      ...
      `DV_CHECK_EQ(cfg.rv_dm_v1_vif.debug_req, data)
    ...
    repeat ($urandom_range(1, 10)) begin
      ...
      cfg.clk_rst_vif.wait_clks($urandom_range(0, 1000));
      `DV_CHECK_EQ(cfg.rv_dm_v1_vif.ndmreset_req, data)
      cfg.clk_rst_vif.wait_clks($urandom_range(1, 10));
    end
    repeat ($urandom_range(1, 10)) begin
      data = $urandom_range(0, 1);
      // Verify that the dmstatus[*unavail] field tracks the unavailable_i input.
      cfg.rv_dm_v1_vif.unavailable <= data;
      csr_rd(.ptr(jtag_dmi_ral.dmstatus), .value(data));
      `DV_CHECK_EQ(cfg.rv_dm_v1_vif.unavailable,
    ...
    repeat ($urandom_range(1, 10)) begin
      ...
      cfg.clk_rst_vif.wait_clks($urandom_range(0, 1000));
      `DV_CHECK_EQ(cfg.rv_dm_v1_vif.dmactive, data)
      cfg.clk_rst_vif.wait_clks($urandom_range(1, 10));
    end
  endtask : body

endclass : rv_dm_v1_smoke_vseq
