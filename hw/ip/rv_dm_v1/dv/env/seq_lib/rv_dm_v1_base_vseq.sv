...
class rv_dm_v1_base_vseq extends cip_base_vseq #(
    .RAL_T               (rv_dm_v1_regs_reg_block),
    .CFG_T               (rv_dm_v1_env_cfg),
    .COV_T               (rv_dm_v1_env_cov),
    .VIRTUAL_SEQUENCER_T (rv_dm_v1_virtual_sequencer)
  );
  `uvm_object_utils(rv_dm_v1_base_vseq)
  `uvm_object_new
  ...
  task pre_start();
    ...
    cfg.rv_dm_v1_vif.lc_hw_debug_en <= lc_hw_debug_en;
    `DV_CHECK_MEMBER_RANDOMIZE_FATAL(scanmode)
    cfg.rv_dm_v1_vif.scanmode <= scanmode;
    `DV_CHECK_MEMBER_RANDOMIZE_FATAL(unavailable)
    cfg.rv_dm_v1_vif.unavailable <= unavailable;
    super.pre_start();
  endtask
  ...
  virtual task dut_shutdown();
    ...
    // Check for pending rv_dm_v1 operations and wait for them to complete.
      ...
      begin: isolation_thread
        fork
          forever @m_tl_sba_device_seq.inject_d_chan_intg_err begin
            cfg.rv_dm_v1_vif.disable_tlul_assert_host_sba_resp_svas =
