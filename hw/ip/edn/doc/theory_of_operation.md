...
CSRNG itself will only consider the first 12 words of additional data and silently discard the rest.)
...
In this mode, EDN automatically issues a special reduced-latency `instantiate` command followed by the default `generate` commands.
...
To exit boot-time request mode, firmware has two options:
1. It clears the `EDN_ENABLE` field in [`CTRL`](registers.md#ctrl) to disable EDN immediately.
   This likely causes EDN and the associated CSRNG instance to go out of sync.
   Before re-enabling EDN, CSRNG has to first be disabled and then re-enabled.
2. It clears the `BOOT_REQ_MODE` field in [`CTRL`](registers.md#ctrl).
   Firmware must then wait for the transition of the state machine by polling the `CMD_RDY` field of the [`SW_CMD_STS`](registers.md#sw_cmd_sts) register or by polling the [`MAIN_SM_STATE`](registers.md#main_sm_state) register.
   Upon exiting boot-time request mode this way, EDN issues an `uninstantiate` command to destroy the associated CSRNG instance.
...
Before entering auto request mode, it is the responsibility of firmware to first load the required `generate` and `reseed` commands into the respective command replay FIFOs via the [`GENERATE_CMD`](registers.md#generate_cmd) and [`RESEED_CMD`](registers.md#reseed_cmd) registers.
...
Before entering auto request mode, firmware must configure a non-zero value in the [`MAX_NUM_REQS_BETWEEN_RESEEDS`](registers.md#max-num-reqs-between-reseeds) register.
...
Auto request mode can then be entered by programming the [`CTRL`](registers.md#ctrl) register with `EDN_ENABLE` and `AUTO_REQ_MODE` fields asserted.
...
Upon entering auto request mode, firmware must first generate an `instantiate` command for the EDN-associated instance via the [`SW_CMD_REQ`](registers.md#sw_cmd_req) register.
Once the CSRNG instance has been instantiated, and the `generate` and `reseed` commands have been loaded,
...
Unlike when exiting from boot-time request mode, EDN does not automatically trigger an `uninstantiate` command when leaving auto request mode.
