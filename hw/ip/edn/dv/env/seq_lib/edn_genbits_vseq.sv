...
class edn_genbits_vseq extends edn_base_vseq;
  ...
  task body();
    ...
    m_endpoint_pull_seq[boot_requester] = push_pull_host_seq#(edn_pkg::FIPS_ENDPOINT_BUS_WIDTH)::type_id::
        create($sformatf("m_endpoint_pull_seq[%0d]", boot_requester));

    // Verify boot_req_mode cmds, continue after disable
    if (cfg.boot_req_mode == MuBi4True) begin
      m_endpoint_pull_seq[boot_requester].num_trans = csrng_pkg::GENBITS_BUS_WIDTH/ENDPOINT_BUS_WIDTH;
