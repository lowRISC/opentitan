...
class lc_ctrl_v1_base_vseq extends cip_base_vseq #(
  .RAL_T              (lc_ctrl_v1_reg_block),
  .CFG_T              (lc_ctrl_v1_env_cfg),
  .COV_T              (lc_ctrl_v1_env_cov),
  .VIRTUAL_SEQUENCER_T(lc_ctrl_v1_virtual_sequencer)
);
  `uvm_object_utils(lc_ctrl_v1_base_vseq)
  ...
  virtual task pre_start();
    ...
    cfg.lc_ctrl_v1_vif.set_test_sequence_typename(this.get_type_name());
    super.pre_start();
  endtask

  virtual task dut_init(string reset_kind = "HARD");
    // Clear static signals so we get transitions for toggle coverage
    cfg.lc_ctrl_v1_vif.clear_static_signals();
    // OTP inputs `lc_state` and `lc_cnt` need to be stable before lc_ctrl_v1's reset is deasserted
    if (do_lc_ctrl_v1_init) begin
      drive_otp_i();
      cfg.pwr_lc_vif.drive_pin(LcPwrInitReq, 0);
    end
    super.dut_init();
    if (do_lc_ctrl_v1_init) lc_ctrl_v1_init();
  endtask
  ...
  virtual task drive_otp_i(bit rand_otp_i = 1);
    ...
    cfg.lc_ctrl_v1_vif.init(.lc_state(lc_state), .lc_cnt(lc_cnt), .otp_device_id(cfg.otp_device_id),
