...
class lc_ctrl_v1_errors_vseq extends lc_ctrl_v1_smoke_vseq;

  // Process to handle alerts
  protected process handle_alerts_process;
  // Error injection control
  rand lc_ctrl_v1_err_inj_t err_inj;
  ...
  `uvm_object_utils_begin(lc_ctrl_v1_errors_vseq)
  ...
  virtual task post_start();
    `uvm_info(`gfn, "post_start: Task called for lc_ctrl_v1_errors_vseq", UVM_MEDIUM)
  ...
  task body();
    ...
    for (int i = 1; i <= num_trans; i++) begin
      ...
      if (cfg.err_inj.token_mux_ctrl_redun_err || cfg.err_inj.token_mux_digest_err) begin
        ...
        wait(cfg.lc_ctrl_v1_vif.lc_ctrl_v1_fsm_state inside {PostTransSt, InvalidSt, EscalateSt});
  ...
  virtual function void update_err_inj_cfg();
    cfg.err_inj = err_inj;
    // Debug signal in lc_ctrl_v1_if
    cfg.lc_ctrl_v1_vif.err_inj <= err_inj;
  ...
  virtual function void do_print(uvm_printer printer);
    ...
    // err_inj
    printer.print_generic(.name("err_inj"), .type_name("lc_ctrl_v1_err_inj_t"), .size($bits(err_inj)),
    ...
    forever begin
      ...
      if (err_inj.flash_rma_error_rsp) begin
        ...
        if(err_inj.flash_rma_rsp_mubi_err) begin
          wait(cfg.lc_ctrl_v1_vif.lc_ctrl_v1_fsm_state inside {FlashRmaSt});
  ...
  virtual task security_escalation_inject();
    ...
    wait(cfg.lc_ctrl_v1_vif.lc_ctrl_v1_fsm_state == security_escalation_err_inj_state);
    ...
    if (security_escalation_err_inj_state == TokenHashSt && $urandom_range(0, 1)) begin
      wait(cfg.lc_ctrl_v1_vif.token_hash_ack_i == 1);
    end
  ...
  virtual task token_mux_ctrl_err_inject();
    ...
    // Wait for FSM to reach state we want to inject the error
    wait(cfg.lc_ctrl_v1_vif.lc_ctrl_v1_fsm_state == inj_state);

    // Read the correct token index value
    good_idx = idx_to_change ? cfg.lc_ctrl_v1_vif.token_idx0 : cfg.lc_ctrl_v1_vif.token_idx1;
    // Randomize a bad token index
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(bad_idx, bad_idx != good_idx;)
    // Force RTL
    cfg.lc_ctrl_v1_vif.force_token_idx(idx_to_change, bad_idx);
    `uvm_info(`gfn, $sformatf(
              "token_mux_ctrl_err_inject: detected FSM state %s, forcing %h to token_idx%0d",
              cfg.lc_ctrl_v1_vif.lc_ctrl_v1_fsm_state.name,
              bad_idx,
              idx_to_change
              ), UVM_MEDIUM)
    ...
    // Release RTL
    cfg.lc_ctrl_v1_vif.release_token_idx(idx_to_change);
    `uvm_info(`gfn, $sformatf(
  ...
  virtual task token_mux_digest_err_inject();
    ...
    if (inj_state == TokenHashSt) wait(cfg.lc_ctrl_v1_vif.token_hash_ack_i == 1);

    // Read the correct token index value
    good_token = token_to_change ? cfg.lc_ctrl_v1_vif.hashed_token_i :
        cfg.lc_ctrl_v1_vif.hashed_token_mux;
    ...
    // Force RTL
    cfg.lc_ctrl_v1_vif.force_hashed_token(token_to_change, bad_token);
    `uvm_info(`gfn, $sformatf(
    ...
    // Release RTL
    cfg.lc_ctrl_v1_vif.release_hashed_token(token_to_change);
    `uvm_info(`gfn, $sformatf(
