...
class lc_ctrl_v1_volatile_unlock_smoke_vseq extends lc_ctrl_v1_smoke_vseq;

  rand dec_lc_state_e next_state;
  rand bit next_state_is_testunlock0;

  `uvm_object_utils(lc_ctrl_v1_volatile_unlock_smoke_vseq)
  `uvm_object_new
  ...
  task body();
    ...
    if (lc_cnt != LcCnt24) begin
      ...
      if (next_state == DecLcStTestUnlocked0) begin
        ...
        `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.strap_en_override_o, 1);
        ...
        `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.strap_en_override_o, 0);
  ...
  virtual task sw_transition_req(bit [TL_DW-1:0] next_lc_state, bit [TL_DW*4-1:0] token_val);
    ...
    csr_wr(ral.transition_cmd, 'h01);
  endtask
endclass : lc_ctrl_v1_volatile_unlock_smoke_vseq
