...
class lc_ctrl_v1_env_cfg extends cip_base_env_cfg #(
  .RAL_T(lc_ctrl_v1_reg_block)
);
  // DUT parameters
  lc_ctrl_v1_parameters_cfg parameters_cfg;
  ...
  lc_ctrl_v1_test_phase_e test_phase;
  ...
  `uvm_object_utils_begin(lc_ctrl_v1_env_cfg)
  `uvm_object_utils_end

  `uvm_object_new

  virtual function void initialize(bit [31:0] csr_base_addr = '1);
    list_of_alerts = lc_ctrl_v1_env_pkg::LIST_OF_ALERTS;
    tl_intg_alert_name = "fatal_bus_integ_error";
    sec_cm_alert_name = "fatal_state_error";
    super.initialize(csr_base_addr);

    // Find parameters on config db
    if (!uvm_config_db#(lc_ctrl_v1_parameters_cfg)::get(
  ...
  virtual function void set_test_phase(lc_ctrl_v1_test_phase_e test_phase);
    // Set in config object
    this.test_phase = test_phase;
    // And interface
    lc_ctrl_v1_vif.test_phase <= test_phase;
    ->set_test_phase_ev;
  endfunction

  virtual function lc_ctrl_v1_test_phase_e get_test_phase();
