...
class lc_ctrl_v1_scoreboard extends cip_base_scoreboard #(
  .CFG_T(lc_ctrl_v1_env_cfg),
  .RAL_T(lc_ctrl_v1_reg_block),
  .COV_T(lc_ctrl_v1_env_cov)
);
  `uvm_component_utils(lc_ctrl_v1_scoreboard)
  ...
  virtual task check_lc_output();
    forever begin
      @(posedge cfg.pwr_lc_vif.pins[LcPwrDoneRsp] && cfg.en_scb) begin
        ...
        if (cfg.err_inj.otp_secrets_valid_mubi_err && lc_state_e'(cfg.lc_ctrl_v1_vif.otp_i.state)
  ...
  virtual task process_otp_prog_rsp();
    ...
    forever begin
      ...
      if (!cfg.err_inj.lc_fsm_backdoor_err && !cfg.err_inj.count_backdoor_err) begin
        ...
        // are unpredictable
        `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.otp_vendor_test_ctrl_o, predict_otp_vendor_test_ctrl())
      end
    end
  endtask
  ...
  virtual task process_jtag_riscv();
    ...
    forever begin
      ...
      if ((jt_item.op === DmiRead || jt_item.op === DmiWrite) && jt_item.status === DmiNoErr) begin
        ...
        process_tl_access(tl_item, AddrChannel, "lc_ctrl_v1_reg_block");
        process_tl_access(tl_item, DataChannel, "lc_ctrl_v1_reg_block");
  ...
  virtual function void check_lc_outputs(lc_outputs_t exp_o = '{default: lc_ctrl_v1_pkg::Off},
    ...
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_hw_debug_en_o, exp_o.lc_hw_debug_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_cpu_en_o, exp_o.lc_cpu_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_keymgr_en_o, exp_o.lc_keymgr_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_escalate_en_o, exp_o.lc_escalate_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_owner_seed_sw_rw_en_o, exp_o.lc_owner_seed_sw_rw_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_iso_part_sw_rd_en_o, exp_o.lc_iso_part_sw_rd_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_iso_part_sw_wr_en_o, exp_o.lc_iso_part_sw_wr_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_seed_hw_rd_en_o, exp_o.lc_seed_hw_rd_en_o, msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.lc_creator_seed_sw_rw_en_o, exp_o.lc_creator_seed_sw_rw_en_o,
                 msg)
    `DV_CHECK_EQ(cfg.lc_ctrl_v1_vif.clk_byp_req_o, exp_clk_byp_req, msg)
  endfunction
      ...
      case (csr.get_name())
        ...
        "hw_revision0": begin
          `DV_CHECK_FATAL(
              ...
              .value(LcCtrlSiliconCreatorId[lc_ctrl_v1_reg_pkg::SiliconCreatorIdWidth-1:0]),
              .kind(UVM_PREDICT_READ)))
          `DV_CHECK_FATAL(
              ral.hw_revision0.product_id.predict(
              .value(LcCtrlProductId[lc_ctrl_v1_reg_pkg::ProductIdWidth-1:0]),
              .kind(UVM_PREDICT_READ)))
        end
        ...
        "hw_revision1": begin
          `DV_CHECK_FATAL(
              ...
              .value(LcCtrlRevisionId[lc_ctrl_v1_reg_pkg::RevisionIdWidth-1:0]),
              .kind(UVM_PREDICT_READ)))
        end
  ...
  virtual function bit [31:0] predict_lc_state();
    ...
    unique case (cfg.test_phase)
      LcCtrlTestInit, LcCtrlIterStart, LcCtrlLcInit, LcCtrlDutInitComplete,
        ...
        lc_state_single_exp = dec_lc_state(lc_state_e'(cfg.lc_ctrl_v1_vif.otp_i.state));
      end
  ...
  virtual function dec_lc_cnt_t predict_lc_transition_cnt();
    ...
    unique case (cfg.test_phase)
      ...
      LcCtrlDutReady, LcCtrlWaitTransition: begin
        ...
        lc_transition_cnt_exp = dec_lc_cnt(lc_cnt_e'(cfg.lc_ctrl_v1_vif.otp_i.count));
      end
  ...
  virtual function otp_ctrl_pkg::lc_otp_program_req_t predict_otp_prog_req();
    ...
    const lc_state_e LcStateIn = lc_state_e'(cfg.lc_ctrl_v1_vif.otp_i.state);
    const lc_cnt_e LcCntIn = lc_cnt_e'(cfg.lc_ctrl_v1_vif.otp_i.count);
