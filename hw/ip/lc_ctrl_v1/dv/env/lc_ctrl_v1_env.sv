...
class lc_ctrl_v1_env extends cip_base_env #(
  .CFG_T              (lc_ctrl_v1_env_cfg),
  .COV_T              (lc_ctrl_v1_env_cov),
  .VIRTUAL_SEQUENCER_T(lc_ctrl_v1_virtual_sequencer),
  .SCOREBOARD_T       (lc_ctrl_v1_scoreboard)
);
  `uvm_component_utils(lc_ctrl_v1_env)
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(lc_ctrl_v1_vif)::get(this, "", "lc_ctrl_v1_vif", cfg.lc_ctrl_v1_vif)) begin
      `uvm_fatal(`gfn, "failed to get lc_ctrl_v1_vif from uvm_config_db")
    end
