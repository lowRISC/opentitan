...
module usbuart_core (
  ...
  assign unused_ctrl_q = {
    reg2hw.ctrl.nf.q, reg2hw.ctrl.llpbk.q, reg2hw.ctrl.parity_en.q, reg2hw.ctrl.parity_odd.q
  };
  ...
  always_ff @(posedge clk_i or negedge rst_ni)
    if (!rst_ni) nco_sum <= 17'h0;
    else if (tx_enable || rx_enable) nco_sum <= {1'b0, nco_sum[15:0]} + {1'b0, reg2hw.ctrl.nco.q};
  ...
  tick_baud_x16 ? (rx_timeout_count + 24'd1) : rx_timeout_count;
  ...
  assign
      event_rx_break_err = ~sys_usb_sense | (reg2hw.ctrl.rxblvl.q == 0) ? host_lost : host_timeout;
