...
module padring_assert_fpv (
  ...
  `ASSERT(MioIn_A, mio_in_o[mio_sel] == mio_pad_io[mio_sel] ^ mio_attr_i[mio_sel][0], clk_pad_i,
          !rst_pad_ni)

  `ASSERT(MioInBackwardCheck_A,
          ##1 !$stable(mio_in_o[mio_sel]) |->
              !$stable(mio_pad_io[mio_sel]) || !$stable(mio_attr_i[mio_sel][0]),
          clk_pad_i, !rst_pad_ni)

  `ASSERT(MioOutNormal_A,
          mio_oe_i[mio_sel] && !mio_attr_i[mio_sel][1] |-> mio_output_value == mio_pad_io[mio_sel],
          clk_pad_i, !rst_pad_ni)

  `ASSERT(MioOutOd0_A,
          mio_oe_i[mio_sel] && mio_attr_i[mio_sel][1] && !mio_output_value |->
  ...
  `ASSERT(MioOutOd1_A,
          mio_oe_i[mio_sel] && mio_attr_i[mio_sel][1] && mio_output_value
              |-> mio_pad_io[mio_sel] === 1'b0 || mio_pad_io[mio_sel] === 1'b1 ||
              mio_pad_io[mio_sel] === 1'bz || mio_pad_io[mio_sel] === 1'bx,
          clk_pad_i, !rst_pad_ni)

  // TODO: find a better way to test high-Z?
  `ASSERT(MioOe_A,
          !mio_oe_i[mio_sel] |-> mio_pad_io[mio_sel] === 1'b0 || mio_pad_io[mio_sel] === 1'b1 ||
              mio_pad_io[mio_sel] === 1'bz || mio_pad_io[mio_sel] === 1'bx,
          clk_pad_i, !rst_pad_ni)
  ...
  `ASSERT(DioIn_A, dio_in_o[dio_sel] == dio_pad_io[dio_sel] ^ dio_attr_i[dio_sel][0], clk_pad_i,
          !rst_pad_ni)

  `ASSERT(DioInBackwardCheck_A,
          ##1 !$stable(dio_in_o[dio_sel]) |->
              !$stable(dio_pad_io[dio_sel]) || !$stable(dio_attr_i[dio_sel][0]),
          clk_pad_i, !rst_pad_ni)

  `ASSERT(DioOutNormal_A,
          dio_oe_i[dio_sel] && !dio_attr_i[dio_sel][1] |-> dio_output_value == dio_pad_io[dio_sel],
          clk_pad_i, !rst_pad_ni)

  `ASSERT(DioOutOd0_A,
          dio_oe_i[dio_sel] && dio_attr_i[dio_sel][1] && !dio_output_value |->
  ...
  `ASSERT(DioOutOd1_A,
          dio_oe_i[dio_sel] && dio_attr_i[dio_sel][1] && dio_output_value
              |-> dio_pad_io[dio_sel] === 1'b0 || dio_pad_io[dio_sel] === 1'b1 ||
              dio_pad_io[dio_sel] === 1'bz || dio_pad_io[dio_sel] === 1'bx,
          clk_pad_i, !rst_pad_ni)

  // TODO: find a better way to test high-Z?
  `ASSERT(DioOe_A,
          !dio_oe_i[dio_sel] |-> dio_pad_io[dio_sel] === 1'b0 || dio_pad_io[dio_sel] === 1'b1 ||
              dio_pad_io[dio_sel] === 1'bz || dio_pad_io[dio_sel] === 1'bx,
          clk_pad_i, !rst_pad_ni)
