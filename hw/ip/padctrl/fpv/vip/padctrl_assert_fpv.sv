...
module padctrl_assert_fpv (
  ...
  `ASSERT(MioAttr_A,
          padctrl.reg2hw.mio_pads[mio_sel].qe |=> mio_attr_o[mio_sel] ==
              $past(padctrl.reg2hw.mio_pads[mio_sel].q & mio_warl_masks[mio_sel]))
  `ASSERT(MioBackwardCheck_A,
          ##2 !$stable(mio_attr_o[mio_sel])
  ...
  `ASSERT(DioAttr_A,
          padctrl.reg2hw.dio_pads[dio_sel].qe |=> dio_attr_o[dio_sel] ==
              $past(padctrl.reg2hw.dio_pads[dio_sel].q & dio_warl_masks[dio_sel]))
  `ASSERT(DioBackwardCheck_A,
          ##2 !$stable(dio_attr_o[dio_sel])
