...
module usbdev_usbif  #(
  ...
  // Assertions //
  ////////////////
  `OCAH_OT_ASSERT_INIT(ParamNEndpointsValid, (NEndpoints > 0) && (NEndpoints <= 16))
  `OCAH_OT_ASSERT_INIT(ParamAVFifoWidthValid, AVFifoWidth > 0)
  `OCAH_OT_ASSERT_INIT(ParamRXFifoWidthValid, RXFifoWidth > 0)
  `OCAH_OT_ASSERT_INIT(ParamMaxPktSizeByteValid, MaxPktSizeByte == 64)
  `OCAH_OT_ASSERT_INIT(ParamNBufValid, NBuf > 1)
  // The SRAM should be large enough for all the buffers of max size (4-byte
  // data width)
  `OCAH_OT_ASSERT_INIT(ParamSramAwValid, SramAw >= NBufWidth + PktW - 2)
