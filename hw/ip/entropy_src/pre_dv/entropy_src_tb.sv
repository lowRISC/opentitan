...
module entropy_src_tb;
  ...
  initial begin
    ...
    if (interrupt_test) begin
      ...
      cmp_reg({
        26'b0, ENTROPY_SRC_INTR_STATE_OFFSET
      }, 32'h000000000, 32'hffff_ffff);  // check to see that interrupts are off
      // test intrp 0
      wr_reg({26'b0, ENTROPY_SRC_INTR_TEST_OFFSET}, 32'h0000_0001);
      cmp_reg({
        26'b0, ENTROPY_SRC_INTR_STATE_OFFSET
      }, 32'h000000001, 32'hffff_ffff);  // check interrupt state
      wr_reg({26'b0, ENTROPY_SRC_INTR_STATE_OFFSET}, 32'h0000_0001);
      cmp_reg({
        26'b0, ENTROPY_SRC_INTR_STATE_OFFSET
      }, 32'h000000000, 32'hffff_ffff);  // check to see that interrupts are off
      ...
      cmp_reg({
        26'b0, ENTROPY_SRC_INTR_STATE_OFFSET
      }, 32'h000000002, 32'hffff_ffff);  // check interrupt state
    ...
    if (lfsr_update_test) begin
      ...
      repeat (10) @(posedge clk);
      wr_reg({
        26'b0, ENTROPY_SRC_ES_SEED_OFFSET
      }, 32'h2222_2222);  // change seed, should not write since fifo is not full
      ...
      cmp_reg({
        26'b0, ENTROPY_SRC_ES_FDEPTHST_OFFSET
      }, 32'h000000020, 32'hffff_ffff);  // check to see if FIFO is full
      wr_reg({
        26'b0, ENTROPY_SRC_ES_SEED_OFFSET
      }, 32'h3333_3333);  // change seed, should write since init bit is on
      wr_reg({26'b0, ENTROPY_SRC_ES_CTRL_OFFSET}, 32'h0000_0001);  // init bit on
      wr_reg({26'b0, ENTROPY_SRC_ES_CTRL_OFFSET}, 32'h0000_0000);  // init bit off
      repeat (10) @(posedge clk);
      wr_reg({
        26'b0, ENTROPY_SRC_ES_REGEN_OFFSET
      }, 32'h0000_0001);  // clear write enable register, make it off
      wr_reg({
        26'b0, ENTROPY_SRC_ES_SEED_OFFSET
      }, 32'h4444_4444);  // change seed, should not write since lock seed bit is on
      cmp_reg({
        26'b0, ENTROPY_SRC_ES_SEED_OFFSET
      }, 32'h3333_3333, 32'hffff_ffff);  // check to see that seed did not change
      wr_reg({26'b0, ENTROPY_SRC_ES_CTRL_OFFSET}, 32'h0000_0001);  // init bit on
      wr_reg({26'b0, ENTROPY_SRC_ES_CTRL_OFFSET}, 32'h0000_0000);  // init bit off
