...
class entropy_src_scoreboard extends cip_base_scoreboard#(
  ...
  // and use that to anticipate any alerts that may come about background disable events
  bit fifos_cleared = 1;
  ...
  // 32-bit segment of this seed (as determined by seed_tl_read_cnt)
  ...
  // To track interrupt events we need to identify interrupts have
  ...
  // Due to subtleties in timing, the DUT resets the Observe FIFO with a unique delay
  typedef enum int {
    HardReset,
    Disable,
    Enable,
  ...
  function void update_watermark(string test, int value, bit disabling = 0);
    ...
    // disabled.
    if ((test == "adaptp_hi") || (test == "adaptp_lo") || (test == "bucket") ||
        (test == "markov_hi") || (test == "markov_lo"))
      windowed_test = 1;
    else begin
      windowed_test = 0;
    end
    ...
    if (rst_type == Enable) begin
      fork
        begin
          ...
          bit ht_watermark_high = (ht_watermark_num == REPCNT_HI)  ||
                                  (ht_watermark_num == REPCNTS_HI) ||
                                  (ht_watermark_num == ADAPTP_HI)  ||
                                  (ht_watermark_num == BUCKET_HI)  ||
                                  (ht_watermark_num == MARKOV_HI)  ||
                                  (ht_watermark_num == EXTHT_HI);
  ...
  virtual function void check_redundancy_val(string reg_name, string mubi_field,
    ...
    // ALERT_THRESHOLD in which the threshold field must equal the inverse of the
