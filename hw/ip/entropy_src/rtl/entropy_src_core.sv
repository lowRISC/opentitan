...
module entropy_src_core import entropy_src_pkg::*; #(
  ...
  localparam int EsEnPulseCopies = 6;
 ...
 // set the err code type bits
  ...
  assign es_bypass_mode_d = module_en_pulse_fo[3] ?
  ...
  assign pfifo_postht_clr = fw_ov_mode_entropy_insert ? !es_enable_fo[6] : module_en_pulse_fo[4];
  ...
  assign observe_fifo_thresh_met = fw_ov_mode && (observe_fifo_thresh != '0) &&
         (observe_fifo_thresh <= sfifo_observe_depth) && es_enable_fo[7];
  ...
  assign sfifo_observe_clr  = ~es_enable_fo[8];
  ...
  assign pfifo_bypass_clr = !es_enable_fo[10];
  ...
  assign stale_seed_processing = ~es_bypass_mode & ~fw_ov_mode_entropy_insert &
                                 cs_aes_halt_req & ~es_enable_fo[11];
  ...
  assign main_sm_extd_en_n_d = cs_aes_halt_req && !es_enable_fo[12] ? 1'b0 :
                               mubi4_test_true_strict(sha3_done_q) ? 1'b1 :
                               main_sm_extd_en_n_q;

  // Also, we keep the main state machine enabled while there is SHA data in flight
  assign cond_data_in_flight = ~es_bypass_mode &
                                 ( sfifo_esrng_pop  |
                                   pfifo_esbit_pop  |
                                   pfifo_postht_pop |
                                   pfifo_precon_pop );

  assign main_sm_enable = (es_enable_fo[13] | cond_data_in_flight) & main_sm_extd_en_n_q;
  ...
  assign fips_compliance = !es_bypass_mode && es_enable_fo[14] && !rng_bit_en;
  ...
  assign sfifo_esfinal_clr  = !es_enable_fo[15];
  ...
  assign pfifo_swread_clr = !(es_enable_fo[18] && es_data_reg_rd_en);
  assign pfifo_swread_pop =  es_enable_fo[19] && sw_es_rd_pulse;

  // set the es entropy to the read reg
  assign es_data_reg_rd_en = es_enable_fo[20] && efuse_es_sw_reg_en && entropy_data_reg_en_pfe;
