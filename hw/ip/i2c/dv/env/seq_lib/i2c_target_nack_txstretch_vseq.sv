...
class i2c_target_nack_txstretch_vseq extends i2c_target_runtime_base_vseq;
  ...
  virtual task initialization();
    ...
    // correctly predict the result when mid-txn NACKs are returned by the DUT. This is a testbench
    // deficiency, and is largely due to the architecture which forms expectations in DUT-Target
    // mode directly from the stimulus items we send to the agent. We also do not yet properly
    // model these timeout behaviours, so are unable to predict when the DUT will begin to NACK.
  ...
  virtual task proc_intr_txstretch();
    // Invoke the parent handler, which adds data into the TXFIFO if there is space
    super.proc_intr_txstretch();

    // Do not clear the appropriate tx_pending bits
    // This is what continues the stretch, to eventually elicit the timeout behaviour.
