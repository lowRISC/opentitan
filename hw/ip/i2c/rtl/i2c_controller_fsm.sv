...
#(
  ...
  assign event_sda_interference_o = (host_idle_o & host_enable_i & !sda_i) |
                                    (en_sda_interf_det & arbitration_lost_i);
      ...
      Idle : begin
        if (host_enable_i) begin
          if (unhandled_unexp_nak_i || unhandled_nak_timeout_i || halt_controller_i) begin
            ...
            // occurs.
            // Note that we may also halt here if arbitration was lost, so software may fix up the
            // FIFOs before beginning a new transaction.
