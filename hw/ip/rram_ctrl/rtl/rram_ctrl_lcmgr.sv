...
#(
  ...
  always_comb begin
    ...
    unique case (state_q)
      ...
      // This state drives the outer loop of the RMA wipe sequence, iterating
      // through each entry in RmaWipeEntries[]. It asserts rma_wipe_req to
      // activate the inner wipe FSM below, which handles the actual page-by-page
      // write and read-back verification for the current entry's address range.
      // Once the inner FSM signals rma_wipe_done, this state either advances
      // rma_wipe_idx to the next entry or exits when all entries are complete.
      StRmaWipe: begin
        phase          = PhaseRma;
        lfsr_en        = 1'b1;
        rma_wipe_req_d = 1'b1;
      ...
      StRmaRsp: begin
        phase            = PhaseNone;
        // The original idea was to disable access but this does not work because lc_ctrl tries to
        // access the OTP after an RMA operation: https://github.com/lowRISC/opentitan/issues/30948
  ...
  // Inner FSM: handles the actual wipe and verify sequence for a single
  // RmaWipeEntries[] entry. It loops page-by-page through the entry's address
  // range, writing random data and reading it back for XOR verification.
  // When all pages in the current entry are done it asserts rma_wipe_done,
  // which is consumed by the outer FSM (StRmaWipe) above to advance to the
  // next entry or conclude the wipe sequence.
  logic fsm_err;
