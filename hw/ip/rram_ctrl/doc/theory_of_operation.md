...
The RMA entry process wipes the creator and owner info pages, and the non-OTP portion of the data partition, by overwriting them with pseudo-random values generated by a 64-bit LFSR.
...
|---|---|---|
| Info | 1 | `CreatorInfoPage` |
| Info | 2 | `OwnerInfoPage` |
| Data | 0-4090 | All non-OTP data pages |

Info pages other than the creator/owner pages, and the OTP region (the top `OtpPages` data pages), are not wiped by this table.
`IsolatedInfoPage` is intentionally excluded: it is written with a high-endurance operation and cannot be overwritten by the normal wipe/write path.
...
The `PhaseRma` rules above grant the access needed to wipe the `RmaWipeEntries` regions.
