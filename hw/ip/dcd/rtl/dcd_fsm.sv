...
module dcd_fsm (
  ...
  assign wakeup_timer_cnt_d = (wakeup_timer_cnt_en) ? wakeup_timer_cnt_q + 1'b1 : wakeup_timer_cnt_q;
  ...
  always_ff @(posedge clk_aon_i or negedge rst_slow_ni) begin: i_chn0_val_we_reg
    if (!rst_slow_ni) begin
      chn0_val_we    <= '0;
    end
    else if (cfg_fsm_rst) begin
  ...
  always_ff @(posedge clk_aon_i or negedge rst_slow_ni) begin: i_chn1_val_we_reg
    if (!rst_slow_ni) begin
      chn1_val_we    <= '0;
    end
    else if (cfg_fsm_rst) begin
  ...
  always_ff @(posedge clk_aon_i or negedge rst_slow_ni) begin: i_chn0_val_reg
    if (!rst_slow_ni) begin
      chn0_val    <= '0;
    end
    else if (cfg_fsm_rst) begin
  ...
  always_ff @(posedge clk_aon_i or negedge rst_slow_ni) begin: i_chn1_val_reg
    if (!rst_slow_ni) begin
      chn1_val    <= '0;
    end
    else if (cfg_fsm_rst) begin
  ...
  for (genvar k = 0 ; k < NumAdcFilter ; k++) begin : gen_fst_lp_match
    assign fst_lp_match[k] = ((lp_sample_cnt_q == 8'd1) && (fsm_state_q == LP_EVAL)) ? dcd_match[k] : 1'b0;
  end
  ...
  typedef enum logic [3:0] {
                            PWRDN = 4'h0,//ADC is in the power down state
                            PWRUP = 4'h1,//ADC is being powered up
                            IDLE = 4'h2,//ADC is powered up after the pwrup_timer
                            ONEST_0 = 4'h3,//ADC is in oneshot mode; sample channel0 value
                            ONEST_021 = 4'h4,//ADC is in oneshot mode; transition from chn0 to chn1
                            ONEST_1 = 4'h5,//ADC is in oneshot mode; sample channel1 value
                            LP_0 = 4'h6,//ADC is in low-power mode, sample channel0 value
                            LP_021 = 4'h7,//ADC is in low-power mode, transition from chn0 to chn1
                            LP_1 = 4'h8,//ADC is in low-power mode, sample channel1 value
                            LP_EVAL = 4'h9,//ADC is in low-power mode, evaluate if there is a match
                            LP_SLP = 4'ha,//ADC is in low-power mode, go to sleep
                            LP_PWRUP = 4'hb,//ADC is in low-power mode, being powered up
