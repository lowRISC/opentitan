...
class xbar_tl_host_seq extends tl_host_seq;
  ...
  virtual function void randomize_req(tl_seq_item req, int idx);
    ...
    if (!(req.randomize() with {
        ...
        if (is_mapped_addr) {
          a_addr inside {[xbar_devices[device_id].addr_ranges[addr_range_id].start_address :
                          xbar_devices[device_id].addr_ranges[addr_range_id].end_address]};
        } else {
          foreach (xbar_devices[device_id].addr_ranges[i]) {
            !(a_addr inside {[xbar_devices[device_id].addr_ranges[i].start_address :
                              xbar_devices[device_id].addr_ranges[i].end_address]});
          }
        }})) begin
