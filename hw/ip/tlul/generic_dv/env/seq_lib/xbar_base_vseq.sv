...
class xbar_base_vseq extends dv_base_vseq#(
    .CFG_T(xbar_env_cfg), .COV_T(xbar_env_cov), .VIRTUAL_SEQUENCER_T(xbar_virtual_sequencer)
);
  ...
  constraint req_cnt_c {
    foreach (host_seq[i]) {host_seq[i].req_cnt inside {[min_req_cnt : max_req_cnt]};}
  }
  ...
  virtual function void seq_init();
    ...
    foreach (host_seq[i]) begin
      host_seq[i] =
          xbar_tl_host_seq::type_id::create($sformatf("%0s_seq", xbar_hosts[i].host_name));
      ...
      foreach (xbar_devices[j]) begin
        if (is_valid_path(xbar_hosts[i].host_name, xbar_devices[j].device_name)) begin
          `uvm_info(get_full_name,
                    $sformatf("Add device %0s to seq %0s", xbar_devices[i].device_name,
                              host_seq[i].get_name()),
                    UVM_HIGH)
    ...
    foreach (device_seq[i]) begin
      device_seq[i] =
          tl_device_seq::type_id::create($sformatf("%0s_seq", xbar_devices[i].device_name));
    end
  ...
  virtual task run_host_seq(uint host_id);
    ...
    `uvm_info(get_full_name(),
              $sformatf("%0s finished sending %0d requests", host_seq[host_id].get_full_name(),
                        host_seq[host_id].req_cnt),
              UVM_LOW)
  endtask
