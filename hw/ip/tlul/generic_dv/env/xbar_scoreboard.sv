...
class xbar_scoreboard extends scoreboard_pkg::scoreboard#(
    .ITEM_T(tl_seq_item), .CFG_T(xbar_env_cfg), .COV_T(xbar_env_cov)
);
  ...
  virtual function void process_src_packet(input tl_seq_item tr, input string port_name,
  ...
  virtual function void process_dst_packet(input tl_seq_item tr, input string port_name,
                                           output tl_seq_item transformed_tr);
