...
class uart_scoreboard extends cip_base_scoreboard#(
  ...
  virtual function void predict_tx_watermark_intr(uint tx_q_size = tx_q.size);
    uint
        watermark = get_watermark_bytes_by_level(ral.fifo_ctrl.txilvl.get_mirrored_value(), UartTx);
  ...
  virtual function void predict_rx_watermark_intr(uint rx_q_size = rx_q.size);
    uint
        watermark = get_watermark_bytes_by_level(ral.fifo_ctrl.rxilvl.get_mirrored_value(), UartRx);
      ...
      "wdata": begin
        ...
        if (write && channel == AddrChannel) begin
          ...
          if (tx_q.size() < UART_FIFO_DEPTH) begin
            ...
            `uvm_info(`gfn,
                      $sformatf("Drop tx item: %0h, tx_q size: %0d, uart_tx_clk_pulses: %0d",
          ...
          case (channel)
            ...
            DataChannel: begin  // check at data phase
              ...
              if (!(tx_enabled && tx_q_size_at_addr_phase == UART_FIFO_DEPTH &&
                ...
                `DV_CHECK_EQ(get_field_val(ral.status.txfull, item.d_data), tx_full_exp,
                             $sformatf(
              ...
              if (!(rx_enabled && is_in_ignored_period(UartRx) &&
                ...
                `DV_CHECK_EQ(get_field_val(ral.status.rxfull, item.d_data), rx_full_exp,
                             $sformatf(
              ...
              if (!(tx_q_size_at_addr_phase == 0 && is_in_ignored_period(UartTx))) begin
                `DV_CHECK_EQ(get_field_val(ral.status.txidle, item.d_data), tx_idle_exp,
                             $sformatf(
              ...
              if (!(uart_rx_clk_pulses inside {1, uart_bits}) || !rx_enabled) begin
                `DV_CHECK_EQ(get_field_val(ral.status.rxidle, item.d_data), rx_idle_exp,
                             $sformatf("check rx_idle fail: rx_en = %0d, uart_rx_clk_pulses = %0d",
  ...
  function void compare(uart_item act, uart_item exp, string dir = "TX");
    if (!act.compare(exp)) begin
      `uvm_error(`gfn,
                 $sformatf("%s item mismatch!\nexp:\n%0s\nobs:\n%0s", dir, exp.sprint(),
                           act.sprint()))
    end else begin
      `uvm_info(`gfn,
                $sformatf("%s item match!\nexp:\n%0s\nobs:\n%0s", dir, exp.sprint(), act.sprint()),
                UVM_HIGH)
    end
  endfunction
