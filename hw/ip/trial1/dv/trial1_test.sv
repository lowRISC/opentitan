...
module trial1_test (
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) rwtype5_capture <= 32'h0;
    else if (reg2hw.rwtype5.qe) rwtype5_capture <= reg2hw.rwtype5.q;
  end

  logic [31:0] rwtype6_capture;
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) rwtype6_capture <= 32'hc8c8c8c8;
    else if (reg2hw.rwtype6.qe) rwtype6_capture <= reg2hw.rwtype6.q;
  end
  ...
  logic my_rotype1_de;
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) rotype1_capture <= 32'h66aa66aa;
    else if (my_rotype1_de) rotype1_capture <= my_rotype1_d;
  end
  // externalized register
  ...
  initial begin
    ...
    if (errorcount) $display("ERROR: completed test with %d errors", errorcount);
    else $display("TEST PASSED CHECKS");
  end
