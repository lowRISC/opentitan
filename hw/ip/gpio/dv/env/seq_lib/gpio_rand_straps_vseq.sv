...
class gpio_rand_straps_vseq extends gpio_base_vseq;
  ...
  // Delay used to have some clocks cycles between driving the gpio_in and reading the straps registers
  rand uint m_delay;
  ...
  constraint delay_c {
    m_delay dist {0 :/ 20, [1:5] :/ 40, [6:15] :/ 30, [20:25] :/ 10};
  }
  ...
  task test_straps_gpio_in();
    ...
    `DV_CHECK_MEMBER_RANDOMIZE_WITH_FATAL(m_delay, m_delay >= 1;)
    cfg.clk_rst_vif.wait_clks(m_delay);
    ...
    csr_mirror(.ptr(ral.hw_straps_data_in));

    // Read the hw_straps_data_in_valid and check the expected value in the scoreboard
    csr_mirror(.ptr(ral.hw_straps_data_in_valid));

    // Random wait
    `DV_CHECK_MEMBER_RANDOMIZE_WITH_FATAL(m_delay, m_delay >= 0;)
    cfg.clk_rst_vif.wait_clks(m_delay);
  ...
  task check_transaction(string txn_desc, bit is_first);
    ...
    // Random wait
    `DV_CHECK_MEMBER_RANDOMIZE_WITH_FATAL(m_delay, m_delay >= 2;)
    cfg.clk_rst_vif.wait_clks(m_delay);
  ...
  task body();
    ...
    for (uint tr_num = 0; tr_num < num_trans; tr_num++) begin
      ...
      `DV_CHECK_MEMBER_RANDOMIZE_FATAL(m_delay)

      cfg.clk_rst_vif.wait_clks(m_delay);
      `uvm_info(msg_id, $sformatf("m_delay = %0d", m_delay), UVM_HIGH)
