...
class gpio_scoreboard extends cip_base_scoreboard #(.CFG_T (gpio_env_cfg),
  ...
  // Catches the gpio data inputs/outputs changes from the gpio_data_monitor.
  virtual function void write_data(gpio_seq_item item);

    logic [NUM_GPIOS-1:0] prev_gpio_i;
    gpio_transition_t [NUM_GPIOS-1:0] gpio_i_transition;

    // Assign the item gpio_i to the previous gpio_i
    prev_gpio_i = item.cio_gpio_i;

    // Coverage Sampling: gpio pin values' coverage
    sample_data_coverage();

    `uvm_info(`gfn, $sformatf("Check gpio_i_driven = 0x%0h [%0b]", gpio_i_driven, gpio_i_driven),
      UVM_HIGH)

    // Based on pins output enable, evaluate the driven values
    // should be from gpio_i or 1'bz.
    gpio_i_driven = model.evaluate_data_in_out();

    `uvm_info(`gfn, $sformatf("Check prev_gpio_i = 0x%0h [%0b]", prev_gpio_i, prev_gpio_i),
      UVM_HIGH)

    // Predict effect on gpio pins and check.
    gpio_predict_and_compare();

    if (prev_gpio_i !== cfg.gpio_vif.pins) begin
      model.process_gpio_transition(gpio_i_transition);
  ...
  function void sample_data_coverage();
    // Coverage Sampling: gpio pin values' coverage
    if (cfg.en_cov) begin
  ...
  virtual function void write_strap(gpio_seq_item item);
  ...
  function void gpio_predict_and_compare(uvm_reg csr = null);
    ...
    if (csr != null) begin
      `uvm_info(`gfn, $sformatf("csr.get_name() %s", csr.get_name()),
          UVM_HIGH)
      // process the csr req
      case (csr.get_name())
        "data_in": begin
          gpio_data_check = 1'b0;
          `uvm_info(`gfn, $sformatf("data check %0b", gpio_data_check),
          UVM_HIGH)
        end
        "direct_out": begin
  ...
  // Function : check gpio data
  function void check_gpio_data(
    bit gpio_data_check,
    logic [NUM_GPIOS-1:0] pred_val_gpio_pins,
    uvm_reg csr = null,
    string msg_id
  ...
  virtual function void gpio_interrupt_predict(
    ...
    // Sample coverage for gpio interrupt types
    sample_gpio_intr_ctrl_en_cov (
    intr_ctrl_en_rising,
    ...
    begin
      ...
      sample_cross_cov_intr_events(
      intr_enable,
      rising_edge_intr_events,
      falling_edge_intr_events,
      intr_ctrl_en_rising,
      intr_ctrl_en_falling
      );
    ...
    begin
      ...
      // Coverage Sampling: Cross coverage of (edge tiggered intr type)x(enable)x(state)
      // when type is enabled
      sample_cross_cov_lvlhigh_events(
      intr_enable,
      lvlhigh_intr_events,
      intr_ctrl_en_lvlhigh,
      lvllow_intr_events,
    ...
    sample_interrupts_index_enable_status(
    intr_enable,
    exp_intr_status,
    last_intr_update_except_clearing,
    last_intr_test_event,
    cleared_intr_bits
    );
  ...
  // Function: Sample coverage for gpio interrupt types
  function void sample_gpio_intr_ctrl_en_cov (
    bit [TL_DW-1:0] intr_ctrl_en_rising,
    bit [TL_DW-1:0] intr_ctrl_en_falling,
    bit [TL_DW-1:0] intr_ctrl_en_lvlhigh,
    bit [TL_DW-1:0] intr_ctrl_en_lvllow
    );
  ...
  endfunction : sample_gpio_intr_ctrl_en_cov

  // Function: sample_cross_cov_intr_events
  // This function samples cross coverage between rising_edge_intr_events,
  // falling_edge_intr_events, intr_ctrl_en_rising and intr_ctrl_en_falling
  function void sample_cross_cov_intr_events(
    bit [TL_DW-1:0] intr_enable,
    bit [TL_DW-1:0] rising_edge_intr_events,
    bit [TL_DW-1:0] falling_edge_intr_events,
    bit [TL_DW-1:0] intr_ctrl_en_rising,
    bit [TL_DW-1:0] intr_ctrl_en_falling
    );
    if (cfg.en_cov) begin
  ...
  endfunction : sample_cross_cov_intr_events

  // Coverage Sampling: Cross coverage of (edge tiggered intr type)x(enable)x(state)
  // when type is enabled
  function void sample_cross_cov_lvlhigh_events(
    bit [TL_DW-1:0] intr_enable,
    bit [TL_DW-1:0] lvlhigh_intr_events,
    bit [TL_DW-1:0] intr_ctrl_en_lvlhigh,
    bit [TL_DW-1:0] lvllow_intr_events,
    bit [TL_DW-1:0] intr_ctrl_en_lvllow
    );
  ...
  endfunction : sample_cross_cov_lvlhigh_events

  function void sample_interrupts_index_enable_status(
    bit [TL_DW-1:0] intr_enable,
    bit [TL_DW-1:0] exp_intr_status,
    bit [TL_DW-1:0] last_intr_update_except_clearing,
    bit [TL_DW-1:0] last_intr_test_event,
  ...
  endfunction : sample_interrupts_index_enable_status
