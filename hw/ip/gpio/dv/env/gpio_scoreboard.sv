...
class gpio_scoreboard extends cip_base_scoreboard#(
  ...
  virtual task process_tl_access(tl_seq_item item, tl_channels_e channel = DataChannel);
    ...
    if (channel == AddrChannel) begin
      ...
      if (data_in_update_queue.size() > 0) begin
        if (data_in_update_queue[$].needs_update ==
            1'b1 && ((crnt_time - data_in_update_queue[$].eval_time) / clk_period) > 1) begin
          void'(ral.data_in.predict(
          .value(data_in_update_queue[$].reg_value),
          .kind(UVM_PREDICT_READ)
          ));
        end else if (data_in_update_queue[$ - 1].needs_update == 1'b1) begin
          // Use previous updated value for "data_in" prediction
          void'(ral.data_in.predict(
          .value(data_in_update_queue[$ - 1].reg_value),
          .kind(UVM_PREDICT_READ)
          ));
        end
      end
      ...
      if (intr_state_update_queue.size() > 0) begin
        ...
        if (intr_state_update_queue[$].needs_update ==
            1'b1 && ((crnt_time - intr_state_update_queue[$].eval_time) / clk_period) > 1) begin
          void'(ral.intr_state.predict(
          .value(intr_state_update_queue[$].reg_value),
          .kind(UVM_PREDICT_READ)
          ));
        end else if (intr_state_update_queue[$ - 1].needs_update == 1'b1) begin
          // Use previous updated value for "intr_state" prediction
          void'(ral.intr_state.predict(
          .value(intr_state_update_queue[$ - 1].reg_value),
          .kind(UVM_PREDICT_READ)
          ));
        end
      end
      ...
      if (write) begin
        if (csr.get_name() == "intr_state") begin
          ...
          if (intr_state_update_queue.size() > 0) begin
            ...
            `uvm_info(`gfn,
                      $sformatf("Entry taken out for clearing is %0p",
                                intr_state_write_to_clear_update),
                      UVM_HIGH)
            // Update time
                ...
                foreach (cleared_intr_bits[each_bit]) begin
                  if (cleared_intr_bits[each_bit]) begin
                    if (last_intr_update_except_clearing[each_bit]) begin
                      cov.sticky_intr_cov[{
                        "gpio_sticky_intr_pin", $sformatf("%0d", each_bit)
                      }].sample(1'b1);
                    end else begin
                      cov.sticky_intr_cov[{
                        "gpio_sticky_intr_pin", $sformatf("%0d", each_bit)
            ...
            if (cfg.en_cov && (!uvm_re_match("*out*", csr.get_name()) ||
              ...
              if (!uvm_re_match("masked*", csr.get_name())) begin
                ...
                for (uint each_pin = 0; each_pin < NUM_GPIOS / 2; each_pin++) begin
                  cov.out_oe_mask_data_cov_objs[each_pin][csr.get_name()
                      ].var1_var2_cg.sample(mask[each_pin], data[each_pin]);
                end
              end
            end
        ...
        if (do_read_check) begin
          ...
          if (csr.get_name() == "intr_state") begin
            bit [TL_DW-1:0] intr_state = (intr_state_update_queue.size() > 0) ?
                intr_state_update_queue[$].reg_value : csr.get_mirrored_value();
            bit [TL_DW-1:0] pred_val_intr_pins = intr_state & ral.intr_enable.get_mirrored_value();
  ...
  virtual task monitor_gpio_i();
    ...
    forever begin : monitor_pins_if
      ...
      `uvm_info(`gfn,
                $sformatf("cfg.gpio_vif.pins = %0h, under_reset = %0b", cfg.gpio_vif.pins,
                          cfg.under_reset),
                UVM_HIGH)
      if (cfg.under_reset == 1'b0) begin
        ...
        foreach (cfg.gpio_vif.pins_oe[pin_num]) begin
          ...
          `uvm_info(`gfn,
                    $sformatf("pins_oe[%0d] = %0b pins_o[%0d] = %0b gpio_i_driven[%0d] = %0b",
        ...
        `uvm_info(`gfn,
                  $sformatf("pins = 0x%0h [%0b]) gpio_i_driven = 0x%0h [%0b]", cfg.gpio_vif.pins,
                            cfg.gpio_vif.pins, gpio_i_driven, gpio_i_driven),
                  UVM_HIGH)
        ...
        `uvm_info(`gfn,
                  $sformatf("prv_gpio_i_pins_o = 0x%0h [%0b]", prv_gpio_i_pins_o,
                            prv_gpio_i_pins_o),
                  UVM_HIGH)
        `uvm_info(`gfn,
                  $sformatf("prv_gpio_i_pins_oe = 0x%0h [%0b]", prv_gpio_i_pins_oe,
                            prv_gpio_i_pins_oe),
                  UVM_HIGH)
      end
  ...
  function void gpio_predict_and_compare(uvm_reg csr = null);
    ...
    if (gpio_data_check == 1'b1) begin
      ...
      `uvm_info(msg_id,
                $sformatf("data_out_effect_on_gpio_i = 0x%0h [%0b]", data_out_effect_on_gpio_i,
      ...
      `uvm_info(msg_id,
                $sformatf("pred_val_gpio_pins = %0h(%0b)", pred_val_gpio_pins, pred_val_gpio_pins),
                UVM_HIGH)
  ...
  virtual function void gpio_interrupt_predict(
    ...
    if (cfg.en_cov) begin
      foreach (intr_ctrl_en_rising[each_bit]) begin
        cov.intr_ctrl_en_cov_objs[each_bit]["intr_ctrl_en_rising"
            ].sample(intr_ctrl_en_rising[each_bit]);
        cov.intr_ctrl_en_cov_objs[each_bit]["intr_ctrl_en_falling"
            ].sample(intr_ctrl_en_falling[each_bit]);
        cov.intr_ctrl_en_cov_objs[each_bit]["intr_ctrl_en_lvlhigh"
            ].sample(intr_ctrl_en_lvlhigh[each_bit]);
        cov.intr_ctrl_en_cov_objs[each_bit]["intr_ctrl_en_lvllow"
            ].sample(intr_ctrl_en_lvllow[each_bit]);
      end
    end
    ...
    begin
        ...
        foreach (rising_edge_intr_events[each_bit]) begin
          if (gpio_i_transition[each_bit].transition_occurred) begin
            rising_edge_intr_events[each_bit] =
                gpio_i_transition[each_bit].is_rising_edge & intr_ctrl_en_rising[each_bit];
            falling_edge_intr_events[each_bit] =
                !gpio_i_transition[each_bit].is_rising_edge & intr_ctrl_en_falling[each_bit];
          end
        end
      ...
      if (cfg.en_cov) begin
        foreach (rising_edge_intr_events[each_bit]) begin
          cov.intr_event_type_cov_objs[each_bit]["intr_event_rising"
              ].intr_type_cg.sample(intr_ctrl_en_rising[each_bit], intr_enable[each_bit],
                                    rising_edge_intr_events[each_bit]);
          cov.intr_event_type_cov_objs[each_bit]["intr_event_falling"
              ].intr_type_cg.sample(intr_ctrl_en_falling[each_bit], intr_enable[each_bit],
                                    falling_edge_intr_events[each_bit]);
        end
      end
    end
    ...
    begin
      ...
      for (uint each_bit = 0; each_bit < TL_DW; each_bit++) begin
        lvlhigh_intr_events[each_bit] =
            (cfg.gpio_vif.pins[each_bit] == 1'b1) && (intr_ctrl_en_lvlhigh[each_bit] == 1'b1);
        lvllow_intr_events[each_bit] =
            (cfg.gpio_vif.pins[each_bit] == 1'b0) && (intr_ctrl_en_lvllow[each_bit] == 1'b1);
    ...
    `uvm_info(msg_id,
              $sformatf("Predicted interrupt status = 0x%0h [%0b]", exp_intr_status,
                        exp_intr_status),
              UVM_HIGH)
    begin
