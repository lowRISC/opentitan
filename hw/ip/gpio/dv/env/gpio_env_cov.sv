...
class gpio_env_cov extends cip_base_env_cov#(
    .CFG_T(gpio_env_cfg)
);
  ...
  covergroup data_out_data_oe_data_in_cross_cg(
    ...
    option.name = name;
    cp_pin: coverpoint pin {bins bins_for_gpio_bits[] = {[0 : NUM_GPIOS - 1]};}
    cp_cross_all: cross cp_pin, data_out, data_oe, data_in{
      ...
      illegal_bins data_oe_1_data_out_0_data_in_1 = binsof (data_out) intersect {
        ...
        1
      } && binsof (data_in) intersect {
        1
      };
      illegal_bins data_oe_1_data_out_1_data_in_0 = binsof (data_out) intersect {
        1
      } && binsof (data_oe) intersect {
        1
      } && binsof (data_in) intersect {
      ...
      foreach (intr_state_cov_obj[each_pin]) begin
        ...
        foreach (intr_types[each_type]) begin
          ...
          intr_ctrl_en_cov_objs[each_pin][{
            "intr_ctrl_en_", intr_types[each_type]
          }] = new({
            "intr_ctrl_en_", intr_types[each_type], $sformatf("_pin%0d_cov", each_pin)
          });
          // Per pin coverage for different types of interrupt event occurrences
          intr_event_type_cov_objs[each_pin][{
            "intr_event_", intr_types[each_type]
          }] = new({
            "intr_event_", intr_types[each_type], $sformatf("_pin%0d", each_pin)
          });
        end
        // Per pin coverage for values of different *out* and *oe* registers
        foreach (out_oe_reg_names[each_reg]) begin
          out_oe_cov_objs[each_pin][out_oe_reg_names[each_reg]] = new({
            out_oe_reg_names[each_reg], $sformatf("_cov_obj_pin%0d", each_pin)
          });
        end
        data_out_data_oe_cov_obj[each_pin] =
            new($sformatf("data_out_data_oe_cov_obj_pin%0d", each_pin));
        ...
        sticky_intr_cov[{
          "gpio_sticky_intr_pin", $sformatf("%0d", each_pin)
        }] = new(
      ...
      foreach (out_oe_mask_data_cov_objs[each_pin]) begin
        foreach (out_oe_reg_names[each_reg]) begin
          if (!uvm_re_match("masked*", out_oe_reg_names[each_reg])) begin
            out_oe_mask_data_cov_objs[each_pin][out_oe_reg_names[each_reg]] = new({
              out_oe_reg_names[each_reg], $sformatf("_mask_data_cov_obj_pin%0d", each_pin)
            });
          end
        end
      end
