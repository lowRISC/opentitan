...
#(
  ...
  for (genvar i = 0; i < NumInpPeriodCounters; i++) begin : gen_inp_prd_cnt
    ...
    assign sampled_inp_d[i] = prescaler_cnt_reached[i] ?
                              data_in_d[reg2hw.inp_prd_cnt_ctrl[i].input_select.q[IOWidth-1:0]] :
                              sampled_inp_q[i];
    ...
    // When the SW-visible register gets updated, write the incremented counter value (with
    // saturation).
    assign hw2reg.inp_prd_cnt_val[i].d = inp_prd_cnt_q[i] != '1 ? inp_prd_cnt_q[i] + 32'd1
                                                                : inp_prd_cnt_q[i];
