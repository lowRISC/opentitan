...
module prim_ascon_duplex
 ...
 (
  ...
  input  logic   [4:0] valid_bytes_i,
  input  mubi4_t       last_AD_block_i,
  input  mubi4_t       last_MSG_block_i,
  input  logic         data_valid_i,
  output logic         data_rd_o,

  // Cipher Output Port
  output logic [127:0] data_o,
  input  logic         data_rd_i,
...
// Padding:
// 1) Associated Data
//    a) empty AD: No padding, AD-processing phase is skiped!
...
assign valid_bytes_mask = bin2thermo(valid_bytes_i);

logic [127:0] padding_byte_mask;
assign padding_byte_mask = get_padding_mask(valid_bytes_i);
...
always_comb begin
  if (mubi4_test_true_strict(complete_block)) begin
      ...
      data_in_padded = data_in_valid_bytes | padding_byte_mask;
  ...
  unique case (fsm_state_q)
    ...
    AbsorbAD: begin
      ...
      if (data_valid_i) begin
        data_rd_o = 1'b1; // data_valid_i is registered. We don't create a loop here!
        ascon_state_update = 1'b1;
        if (mubi4_test_true_strict(last_AD_block_i)) begin
          if (mubi4_test_true_strict(complete_block)) begin
    ...
    AbsorbMSG: begin
      if (data_valid_i) begin
        ...
        data_we_o = 1'b1;
        ascon_state_update = 1'b1;
        if (mubi4_test_true_strict(last_MSG_block_i)) begin
    ...
    default: sparse_fsm_error = 1'b 1;
  endcase
end
