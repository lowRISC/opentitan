...
#(
  ...
  assign digest_on_blk_o = (update_digest_d || update_digest_q)  && (fifo_st_q == FifoIdle) && (
      (digest_mode_flag_q == SHA2_256                 && message_length_i[8:0] == '0) ||
      (digest_mode_flag_q inside {SHA2_384, SHA2_512} && message_length_i[9:0] == '0));

  assign update_digest_d = update_digest   ? 1'b1 :
                           digest_on_blk_o ? 1'b0 : update_digest_q;
