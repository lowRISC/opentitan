...
//
// Leading one detector baed on parallel prefix computation
...
module prim_leading_one_ppc #(
  ...
  input        [ N-1:0]    in_i,
  output logic [ N-1:0]    leadig_one_o,
  ...
  assign leadig_one_o = ppc_out ^ {ppc_out[N-2:0], 1'b0};
  assign ppc_out_o    = ppc_out;

  always_comb begin
    idx_o = '0;
    for (int unsigned i = 0 ; i < N ; i++) begin
      if (leadig_one_o[i]) begin
        idx_o = i[IdxW-1:0];
      end
    end
  end
