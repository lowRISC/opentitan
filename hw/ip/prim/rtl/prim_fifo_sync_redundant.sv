...
`ifdef INC_ASSERT
  // This module duplicates the fifo and raises err_o if the two copies disagree. To set the logic
  // value below to one, the user must use the ASSERT_PRIM_FIFO_SYNC_ERROR_TRIGGERS_ALERT1 macro
  // (which checks that the error signal causes an alert).
  //
