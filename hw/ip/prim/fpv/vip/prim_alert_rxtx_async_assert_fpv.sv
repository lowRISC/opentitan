...
module prim_alert_rxtx_async_assert_fpv (
  ...
  logic error_present;
  assign error_present =
      ping_err_pi | ping_err_ni | ack_err_pi | ack_err_ni | alert_err_pi | alert_err_ni;
  ...
  `ASSUME_FPV(PingEnStaysAsserted0_M,
              ping_en_i
  ...
  sequence FullHandshake_S;
    $rose(prim_alert_rxtx_async_fpv.alert_pd) ##[3:5] $rose(prim_alert_rxtx_async_fpv.ack_pd) &&
        $stable(prim_alert_rxtx_async_fpv.alert_pd) ##[3:5]
        $fell(prim_alert_rxtx_async_fpv.alert_pd) && $stable(prim_alert_rxtx_async_fpv.ack_pd) ##[
        3:5] $fell(prim_alert_rxtx_async_fpv.ack_pd) && $stable(prim_alert_rxtx_async_fpv.alert_pd);
  endsequence

  // note: injected errors may lockup the FSMs, and hence the full HS can
  // only take place if both FSMs are in a sane state
  `ASSERT(PingHs_A,
          ##1 $changed(prim_alert_rxtx_async_fpv.ping_pd) && (
  ...
  `ASSERT(AlertHs_A,
          alert_i && (prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q ==
                      prim_alert_rxtx_async_fpv.i_prim_alert_sender.Idle) && (
              prim_alert_rxtx_async_fpv.i_prim_alert_receiver.state_q ==
                  prim_alert_rxtx_async_fpv.i_prim_alert_receiver.Idle) |-> ##[0:5] FullHandshake_S,
          clk_i, !rst_ni || error_setreg_q)
  ...
  `ASSERT(AlertCheck0_A,
          !ping_en_i [* 10] ##1 $rose(alert_i) && (
              prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q ==
                  prim_alert_rxtx_async_fpv.i_prim_alert_sender.Idle) |-> ##[3:5] alert_o,
          clk_i, !rst_ni || ping_en_i || error_setreg_q)
  ...
  `ASSERT(FsmLivenessSender_A,
          !error_present [* 2]
              ##1 !error_present && (prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q !=
                                     prim_alert_rxtx_async_fpv.i_prim_alert_sender.Idle) |->
  ...
  `ASSERT(FsmLivenessReceiver_A,
          !error_present [* 2]
              ##1 !error_present && (prim_alert_rxtx_async_fpv.i_prim_alert_receiver.state_q !=
