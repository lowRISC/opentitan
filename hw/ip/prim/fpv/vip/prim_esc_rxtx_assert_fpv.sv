...
module prim_esc_rxtx_assert_fpv (
  ...
  `ASSERT(PingRespCheck_A, !error_present [* 4] ##1 $rose(ping_en_i) |-> ##[0:4] ping_ok_o, clk_i,
          !rst_ni || error_present)

  // be more specific (i.e. use throughout)
  `ASSERT(EscRespCheck_A,
          ##1 esc_en_i |-> ##[0:1] prim_esc_rxtx_fpv.esc_rx_out.resp_p ##1
      !prim_esc_rxtx_fpv.esc_rx_out.resp_p,
          clk_i, !rst_ni || error_present)
  ...
  `ASSERT(FsmLivenessSender_A,
          (prim_esc_rxtx_fpv.i_prim_esc_sender.state_q != prim_esc_rxtx_fpv.i_prim_esc_sender.Idle)
              |-> strong (##[1:$] (prim_esc_rxtx_fpv.i_prim_esc_sender.state_q ==
                                   prim_esc_rxtx_fpv.i_prim_esc_sender.Idle)),
          clk_i, !rst_ni || error_present)
  `ASSERT(FsmLivenessReceiver_A,
          (prim_esc_rxtx_fpv.i_prim_esc_receiver.state_q !=
