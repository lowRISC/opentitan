...
#include "Vascon_duplex_tb.h"
#include "sim_ctrl_extension.h"
#include "verilated_toplevel.h"
#include "verilator_sim_ctrl.h"

class ASCONDUPLEXTB : public SimCtrlExtension {
  using SimCtrlExtension::SimCtrlExtension;

 public:
  ASCONDUPLEXTB(ascon_duplex_tb *top);

  void OnClock(unsigned long sim_time);

 private:
  ascon_duplex_tb *top_;
};

// Constructor:
// - Set up top_ ptr
ASCONDUPLEXTB::ASCONDUPLEXTB(ascon_duplex_tb *top)
    : SimCtrlExtension{}, top_(top) {}

// Function called once every clock cycle from SimCtrl
void ASCONDUPLEXTB::OnClock(unsigned long sim_time) {
  if (top_->test_done_o) {
...
int main(int argc, char **argv) {
  ...
  ASCONDUPLEXTB asconduplextb(&top);
  simctrl.RegisterExtension(&asconduplextb);
