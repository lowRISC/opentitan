...
From the OpenTitan top level execute
   ...
   ```sh
   fusesoc --cores-root=. run --setup --build lowrisc:dv_verilator:ascon_duplex_tb
   ```
to build the testbench and afterwards

   ```sh
   ./build/lowrisc_dv_verilator_ascon_duplex_tb_0/default-verilator/Vascon_duplex_tb \
     --trace
   ```
to run it.
...
- `rtl/ascon_duplex_tb.sv`: SystemVerilog testbench, signals test end and
  result (pass/fail) to C++ via output ports.
- `cpp/ascon_duplex_tb.cc`: Contains main function and instantiation of SimCtrl,
