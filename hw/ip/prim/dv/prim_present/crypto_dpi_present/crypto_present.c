...
// C file used to generate a lib that can be reused in python
#include "present.inc"
...
extern uint64_t c_encrypt(uint64_t plaintext, uint64_t key_high,
...
extern uint64_t c_decrypt(uint64_t ciphertext, uint64_t key_high,
                          uint64_t key_low, uint8_t num_rounds,
                          uint8_t key_size_80) {
