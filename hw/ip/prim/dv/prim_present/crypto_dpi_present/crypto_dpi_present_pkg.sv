...
package crypto_dpi_present_pkg;
  ...
  import "DPI-C" context function void c_dpi_key_schedule(
      input longint unsigned key_high, input longint unsigned key_low,
      input int unsigned num_rounds, input int unsigned key_size_80,
      output bit [NumRounds:0][63:0] key_schedule);

  import "DPI-C" context function longint c_dpi_encrypt(
      input longint unsigned plaintext, input longint unsigned key_high,
      input longint unsigned key_low, input int unsigned num_rounds,
      input int unsigned key_size_80);

  import "DPI-C" context function longint c_dpi_decrypt(
      input longint unsigned ciphertext, input longint unsigned key_high,
      input longint unsigned key_low, input int unsigned num_rounds,
      input int unsigned key_size_80);

  // Helper Functions
  function automatic void get_keys(input bit [127:0] key, input bit key_size_80,
                                   output bit [63:0] key_high, output bit [63:0] key_low);
  ...
  function automatic void sv_dpi_present_get_key_schedule(
      input bit [127:0] key, input bit key_size_80, output bit [NumRounds:0][63:0] key_schedule);
  ...
  function automatic void sv_dpi_present_encrypt(input bit [63:0] plaintext, input bit [127:0] key,
  ...
  function automatic void sv_dpi_present_decrypt(input bit [NumRounds-1:0][63:0] ciphertext,
                                                 input bit [127:0] key, input bit key_size_80,
    ...
    for (int i = 1; i <= NumRounds; i++) begin
      decrypted_values[i - 1] =
          c_dpi_decrypt(ciphertext[i - 1], key_high, key_low, i + 1, key_size_80);
    end
