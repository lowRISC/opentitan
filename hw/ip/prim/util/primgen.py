...
def _parse_module_header_verible(generic_impl_filepath, module_name):
    ...
    if not header:
        raise ValueError("Unable to extract module header from %s." % (generic_impl_filepath,))

    name = header.find(
        {"tag": ["SymbolIdentifier", "EscapedIdentifier"]}, iter_=PreOrderTreeIterator
    )
    if not name:
        raise ValueError("Unable to extract module name from %s." % (generic_impl_filepath,))
    ...
    parameters = set()
    if parameters_list:
        for parameter in sorted(parameters_list.iter_find_all({"tag": "kParamDeclaration"})):
            if parameter.find({"tag": "parameter"}):
                parameter_id = parameter.find({"tag": ["SymbolIdentifier", "EscapedIdentifier"]})
...
def _generate_prim_pkg(gapi):
    ...
    prim_pkg_sv_tpl_filepath = os.path.join(os.path.dirname(__file__), "primgen", "prim_pkg.sv.tpl")
    ...
    prim_pkg_core_src = os.path.join(os.path.dirname(__file__), "primgen", "prim_pkg.core.tpl")
...
def _create_instances(prim_name, techlibs, parameters):
    ...
    # generated wrapper.
    techlibs_wo_generic = sorted([techlib for techlib in techlibs if techlib != "generic"])
    ...
    for pos, techlib in enumerate(techlibs_generic_last):
        ...
        s += "begin : gen_{techlib}\n" + _instance_sv(prim_name, techlib, parameters) + "end"

        if not is_last:
            s += " "

        out += s.format(
            prim_name=prim_name, techlib=techlib, techlib_enum=_enum_name_for_techlib(techlib)
        )
    return out
...
def _generate_abstract_impl(gapi):
    ...
    if "generic" not in techlibs:
        raise ValueError("Techlib generic is required, but not found for primitive %s." % prim_name)
    ...
    print("Abstract primitive written to %s" % (os.path.abspath(abstract_prim_sv_filepath),))
