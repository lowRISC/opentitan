...
class MemChunk:
    ...
    def __str__(self) -> str:
        return "MemChunk(@{:#x}, words_len={})".format(self.base_addr, len(self.words))
...
class MemFile:
    ...
    def __str__(self) -> str:
        return "MemFile(width={}, chunks_len={})".format(self.width, len(self.chunks))
    ...
    def _parse_line(width: int, line: str) -> Tuple[int, List[int]]:
        ...
        if addr_match is None:
            raise ValueError(
                "Bad line format: first token is {!r}, "
                "which is not in the right format for an address.".format(tokens[0])
            )
        ...
        for idx, word_tok in enumerate(tokens[1:]):
            try:
                ...
                raise ValueError(
                    "Word {} of the line is invalid: {!r} is not a hex number.".format(
                        idx + 1, word_tok
                    )
                ) from None
            ...
            if word < 0 or word >> width:
                raise ValueError(
                    "Word {} of the line is {!r}, which "
                    "does not fit in an unsigned {}-bit number.".format(idx + 1, word_tok, width)
                )
            words.append(word)
    ...
    def _load_preproc(width: int, infile: IO[str]) -> "MemFile":
        ...
        for line in infile:
            ...
            if line_addr < chunk_end:
                raise ValueError(
                    "Cannot read data starting at {:#x}: "
                    "we're already at {:#x}, so this would "
                    "go backwards.".format(line_addr, chunk_end)
                )
            if line_addr == chunk_end:
    ...
    def load_elf32(infile: BinaryIO, base_addr: int) -> "MemFile":
        ...
        for segment in elf_file.iter_segments():
            ...
            if seg_lma < 0:
                raise ValueError(
                    "ELF file contains a segment starting at "
                    "{:#x}, so cannot be loaded relative to base "
                    "address {:#x}.".format(base_addr + seg_lma, base_addr)
                )
        ...
        for lma, top, data in segments:
            if lma < next_addr:
                raise ValueError(
                    "ELF file contains overlapping segments with "
