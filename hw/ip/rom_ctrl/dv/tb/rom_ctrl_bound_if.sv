...
//
// To solve the problem, this interface instantiates rom_ctrl_if inside it called u_rom_ctrl_if and
// the testbench passes *that* (non-parameterised) interface to the environment.
//
...
interface rom_ctrl_bound_if #(
  ...
  if (Bound) begin : gen_bound
    if (!SecDisableScrambling) begin : gen_enable_scrambling
      ...
      // Override the kmac_data_i.rsp_valid to be high for a cycle. On a posedge of
      // force_kmac_rsp_valid, force kmac_data_i.rsp_valid to be true.
      //
      // The override lasts until the next negedge of the clock or reset is asserted. When the
      // override is complete, this flips kmac_rsp_valid_forced.
      initial begin
    ...
    // Watch the bus_rom_idx override ports in u_rom_ctrl_if. On a posedge of override_bus_rom_idx,
    // use the value from desired_bus_rom_idx to override the next request that comes out of
    // u_tl_adapter_rom. This means that operation will end up asking for the given word instead of
    // the one it expected.
    //
    // The override lasts until the A channel valid signal drops again or reset is asserted. When the
    // override is complete, this flips bus_rom_idx_overridden.
