...
class rom_ctrl_common_vseq extends rom_ctrl_base_vseq;
  ...
  extern function void splat_fifo_storage(string fifo_path, int unsigned depth);
  extern virtual function void inject_intg_fault_in_passthru_mem(
                                   dv_base_mem mem,
                                   bit [bus_params_pkg::BUS_AW-1:0] addr);
  extern function bit is_ptr_in_prim_counts_fifo(string path, string fifo_path);
...
function void rom_ctrl_common_vseq::splat_fifo_storage(string fifo_path, int unsigned depth);
...
function void rom_ctrl_common_vseq::inject_intg_fault_in_passthru_mem(
  ...
  rdata = cfg.rom_ctrl_util_h.rom_encrypt_read32(addr, RND_CNST_SCR_KEY,
  ...
  cfg.rom_ctrl_util_h.rom_encrypt_write32_integ(addr, rdata, RND_CNST_SCR_KEY, RND_CNST_SCR_NONCE,
                                                1'b1, flip_bits);
endfunction

// Return 1 if path is a pointer in the prim_count associated with the fifo at fifo_path
function bit rom_ctrl_common_vseq::is_ptr_in_prim_counts_fifo(string path, string fifo_path);
...
function void rom_ctrl_common_vseq::sec_cm_fi_ctrl_svas(sec_cm_base_if_proxy if_proxy,
                                                        bit enable);
