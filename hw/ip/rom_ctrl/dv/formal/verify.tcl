...
# feature and doesn't need testing with e.g. a stopat. Disable the cover property instead. To make
# sure this doesn't change with a change to the design, the tb contains the SingleWritableReg_A
# assertion.
...
foreach fifo_pr { { u_reqfifo ReqFifo } { u_sramreqfifo SramReqFifo } { u_rspfifo RspFifo } } {
    ...
    foreach ptr_pr { { u_wptr Wptr } { u_rptr Rptr } } {
        ...
        # These two assertions claim that prim_count saturates (on both the up and down count). The
        # surrounding logic checks that this can never happen (because the counters are used for
        # wrapping fifo pointers). This might mean that we could potentially simplify the design
        # slightly, but there's definitely no problem with waiving precondition coverage.
...
# where the checker counter can be given a different value, which allows us to skip most of both
