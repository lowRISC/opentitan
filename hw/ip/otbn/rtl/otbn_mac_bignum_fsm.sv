...
(
  ...
  always_comb begin
    ...
    for (int unsigned cycle = 0; cycle < LatencyVec; cycle++) begin
      automatic logic [1:0] elem_idx = 2'(cycle) + shuffle_offset;

      contrl_vec[cycle].acc_wr_en_raw  = 1'b1;
      predec_vec[cycle].op_a_qw_sel    = elem_idx;
      predec_vec[cycle].op_b_elem0_sel = 3'({elem_idx, 1'b0});
      predec_vec[cycle].op_b_elem1_sel = 3'({elem_idx, 1'b1});
      predec_vec[cycle].acc_qw_sel     = (VLEN/QWLEN)'(unsigned'(1) << elem_idx);
  ...
  always_comb begin
    ...
    for (int unsigned cycle = 0; cycle < LatencyMod; cycle++) begin
      automatic logic [1:0] elem_idx = 2'(cycle / LatencyMontgMul) + shuffle_offset;

      contrl_mod[cycle]                = contrl_mod_mul[cycle % LatencyMontgMul];
      predec_mod[cycle]                = predec_mod_mul[cycle % LatencyMontgMul];
      predec_mod[cycle].op_a_qw_sel    = elem_idx;
      predec_mod[cycle].op_b_elem0_sel = 3'({elem_idx, 1'b0});
      predec_mod[cycle].op_b_elem1_sel = 3'({elem_idx, 1'b1});
      predec_mod[cycle].acc_qw_sel     = (VLEN/QWLEN)'(unsigned'(1)) << elem_idx;
    end
