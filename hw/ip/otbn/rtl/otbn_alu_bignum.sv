...
(
  ...
  input logic                         sec_wipe_mod_zero_i,
  ...
  for (genvar i_word = 0; i_word < BaseWordsPerWLEN; i_word++) begin : g_mod_words
    ...
    always_comb begin
      ...
      unique case(1'b1)
        ...
        sec_wipe_mod_zero_i: mod_d[i_word*32+:32] = 32'd0;
        default:             mod_d[i_word*32+:32] = ispr_bignum_wdata_i[i_word*32+:32];
      endcase

    `ASSERT(ModSecWipeSelOneHot, $onehot0({sec_wipe_mod_urnd_i, sec_wipe_mod_zero_i}))
    ...
    assign mod_wr_en[i_word] = ispr_init_i |
      ...
      sec_wipe_mod_urnd_i | sec_wipe_mod_zero_i;
  end
