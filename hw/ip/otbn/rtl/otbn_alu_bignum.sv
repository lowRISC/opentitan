 ...
 * (AND,OR,XOR,NOT), a vector transposer and 24-bit to 32-bit packing and unpacking wiring.
 *
 ...
 * on the carries of adder X and Y. For all other add/sub operations Y computes the operation with
 * one of the inputs supplied by the shifter and the other from operand_a.
 *
 ...
 * inputs together before shifting with the A input in the upper (256-bit) half {A/0, B}. This
 * allows the shifter to pass through operand B simply by not performing a shift.
 *
 * The shifter result has no direct connection to the final result MUX but the result is pushed
 * through the logic block with operand A set to 0 and the logic operation set to OR. This saves
 * one input to the result MUX.
 *
 ...
 * +---------+      |                 |               |               |
 * | Modulo  |      |                 |               |               |
 * | Result  |   Y result      unpacked result   logic result  transpose result
 * | Select. |      |                 |               |               |
 * +---------+      |                 |               |               |
 *      |           |                 |               |               |
 *  MOD result      |                 |               |               |
...
(
  ...
  // SEC_CM: DATA_REG_SW.SCA
  prim_blanker #(.Width(8*24)) u_shifter_res_blanker (
  ...
  // Despite mod_no_intg_q_replicated is not blanked and thus a onehot MUX should be used to avoid
  // mixing the two inputs a regular predecoded MUX is sufficient. The reason is that the SW can
  // control the value of MOD and therefore control what is mixed.
  ...
  /////////////////////////////
  logic [WLEN-1:0] mod_res;
  logic            mod_res_adder_y_used;

  // There is no blanking required between the adder Y result and this selection logic for
  // non-modulo instructions. The selection logic will only mix the result with '0. This is save
  // because it is the case as when other blanked paths are combined.
  otbn_mod_result_selector #(
      .LVLEN(VLEN),
      .LVChunkLEN(VChunkLEN)
    ) u_mod_result_selector (
    ...
    .result_o        (mod_res),
    .adder_y_used_o  (mod_res_adder_y_used)
  );
  ...
  always_comb begin
    ...
    unique case (operation_i.op)
      ...
      AluOpBignumAddvm: begin
        ...
        expected_adder_x_carries_in  = '0;
        expected_adder_x_op_b_invert = 1'b0;
        expected_adder_y_carries_top = {(NVecProc-1){1'b1}};
        adder_y_carry_0_in           = 1'b1;
        expected_adder_y_op_b_invert = 1'b1;
      ...
      AluOpBignumSubm: begin
        ...
        // be applied or not).
        expected_adder_x_carries_in  = NVecProc'(1);
      ...
      AluOpBignumSubvm: begin
        ...
        expected_adder_x_carries_in  = {NVecProc{1'b1}};
        expected_adder_x_op_b_invert = 1'b1;
        expected_adder_y_carries_top = '0;
        adder_y_carry_0_in           = 1'b0;
        expected_adder_y_op_b_invert = 1'b0;
  ...
  always_comb begin
    ...
    unique case(operation_i.op)
      ...
      // The selection of result of adder x or y is implemented in the otbn_mod_result_selector.
      // The comments in this mux apply to the 256b case.
      ...
      AluOpBignumAddm,
      AluOpBignumAddvm: begin
        // `adder_y_res` is always used: either as condition to evaluate the MOD test or as actual
        // result.
        adder_y_res_used   = 1'b1;
        operation_result_o = mod_res;
      end
      ...
      AluOpBignumSubm,
      AluOpBignumSubvm: begin
        adder_y_res_used   = mod_res_adder_y_used;
        operation_result_o = mod_res;
      end
