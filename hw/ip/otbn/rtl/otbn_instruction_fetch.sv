...
#(
  ...
  // a duplicate inside BN MAC. This one here generates the predecoded signals which do change
  // during the execution. These signals are compared to the ones generated by the FSM in BN MAC.
  // It operates one cycle in advance and uses the control signals generated by the predecoder. The
  // one inside BN MAC operates on the decoded control signals. This gives redundancy similar to
  // a lockstep configuration.
  otbn_mac_bignum_fsm #(
    // Any state error (e.g., faulted counter) of this FSM will be detected by the FSM in the
    // BN MAC. If an attack changes this FSM's state, then either:
    // - Wrong predec signals are forwarded -> Detected by BN MAC in the next cycle
    // - Valid predec signals are forwarded -> Attack has no effect. Even if all following cycles
    //   forward "valid" control signals, then the attack has no effect. And if at least one
    //   following state mismatches, it is detected.
    // Therefore, we can ignore the state error here. This ensures that all multi-cycle
    // multiplications escalate at the earliest when they are executed (like other instructions).
    .EnableAlertTriggerSVA(0)
    ) u_mac_bignum_fsm (
    .clk_i,
    .rst_ni,

    // Start as soon as the instruction is fetched the first time. op_en will be high in this case.
    .start_i         (insn_fetch_en),
    // We must use the _d signals such that we keep the signals from the instruction where we
    // started the execution as when stalling the predecoder already decodes the next instruction.
    .mac_en_i        (mac_bignum_predec_d.op_en),
    .is_lane_i       (mac_bignum_predec_d.is_lane),
    .lane_index_i    (mac_bignum_predec_d.lane_index),
    .op_a_qw_sel_i   (mac_bignum_predec_d.op_a_qw_sel_raw),
    .op_b_elem0_sel_i(mac_bignum_predec_d.op_b_elem0_sel_raw),
    .op_b_elem1_sel_i(mac_bignum_predec_d.op_b_elem1_sel_raw),
    .is_vec_i        (mac_bignum_predec_d.is_vec),
    .is_mod_i        (mac_bignum_predec_d.is_mod),

    .contrl_o    (/* unused here */),
    .predec_dyn_o(mac_bignum_predec_dyn),
    .is_busy_o   (mac_bignum_is_busy),

    .sec_wipe_i(sec_wipe_mac_urnd_i),

    .state_err_o(/* Ignored, see comment above. */)
  );
  ...
  // Take the signals from the FSM only when it is active (an instruction starts or is ongoing).
  assign mac_bignum_predec_dyn_d = (insn_fetch_en || mac_bignum_is_busy) ? mac_bignum_predec_dyn :
                                                                           mac_bignum_predec_dyn_q;
