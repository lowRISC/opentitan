...
def _get_pcs_for_mnemonics(program: OTBNProgram, mnems: List[str]) -> List[int]:
    """Gets all PCs in the program holding the given instruction."""
    return [pc for (pc, (insn, _)) in program.insns.items() if insn.mnemonic in mnems]
...
def _check_loop_iterations(program: OTBNProgram, loops: List[CodeSection]) -> CheckResult:
        ...
        if insn.mnemonic == "loopi" and operands["iterations"] <= 0:
            out.err(
                "Bad number of loop iterations ({}) at PC {:#x}: {}".format(
                    operands["iterations"], loop.start, insn.disassemble(loop.start, operands)
                )
            )
    return out


def _check_loop_end_insns(program: OTBNProgram, loops: List[CodeSection]) -> CheckResult:
    ...
    for loop in loops:
        ...
        if not loop_end_insn.straight_line:
            out.err(
                "Control flow instruction ({}) at end of loop at PC {:#x} "
                "(loop starting at PC {:#x})".format(loop_end_insn.mnemonic, loop.end, loop.start)
            )
    return out


def _check_loop_inclusion(program: OTBNProgram, loops: List[CodeSection]) -> CheckResult:
    ...
    for loop in loops:
        for other in loops:
            if other.start in loop and other.end not in loop:
                out.err(
                    "Inner loop ends after outer loop (inner loop {}, outer loop {})".format(
                        other, loop.pretty()
                    )
                )
...
def _check_loop_branching(program: OTBNProgram, loops: List[CodeSection]) -> CheckResult:
...
def _check_loop_stack(program: OTBNProgram, loops: List[CodeSection]) -> CheckResult:
    ...
    for loop in loops:
        if loop.end in end_addrs:
            out.err(
                ...
                "another loop; consider adding a NOP instruction.".format(loop.start)
            )
