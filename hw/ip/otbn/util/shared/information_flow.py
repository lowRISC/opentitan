...
class InformationFlowGraph:
    ...
    def __deepcopy__(self, memo: Optional[Dict[int, Any]]) -> "InformationFlowGraph":
    ...
    def pretty(self, indent: int = 0) -> str:
        ...
        prefix = " " * indent
        flow_strings = {sink: ",".join(sorted(sources)) for sink, sources in self.flow.items()}
        ...
        for sink in sorted(self.flow.keys()):
            ...
            lines.append("{}{}{} -> {}".format(prefix, sources_str, padding, sink))
        return "\n".join(lines)
...
class InsnInformationFlowNode:
    ...
    def evaluate(self, op_vals: Dict[str, int], constant_regs: Dict[str, int]) -> str:
    ...
    def __init__(self, op: Operand):
        if not isinstance(op.op_type, RegOperandType):
            raise ValueError(
                ...
                "node from a non-register operand {} of type {}".format(op.name, op.op_type)
            )
        self.op = op
    ...
    def __init__(self, op: Operand):
        if not isinstance(op.op_type, RegOperandType):
            raise ValueError(
                ...
                "flow node from a non-register operand {} of type {}".format(op.name, op.op_type)
            )
        if not op.op_type.reg_type == "gpr":
            raise ValueError(
                "Attempt to construct an indirect-WDR operand information "
                "flow node from a non-GPR register operand {} of type {}".format(
                    op.name, op.op_type.reg_type
                )
            )
        self.op = op
...
class InsnGroupFlagNode(InsnInformationFlowNode):
    ...
    def __init__(self, flag: str):
        ...
        if flag not in FLAG_NAMES:
            raise ValueError(
                'Invalid flag name: "{}". Valid names are: {}'.format(flag, FLAG_NAMES)
            )
        self.flag = flag
    ...
    def evaluate(self, op_vals: Dict[str, int], constant_regs: Dict[str, int]) -> str:
        if "flag_group" not in op_vals:
            raise ValueError(
                "Operand flag_group not found in provided operand values: {}".format(op_vals.keys())
            )
...
def _parse_iflow_nodes(
    node: str, what: str, operands: List[Operand]
) -> List[InsnInformationFlowNode]:
        ...
        if node == op.name:
            if not isinstance(op.op_type, RegOperandType):
                raise RuntimeError(
                    ...
                    "immediate operands cannot be information-flow nodes.".format(
            ...
            if gpr == op.name:
                if not (isinstance(op.op_type, RegOperandType) and op.op_type.reg_type == "gpr"):
                    raise RuntimeError(
                        ...
                        "(type {}). Only GPRs can be indirect references.".format(
                            gpr, node, type(op.op_type)
                        )
                    )
    ...
    if flag_group_str == "flags":
        ...
        if flag == "all":
            return [InsnConstantNode("{}-{}".format(flag_group_str, flag)) for flag in FLAG_NAMES]
...
def _parse_iflow_test(
    test_yml: object, what: str, operands: List[Operand]
) -> InsnInformationFlowTest:
    ...
    try:
        ...
        raise ValueError(
            "Value {} in test {} for {} must be an integer.".format(value_str, test, what)
        )
    ...
    if constructor is None:
        raise ValueError("Unrecognized comparison {} for {}".format(comparison, what))
...
class InsnInformationFlowRule:
    ...
    def __init__(
        self,
        flows_to: Sequence[InsnInformationFlowNode],
    ...
    def evaluate(
        self, op_vals: Dict[str, int], constant_regs: Dict[str, int]
    ...
    @staticmethod
    def from_yaml(
        yml: object, what: str, insn_operands: List[Operand]
    ) -> "InsnInformationFlowRule":
        ...
        for node_yml in check_list(rule["to"], to_what):
            ...
            nodes = _parse_iflow_nodes(check_str(node_yml, node_what), node_what, insn_operands)
...
class InsnInformationFlow:
    ...
    def required_constants(self, op_vals: Dict[str, int]) -> Set[str]:
        """Returns the names of regs that must be constant for `evaluate()`."""
        return {const for rule in self.rules for const in rule.required_constants(op_vals)}
    ...
    @staticmethod
    def from_yaml(
        yml: Optional[object], what: str, insn_operands: List[Operand]
    ) -> "InsnInformationFlow":
        if yml is None:
            # Default behavior: all source registers flow to all destination
            # registers, no other information flow or special registers
            sources = [
                InsnRegOperandNode(op)
                for op in insn_operands
                if isinstance(op.op_type, RegOperandType) and op.op_type.is_src()
            ]
            dests = [
                InsnRegOperandNode(op)
                for op in insn_operands
                if isinstance(op.op_type, RegOperandType) and op.op_type.is_dest()
            ]
            return InsnInformationFlow([InsnInformationFlowRule(dests, sources, MultiTest([]))])

        rule_list = check_list(yml, what)
        rules = []
        for idx, rule_yml in enumerate(rule_list):
            rule_what = "rule at index {} for {}".format(idx, what)
            rule = InsnInformationFlowRule.from_yaml(rule_yml, rule_what, insn_operands)
            rules.append(rule)
