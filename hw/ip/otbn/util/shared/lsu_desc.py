...
class LSUDesc:
    ...
    def __init__(self, lsu_type: str, target: List[str], idx_width: int) -> None:
    ...
    def from_yaml(yml: object, what: str) -> "LSUDesc":
        ...
        if lsu_type not in LSUDesc.TYPES:
            raise ValueError(
                "{} is {!r}, but should be one of the following: {}.".format(
                    type_what, lsu_type, ", ".join(repr(t) for t in LSUDesc.TYPES)
                )
            )
        ...
        if isinstance(target, str):
            ...
            if not isinstance(target, list):
                raise ValueError(
                    "target field for {} should be a string or a "
                    "list of strings, but actually has type {}.".format(what, type(target).__name__)
                )
            for idx, part in enumerate(target):
                elt_what = "element {} of the target list for {}".format(idx, what)
                target_parts.append(check_str(part, elt_what))

        if lsu_type.startswith("mem-"):
            if "bytes" not in yd:
                raise ValueError(
                    "{} defines a memory operation, so requires "
                    "a bytes field (how many bytes does this "
                    "operation touch?)".format(what)
                )
            idx_width = check_int(yd["bytes"], "bytes field for " + what)
        else:
            if "bytes" in yd:
                raise ValueError(
                    "{} isn't a memory operation, so cannot have a bytes field.".format(what)
                )
            idx_width = 1
