...
class LoopStart(ControlLoc):
    ...
    def pretty(self) -> str:
        return "<loop from {:#x}-{:#x}>".format(self.loop_start_pc, self.loop_end_pc)
...
class LoopEnd(Cycle):
    ...
    def pretty(self) -> str:
        return "<loop end: back to {:#x}>".format(self.loop_start.loop_start_pc)
...
class ControlGraph:
    ...
    def __init__(self, start: int, graph: Dict[int, Tuple[CodeSection, List[ControlLoc]]]):
    ...
    def get_entry(self, pc: int) -> Tuple[CodeSection, List[ControlLoc]]:
        ...
        try:
            ...
            raise ValueError(
                "PC {:#x} not found in control graph (PCs in graph are: {})".format(pc, graph_pcs)
            ) from None
    ...
    def _pretty_lines(
        ...
        non_special_edges = [
            loc for loc in edges if not (loc.is_special() or isinstance(loc, LoopEnd))
        ]
                ...
                if isinstance(loc, Ret) and len(call_stack) > 0:
                    ...
                    out += [
                        ...
                        for indent, line in self._pretty_lines(
                            program, call_stack[0], concise, already_printed, call_stack[1:]
                        )
                    ]
                if isinstance(loc, LoopStart):
                    out += [
                        (indent + child_indent + 2, line)
                        for indent, line in self._pretty_lines(
                            program, loc.loop_start_pc, concise, already_printed, call_stack
                        )
                    ]
            else:
    ...
    def pretty(self, program: OTBNProgram, indent: int = 0, concise: bool = False) -> str:
        ...
        line_strings = ["{}{}".format(" " * lindent, line) for (lindent, line) in lines]
        return "\n".join(line_strings)
...
def _get_next_control_locations(insn: Insn, operands: Dict[str, int], pc: int) -> List[ControlLoc]:
    ...
    if insn.mnemonic == "beq" or insn.mnemonic == "bne":
        ...
        if operands["offset"] != 0:
            raise RuntimeError(
                ...
                "supported by this check.".format(pc, insn.disassemble(pc, operands))
            )
    ...
    raise RuntimeError(
        "Unrecognized control flow instruction (straight-line=false) at PC {:#x}: {}".format(
            pc, insn.disassemble(pc, operands)
        )
    )


def _populate_control_graph(
    graph: ControlGraph, program: OTBNProgram, start_pc: int, loop_stack: List[LoopStart]
) -> None:
        ...
        for loc in locs:
            if not loc.is_special():
                ...
                if len(loop_stack) >= LOOP_STACK_SIZE - 1:
                    loop_start_pcs = [loop.loop_start_pc for loop in [loc] + loop_stack]
                    raise RuntimeError(
                        "Loop stack overflow at PC {} (loop stack includes "
                        "loops starting at the following PCs: {})".format(pc, loop_start_pcs)
                    )
                _populate_control_graph(graph, program, loc.loop_start_pc, [loc] + loop_stack)
            elif isinstance(loc, ImemEnd) or isinstance(loc, Ecall) or isinstance(loc, Ret):
                # Nothing more to do
                pass
            else:
                raise ValueError(
                    "Control-flow location of type {} is special "
                    "but does not have a recognized special type.".format(type(loc))
                )
    ...
    graph.graph[start_pc] = (CodeSection(start_pc, program.max_pc()), [ImemEnd()])
    return


def _label_cycles(
    program: OTBNProgram, graph: ControlGraph, start_pc: int, visited_pcs: Set[int]
) -> None:
...
def subroutine_control_graph(program: OTBNProgram, subroutine: str) -> ControlGraph:
