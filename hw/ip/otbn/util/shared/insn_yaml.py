...
from serialize.parse_helpers import (
    check_keys,
    check_str,
    check_bool,
    check_list,
...
class Insn:
    def __init__(
        self, yml: object, encoding_schemes: Optional[EncSchemes], isrs: Optional[IsrMaps]
    ) -> None:
        yd = check_keys(
            yml,
            "instruction",
        ...
        if encoding_yml is not None:
            ...
            self.encoding = Encoding(encoding_yml, encoding_schemes, self.mnemonic)

        self.operands = [
            Operand(y, self.mnemonic, self.encoding, isrs)
            for y in check_list(yd["operands"], "operands for " + what)
        ]
        self.name_to_operand = index_list("operands for " + what, self.operands, lambda op: op.name)
        ...
        for op in self.operands:
            if op.abbrev is not None:
                if op.abbrev in operand_names:
                    raise ValueError(
                        "The name {!r} appears as an operand or "
                        "abbreviation more than once for "
                        "instruction {!r}.".format(op.abbrev, self.mnemonic)
                    )
        ...
        if self.encoding is not None:
            ...
            missing_ops = set(self.encoding.op_to_field_name.keys()) - set(
                self.name_to_operand.keys()
            )
            if missing_ops:
                raise ValueError(
                    "Encoding scheme for {} specifies some non-existent operands: {}.".format(
                        what, ", ".join(list(missing_ops))
                    )
                )

        self.rv32i = check_bool(yd.get("rv32i", False), "rv32i flag for " + what)
        self.uses_isr = check_bool(yd.get("uses_isr", False), "uses_isr flag for " + what)
        self.glued_ops = check_bool(yd.get("glued-ops", False), "glued-ops flag for " + what)
        ...
        if "errs" in yd:
            ...
            for idx, err_desc in enumerate(y_errs):
                self.errs.append(check_str(err_desc, "element {} of the {}".format(idx, errs_what)))

        raw_syntax = get_optional_str(yd, "syntax", what)
        if raw_syntax is not None:
            self.syntax = InsnSyntax.from_yaml(self.mnemonic, raw_syntax.strip())
        else:
            self.syntax = InsnSyntax.from_list([op.name for op in self.operands])
        ...
        if set(self.name_to_operand.keys()) != self.syntax.op_set:
            raise ValueError(
                "Operand syntax for {!r} doesn't have the "
        ...
        self.python_pseudo_op = check_bool(
            yd.get("python-pseudo-op", False), "python-pseudo-op flag for " + what
        )
        if self.python_pseudo_op and self.encoding is not None:
            raise ValueError(
                "{} specifies an encoding and also sets python-pseudo-op.".format(what)
            )
            ...
            for idx, item in enumerate(lpo_lst):
                if not isinstance(item, str):
                    raise ValueError(
                        ...
                        "{} is {!r}, which is not a string.".format(idx, what, item)
                    )
            self.literal_pseudo_op = cast(Optional[List[str]], lpo_lst)

            if self.python_pseudo_op:
                raise ValueError(
                    "{} specifies both python-pseudo-op and literal-pseudo-op.".format(what)
                )
            if self.encoding is not None:
                raise ValueError(
                    "{} specifies both an encoding and literal-pseudo-op.".format(what)
                )
        ...
        if lsu_yaml is None:
            ...
            self.lsu = LSUDesc.from_yaml(lsu_yaml, "lsu field for {}".format(what))
            for idx, op_name in enumerate(self.lsu.target):
                if op_name not in self.name_to_operand:
                    raise ValueError(
                        "element {} of the target for the lsu "
                        "field for {} is {!r}, which is not a "
                        "operand name of the instruction.".format(idx, what, op_name)
                    )

        self.straight_line = yd.get("straight-line", True)

        iflow_what = "iflow field for {}".format(what)
        self.iflow = InsnInformationFlow.from_yaml(yd.get("iflow", None), iflow_what, self.operands)

    def enc_vals_to_op_vals(self, cur_pc: int, enc_vals: Dict[str, int]) -> Dict[str, int]:
    ...
    def disassemble(self, cur_pc: int, op_vals: Dict[str, int]) -> str:
...
class InsnGroup:
    def __init__(
        ...
        insns_path = os.path.normpath(os.path.join(os.path.dirname(path), insns_rel_path))
        insns_yaml = load_yaml(insns_path, insns_what)
        try:
            self.insns = [
                Insn(i, encoding_schemes, isrs) for i in check_list(insns_yaml, insns_what)
            ]
        except ValueError as err:
            raise RuntimeError(
                "Invalid schema in YAML file at {!r}: {}".format(insns_path, err)
            ) from None
...
class InsnGroups:
    def __init__(
        ...
        self.groups = [
            InsnGroup(path, encoding_schemes, y, isrs) for y in check_list(yml, "insn-groups")
        ]
        if not self.groups:
            raise ValueError(
                "Empty list of instruction groups: we need at least one as a base group."
            )
        self.key_to_group = index_list("insn-groups", self.groups, lambda ig: ig.key)


class InsnsFile:
    def __init__(self, path: str, yml: object, isrs: Optional[IsrMaps]) -> None:
        yd = check_keys(yml, "top-level", ["insn-groups"], ["encoding-schemes"])
        ...
        if enc_scheme_path is None:
            ...
            try:
                ...
                raise RuntimeError(
                    "Invalid schema in YAML file at {!r}: {}".format(es_path, err)
                ) from None

        self.groups = InsnGroups(path, self.encoding_schemes, yd["insn-groups"], isrs)
        ...
        self.mnemonic_to_insn = index_list("insns", self.insns, lambda insn: insn.mnemonic.lower())

        masks_exc, ambiguities = self._get_masks()
        if ambiguities:
            raise ValueError("Ambiguous instruction encodings: " + ", ".join(ambiguities))
    ...
    def _get_masks(self) -> Tuple[Dict[str, Tuple[int, int]], List[str]]:
        ...
        for mnem0, mnem1 in itertools.combinations(masks_inc.keys(), 2):
            ...
            if ambiguous_mask == (1 << 32) - 1:
                ambiguities.append(
                    "{!r} and {!r} both match bit pattern {:#010x}".format(mnem0, mnem1, m1 & ~m0)
                )
...
def load_file(path: str, isrs: Optional[IsrMaps]) -> InsnsFile:
    ...
    try:
        ...
        raise RuntimeError("Invalid schema in YAML file at {!r}: {}".format(path, err)) from None
...
def make_isr_dict(path: str) -> IsrMap:
    ...
    try:
        ...
        raise RuntimeError(
            "Invalid schema in ISR YAML file at {!r}: {}".format(path, err)
        ) from None
...
def load_insns_yaml() -> InsnsFile:
    ...
    _DEFAULT_INSNS_FILE = load_file(os.path.join(data_path, "insns.yml"), IsrMaps(csrs, wsrs))
