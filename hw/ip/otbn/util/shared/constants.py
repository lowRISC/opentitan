...
class ConstantContext:
    ...
    def __deepcopy__(self, memo: Optional[Dict[int, Any]]) -> "ConstantContext":
...
def parse_required_constants(constants: List[str]) -> Dict[str, int]:
    ...
    for token in constants:
        ...
        if not is_gpr_name(reg):
            raise ValueError(
                f"Cannot parse required constant {token}: {reg} is not a valid GPR name."
            )
        ...
        if value < 0 or value > GPR_MAX:
            raise ValueError(
                f"Cannot parse required constant {token}: {value} is out of range [0, GPR_MAX]."
            )
        out[reg] = value
