...
class EncodingField:
    ...
    def __init__(self, value: Union[BoolLiteral, str], scheme_field: EncSchemeField) -> None:
        self.value = value
        self.scheme_field = scheme_field

    @staticmethod
    def from_yaml(as_str: str, scheme_field: EncSchemeField, what: str) -> "EncodingField":
    ...
    def __init__(self, yml: object, schemes: EncSchemes, mnemonic: str):
        ...
        for field_name, scheme_field in scheme_fields.fields.items():
            if scheme_field.value is not None:
                ...
                field_what = "value for {} field in encoding for instruction {!r}".format(
                    field_name, mnemonic
                )
                ef_val = check_str(ydm[field_name], field_what)
                scheme_field = scheme_fields.fields[field_name]
                field = EncodingField.from_yaml(ef_val, scheme_field, field_what)
