...
def _get_elf_mem_data(
    elf_file: ELFFile, imem_desc: _MemDesc, dmem_desc: _MemDesc
) -> Tuple[bytes, bytes]:
...
def read_elf(path: str) -> Tuple[bytes, bytes, Dict[str, int]]:
    ...
    with open(path, "rb") as handle:
        ...
        imem_bytes, dmem_bytes = _get_elf_mem_data(elf_file, imem_desc, dmem_desc)
    ...
    if len(imem_bytes) & 3:
        raise RuntimeError(
            "ELF file at {!r} has imem data of length {}: not a multiple of 4.".format(
                path, len(imem_bytes)
            )
        )
