...
class EncSchemeField:
    ...
    def __init__(self, bits: BitRanges, value: Optional[BoolLiteral]) -> None:
        self.bits = bits
        self.value = value
    ...
    def from_yaml(yml: object, what: str) -> "EncSchemeField":
        ...
        if isinstance(yml, dict):
            ...
            if not (isinstance(bits_yml, str) or isinstance(bits_yml, int)):
                raise ValueError(
                    "{} is of type {}, not a string or int.".format(
                        bits_what, type(bits_yml).__name__
                    )
                )
                ...
                if not isinstance(val_yml, str):
                    raise ValueError(
                        ...
                        "base 10 and value should be in base 2).".format(
                            value_what, type(val_yml).__name__
                        )
                    )
                raw_value = val_yml
            ...
            raise ValueError(
                "{} is a {}, but should be a dict, string or integer.".format(
                    what, type(yml).__name__
                )
            )
        ...
        if raw_value is not None:
            ...
            if bits.width != value.width:
                raise ValueError(
                    "{} has bits that imply a width of {}, but a value with width {}.".format(
...
class EncSchemeImport:
    ...
    def __init__(self, yml: object, importer_name: str) -> None:
        as_str = check_str(yml, "value for import in encoding scheme {!r}".format(importer_name))
        ...
        if not match:
            raise ValueError(
                "Malformed encoding scheme inheritance by scheme {!r}: {!r}.".format(
            ...
            for arg in args:
                ...
                if len(arg_parts) != 2:
                    raise ValueError(
                        ...
                        "equals signs (should have exactly one).".format(when, len(arg_parts) - 1)
                    )

                field_name = arg_parts[0]
                field_what = (
                    "literal value for field {!r} when inheriting "
                ...
                if field_name in self.settings:
                    raise ValueError(
                        "{}, found multiple arguments assigning values to the field {!r}.".format(
                            when, field_name
                        )
                    )
    ...
    def apply_settings(self, esf: "EncSchemeFields", what: str) -> "EncSchemeFields":
        ...
        for name, literal in self.settings.items():
            ...
            if old_field is None:
                raise ValueError(
                    "{} sets unknown field {!r} from {!r}.".format(what, name, self.parent)
                )

            if old_field.bits.width != literal.width:
                raise ValueError(
                    "{} sets field {!r} from {!r} with a literal "
                    "of width {}, but the field has width {}.".format(
                        what, name, self.parent, literal.width, old_field.bits.width
                    )
                )
...
class EncSchemeFields:
    ...
    def __init__(self, fields: Dict[str, EncSchemeField], op_fields: Set[str], mask: int) -> None:
        self.fields = fields
    ...
    def from_yaml(yml: object, name: str) -> "EncSchemeFields":
        if not isinstance(yml, dict):
            raise ValueError(
                "fields for encoding scheme {!r} should be a dict, but we saw a {}.".format(
                    name, type(yml).__name__
                )
            )
        ...
        for key, val in yml.items():
            if not isinstance(key, str):
                raise ValueError(
                    "{!r} is a bad key for a field name of "
                    "encoding scheme {} (should be str, not {}).".format(
                        key, name, type(key).__name__
                    )
                )
        ...
        if overlaps:
            raise ValueError(
                "Direct fields for encoding scheme {} have "
                "overlapping ranges (mask: {:#08x})".format(name, overlaps)
            )
    ...
    def merge_in(self, right: "EncSchemeFields", when: str) -> None:
        for name, field in right.fields.items():
            if name in self.fields:
                raise ValueError("Duplicate field name: {!r} {}.".format(name, when))
            ...
            if overlap:
                raise ValueError(
                    ...
                    "intersection {:08x}) {}.".format(self.mask, field.bits.mask, overlap, when)
                )
...
class EncScheme:
    def __init__(self, yml: object, name: str) -> None:
        ...
        self.direct_fields = (
            EncSchemeFields.from_yaml(fields_yml, name)
        ...
        parents = (
            [EncSchemeImport(y, name) for y in check_list(parents_yml, parents_what)]
            if parents_yml is not None
            else []
        )
        self.parents = index_list(parents_what, parents, lambda imp: imp.parent)
...
class EncSchemes:
    def __init__(self, yml: object) -> None:
        if not isinstance(yml, dict):
            raise ValueError(
                "value for encoding-schemes is expected to be "
                "a dict, but was actually a {}.".format(type(yml).__name__)
            )
        ...
        for key, val in yml.items():
            if not isinstance(key, str):
                raise ValueError(
                    "{!r} is a bad key for an encoding scheme name (should be str, not {}).".format(
                        key, type(key).__name__
                    )
                )
            self.schemes[key] = EncScheme(val, key)

    def _resolve(self, name: str, user: str, stack: List[str]) -> EncSchemeFields:
        ...
        if name in stack:
            raise RuntimeError(
                "Circular inheritance of encoding schemes: {}".format(" -> ".join(stack + [name]))
            )

        # Does the scheme actually exist?
        scheme = self.schemes.get(name)
        if scheme is None:
            raise ValueError("{} requires undefined encoding scheme {!r}.".format(user, name))
        ...
        for pname, pfields in resolved_parents.items():
            when = "merging fields of scheme {} into already merged fields of {}".format(
                pname, ", ".join(parent_names_so_far)
            )
            merged.merge_in(pfields, when)
            parent_names_so_far.append(repr(pname))

        # Now try to merge in any direct fields
        when = "merging direct fields of scheme {} into fields from parents".format(name)
    ...
    def resolve(self, name: str, mnemonic: str) -> EncSchemeFields:
        ...
        if missing:
            raise ValueError(
                "Fields for encoding scheme {} miss some bits (mask: {:#08x})".format(name, missing)
            )
