...
def find_tool(tool_name: str) -> str:
    ...
    if configured_tool_path is not None:
        if not os.path.exists(configured_tool_path):
            raise RuntimeError(
                "No such file: {!r} (derived from the "
                "{!r} environment variable when trying "
                "to find the {!r} tool).".format(configured_tool_path, tool_env_var, tool_name)
            )
    ...
    if toolchain_path is not None:
        ...
        if not os.path.exists(tool_path):
            raise RuntimeError(
                ...
                "trying to find the {!r} tool).".format(tool_path, tool_name)
            )
        return tool_path
    ...
    raise RuntimeError(
        "Unable to find {!r} in PATH or in {!r}. Set the {!r} "
        "or TOOLCHAIN_PATH environment variable if you "
        "installed your RISC-V toolchain in an alternate "
        "location.".format(expanded, default_location, tool_env_var)
    )
