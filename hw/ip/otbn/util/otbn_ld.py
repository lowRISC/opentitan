...
def interpolate_linker_script(in_path: str, out_path: str) -> None:
    ...
    try:
        ...
        raise RuntimeError("Failed to open output file at {!r}.".format(out_path)) from None


@contextmanager
def mk_linker_script() -> Iterator[str]:
    ld_in = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "otbn.ld.tpl"))
...
def run_ld(ld_script: Optional[str], args: List[str]) -> int:
    ...
    try:
        ...
        sys.stderr.write(
            "Unknown command: {!r}. (is it installed and on your PATH?)\n".format(ld_name)
        )
        return 127
