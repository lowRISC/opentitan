...
For each vector element:
- Execute with the two extreme values of `MOD` (zero and all ones) and
intermediate values.
  Tracked as `mod_cp`.
- A non-negative intermediate result with a nonzero `MOD` (so `MOD` is not added).
  Tracked as `diff_nonneg_elemX_cp` where `X` represents the vector element index.- An intermediate result that exactly equals a nonzero `-MOD`.
