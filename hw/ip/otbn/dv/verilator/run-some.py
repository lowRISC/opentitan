...
def main() -> int:
    ...
    parser.add_argument(
        "--count", type=int, default=10, help="Number of binaries to generate and run"
    )
    ...
    if subprocess.run(gb_cmd, check=False).returncode:
        print(
            "Failed to run generate-binaries (command: {})".format(
                " ".join([shlex.quote(arg) for arg in gb_cmd])
            ),
            file=sys.stderr,
        )
        return 1
...
def write_ninja(handle: TextIO, destdir: str, seed: int, count: int) -> None:
    ...
    # Rules to run them
    handle.write(
        f"rule run\n  command = REPO_TOP={projdir_from_destdir} $tb --load-elf $in >$out\n\n"
    )
