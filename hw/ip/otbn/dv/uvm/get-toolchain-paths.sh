...
if [[ -n ${BAZEL_CACHE} ]]; then
  ...
  ${BAZEL_CMD} fetch \
    ...
    @lowrisc_rv32imcb_$(uname -m)_files//...
else
  BAZEL_CMD="./bazelisk.sh"
  ${BAZEL_CMD} fetch @lowrisc_rv32imcb_$(uname -m)_files//...
fi

# Set environment variables for the RV32 linker and assembler.
RV32_TOOL_LD=$(${BAZEL_CMD} query \
  "deps(@lowrisc_rv32imcb_$(uname -m)_files//:bin/riscv32-unknown-elf-ld)" \
  --output location | cut -f1 -d:)
RV32_TOOL_AS=$(${BAZEL_CMD} query \
  "deps(@lowrisc_rv32imcb_$(uname -m)_files//:bin/riscv32-unknown-elf-as)" \
  --output location | cut -f1 -d:)
export RV32_TOOL_LD
export RV32_TOOL_AS
