...
interface otbn_loop_if #(
  ...
  // This avoids parametrizing this interface (which requires manually adjusting the default value
  ...
  function static void randomize_loop_addrs_info();
    ...
    `DV_CHECK_STD_RANDOMIZE_FATAL(loop_start, "Randomization failed!",
                                  "otbn_loop_if.randomize_loop_addrs_info.loop_start");
    `DV_CHECK_STD_RANDOMIZE_FATAL(loop_end, "Randomization failed!",
                                  "otbn_loop_if.randomize_loop_addrs_info.loop_end");

    // Add a check to abort in case the a too large IMEM address is configured and this function
    // is not properly adapted. With >15 bits the start and end addresses would not fit into 32
    // bits anymore.
    `DV_CHECK_FATAL(ImemAddrWidth < 16,
                   "Specified IMEM is too large for randomize_loop_addrs_info.",
                   "otbn_loop_if.randomize_loop_addrs_info.IMEM_size_check");
    ...
    force tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_addrs_intg = loop_addrs_padded_intg[38:32];
    force tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_start = loop_start;
    force tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_end = loop_end;
  endfunction

  // Release the forcing of the loop_addr_info.
  function automatic void release_loop_addrs_info();
    release tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_addrs_intg;
    release tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_start;
    release tb.dut.u_otbn_core.u_otbn_controller.u_otbn_loop_controller.current_loop.
      loop_addr_info.loop_end;
  endfunction
