...
`undef DEF_FAC_CP
  ...
  covergroup insn_bn_addv_cg
    ...
    `define DEF_ADDV_OVERFLOW_CP(elem) \
      `DEF_SEEN_CP(overflow_elem``elem``_cp, \
          ...
          {1'b0, wdr_operand_b[32*elem +: 32]} >= (33'b1 << 32))))
  ...
  covergroup insn_bn_addvm_cg
    ...
    // Define the three MOD-related sum coverpoints per element together
    // Case 1: A sum that is less than a non-zero MOD (so no MOD subtraction occurs)
    // Case 2: Sum exactly equals a nonzero MOD (subtracting down to zero)
    // Case 3: Sum is greater than a nonzero MOD, but not twice as big (since operation according to ISA
    // operates correctly providing that result is less than 2*MOD)
    `define DEF_ADDVM_SUM_MOD_CP(elem) \
    ...
    `DEF_ADDVM_SUM_MOD_CP(0)
    `DEF_ADDVM_SUM_MOD_CP(1)
    `DEF_ADDVM_SUM_MOD_CP(2)
    `DEF_ADDVM_SUM_MOD_CP(3)
    `DEF_ADDVM_SUM_MOD_CP(4)
    `DEF_ADDVM_SUM_MOD_CP(5)
    `DEF_ADDVM_SUM_MOD_CP(6)
    `DEF_ADDVM_SUM_MOD_CP(7)
    `undef DEF_ADDVM_SUM_MOD_CP
  ...
  covergroup insn_bn_subvm_cg
    ...
    // Define the three MOD-related difference SUBVM coverpoints per element together
    // Case 1: A non-negative result with a non-zero MOD (so no MOD addition occurs)
    // Case 2: A difference that exactly equals a non-zero -MOD (adding back up to zero)
    //    We want to see A - B = -MOD. Rearrange this (by adding MOD+B to both sides) to get
    //    A + MOD = B.
    // Case 3: A negative intermidate difference with a non-zero MOD, so MOD is added back to give a positive result, and
    // checking that the difference is greater than at least -MOD, which is required for correct ISA operation

    `define DEF_SUBVM_DIFF_MOD_CP(elem) \
      ...
      `DEF_SEEN_CP(diff_minus_mod_elem``elem``_cp, \
        (mod != 0) && \
        (wdr_operand_a[32*elem+:32] + mod == wdr_operand_b[32*elem+:32])) \
      `DEF_SEEN_CP(diff_neg_elem``elem``_cp, \
        (mod != 0) && \
        ({1'b0, wdr_operand_a[32*elem+:32]} < {1'b0, wdr_operand_b[32*elem +:32]}) && \
        ({1'b0, wdr_operand_a[32*elem+:32]} + {1'b0, mod} > {1'b0, wdr_operand_b[32*elem+:32]}))

    `DEF_SUBVM_DIFF_MOD_CP(0)
    `DEF_SUBVM_DIFF_MOD_CP(1)
    `DEF_SUBVM_DIFF_MOD_CP(2)
    `DEF_SUBVM_DIFF_MOD_CP(3)
    `DEF_SUBVM_DIFF_MOD_CP(4)
    `DEF_SUBVM_DIFF_MOD_CP(5)
    `DEF_SUBVM_DIFF_MOD_CP(6)
    `DEF_SUBVM_DIFF_MOD_CP(7)
    `undef DEF_SUBVM_DIFF_MOD_CP
  endgroup
  ...
  covergroup insn_bn_mulv_cg
    ...
    `define DEF_MULV_OVERFLOW_CP(elem) \
      `DEF_SEEN_CP(overflow_elem``elem``_cp, \
          (({32'b0, wdr_operand_a[32*elem +: 32]} * \
          {32'b0, wdr_operand_b[32*elem +: 32]} > (33'b1 << 32))))
  ...
  covergroup insn_bn_mulvl_cg
    ...
    `define DEF_MULVL_OVERFLOW_CP(elem) \
      `DEF_SEEN_CP(overflow_elem``elem``_cp, \
          (({32'b0, wdr_operand_a[32*elem+:32]} * \
          {32'b0, wdr_operand_b[32*insn_data[30:28]+:32]} > (33'b1 << 32))))
  ...
  covergroup insn_bn_mulvm_cg
    ...
    // Combined MULVM checks: gt/eq/lt relative to MOD and overflow
    // Case 1: Checking for when the multiplications give a result greater than/equal to mod
    // Case 2: Checking for when the multiplication gives a result that is less than mod
    // Case 3: Checking for overflow
    `define DEF_MULVM_MOD_OVERFLOW_CP(elem) \
    ...
    `DEF_MULVM_MOD_OVERFLOW_CP(0)
    `DEF_MULVM_MOD_OVERFLOW_CP(1)
    `DEF_MULVM_MOD_OVERFLOW_CP(2)
    `DEF_MULVM_MOD_OVERFLOW_CP(3)
    `DEF_MULVM_MOD_OVERFLOW_CP(4)
    `DEF_MULVM_MOD_OVERFLOW_CP(5)
    `DEF_MULVM_MOD_OVERFLOW_CP(6)
    `DEF_MULVM_MOD_OVERFLOW_CP(7)
    `undef DEF_MULVM_MOD_OVERFLOW_CP
  ...
  covergroup insn_bn_mulvml_cg
    ...
    // Combined MULVML checks: gt/ls relative to MOD and overflow
    // Case 1: Checking for when the multiplications give a result greater than/equal to mod
    // Case 2: Checking for when the multiplication gives a result that is less than mod
    // Case 3: Checking for overflow
    `define DEF_MULVML_MOD_OVERFLOW_CP(elem) \
      `DEF_SEEN_CP(gt_mod_elem``elem``_cp, \
        wide_prod >= {1'b0,mod}) \
      `DEF_SEEN_CP(ls_mod_elem``elem``_cp, \
        wide_prod < {1'b0,mod}) \
    ...
    `DEF_MULVML_MOD_OVERFLOW_CP(0)
    `DEF_MULVML_MOD_OVERFLOW_CP(1)
    `DEF_MULVML_MOD_OVERFLOW_CP(2)
    `DEF_MULVML_MOD_OVERFLOW_CP(3)
    `DEF_MULVML_MOD_OVERFLOW_CP(4)
    `DEF_MULVML_MOD_OVERFLOW_CP(5)
    `DEF_MULVML_MOD_OVERFLOW_CP(6)
    `DEF_MULVML_MOD_OVERFLOW_CP(7)
    `undef DEF_MULVML_MOD_OVERFLOW_CP
  endgroup
    ...
    case (mnem)
      ...
      mnem_bn_addvm: begin
        ...
        for (int elem = 0; elem < 8; elem++) begin
          wide_sum[33*elem+:33] =
            {1'b0, rtl_item.wdr_operand_a[32*elem+:32]} +
            {1'b0, rtl_item.wdr_operand_b[32*elem+:32]};
        end
      ...
      mnem_bn_mulvml: begin
        ...
        for (int elem = 0; elem < 8; elem++) begin
          wide_prod[33*elem+:33] =
          {1'b0, rtl_item.wdr_operand_a[32*elem+:32]} *
          {1'b0, rtl_item.wdr_operand_b[32*insn_data[30:28]+:32]} *
          (2**(-32));
        end
