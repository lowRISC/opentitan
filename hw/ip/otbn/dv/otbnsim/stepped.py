...
def read_word(arg_name: str, word_data: str, bits: int) -> int:
    ...
    try:
        ...
        raise ValueError(
            "Failed to read {!r} as an integer for <{}> argument.".format(word_data, arg_name)
        ) from None

    if value < 0 or value >> bits:
        raise ValueError(
            "<{}> argument is {!r}: not representable in {!r} bits.".format(
                arg_name, word_data, bits
            )
        )
...
def on_add_loop_warp(sim: OTBNSim, args: List[str]) -> Optional[OTBNSim]:
    ...
    try:
        ...
        raise ValueError("Bad argument to add_loop_warp: {}".format(err)) from None
...
def on_set_keymgr_value(sim: OTBNSim, args: List[str]) -> Optional[OTBNSim]:
    ...
    sim.state.wsrs.set_sideload_keys(key0 if valid else None, key1 if valid else None)
