...
class Dmem:
    ...
    def __init__(self) -> None:
        ...
        if dmem_size % 32:
            raise RuntimeError("DMEM size ({}) is not divisible by 32.".format(dmem_size))
    ...
    def _load_5byte_le_words(self, data: bytes) -> None:
        ...
        if len(data) % 5:
            raise ValueError(
                "Trying to load {} bytes of data, which is not a multiple of 5.".format(len(data))
            )
        ...
        if len_data_32 > len_mem_32:
            raise ValueError(
                "Trying to load {} bytes of data, but DMEM is only {} bytes long.".format(
                    4 * len_data_32, 32 * len(self.data)
                )
            )
        ...
        for idx32, (vld, u32) in enumerate(struct.iter_unpack("<BI", data)):
            if vld not in [0, 1]:
                raise ValueError(
                    "The validity byte for 32-bit word {} "
                    "in the input data is {}, not 0 or 1.".format(idx32, vld)
                )
