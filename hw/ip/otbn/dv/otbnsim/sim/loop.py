...
class TraceLoopStart(Trace):
    ...
    def trace(self) -> str:
        return "Starting loop {}, {} iterations, bodysize: {}".format(
            self.depth, self.iterations, self.bodysize
        )
...
class TraceLoopIteration(Trace):
    ...
    def trace(self) -> str:
        return "Finished iteration {}/{} of loop {}".format(self.iteration, self.total, self.depth)
...
class LoopStack:
    ...
    def start_loop(self, start_addr: int, loop_count: int, insn_count: int) -> None:
        """Start a loop.
    ...
    def step(self, pc: int, warps: Dict[int, int]) -> Optional[int]:
        ...
        if self.is_last_insn_in_loop_body(pc):
            ...
            self.trace.append(TraceLoopIteration(len(self.stack), loop_idx, top.loop_count))
            return ret_val
