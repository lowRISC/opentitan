...
def decode_words(base_addr: int, data: List[Tuple[bool, int]]) -> List[OTBNInsn]:
...
def decode_file(base_addr: int, path: str) -> List[OTBNInsn]:
    ...
    if len(raw_bytes) % 5:
        raise ValueError(
            "Trying to load {} bytes of data from {}, which is not a multiple of 5.".format(
                path, len(raw_bytes)
            )
        )
    ...
    for idx32, (vld, u32) in enumerate(struct.iter_unpack("<BI", raw_bytes)):
        if vld not in [0, 1]:
            raise ValueError(
                "The validity byte for 32-bit word {} at {} is {}, not 0 or 1.".format(
                    idx32, path, vld
                )
            )
