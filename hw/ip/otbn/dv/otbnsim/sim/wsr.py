...
class URNDWSR(ISPR):
    ...
    URND_CTRL_ENABLED_OFFSET = 0
    STOPPED_OFFSET = 1
    RESTORING_OFFSET = 2
    USED_WHILE_STOPPED_OFFSET = 3
    STATE_SIZE_OFFSET = 16
    STATE_SIZE_MASK = 0x3ff  # 10 bits
    PART_SEED_SIZE_OFFSET = 26
    PART_SEED_SIZE_MASK = 0x3f  # 6 bits

    CMD_STOP_MASK = 0x1
    CMD_START_MASK = 0x2
    CMD_RESTORE_MASK = 0x4
    ...
    def get_state(self) -> int:
        '''Return the current PRNG state if URND control is enabled. Otherwise return zero.'''
        if not self.urnd_ctrl_enabled:
            return 0
    ...
    def mark_consumed(self) -> None:
        '''Mark URND as consumed this cycle, forcing the PRNG to advance on commit even if
        stopped.'''
    ...
    def step(self, predec_read: bool = False) -> None:
        ...
        if predec_read and self.stopped:
            ...
            self._trivium.step()
            self._value = self._trivium.keystream()
            self._value_permuted = False

        # Schedule a state update and readout the raw keystream. The state update is always
        # speculative. The URND commit()/abort() decide whether the state update actually takes
        # effect. The permutation is applied only when necessary.
        self._trivium.update()
