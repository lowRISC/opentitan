...
def find_simple_tests() -> List[Tuple[str, str]]:
    ...
    for subdir, _, files in os.walk(root):
        ...
        for basename, asm_file in asm_files.items():
            ...
            ret.append((os.path.join(dirname, asm_file), os.path.join(dirname, exp_file)))
        ...
        for basename, exp_file in exp_files.items():
            if basename not in asm_files:
                raise RuntimeError(
                    ...
                    "that generates the expected values.".format(dirname, exp_file, basename)
                )
...
def test_count(tmpdir: py.path.local, asm_file: str, expected_file: str) -> None:
    ...
    cmd = [os.path.join(SIM_DIR, "standalone.py"), "--dump-regs", "-", elf_file]
    sim_proc = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, universal_newlines=True)
