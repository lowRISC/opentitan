    ...
    def __init__(self, what: str, yml: object):
        ...
        for key, value in yml.items():
            if not isinstance(key, str):
                raise ValueError("{} had key {!r}, which is not a string.".format(what, key))
            try:
                fval = float(value)
            except ValueError:
                raise ValueError(
                    "{} at key {!r} had value {!r}, which is not a float.".format(what, key, value)
                ) from None

            if fval < 0:
                raise ValueError(
                    "{} at key {!r} had negative value {}, which is not a valid weight.".format(
                        what, key, fval
                    )
                )
...
class MinMaxes:
        ...
        for key, value in yml.items():
            ...
            if not is_min:
                if not key.startswith("max-"):
                    raise ValueError(
                        '{} had key {!r}, which does not start with "min-" or "max-".'.format(
                            what, key
                        )
                    )
            ...
            try:
                ...
                raise ValueError(
                    "{} at key {!r} had value {!r}, which is not an integer.".format(
                        what, key, value
                    )
                ) from None
    ...
    def get_range(self, key: str, default_min: int, default_max: int) -> Tuple[int, int]:
        """Get a (min, max) pair from the dictionary"""
        return (self.min_values.get(key, default_min), self.max_values.get(key, default_max))
...
class Inheritance:
    ...
    def __init__(self, item_num: int, yml: object):
        if isinstance(yml, str):
            ...
            cfgs = check_str(yd["cfgs"], "cfgs field for inheritance list {}".format(item_num))
            ...
            if isinstance(yw, float) or isinstance(yw, int):
                ...
                try:
                    ...
                    raise ValueError(
                        "The weight in inheritance list {} is {!r}, not a valid float.".format(
                            item_num, yw
                        )
                    )
            else:
                raise ValueError(
                    "The weight in inheritance list {} is not a number or a string.".format(
                        item_num
                    )
                )
            ...
            if weight <= 0:
                raise ValueError(
                    "The weight in inheritance list {} is {}, which is not positive.".format(
                        item_num, weight
                    )
                )
        ...
        for n in self.names:
            if not n:
                raise ValueError("Empty name in list of config names: {}".format(self.names))


class Config:
    """An object representing configuration for a RIG run"""

    def __init__(self, cfg_dir: str, path: str, known_names: Set[str], yml: object):
        yd = check_keys(yml, "top-level", [], ["gen-weights", "insn-weights", "inherit", "ranges"])
        ...
        if isinstance(y_inherit, str):
            ...
            raise ValueError("inherit field for config at {} is not a string or list.".format(path))

        inhs = [Inheritance(idx + 1, elt) for idx, elt in enumerate(inherit_lst)]
        if not inhs:
            merged_ancestors = None
        else:
            # There's at least one possible list of parents. Pick one.
            parents = random.choices([i.names for i in inhs], weights=[i.weight for i in inhs])[0]
            # The Inheritance class constructor ensures that parents is
            # nonempty.
            assert parents
            merged_ancestors = Config.load_and_merge(cfg_dir, parents, known_names)
    ...
    @staticmethod
    def load(cfg_dir: str, name: str, children: Optional[Set[str]] = None) -> "Config":
        if children is not None and name in children:
            raise ValueError("Dependency loop for config: {} includes itself.".format(name))
        ...
        try:
            ...
            raise RuntimeError("Invalid schema in YAML file at {!r}: {}".format(path, err))

    @staticmethod
    def load_and_merge(cfg_dir: str, names: List[str], children: Set[str]) -> "Config":
        assert names
