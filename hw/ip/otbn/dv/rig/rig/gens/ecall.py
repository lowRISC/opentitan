...
class ECall(SnippetGen):
    ...
    def __init__(self, cfg: Config, insns_file: InsnsFile) -> None:
        ...
        if ecall_insn.operands:
            raise RuntimeError(
                "ECALL instruction in instructions file has a nonempty list of operands."
            )

        if ecall_insn.lsu:
            raise RuntimeError(
                "ECALL instruction in instructions file has unexpected LSU information."
            )
        ...
        if cfg.insn_weights.values.get("ecall") is not None:
            raise ValueError(
                "Config at {} configures the instruction "
                "weight of ECALL. This has no effect: configure "
                "the weight of the ECall generator instead.".format(cfg.path)
            )
