...
impl_list = [impl_dut.replace(rtl_path, "").replace(".sv", "") for impl_dut in impl_list]
...
sv2v_cmd = ["sv2v", "-I", prim_path] + dep_list + [rtl_path + impl_gold + ".sv"]
...
for impl_dut in impl_list:
    ...
    sv2v_cmd = ["sv2v", "-I", prim_path] + dep_list + [rtl_path + impl_dut + ".sv"]
    ...
    if "masked" in impl_dut:
        ...
        # Change module name
        replace_module_name("scratch/" + file_wrap, "aes_sbox_masked", impl_dut)
    ...
    # Change module name
    if "masked" in impl_dut:
        replace_module_name("scratch/aes_sbox_dut.v", impl_dut + "_wrapper", "aes_sbox_dut")
    else:
    ...
    if (len(lec_lines) == 0) or (len(lec_lines) != num_lec_success) or (num_lec_failed > 0):
        print("LEC failed: \t\t\t" + impl_dut + ".sv\n-> " + "Check " + lec_log + " for details.")
