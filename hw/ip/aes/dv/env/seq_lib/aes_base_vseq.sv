...
class aes_base_vseq extends cip_base_vseq #(
  ...
  // Send an AES message
  //
  // Reset behaviour depends on the m_external_reset configuration bit:
  //  - If m_external_reset=1, this task will not generate its own resets. When an event that locks
  //    up the block happens, the task will just wait for a reset to be generated elsewhere and then
  //    return.
  //  - If m_external_reset=0, the task checks that no external reset has been generated.
  ...
  virtual task config_and_transmit (
    ...
    if (manual_operation && !rst_set) begin
      trigger();
      if (cfg.under_reset) return;

      // When in AES-GCM mode, trigger a second time. The first trigger encrypted and loaded the
      // all-zero block. Now, we will encrypt the initial counter block and load it into the GHASH
      // block.
      if (cfg_item.mode == AES_GCM && new_msg) begin
        trigger();
        if (cfg.under_reset) return;
      end
    end
  ...
  virtual task wait_for_fatal_alert_and_reset ();
    ...
    if (m_external_reset) begin
      // Because m_external_reset is true, we don't want to apply our own reset, and should just
      // wait for one to be applied from outside. To ensure this doesn't spin for an enormous
      // timeout, bound the wait at 10ms (*much* more than we'd normally expect to wait in
      ...
      fork : isolation_fork begin
        fork
          ...
          begin
            #10ms;
            `uvm_fatal(get_full_name(),
                       "After generating a fatal alert, the block ran for 10ms without a reset.")
          end
        join_any
        disable fork;
      end join
  ...
  //
  // If m_external_reset is true, this task will not generate its own resets. When an event that
  // locks up the block happens, the task will just wait for a reset to be generated elsewhere and
  // then return. If m_external_reset is false, the task checks that no external reset has been
  // generated.
  ////////////////////////////////////////////////////////////////////////////////////////////
