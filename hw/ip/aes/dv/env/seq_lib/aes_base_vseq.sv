...
class aes_base_vseq extends cip_base_vseq #(
  ...
  virtual task set_mode(bit mode);
    bit [31:0] read_val;
    csr_rd(.ptr(ral.ctrl), .value(read_val));
    `uvm_info(`gfn, $sformatf("read ctrl reg: %02h", read_val), UVM_DEBUG);

    read_val[0] = mode;
    csr_wr(.csr(ral.ctrl), .value(read_val));
    `uvm_info(`gfn, $sformatf("wrote new ctrl value: %02h", read_val), UVM_DEBUG);
