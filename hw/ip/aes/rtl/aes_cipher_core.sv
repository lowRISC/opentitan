...
#(
  ...
  //
  // Note that for the masked Canright S-Box implementations, it is important that the SubBytes
  // input masks (generated by the PRNG in Round X-1) and the SubBytes output masks (generated by
  // the PRNG in Round X) are independent. Inside the PRNG, this is achieved by selecting the
  // separate LFSR chunks in alternating fashion.
  //
  // Since the input data masks become the SubBytes input masks in the first round, we thus select
  // those bit lanes that are used to form the SubBytes output mask for the masked Canright S-Box
  // implementations, i.e., the LSBs of the per S-Box pseudo-random data. In particular, we have:
  //
  // prd_masking = { prd_key_expand, ... , sb_prd[4], sb_out_mask[4], sb_prd[0], sb_out_mask[0] }
  //
  // Where sb_out_mask[x] contains the SubBytes output mask for byte x (when using masked Canright
  // S-Box) and sb_prd[x] contains additional PRD consumed by SubBytes for byte x. We do the
  // extraction on a row basis.
  ...
  logic [3:0][3:0][(WidthPRDSBox-8)-1:0] unused_sb_prd;
  for (genvar i = 0; i < 4; i++) begin : gen_unused_sb_prd
    assign unused_sb_prd[i] = aes_sb_prd_get(prd_masking[i * WidthPRDRow +: WidthPRDRow]);
  end
  for (genvar i = 0; i < 4; i++) begin : gen_unused_prd_masking
    assign unused_prd_masking[i * WidthPRDRow +: WidthPRDRow] =
        aes_sb_out_mask_prd_concat(data_in_mask_o[i], unused_sb_prd[i]);
  end
