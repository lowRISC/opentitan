...
class rv_dm_env_cfg extends cip_base_env_cfg #(.RAL_T(rv_dm_regs_reg_block));
  ...
  virtual function void initialize();
    ...
    // Configure the RAL model for the mem register block (rv_dm_mem_reg_block) to represent the
    // fact that the hardware completely ignores the instr_type signal. In the design, accesses to
    // both CSRs and the debug ROM are passed straight to dm_top (without information about whether
    // this is a read or a fetch), so there is nothing to stop fetches from CSRs.
    ral_models[mem_ral_name].set_ignores_instr_type(1'b1);
