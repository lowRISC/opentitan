...
class rv_dm_buffered_enable_vseq extends rv_dm_base_vseq;
  ...
  // The bits that are force-enabled. This can be mirrored into the tb.force_enable bits by a call
  // to update_force_enable()
  bit [4:0] force_enable;

  // Mirror our force_enable class variable into the tb.force_enable signal.
  task update_force_enable();
    string force_enable_path = "tb.force_enable";
    `DV_CHECK(uvm_hdl_deposit(force_enable_path, force_enable))
  ...
  task body();
    copy_t copies[$];
    string use_force_path = "tb.use_force";
    ...
    // Turn on the use_force signal in the tb, which will cause tb.force_enable to have an effect.
    `DV_CHECK_FATAL(uvm_hdl_deposit(use_force_path, 1'b1))

    // Compute an initial force_enable signal based on the target copy and mirror it in the
    // tb.force_enable array.
    force_enable = 1 << int'(tgt_copy);
    update_force_enable();
    ...
    // sequence)
    `DV_CHECK_FATAL(uvm_hdl_deposit(use_force_path, 1'b0))
