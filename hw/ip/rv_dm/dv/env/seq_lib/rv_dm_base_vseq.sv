...
class rv_dm_base_vseq extends cip_base_vseq #(
  ...
  // pre_start and it takes effect either through a top-level pin (!pin_disable_late_debug) or a
  // register (reg_late_debug_enable).
  //
  // When one of these bits is true, so one of the associated inputs is driven with MuBi4True, the
  // "debug enable" check is made on lc_hw_debug_en_i instead of lc_dft_en_i.
...
`endif
    ...
    mode_seq.m_has_next_dm_addr = 1'b1;
    ...
    if (!mode_seq.randomize() with {
          ...
          //
          // We don't currently model systems with multiple debug modules. Drive this port with the
          // same value as given as a default for next_dm_addr in rv_dm.hjson.
          m_next_dm_addr == '0;
...
`endif
  ...
  // Update the signals that rv_dm gets from lc_ctrl so that they match the corresponding variables
  // in the virtual sequence (lc_hw_debug_clr, lc_hw_debug_en, lc_init_done).
  ...
  // Update the signals that rv_dm gets from pinmux so that they match the pinmux_hw_debug_en
  ...
  //
  // This extends a function from cip_base_vseq
  function void configure_child_vseq(uvm_sequence child_vseq);
    rv_dm_base_vseq rv_dm_child_vseq;

    super.configure_child_vseq(child_vseq);

    if (!$cast(rv_dm_child_vseq, child_vseq)) begin
      `uvm_fatal(get_full_name(), "Cannot configure child that is not an rv_dm_base_vseq.")
    end

    rv_dm_child_vseq.m_mode_sequencer = m_mode_sequencer;
  endfunction
