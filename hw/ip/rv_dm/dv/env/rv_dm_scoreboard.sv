...
class rv_dm_scoreboard extends cip_base_scoreboard #(
  ...
  // A / D channel transactions that have been seen by the monitor in the environment's
  // m_tl_sba_agent
  uvm_tlm_analysis_fifo #(tl_seq_item)  tl_sba_a_chan_fifo;
  uvm_tlm_analysis_fifo #(tl_seq_item)  tl_sba_d_chan_fifo;

  // JTAG items seen by the environment's m_sba_access_monitor and are not related to the DMI
  // register.
  uvm_tlm_analysis_fifo #(jtag_item)    jtag_non_dmi_dtm_fifo;

  // DMI transactions seen by the environment's m_sba_access_monitor that are not SBA transactions.
  ...
  //
  // This queue is needed because items from sba_access_fifo come in much later: the inferred
  // transactions are marked completed only when the JTAG interface actually reads the SBCS
  // register. So we capture and hold the TL accesses over SBA and compare against SBA access
  // whenever they arrive.
  local tl_seq_item sba_tl_access_q[$];

  // Currently selected non-DMI DTM CSR.
  local uvm_reg selected_dtm_csr;
  ...
  // Update the scoreboard when a reset is seen
  //
  // This implements a function from dv_base_scoreboard.
  extern virtual function void reset(string kind = "HARD");

  // Consume a TL access reported by a monitor
  //
  // This implements cip_base_scoreboard::process_tl_access.
  extern virtual task process_tl_access(tl_seq_item   item,
  ...
  // Receive and process incoming A-channel SBA accesses (through tl_sba_a_chan_fifo)
  extern local task process_tl_sba_a_chan_fifo();

  // Receive and process incoming D-channel SBA accesses (through tl_sba_d_chan_fifo)
  extern local task process_tl_sba_d_chan_fifo();

  // Process a TL access that was seen on the A or D channel by one of the previous two tasks.
  extern local task process_tl_sba_access(tl_seq_item item, tl_channels_e channel);

  // Compare an SBA access and the TL sequence item it triggered
  extern local function void compare_sba_access(sba_access_item sba_item,
                                                tl_seq_item     sba_tl_item);

  // A function called by process_tl_access when we see a D-channel item which is a response on the
  // regs interface.
  extern local function void on_regs_access_d(tl_seq_item item);

  // A function called by process_tl_access when we see a D-channel item which is a response on the
  // mem interface.
  extern local function void on_mem_access_d(tl_seq_item item);

  // Return true if we're using late debug enable, based on top-level inputs and register state.
  extern local function logic using_late_debug_enable();

  // Return whether debug is supposed to be enabled, based on whether we're using late debug enable
  // and on top-level inputs.
  extern local function logic is_debug_enabled();
endclass

function rv_dm_scoreboard::new(string name, uvm_component parent);
  super.new(name, parent);
endfunction

function void rv_dm_scoreboard::build_phase(uvm_phase phase);
...
task rv_dm_scoreboard::run_phase(uvm_phase phase);
...
function void rv_dm_scoreboard::check_phase(uvm_phase phase);
...
function void rv_dm_scoreboard::reset(string kind = "HARD");
  super.reset(kind);
...
task rv_dm_scoreboard::process_tl_access(tl_seq_item item, tl_channels_e channel, string ral_name);
...
function bit rv_dm_scoreboard::predict_tl_err(tl_seq_item   item,
                                              tl_channels_e channel,
                                              string        ral_name);
...
task rv_dm_scoreboard::process_jtag_non_sba_dmi_fifo();
  uvm_reg       csr;
  jtag_dmi_item item;
  bit           do_read_check;
...
task rv_dm_scoreboard::process_tl_sba_a_chan_fifo();
  tl_seq_item item;
  forever begin
...
task rv_dm_scoreboard::process_tl_sba_d_chan_fifo();
  tl_seq_item item;
  forever begin
...
task rv_dm_scoreboard::process_tl_sba_access(tl_seq_item item, tl_channels_e channel);
endtask

function void rv_dm_scoreboard::compare_sba_access(sba_access_item sba_item,
                                                   tl_seq_item sba_tl_item);
  ...
  if (sba_item.bus_op == BusOpRead) begin
    ...
    // SBA system shifts the read data based on transfer size. The higher order bits that are don't
    // care are left alone. The RISCV debug spec does not mandate that they be masked.
    ...
    // SBA system shifts the write data based on transfer size. TLUL adapter further masks the don't
    // care higher order bits for correct ECC computation.
...
function void rv_dm_scoreboard::on_regs_access_d(tl_seq_item item);
  ...
  // and d_data = '1. After checking that this has happened, return: the access should not have had
  // any other effect.
  if (csr == null) begin
...
function void rv_dm_scoreboard::on_mem_access_d(tl_seq_item item);
