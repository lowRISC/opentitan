...
(
  ...
  `OCAH_OT_ASSERT(NdmResetAckNeedsDebug_A, $rose(ndmreset_ack) -> debug_enabled)

  // A debug request can only be passed from the debug module to the hart if debug is enabled
  `OCAH_OT_ASSERT(DebugRequestNeedsDebug_A, $rose(debug_req_o_i) -> debug_enabled)
  ...
  `OCAH_OT_ASSERT(MemTLResponseWithoutDebugIsError_A,
  ...
  `OCAH_OT_ASSERT(SbaTLRequestNeedsDebug_A, $rose(sba_tl_h_o_i.a_valid) -> debug_enabled)
endmodule
