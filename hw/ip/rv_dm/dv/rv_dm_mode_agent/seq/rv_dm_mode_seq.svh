...
class rv_dm_mode_seq extends dv_base_seq #(.REQ (rv_dm_mode_seq_item),
  ...
  extern local function bit [31:0] gen_something_for_bool(int unsigned width,
                                                          bit          strict_bool,
  ...
  // bool_val = strict_val.
  extern local function bit [3:0] gen_lc_tx_t_from_bool(bit strict_val, bit bool_val);
  extern local function bit [3:0] gen_mubi4_t_from_bool(bit strict_val, bit bool_val);
  extern local function bit [7:0] gen_mubi8_t_from_bool(bit strict_val, bit bool_val);

  // Generate a random lc_tx_t value that is neither On nor Off
  extern local function bit [3:0] gen_bad_lc_tx_t();
