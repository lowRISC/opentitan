...
class keymgr_dpe_base_vseq extends cip_base_vseq #(
  ...
  virtual task keymgr_dpe_init();
    ...
    `uvm_info(`gfn, $sformatf({"Top dependent dv parameter > ",
        "# Bootstages: %0d # HW slots: %0d # ROM Digest values: %0d",
        " Size Adv Data: %0d"},
        keymgr_dpe_env_pkg::DvBootStages,
        keymgr_dpe_env_pkg::DvNumInstHwSlot,
        keymgr_dpe_env_pkg::DvNumRomDigestInputs,
        keymgr_dpe_env_pkg::DvDpeAdvDataWidth), UVM_LOW)
  ...
  virtual task keymgr_dpe_erase(
    ...
    `uvm_info(`gfn,
              $sformatf("Start keymgr_dpe_erase"),
              UVM_MEDIUM)
  ...
  // the keymgr_dpe only if its in the available state, otherwise the DUT
  ...
  virtual task keymgr_dpe_disable(
    ...
    `uvm_info(`gfn,
              $sformatf("Start keymgr_dpe_disable"),
              UVM_MEDIUM)
    ...
    if (current_state == keymgr_dpe_pkg::StWorkDpeReset) begin
      if (num_adv_op != 0) begin
        `uvm_error(`gfn,
                   $sformatf({"Invoking the disable operation in the Reset ",
                              "State with the number of request advance call ",
                              "not set to 0 can lead to unexpected behavior."}))
      end
    end else begin
      repeat (num_adv_op) begin
        keymgr_dpe_advance(wait_done);
  ...
  virtual task keymgr_dpe_operations(bit advance_state = $urandom_range(0, 1),
                                     ...
                                     bit wait_done     = 1);
    `uvm_info(`gfn,
              $sformatf({"Start keymgr_dpe_operations num_gen_op %0d",
                        " advance_state %0d"}, num_gen_op, advance_state),
              UVM_MEDIUM)
  ...
  virtual task keymgr_dpe_generate(
    ...
    sema_update_control_csr.get();
    `uvm_info(`gfn,
              $sformatf({"Generate key manager output w/operation %s and",
                        " dest %s"}, operation.name, key_dest.name),
              UVM_MEDIUM)
