...
module rv_plic_assert_fpv #(
  ...
  `ASSERT(LevelTriggeredIp_A,
          $rose(ip[src_sel]) |-> $past(le[src_sel]) || $past(intr_src_i[src_sel]))

  `ASSERT(EdgeTriggeredIp_A,
          $rose(ip[src_sel]) |-> !$past(le[src_sel]) || $rose($past(intr_src_i[src_sel])))

  // when interrupt is trigger, and nothing claimed yet, then next cycle should assert IP.
  `ASSERT(LevelTriggeredIpWithClaim_A,
          !le[src_sel] && intr_src_i[src_sel] && !claimed |=> ip[src_sel])

  `ASSERT(EdgeTriggeredIpWithClaim_A,
          le[src_sel] && $rose(intr_src_i[src_sel]) && !claimed |=> ip[src_sel])
  ...
  `ASSERT(TriggerIrqForwardCheck_A,
          ip[src_sel] && prio[src_sel] > threshold[tgt_sel] && max_priority && ie[tgt_sel][src_sel]
              |=> irq_o[tgt_sel])

  `ASSERT(TriggerIrqBackwardCheck_A,
          $rose(irq_o[tgt_sel]) |-> $past(irq) && (irq_id_o[tgt_sel]) == $past(i_high_prio))
  ...
  `ASSERT(IdChangeWithIrq_A,
          !$stable(irq_id_o[tgt_sel]) && irq_id_o[tgt_sel] != 0 |->
