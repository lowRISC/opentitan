...
module cheriot_access_check #(
  ...
  // A full, word-granular TL-UL access to this device.
  localparam int unsigned                FullWordSizeInt = $clog2(top_pkg::TL_DBW);
  localparam logic [top_pkg::TL_SZW-1:0] FullWordSize    = top_pkg::TL_SZW'(FullWordSizeInt);
  ...
  always_comb begin: proc_allow_forward
    ...
    if(prim_mubi_pkg::mubi4_test_true_strict(cheriot_ena_i)) begin
      ...
      // Check whether we have a valid operation: only full accesses are permitted.
      allowed_operation = tl_h_i.a_opcode inside {PutFullData, Get};

      // Forward if address range is valid, the address is word-aligned, the operation is
      // allowed, and we can use the device in the currently active mode.
      allow_forward = tl_h_i.a_address      >= CheriotBaseAddr &&
                      tl_h_i.a_address      <  CheriotTopAddr  &&
                      tl_h_i.a_address[1:0] == 2'b00           &&
                      tl_h_i.a_size         == FullWordSize    &&
                      allowed_operation;
    end
  end
