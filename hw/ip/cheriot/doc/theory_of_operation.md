...
For Earl Grey - 128 KiB main SRAM at `0x1000_0000`, 2 MiB NVM at `0x2000_0000`, meta SRAM at
`0x1100_0000` - it is:

| Offset  | Size   | Region                             | Reachable from |
|---------|--------|------------------------------------|----------------|
| `0x0000` | 2 KiB  | Main SRAM revocation bitmap        | `revbm_tl_d`, `corerevbm_tl` |
| `0x0800` | 32 KiB | NVM capability tags                | RMW filter |
| `0x8800` | 2 KiB  | Main SRAM capability tags          | RMW filter |
| `0x9000` | -      | top                                | - |
...
A meta lookup is needed when CHERIoT mode is enabled, we access memory capable of holding
capabilities and either:
...
The FIFO depth sets the number of outstanding transactions the subsystem supports; it is 2 in the
Earl Grey instantiation. Error responses from either the host or the meta path are merged into
`d_error` towards the core.
...
| Meta SRAM controller error | Forwarded as `d_error` towards the core |
...
The first four are reachable by software and surface as a bus fault in the core, so they must not
raise an alert. The last three indicate a fault-injection attempt and latch the fatal alert until
reset. There is no interrupt.
