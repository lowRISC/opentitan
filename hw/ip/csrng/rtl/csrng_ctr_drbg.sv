...
module csrng_ctr_drbg import csrng_pkg::*; (
  ...
  for (genvar i = 0; i < NumApps; i++) begin : g_assign_gen_adata
    ...
    always_comb begin
      ...
      end else if (capt_adata && !generate_adata_vld_q[i]) begin
        generate_adata_vld_d[i] = 1'b1;
        generate_adata_d[i]     = core_data.pdata;
      end else if (rsp_vld_o && req_glast_i && (core_data.inst_id == i)) begin
        generate_adata_vld_d[i] = 1'b0;
      end
    end
  end
  ...
  if (CtrLen < BlkLen) begin : g_ctr_sized_lsb
    ...
    // Need to distinguish this case as the slice select into core_data.v above would otherwise
    // yield an incorrect range ([BlkLen-1:BlkLen])
