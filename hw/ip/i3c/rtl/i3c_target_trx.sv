...
#(
  ...
  // transmission because PRE1 has the opposite sense for the first word, in order to support an
  ...
  if (UseTokens) begin : gen_use_tokens
    ...
    // from SCL to SDA. Start requests - which occur only during the Bus Idle condition, so the
    // transceiver is inactive - must lower SDA to prompt the Active Controller.
  ...
  for (genvar t = 0; t < NumTargets; t++) begin : gen_dready
    ...
    // because this easier than trying to retract the data later. Also, the target count is low.
  end
  assign trx_ctready_o = tx_starting & ccc_command;
  // Indication to the core how much data has been accepted, since this is mode-dependent; the core
  // may then update the presented read data accordingly.
