...
#(
  ...
  // TODO(#31304): Auto-Command behavior is presently not implemented.
  logic [7:0] mdb;
  ...
  always_comb begin
    ...
    case (state_q)
      ...
      // Controller Role Requests. TODO(#31308): No support for CRR presently.
      ...
      // TODO(#31304): Implement this as more than a single issue; in more general use it shall be
      // issued following a RSTACT CCC Command Descriptor with TOC set.
  ...
  // - ensure that we don't lose Command Descriptor words when the Controller logic is disabled.
  assign cmd_desc_rready_o = &{enable_i, cmd_enabled, cmd_complete | ~cmdq_lo_valid};
  ...
  // TODO(#31304): Check the semantics of IBI NACK. Normally the Controller is required to send
  // `DISEC` afterwards which means that retrying will not occur, so perhaps it's a bit
  // heavy-handed.
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
  ...
  // TODO(#31305): We should also report data loss somehow.
  // wire ibi_stat_loss = ;
  // wire ibi_data_loss = ;
  ...
  // TODO(#31308) Controller Role Requests are always rejected, pending Handoff implementation.
  assign arb_nack = (ibi_req ? ibi_reject : reg2hw_i.hc_control.hot_join_ctrl.q) | cr_role_req;
