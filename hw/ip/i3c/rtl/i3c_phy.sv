...
module i3c_phy #(
  ...
  `ifdef VERILATOR
  // Serial CLock output.
  assign scl = scl_pp_en_i ? scl_i : (scl_pu_en_i ? 1'b1 : 1'bZ);
  // Serial DA output.
  for (genvar d = 0; d < NumSDALanes; d++) begin
    assign sda[d] = sda_pp_en_i ? sda_i[d] :
  ...
  // Open drain drivers.
  for (genvar d = 0; d < NumSDALanes; d++) begin
    assign (strong0, weak1) sda[d] = (sda_od_en_i & !sda_i[d]) ? 1'b0 : 1'bZ;
  end
