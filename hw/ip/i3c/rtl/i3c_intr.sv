...
module i3c_intr #(
  ...
  // Whether each interrupt is an `Event` type (pulse), rather than always reflecting the current
  // `Status`.
  parameter logic [Width-1:0] Event = 0
) (
  ...
  // `Event` type interrupts only ever set the Interrupt Status bit, and it will be cleared by
  // software at a later time.
  //
  // `Status` type interrupts need both to set and to clear the Interrupt status bit in response
  // to the hardware state change, e.g. data level in FIFO.
  ...
  assign clr_status = (status_i & ~event_i) & ~Event;

  // Drive out OpenTitan-style 'data enable' and 'data' values.
  // Note: `set_status` overrides `clear_status`; this is essential for forcing to work on
  //       Status-type (level-sensitive) interrupts, and also ensures that newly-arriving Event-type
  //       interrupts are still seen when they collide with clear requests from software.
  assign status_de_o = set_status | clr_status;
  assign status_d_o  = set_status;
