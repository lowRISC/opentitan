...
#(
  ...
  // TODO(#31307): Implement this debug/safety functionality.
  ...
  // TODO(#31305): Error and Abort handling incomplete.
  ...
  wire sdr_read_ending = &{sdr_read, last_bit, phase_end, state_q == State_ClkLoSampleD,
                           sdr_tbit_drive_q};
  // TODO(#31304): This is presently the only case of early termination that we handle, but there
  // will be others such as HDR-DDR since a request can accommodate two Data Words.
  ...
  wire starting   = &{advance, state_q == State_Idle, trx_dvalid_i};
  // We must lower SDA for the second part of HDR Restart/SDR Repeated Start signaling.
  wire restarting = &{advance, state_q inside {State_RepStart, State_RepStartHiD}};
  // We must lower SDA before we can raise it for STOP signaling.
  wire pre_stop   = &{advance, state_q == State_PreStop};
  // STOP signaling; SDA raised but SCL still driven low.
  wire stop       = &{advance, state_q == State_Stop};
  ...
  // TODO(#31305): We shall perhaps need to make this sticky.
  logic raise_error;
    ...
    if (trx_dvalid_i) begin
      case (trx_dreq_i.req)
        ...
        CReqType_DirectDrive:
          ...
          // TODO(#31307): ... but we have no open drain driver on SCL.
  ...
  always_comb begin
    ...
    case (state_q)
      ...
      // TODO(#31304): The Controller must sit in this state for quite a while if the bus was idle,
  ...
  always_comb begin
    ...
    if (unit_starting) begin
      ...
      // TODO(#31304): References to `state_next_req` should be disappearing from here.
      case (state_q)
        ...
        State_ArbClkLoIdle: begin
          ...
          //  // TODO(#31304): For ENTDAA we must continue reading..
        ...
        State_AddrClkLoHoldD: begin
          if (last_bit) begin
            // TODO(#31304): Temporary; check timing for ACK bit of non-Arb address, including the
  ...
  // target.
  logic sda_driven;
  // TODO(#31305): The action taken on a mismatch, if any, shall depend upon the state.
  ...
  // TODO(#31305): The timing of these error signals must be qualified.
  assign raise_error = |{parity_error, crc5_error};  // TODO(#31305): sda_mismatch, at times.
  ...
  always_comb begin
    trx_rsp_o = '0;
    // TODO(#31305): report SDA mismatch; `i3c_err_status_e` is too limited to carry everything that
    // we shall need to report to the Controller core.
  ...
  always_comb begin
    // TODO(#31307): The transmit/receive states require further qualifications, in particular to
    // report ENTDAA, SETDASA and IBI.
    case (state_q)
      State_ClkLoSetupD,
      State_ClkHiHoldD,
      State_ClkHiSetupD,
