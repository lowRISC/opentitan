...
module i3c_controller_ibi
  import i3c_pkg::*;
#(
  parameter int unsigned Log2DBW = 2  // log2(Data width in bytes)
) (
  input                   clk_i,
  input                   rst_ni,
  ...
  // Configuration.
  input             [7:0] ibi_status_size_i,  // Total of IBI Status Descriptors and Data Queue.
  input             [7:0] data_seg_size_i,    // Maximum size of IBI Data Segments (1-63 DWORDs).

  // Indication of whether the current write terminates the IBI Data Segment.
  output                  last_o,
  ...
  input       [Log2DBW:0] wlen_i,        // Number of bytes of IBI data being written.
  ...
  // with the count of payload bytes written so far.
  ...
  localparam int unsigned SegCntB = SegCntW + Log2DBW;
  ...
  // - HCI 6.9.1 actually requires software to prevent deadlock, but we try to be helpful.
  logic [SegCntW-1:0] seg_dwords;
  assign seg_dwords = (ibi_status_size_clamped < data_seg_size_i) ? ibi_status_size_clamped
                                                                  : data_seg_size_i[SegCntW-1:0];
  // Count of IBI payload bytes written.
  ...
  // - only the final write within the collection of the entire IBI payload may be a partial DWORD;
  //   the transceiver returns complete DWORDs whenever possible.
  logic [SegCntB-1:0] bytes_written;
  logic [SegCntB-1:0] bytes_next;
  // Since the segment cannot exceed 252 bytes, this addition cannot overflow.
  assign bytes_next = bytes_written + (write_i ? wlen_i : Log2DBW'('b0));
  ...
  always_comb begin
    ...
    stat_desc_o.data_length = bytes_next;  // Including bytes from the current write.
  end

  // Indicate whether the current write terminates the current segment; this instructs the
  // controller core to commence another IBI Status Descriptor.
  // - if misconfigured with an illegal zero `IBI_DATA_SEGMENT_SIZE`, this emits single DWORDs.
  assign last_o = (bytes_next[SegCntB-1:Log2DBW] >= seg_dwords);

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) bytes_written <= '0;
    else if (sw_reset_i) bytes_written <= '0;
    else if (enable_i & (init_i | write_i)) begin
      // Reset the byte count when starting an IBI transfer or completing a segment.
      bytes_written <= (init_i | last_o) ? 'b0 : bytes_next;
    end
  end

  // The `last_o` termination logic requires the caller never to present more than a single DWORD.
  `ASSERT(WLenValidA, !enable_i || !write_i || (wlen_i <= (1 << Log2DBW)))
