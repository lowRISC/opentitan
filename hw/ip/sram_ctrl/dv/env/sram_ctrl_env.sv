...
class sram_ctrl_env #(parameter int AddrWidth = 10) extends cip_base_env #(
  ...
  function void build_phase(uvm_phase phase);
    ...
    // Get the sram_ctrl_util interface
    if (!uvm_config_db#(sram_ctrl_util)::get(this, "", "sram_ctrl_util",
                                             cfg.sram_ctrl_util_h)) begin
      `uvm_fatal(`gfn, "failed to get sram_ctrl_util from uvm_config_db")
    end
