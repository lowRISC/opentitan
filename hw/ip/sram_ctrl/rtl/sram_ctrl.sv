...
#(
  ...
  // Interrupt Requests
  output logic                            sram_parity_error_o,
  ...
  // Clear when writing 1 to error bit.
  assign parity_error_d = (reg2hw.status.error.q && reg2hw.status.error.qe) ?
                          sram_scr_parity_err :
                          sram_scr_parity_err | parity_error_q;

  logic unused_bits;
  assign unused_bits = ^{reg2hw.status.scr_key_valid,        // this bit cannot be cleared via SW
                         reg2hw.status.scr_key_seed_valid};  // this bit cannot be cleared via SW
  // Control register
  logic key_req;
  ...
  assign hw2reg.error_address.de       = sram_scr_parity_err;
  ...
  prim_intr_hw #(
    ...
    .clk_i,
    .rst_ni,
    .event_intr_i           ( sram_scr_parity_err  ),
  ...
  ///////////////////////////
  // Srambling Key Request //
  ///////////////////////////

  // The srambling key and nonce have to be requested from the OTP controller via a req/ack
  ...
  prim_sync_reqack u_prim_sync_reqack (
    .clk_src_i  ( clk_i              ),
    .rst_src_ni ( rst_ni             ),
    .clk_dst_i  ( clk_fixed_i        ),
    .rst_dst_ni ( rst_fixed_ni       ),
  ...
  prim_ram_1p_scr #(
    ...
    .wmask_i  ( tlul_wmask     ),
    .rdata_o  ( tlul_rdata     ),
    .rvalid_o ( tlul_rvalid    ),
    .rerror_o ( tlul_rerror    ),
    .raddr_o  ( sram_scr_raddr ),
    .cfg_i    ( sram_scr_cfg   )
  );

  assign sram_scr_parity_err = tlul_rerror[1];
  ...
  `ASSERT_KNOWN(IntrSramParityErrorKnown_A,  sram_parity_error_o)
