...
// Randomly set dut.ram_cfg_sys2spi_i / dut.ram_cfg_spi2sys_i and check this value is
// propagated to the backing prim memory (cfg_i).
class spi_device_ram_cfg_vseq extends spi_device_base_vseq;
  `uvm_object_utils(spi_device_ram_cfg_vseq)
  `uvm_object_new

  virtual task body();
    prim_ram_1r1w_pkg::ram_1r1w_cfg_req_t src_ram_cfg, dst_ram_cfg, egress_ram_cfg, ingress_ram_cfg;
    string src_path     = `SRAM_TYPE == spi_device_pkg::SramType2p ?
                          "tb.dut.ram_cfg_sys2spi_i" : "tb.dut.ram_cfg_spi2sys_i";
