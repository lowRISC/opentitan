...
class spi_device_base_vseq extends cip_base_vseq#(
  ...
  virtual task spi_host_xfer_byte(bit [7:0] host_data, ref bit [7:0] device_data);
    ...
    `DV_CHECK_RANDOMIZE_WITH_FATAL(m_spi_host_seq, data.size() == 1;
                                   data[0]     == host_data;)
  ...
  virtual task spi_host_xfer_word(bit [31:0] host_data, ref bit [31:0] device_data);
    ...
    `uvm_create_on(m_spi_host_seq, p_sequencer.spi_sequencer_h)
    `DV_CHECK_RANDOMIZE_WITH_FATAL(m_spi_host_seq, data.size() == SRAM_WORD_SIZE;
  ...
  virtual task write_device_words_to_send(bit [31:0] device_data[$]);
    ...
    foreach (device_data[i]) begin
      ...
      `uvm_info(`gfn, $sformatf({
                ...
                }, tx_wptr[SRAM_MSB:0], tx_wptr[SRAM_PTR_PHASE_BIT], tx_sram_size_bytes,
                    tx_wptr_addr),
                UVM_MEDIUM)
  ...
  virtual task read_host_words_rcvd(uint num_words, ref bit [31:0] host_data[$]);
    ...
    repeat (num_words) begin
      ...
      `uvm_info(`gfn, $sformatf({
                ...
                }, rx_rptr[SRAM_MSB:0], rx_rptr[SRAM_PTR_PHASE_BIT], rx_sram_size_bytes,
                    rx_rptr_addr),
                UVM_MEDIUM)
  ...
  virtual task read_tx_avail_bytes(sram_avail_type_e avail_type, ref uint avail_bytes);
    ...
    endcase
    `uvm_info(`gfn,
              $sformatf("TX avail_type = %0s, avail_bytes = %0d", avail_type.name, avail_bytes),
              UVM_MEDIUM)
  endtask
  ...
  virtual task read_rx_avail_bytes(sram_avail_type_e avail_type, ref uint avail_bytes);
    ...
    endcase
    `uvm_info(`gfn,
              $sformatf("RX avail_type = %0s, avail_bytes = %0d", avail_type.name, avail_bytes),
              UVM_MEDIUM)
  endtask
  ...
  virtual task wait_for_tx_avail_bytes(uint req_bytes, sram_avail_type_e avail_type,
    ...
    `DV_SPINWAIT(do begin
      ...
      end while (cur_bytes < req_bytes);, {"wait_for_tx_avail_bytes::", avail_type.name},
                 default_timeout_ns * 2)
    avail_bytes = cur_bytes;
    `uvm_info(`gfn,
              $sformatf("TX req_bytes = %0d, avail_type = %0s, avail_bytes = %0d", req_bytes,
                        avail_type.name, avail_bytes),
              UVM_MEDIUM)
  endtask
  ...
  virtual task wait_for_rx_avail_bytes(uint req_bytes, sram_avail_type_e avail_type,
    ...
    `DV_SPINWAIT(do begin
      ...
      end while (cur_bytes < req_bytes);, {"wait_for_rx_avail_bytes::", avail_type.name},
                 default_timeout_ns * 2)
    avail_bytes = cur_bytes;
    `uvm_info(`gfn,
              $sformatf("RX req_bytes = %0d, avail_type = %0s, avail_bytes = %0d", req_bytes,
                        avail_type.name, avail_bytes),
              UVM_MEDIUM)
  endtask
  ...
  virtual task get_num_xfer_bytes(bit return_out_of_range, ref uint avail_bytes);
    ...
    if (return_out_of_range) begin
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(final_data_bytes, final_data_bytes[1:0] == 0;
