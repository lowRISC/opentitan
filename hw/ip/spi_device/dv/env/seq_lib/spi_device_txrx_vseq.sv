...
class spi_device_txrx_vseq extends spi_device_base_vseq;
  ...
  virtual task process_tx_write(uint xfer_bytes);
    ...
    while (xfer_bytes > 0) begin
      ...
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(tx_write_bytes, tx_write_bytes <= sram_avail_bytes;
      ...
      `uvm_info(`gfn, $sformatf("tx_write_bytes = %0d, sram_avail_bytes = %0d,\
                                xfer_bytes = %0d", tx_write_bytes, sram_avail_bytes, xfer_bytes),
                UVM_MEDIUM)
  ...
  virtual task process_rx_read(uint xfer_bytes);
    ...
    while (xfer_bytes > 0) begin
      ...
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(rx_read_bytes, rx_read_bytes <= sram_avail_bytes;
                                         ...
                                         rx_read_bytes dist {
                                             ...
                                             SRAM_SIZE             :/ 1};)
      `uvm_info(`gfn,
                $sformatf("rx_read_bytes = %0d, sram_avail_bytes = %0d, xfer_bytes =%0d",
                          rx_read_bytes, sram_avail_bytes, xfer_bytes),
                UVM_MEDIUM)
  ...
  virtual task process_spi_xfer();
    ...
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(spi_bytes, spi_bytes <= sram_avail_bytes;
                                       spi_bytes[1:0] == 0;
                                       spi_bytes dist {
                                           [1:SRAM_WORD_SIZE]    :/ 1,
                                           [SRAM_WORD_SIZE+1:20] :/ 3,
