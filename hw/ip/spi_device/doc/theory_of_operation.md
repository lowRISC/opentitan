...
The Status logic processes the three Read Status commands (Read Status 1-3).
The returned data byte for the command is read from the SW-configurable [`FLASH_STATUS`](registers.md#flash_status) register.

The JEDEC submodule returns the JEDEC Manufacturer ID and Device ID.
This is controlled by the SW-configurable [`JEDEC_CC`](registers.md#jedec_cc) and [`JEDEC_ID`](registers.md#jedec_id) registers.
...
Typically, a Read Status command follows any command that would set the `BUSY` bit to check that the bit has cleared - this is sufficient to unblock passthrough for the next command.

This module only processes Read Status commands.
To handle Write Status commands, SW must configure a command information entry with the Write Status command opcode and set the [`CMD_INFO.upload`](registers.md#cmd_info) bit, then handle the uploaded commands in SW.
...
`LAST_READ_ADDR` does not show a read falling into the mailbox region or Read SFDP's command address space - if a read enters either region, `LAST_READ_ADDR` does not update until the read exits the region.
...
This is designed so that SW implements a double-buffering scheme - when the host system accesses one half of the read buffer (1kB), the SW should prepare the other 1kB by copying data from the internal non-volatile memory.
If the received read address crosses the SW-configured watermark address or crosses a read buffer boundary, the HW notifies the SW by way of the [`readbuf_watermark`](registers.md#intr_state) and [`readbuf_flip`](registers.md#intr_state) interrupts respectively.
SW should configure the watermark CSR low enough so that the SW has enough time to copy over the data.
SW should also clear the corresponding interrupt bits so that HW can notify it of future events.
...
The broadcasted *cfg_addr_4b_en* signal affects the read commands for which `CMD_INFO.addr_mode == AddrCfg` in their command information entries.
...
If the size of a command's address field is not 4 bytes (`CMD_INFO.addr_mode != Addr4B`, or `CMD_INFO.addr_mode == AddrCfg` and `ADDR_MODE.addr_4b_en` is unset), bits `[31:24]` are unused.

For example, if `ADDR_SWAP_MASK` is set to `0x000f0000` and `PAYLOAD_SWAP_DATA` is set to `0x000f0000`, bits 0 and 2 of the third address byte are set to 1, bits 1 and 3 of the third address byte are set to 0, and the rest of the address is unchanged.
