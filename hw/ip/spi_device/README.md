...
- Support Serial Flash emulation
  ...
  - 16-depth Command/Address FIFOs and 256B Payload buffer for command uploading for SW
  - 2x 1kB read buffer for read commands
  - 1kB mailbox buffer and configurable mailbox target address
- Support SPI Passthrough to downstream Flash
  - Filtering of inadmissible commands (256-bit filter CSR)
  - SW-configurable translation of address bytes and first 4B of payload
  - HW control of SPI PADs' output enable based on command information list
  - SW-configurable intercepting for Read Status, Read JEDEC ID, Read SFDP, and read access to the mailbox space
...
SPI Device HWIP has two flash-like modes, as well as a TPM mode.
