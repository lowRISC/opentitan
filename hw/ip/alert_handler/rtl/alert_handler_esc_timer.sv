...
(
  ...
  `ASSERT(CheckAccumTrig1,
          accum_trig_i && state_q == Timeout && en_i && !clr_i |=> state_q == Phase0)
  // Check if timeout correctly captured
  `ASSERT(CheckTimeout0,
          state_q == Idle && timeout_en_i && en_i && timeout_cyc_i != 0 && !accum_trig_i |=>
              state_q == Timeout)
  `ASSERT(CheckTimeout1,
          state_q == Timeout && timeout_en_i && cnt_q < timeout_cyc_i && !accum_trig_i |=>
              state_q == Timeout)
  ...
  `ASSERT(CheckTimeoutTrig,
          state_q == Timeout && timeout_en_i && cnt_q == timeout_cyc_i |=> state_q == Phase0)
  ...
  `ASSERT(CheckPhase3,
          state_q == Phase3 && !clr_i && cnt_q >= phase_cyc_i[3] |=> state_q == Terminal)
