...
class alert_handler_scoreboard extends cip_base_scoreboard#(
    .CFG_T(alert_handler_env_cfg), .RAL_T(alert_handler_reg_block), .COV_T(alert_handler_env_cov)
);
          ...
          if (alert_en[index]) begin
            ...
            if (act_item.alert_esc_type == AlertEscSigTrans && !act_item.timeout &&
              ...
              if (loc_alert_en[LocalAlertPingFail]) begin
                ...
                `uvm_info(`gfn,
                          $sformatf("alert %0d ping timeout, timeout_cyc reg is %0d", index,
                                    ral.ping_timeout_cyc.get_mirrored_value()),
                          UVM_LOW);
              end
            end
          end
        ...
        forever begin
          ...
          if (act_item.alert_esc_type == AlertEscSigTrans &&
            ...
            if (loc_alert_en[LocalEscPingFail]) begin
              ...
              `uvm_info(`gfn,
                        $sformatf("esc %0d ping timeout, timeout_cyc reg is %0d", index,
                                  ral.ping_timeout_cyc.get_mirrored_value()),
                        UVM_LOW);
            end
          end
    ...
    fork
      begin
        ...
        if (!under_reset) begin
          ...
          `uvm_info(`gfn,
                    $sformatf("class %0d is triggered, class ctrl=%0h, under_esc=%0b", class_i,
                              class_ctrl, under_esc_classes[class_i]),
                    UVM_DEBUG)
  ...
  virtual function void alert_accum_cal(int class_i);
    ...
    `uvm_info(`gfn,
              $sformatf("alert_accum: class=%0d, alert_cnt=%0d, thresh=%0d, under_esc=%0b", class_i,
                        accum_cnter_per_class[class_i], accum_thresh, under_esc_classes[class_i]),
              UVM_DEBUG)
  endfunction
              ...
              fork
                begin : inc_esc_cnt
                  for (int phase_i = 0; phase_i < NUM_ESC_PHASES; phase_i++) begin
                    int phase_thresh =
                        reg_esc_phase_cycs_per_class_q[class_i][phase_i].get_mirrored_value();
                    bit [TL_DW-1:0] class_ctrl = get_class_ctrl(class_i);
                    int enabled_sig_q[$];
  ...
  virtual function void incr_esc_sig_cnt(const ref int sig_q[$], int class_i);
    foreach (sig_q[i]) begin
      ...
      if (esc_sig_class[index] == (class_i + 1)) begin
        ...
        `uvm_info(`gfn,
                  $sformatf("class_%0d signal_%0d, esc_cnt=%0d", class_i, index,
