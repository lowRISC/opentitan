...
Software can find out the length in use by reading [`CFG.key_length`](registers.md#cfg--key_length) back after configuring HMAC.
...
- Saving and restoring the context is not available: `CMD.hash_stop` and `CMD.hash_continue` are rejected, and [`ERR_CODE`](registers.md#err_code) reports `SwInvalidConfig`.
...
1. Clear [`INTR_STATE.hmac_err`](registers.md#intr_state).
   As long as it is pending, no further error is reported in [`ERR_CODE`](registers.md#err_code).
1. Wait until the key manager provides a valid key again.
   Starting while it is still invalid is blocked with the same error.
1. Restart the operation with `CMD.hash_start` and write the whole message again.
   The part of the message that had been written is discarded together with the intermediate hash, and the context cannot be restored with a sideloaded key.

The configuration in [`CFG`](registers.md#cfg) is retained across an abort and does not need to be written again.
The digest registers hold no valid result after an abort and must not be used.
