...
class hmac_scoreboard extends cip_base_scoreboard #(.CFG_T (hmac_env_cfg),
  ...
  // Reset some variables in case of a reset is happening
  extern virtual function void reset(string kind = "HARD");
  // Clear some variables after hmac_done/invalid_config
  extern virtual function void flush();
  // Model of the message FIFO when is written
  extern virtual task hmac_process_fifo_wr();
  // Model of the message FIFO when is read
  extern virtual task hmac_process_fifo_rd();
  // Model the status of the message FIFO
  extern virtual task hmac_process_fifo_status();
  // Spawn process to check interrupt pins in test mode
  extern virtual task hmac_intr_test();
  // Check interrupt pins in test mode
  extern virtual task hmac_intr_test_pin(int intr_i);
  // Query SHA/HMAC to the C model to get expected digest
  extern virtual function void predict_digest(
    bit [7:0] msg_i[], bit sha_en, bit hmac_en, bit [3:0] digest_size, bit [5:0] key_length);
  ...
  extern virtual task update_err_intr_code(err_code_e err_code_val);
  // Check idle output pin and update status register predicted value
  extern virtual function void check_idle(bit val);
  // Trigger coverage sampling
  extern virtual task monitor_cov();
endclass : hmac_scoreboard


function void hmac_scoreboard::build_phase(uvm_phase phase);
  super.build_phase(phase);
endfunction : build_phase

task hmac_scoreboard::run_phase(uvm_phase phase);
...
virtual task hmac_scoreboard::process_tl_access(tl_seq_item item, tl_channels_e channel, string ral_name);
  uvm_reg csr;
  string  csr_name;
...
virtual function void hmac_scoreboard::reset(string kind = "HARD");
  super.reset(kind);
...
virtual function void hmac_scoreboard::flush();
...
virtual task hmac_scoreboard::hmac_process_fifo_wr();
...
virtual task hmac_scoreboard::hmac_process_fifo_status();
...
// Spawn process to check interrupt pins in test mode
virtual task hmac_scoreboard::hmac_intr_test();
...
// Check interrupt pins in test mode
virtual task hmac_scoreboard::hmac_intr_test_pin(int intr_i);
...
virtual task hmac_scoreboard::hmac_process_fifo_rd();
  bit key_processed = 0;
  fork
...
function void hmac_scoreboard::check_phase(uvm_phase phase);
...
// update predicted digest to ral mirrored value
virtual function void hmac_scoreboard::predict_digest(
  bit [7:0] msg_i[],
...
virtual function void hmac_scoreboard::update_wr_msg_length(int size_bytes);
...
virtual task hmac_scoreboard::update_err_intr_code(err_code_e err_code_val);
...
// Check idle output pin and update status register predicted value
virtual function void hmac_scoreboard::check_idle(bit val);
...
virtual task hmac_scoreboard::monitor_cov();
