...
task hmac_scoreboard::process_tl_access(tl_seq_item item, tl_channels_e channel, string ral_name);
  uvm_reg csr;
  string  csr_name;
  ...
  if (csr == null &&
      !((item.a_addr & addr_mask) inside {[HMAC_MSG_FIFO_BASE:HMAC_MSG_FIFO_LAST_ADDR]})) begin
...
function void hmac_scoreboard::reset(string kind = "HARD");
  super.reset(kind);
...
function void hmac_scoreboard::flush();
...
task hmac_scoreboard::hmac_process_fifo_wr();
...
task hmac_scoreboard::hmac_process_fifo_rd();
  bit key_processed = 0;
  fork
...
// update predicted digest to ral mirrored value
function void hmac_scoreboard::predict_digest(
  bit [7:0] msg_i[],
  bit       use_gmv     = 1,
  bit       sha_en      = 0,
  bit       hmac_en     = 0,
  bit [3:0] digest_size = 0,
  bit [5:0] key_length  = 0);

  bit [7:0] msg_tmp[];
  bit [TL_DW-1:0] big_endian_key[NUM_KEYS];

  // Use mirrored values, otherwise take values from the arguments
  if (use_gmv) begin
    sha_en      = `gmv(ral.cfg.sha_en);
    hmac_en     = `gmv(ral.cfg.hmac_en);
    digest_size = `gmv(ral.cfg.digest_size);
    key_length  = `gmv(ral.cfg.key_length);
  end
...
task hmac_scoreboard::update_err_intr_code(err_code_e err_code_val);
...
// Check idle output pin and update status register predicted value
function void hmac_scoreboard::check_idle(bit val);
...
task hmac_scoreboard::monitor_cov();
