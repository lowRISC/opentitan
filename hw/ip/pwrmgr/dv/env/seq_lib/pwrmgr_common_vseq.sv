  ...
  virtual task check_sec_cm_fi_resp(sec_cm_base_if_proxy if_proxy);
    ...
    case (if_proxy.sec_cm_type)
      SecCmPrimSparseFsmFlop: begin
        `uvm_info(`gfn, $sformatf("chk_slow_st %0d  chk_fast_st %0d",
                                  cfg.pwrmgr_vif.unknown_slow_st,
                                  cfg.pwrmgr_vif.unknown_fast_st), UVM_HIGH)
        ...
        if (cfg.pwrmgr_vif.unknown_slow_st == 1) begin
          `uvm_info(`gfn, "detect unknown slow state", UVM_MEDIUM)
          fork begin
            fork
              begin
                wait(cfg.pwrmgr_vif.fast_state == pwrmgr_pkg::FastPwrStateInvalid &&
                     cfg.pwrmgr_vif.pwr_ast_req.pwr_clamp == 1 &&
                     cfg.pwrmgr_vif.pwr_ast_req.main_pd_n == 0);
              end
              begin
                #5us;
                `uvm_error(`gfn, $sformatf(slow_st_to,
        ...
        if (cfg.pwrmgr_vif.unknown_fast_st == 1) begin
          `uvm_info(`gfn, "detect unknown fast state", UVM_MEDIUM)
          fork begin
            fork
              begin
                wait(cfg.pwrmgr_vif.pwr_rst_req.rst_lc_req == 2'b11 &&
        ...
        fork begin
          fork
            ...
            begin
              #5us;
              `uvm_error(`gfn,
                         $sformatf("rx clk loc esc chk timeout : fast_state %s",
                                   cfg.pwrmgr_vif.fast_state.name))
            end
          join_any
          disable fork;
