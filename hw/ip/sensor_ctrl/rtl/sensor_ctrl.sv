...
module sensor_ctrl import sensor_ctrl_pkg::*; (
  ...
  tlul_pkg::tl_h2d_t tl_host [1];
  tlul_pkg::tl_d2h_t tl_dev [1];

  assign ast_host_o = tl_host[0];
  assign tl_dev[0] = ast_dev_i;
  ...
  sensor_ctrl_reg_top u_reg (
    ...
    .tl_i,
    .tl_o,
    .tl_win_o(tl_host), // directly output to ast
    .tl_win_i(tl_dev), // directly input from ast
    .reg2hw,
    .hw2reg,
    .devmode_i(1'b1)
  );
  ...
  // When in immedaite ack mode, ack alerts as they come
