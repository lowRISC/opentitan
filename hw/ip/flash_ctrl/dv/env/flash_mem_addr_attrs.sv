...
class flash_mem_addr_attrs;
  ...
  function void set_attrs(bit [TL_AW-1:0] addr);
    ...
    bank_start_addr = {
      addr[TL_AW - 1 : FlashMemAddrPageMsbBit + 1], {FlashMemAddrPageMsbBit + 1{1'b0}}
    };
    page_start_addr = {
      addr[FlashMemAddrPageMsbBit : FlashMemAddrLineMsbBit + 1], {FlashMemAddrLineMsbBit + 1{1'b0}}
    };
  ...
  function string sprint();
    return $sformatf({
      {"addr_attrs: addr = 0x%0h, bank_addr = 0x%0h "},
      {"bank_start_addr = 0x%0h, page_start_addr = 0x%0h "},
      {"bank = %0d, page = %0d, line = %0d"}
    }, addr, bank_addr, bank_start_addr, page_start_addr, bank, page, line);
  endfunction
