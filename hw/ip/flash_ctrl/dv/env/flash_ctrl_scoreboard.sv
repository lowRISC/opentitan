...
class flash_ctrl_scoreboard #(
  ...
  function void process_alert(string alert_name, alert_esc_seq_item item);
    ...
    `uvm_info(`gfn, $sformatf("alert %0s detected, alert_status is %s exp:%0d contd:%0d", alert_name,
                              item.alert_handshake_sta, exp_alert[alert_name], exp_alert_contd[alert_name]), UVM_MEDIUM)
