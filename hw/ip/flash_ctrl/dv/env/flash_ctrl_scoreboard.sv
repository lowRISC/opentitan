...
`JDBG(("flash_ral_name:%0s  cnt:%0d", cfg.flash_ral_name, cfg.tlul_core_exp_cnt))
    ...
    if (ral_name == cfg.flash_ral_name) begin
	  ...
	  if (channel == DataChannel) begin
             `DV_CHECK_EQ(item.d_error, 1, $sformatf("On interface %s, TL item: %s, ecc_err:%0d in_err:%0d",
