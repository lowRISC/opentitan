...
class flash_ctrl_hw_rma_vseq extends flash_ctrl_base_vseq;
  ...
  task body();
    ...
    // Iterate
    num_trans = 1;  // Fixed Number of Iterations as Test time is VERY LONG
    for (int i=0; i<num_trans; i++) begin
      ...
      foreach (order_q[i]) begin
        unique case(order_q[i])
          0 : do_flash_op_rma(FlashCreatorPart, flash_ctrl_pkg::FlashOpErase, flash_creator_data,
                READ_CHECK_NORM, 0, 0);
          1 : do_flash_op_rma(FlashOwnerPart, flash_ctrl_pkg::FlashOpErase, flash_owner_data,
                READ_CHECK_NORM, 0, 0);
          2 : do_flash_op_rma(FlashIsolPart, flash_ctrl_pkg::FlashOpErase, flash_isol_data,
                READ_CHECK_NORM, 0, 0);
          3 : do_flash_op_rma(FlashData0Part, flash_ctrl_pkg::FlashOpErase, flash_data0_data,
                READ_CHECK_NORM, data0_part_addr, data0_part_num_words);
          4 : do_flash_op_rma(FlashData1Part, flash_ctrl_pkg::FlashOpErase, flash_data1_data,
                READ_CHECK_NORM, data1_part_addr, data1_part_num_words);
      ...
      foreach (order_q[i]) begin
        unique case(order_q[i])
          0 : do_flash_op_rma(FlashCreatorPart, flash_ctrl_pkg::FlashOpProgram, flash_creator_data,
                READ_CHECK_NORM, 0, 0);
          1 : do_flash_op_rma(FlashOwnerPart, flash_ctrl_pkg::FlashOpProgram, flash_owner_data,
                READ_CHECK_NORM, 0, 0);
          2 : do_flash_op_rma(FlashIsolPart, flash_ctrl_pkg::FlashOpProgram, flash_isol_data,
                READ_CHECK_NORM, 0, 0);
          3 : do_flash_op_rma(FlashData0Part, flash_ctrl_pkg::FlashOpProgram, flash_data0_data,
                READ_CHECK_NORM, data0_part_addr, data0_part_num_words);
          4 : do_flash_op_rma(FlashData1Part, flash_ctrl_pkg::FlashOpProgram, flash_data1_data,
                READ_CHECK_NORM, data1_part_addr, data1_part_num_words);
          default: `uvm_error(`gfn, $sformatf("No case item match, FAIL"))
        endcase
      end
      ...
      foreach (order_q[i]) begin
        unique case(order_q[i])
          0 : do_flash_op_rma(FlashCreatorPart, flash_ctrl_pkg::FlashOpRead, flash_creator_data,
                READ_CHECK_NORM, 0, 0);
          1 : do_flash_op_rma(FlashOwnerPart, flash_ctrl_pkg::FlashOpRead, flash_owner_data,
                READ_CHECK_NORM, 0, 0);
          2 : do_flash_op_rma(FlashIsolPart, flash_ctrl_pkg::FlashOpRead, flash_isol_data,
                READ_CHECK_NORM, 0, 0);
          3 : do_flash_op_rma(FlashData0Part, flash_ctrl_pkg::FlashOpRead, flash_data0_data,
                READ_CHECK_NORM, data0_part_addr, data0_part_num_words);
          4 : do_flash_op_rma(FlashData1Part, flash_ctrl_pkg::FlashOpRead, flash_data1_data,
                READ_CHECK_NORM, data1_part_addr, data1_part_num_words);
          default: `uvm_error(`gfn, $sformatf("No case item match, FAIL"))
        endcase
      end
      ...
      // Delay
      cfg.clk_rst_vif.wait_clks($urandom_range(100, 1000));
      ...
      // READ (Compare Expected Data with Data Read : EXPECT DATA MISMATCH)

      // After Reset and DUT Initialisation, Checkt he FLASH is written randomly and not what
      // was written to it before the RMA took place.
      // This May get an the Occasional Hit ~ 1 in 32^2-1, per location
      ...
      foreach (order_q[i]) begin
        unique case(order_q[i])
          0 : do_flash_op_rma(FlashCreatorPart, flash_ctrl_pkg::FlashOpRead, flash_creator_data,
                READ_CHECK_EXPLICIT, 0, 0);
          1 : do_flash_op_rma(FlashOwnerPart, flash_ctrl_pkg::FlashOpRead, flash_owner_data,
                READ_CHECK_EXPLICIT, 0, 0);
          2 : do_flash_op_rma(FlashIsolPart, flash_ctrl_pkg::FlashOpRead, flash_isol_data,
                READ_CHECK_EXPLICIT, 0, 0);
          3 : do_flash_op_rma(FlashData0Part, flash_ctrl_pkg::FlashOpRead, flash_data0_data,
                READ_CHECK_EXPLICIT, data0_part_addr, data0_part_num_words);
          4 : do_flash_op_rma(FlashData1Part, flash_ctrl_pkg::FlashOpRead, flash_data1_data,
                READ_CHECK_EXPLICIT, data1_part_addr, data1_part_num_words);
          default: `uvm_error(`gfn, $sformatf("No case item match, FAIL"))
        endcase
      end
  ...
  virtual task do_flash_ctrl_access_check();
    ...
    // Arbitrary num_words - Access will fail on the first attempt
    ...
    // Timeout Expected
    wait_flash_op_done_timeout(.timeout_ns(cfg.seq_cfg.read_timeout_ns), .result(result));
