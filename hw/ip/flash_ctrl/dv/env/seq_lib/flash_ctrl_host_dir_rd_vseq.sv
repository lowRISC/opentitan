...
// host direct back to back read test, without and with scrambling
class flash_ctrl_host_dir_rd_vseq extends flash_ctrl_base_vseq;
  ...
  task body();
    ...
    foreach (flash_op_data[k]) begin
      ...
      if (flash_op_data[k] != flash_rd_one_data) begin
        `uvm_error(`gfn, $sformatf(
                   "exp_data: 0x%0h != rd_trans: 0x%0h", flash_op_data[k], flash_rd_one_data))
      end
    end
