...
class flash_ctrl_base_vseq extends cip_base_vseq #(
  ...
  virtual task wait_flash_op_done_timeout(input time timeout_ns = 10_000_000, output bit result);

    // Result 0 - Response Returned - FAIL
    //        1 - Timeout, no response returned - PASS
    ...
    fork
      ...
      begin  // Timeout - Expected
        `uvm_info(`gfn, "Starting Timeout Check ...", UVM_LOW)
        #(timeout_ns);
        timeout = 1;
      end
    join
  ...
  endtask : wait_flash_op_done_timeout
  ...
  virtual task do_flash_op_rma(input flash_sec_part_e part, input flash_op_e op,
    ...
    // Note:
    // Special Partition 0 (used for Creator)  : Bank 0, Information Partition 0, Page 1
    // Special Partition 1 (used for Owner)    : Bank 0, Information Partition 0, Page 2
    // Special Partition 3 (used for Isolated) : Bank 0, Information Partition 0, Page 3
