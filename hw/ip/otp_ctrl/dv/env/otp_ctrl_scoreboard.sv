...
class otp_ctrl_scoreboard #(type CFG_T = otp_ctrl_env_cfg)
  ...
  virtual function void predict_digest_csrs();
    ...
    void'(ral.hw_cfg1_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[HwCfg1Idx]]), .kind(UVM_PREDICT_DIRECT)));
    void'(ral.hw_cfg1_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[HwCfg1Idx] + 1]), .kind(UVM_PREDICT_DIRECT)));
    ...
    void'(ral.secret3_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[Secret3Idx]]), .kind(UVM_PREDICT_DIRECT)));
    void'(ral.secret3_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[Secret3Idx] + 1]), .kind(UVM_PREDICT_DIRECT)));
  endfunction
  ...
  virtual function bit [TL_DW*2-1:0] get_digest_reg_val(int part_idx);
    ...
    case (part_idx)
      ...
      HwCfg1Idx: begin
        digest = {`gmv(ral.hw_cfg1_digest[1]), `gmv(ral.hw_cfg1_digest[0])};
      end
      Secret0Idx: begin
      ...
      Secret3Idx: begin
        digest = {`gmv(ral.secret3_digest[1]), `gmv(ral.secret3_digest[0])};
      end
