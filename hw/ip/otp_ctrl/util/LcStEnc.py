...
def _get_incremental_codewords(config, base_ecc, existing_words):
    ...
    # Only need to spin through data bits that have not been set yet.
    incr_cands = []
    free_bits = base_data.count('0')
    for k in range(1, 2**free_bits):
        # Get incremental dataword.
        incr_cand = _get_data_cand(base_data,
...
def _hd_histogram(existing_words):
    ...
    bars = _hist_to_bars(hist, len(existing_words))

    for bar in _hist_to_bars(hist, len(existing_words)):
        log.info(bar)

    log.info('')
    log.info('Minimum HD: {}'.format(minimum_hd))
    log.info('Maximum HD: {}'.format(maximum_hd))
    log.info('Minimum HW: {}'.format(minimum_hw))
    log.info('Maximum HW: {}'.format(maximum_hw))
...
class LcStEnc():
    ...
    def generate(self):
        '''Generates AB, CD and EF encodings.'''
        ...
        self.gen['stats'] = _hd_histogram(existing_words)
