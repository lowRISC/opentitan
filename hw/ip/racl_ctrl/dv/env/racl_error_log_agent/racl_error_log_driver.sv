...
class racl_error_log_driver extends dv_base_driver #(.ITEM_T (racl_error_log_vec_driver_item),
  ...
  //
  // If this task starts in reset, it this clears the error log input, then waits until reset is
  // cleared before driving the item. It returns immediately (not sending the errors) if reset is
  // asserted in the middle of the delay.
...
task racl_error_log_driver::drive_item(racl_error_log_vec_driver_item item);
  if (!cfg.vif.rst_ni) begin
    cfg.vif.subscriber_cb.errors <= '0;
    wait(cfg.vif.rst_ni);
  end
  ...
  fork begin : isolation_fork
    fork
      wait(!cfg.vif.rst_ni);
      repeat (item.delay) @(cfg.vif.ctrl_cb);
    join_any
    disable fork;
  end join
  if (!cfg.vif.rst_ni) return;
...
task racl_error_log_driver::reset_lines();
  ...
  cfg.vif.errors <= 'x;
  cfg.vif.subscriber_cb.errors <= 'x;

  for (int i = 0; i < 63; i++) begin
    cfg.vif.errors[i].valid <= 1'b0;
    cfg.vif.subscriber_cb.errors[i].valid <= 1'b0;
  end
endtask

task racl_error_log_driver::clear_line(int unsigned i);
  cfg.vif.subscriber_cb.errors[i] <= 'x;
  cfg.vif.subscriber_cb.errors[i].valid <= 1'b0;
endtask

task racl_error_log_driver::set_line(int unsigned i, racl_error_log_item item);
  cfg.vif.subscriber_cb.errors[i].valid           <= 1'b1;
  cfg.vif.subscriber_cb.errors[i].overflow        <= item.overflow;
  cfg.vif.subscriber_cb.errors[i].racl_role       <= item.role;
  cfg.vif.subscriber_cb.errors[i].ctn_uid         <= item.ctn_uid;
  cfg.vif.subscriber_cb.errors[i].read_access     <= item.read_not_write;
  cfg.vif.subscriber_cb.errors[i].request_address <= item.request_address;
endtask
