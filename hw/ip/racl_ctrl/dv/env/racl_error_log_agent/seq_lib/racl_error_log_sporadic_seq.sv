...
class racl_error_log_sporadic_seq
  ...
  // If this bit is set then the sequence will finish as soon as is practical (by telling the driver
  // to stop the current item and then completing, rather than starting the next item).
  local bit stopping;
  ...
  // if there is an item currently being driven (which may take a while, because of delays). The
  // task then waits until the sequence has completed.
...
task racl_error_log_sporadic_seq::body();
  while (!stopping) begin
    ...
    if (!item.randomize() with {
      ...
      foreach(subscribers_with_errors[i]) subscribers_with_errors[i] < cfg.num_subscribing_ips;
      delay dist { [1:99] :/ 1, [100:1000] :/ 10 };
    }) begin
      `uvm_error(`gfn, "Failed to randomize error log vec item");
    end
    finish_item(item);
  end
endtask
