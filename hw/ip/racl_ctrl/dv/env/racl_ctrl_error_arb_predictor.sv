...
task racl_ctrl_error_arb_predictor::watch_errors();
  forever begin
    ...
    // Wait until something appears on a fifo. Since the two fifos are on the same clock, we add a
    // tiny delay to collect something on the other fifo that turns up at the same time.
    fork begin : isolation_fork
      racl_error_log_vec_item internal_item, external_item;
      fork
        begin
