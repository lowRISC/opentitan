...
% for i, _ in enumerate(sw_rsts):
      "sw_rst_ctrl_n_${i}": blocked = `gmv(ral.sw_rst_regwen[${i}]) == 0;
% endfor
      default:
...
% for i in range(len(sw_rsts)):
      "sw_rst_ctrl_n_${i}": begin
...
% endfor
% for i in range(len(sw_rsts)):
      "sw_rst_regwen_${i}"${": begin" if loop.last else ","}
% endfor
        // RW0C, so check.
      end
      default: begin
