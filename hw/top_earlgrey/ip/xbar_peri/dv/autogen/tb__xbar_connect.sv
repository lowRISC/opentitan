// Copyright lowRISC contributors.
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0
//
// tb__xbar_connect generated by `tlgen.py` tool


`CONNECT_CLK(clk_peri_i)

`CONNECT_RST(rst_peri_ni)

xbar_peri dut(
  .clk_peri_i(clk_peri_i),
  .rst_peri_ni(rst_peri_ni)
);

// Host TileLink interface connections
`CONNECT_TL_HOST_IF(main, dut, clk_peri_i, rst_peri_ni)

// Device TileLink interface connections
`CONNECT_TL_DEVICE_IF(uart, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(gpio, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(spi_device, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(rv_timer, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(usbdev, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(pwrmgr, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(rstmgr, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(clkmgr, dut, clk_peri_i, rst_peri_ni)
`CONNECT_TL_DEVICE_IF(sensor_ctrl, dut, clk_peri_i, rst_peri_ni)
