...
module rglts_pdm_3p3v (
  ...
  input por_req_h_ni,                    // POR Request (1.1v) @3.3v
  ...
  output logic por_ack_h_o,              // POR Acknoledge @3.3v
  output logic por_enable_h_o,           // POR Enable @3.3v
...
assign dv_hook = 2'b00;  // Force 2'b11 to reduce LDOs time and indicate calibrated AON clock
...
typedef enum logic [3-1:0] {
  ...
  RGLS_VCM2A = 3'd2,  // MAIN Regulator ON (AON Regulator rN,  Power Switch Enabled->Disabled)
  RGLS_VCAON = 3'd3,  // AON Regulator ON (MAIN Regulator OFF, Power Switch Diabled)
  RGLS_VCA2M = 3'd4,  // AON Regulator ON (MAIN Regulator ON,  Power Switch Diabled->Enabled)
...
logic rgls_rst_h_n;
assign rgls_rst_h_n = vcc_pok_h_i || vcc_pok_str_h_o;
...
always_ff @( posedge clk_src_aon_h_i, negedge rgls_rst_h_n ) begin
  if ( !rgls_rst_h_n ) begin
    ...
    // POR_NI
    por_ack_h_o      <= 1'b0;        // Acknoledge POR only on VCMON
    por_enable_h_o   <= 1'b1;        // Enable POR
    //
    ...
    case ( rgls_sm )
      RGLS_CLDPU: begin
        ...
        // POR_NI
        por_ack_h_o      <= 1'b0;        // Acknoledge POR only on VCMON
        por_enable_h_o   <= 1'b1;        // Block POR during PS Transions
        //
        dly_cnt          <= dly_cnt - 1'b1;
        //
        if (dly_cnt == '0) begin
          por_ack_h_o    <= 1'b1;        // Acknoledge POR only on VCMON
          por_enable_h_o <= 1'b1;        // Block POR during PS Transions
      ...
      RGLS_VCMON: begin
        ...
        if ( !vcc_pok_s_h ) begin
          por_ack_h_o    <= 1'b0;        // Acknoledge POR only on VCMON
          por_enable_h_o <= 1'b0;        // Block POR
          rgls_sm        <= RGLS_BROUT;  // Brownout
        end else if ( !main_pd_h_ni && por_req_h_ni ) begin
      ...
      RGLS_VCM2A: begin
        ...
        // POR_NI
        por_ack_h_o      <= 1'b0;        // Acknoledge POR only on VCMON
        por_enable_h_o   <= 1'b0;        // Block POR during PS Transions
        //
        dly_cnt          <= dly_cnt - 1'b1;
        //
        if ( dly_cnt == '0 ) begin
...
///////////////////////////////////////
assign deep_sleep_h_o = !(main_pd_h_ni && !main_pd_str_h && vcmain_pok_por_h_o);


///////////////////////////////////////
// Flash
///////////////////////////////////////
assign flash_power_down_h_o  = scan_mode_h_i || deep_sleep_h_o;
