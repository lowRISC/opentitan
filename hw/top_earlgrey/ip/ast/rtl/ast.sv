...
////////////////////////////////////////
logic rst_clk0_n, rst_clk1_n;
logic lc_clk_byp_en, clks_aoff;
logic clk0_sel, clk0_aoff, clk0_en_q, clk_io_0;
logic clk1_sel, clk1_aoff, clk1_en_q, clk_io_1;

assign rst_clk0_n = clk_io_osc_val;
assign rst_clk1_n = rst_io_clk_n;
assign lc_clk_byp_en = (lc_clk_byp_req_i == lc_ctrl_pkg::On);
assign clks_aoff = !(clk0_en_q || clk1_en_q);

always_ff @( posedge clk_io_osc, negedge rst_clk0_n ) begin
  if ( !rst_clk0_n ) begin
    clk0_sel  <= 1'b0;
    clk0_aoff <= 1'b0;
  end else begin
    clk0_sel  <= !lc_clk_byp_en;
    clk0_aoff <= clks_aoff;
  end
end
...
always_ff @( posedge clk_io_osc, negedge rst_clk0_n ) begin
  if ( !rst_clk0_n ) begin
    clk0_en_q <= 1'b0;
  end else if ( !clk0_sel ) begin
    clk0_en_q <= 1'b0;
  end else if ( clk0_sel && clk0_aoff ) begin
    clk0_en_q <= 1'b1;
  end
end

assign clk0_en = clk0_sel && (clk0_en_q || clk0_aoff);

prim_clock_gating clk0_ckgt (
  .clk_i ( clk_io_osc && rst_clk0_n ),
  .en_i ( clk0_en ),
  .test_en_i ( 1'b0 ),
  .clk_o ( clk_io_0 )
);

always_ff @( posedge clk_ast_ext_i, negedge rst_clk1_n ) begin
  if ( !rst_clk1_n ) begin
    clk1_sel  <= 1'b0;
    clk1_aoff <= 1'b0;
  end else begin
    clk1_sel  <= lc_clk_byp_en;
    clk1_aoff <= clks_aoff;
  end
end
...
always_ff @( posedge clk_ast_ext_i, negedge rst_clk1_n ) begin
  if ( !rst_clk1_n ) begin
    clk1_en_q <= 1'b0;
  end else if ( !clk1_sel ) begin
    clk1_en_q <= 1'b0;
  end else if ( clk1_sel && clk1_aoff ) begin
    clk1_en_q <= 1'b1;
  end
end

assign clk1_en = clk1_sel && (clk1_en_q || clk1_aoff);

prim_clock_gating clk1_ckgt (
  .clk_i ( clk_ast_ext_i && rst_clk1_n ),
  .en_i ( clk1_en ),
  .test_en_i ( 1'b0 ),
  .clk_o ( clk_io_1 )
);

assign clk_src_io_o = clk_io_0 || clk_io_1;
assign clk_src_io_val_o = clk_io_osc_val || clk1_en;
assign lc_clk_byp_ack_o = clk1_en ? lc_ctrl_pkg::On :
                                    lc_ctrl_pkg::Off;
