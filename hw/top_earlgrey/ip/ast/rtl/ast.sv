...
logic poks_por_n;
logic por_ack;     // (rgls_sm == VCMON)
logic vcaon_pok_por_src;

always_ff @( posedge clk_aon, negedge rst_poks_por_n ) begin
  if ( !rst_poks_por_n ) begin
    poks_por_n <= 1'b0;
  end else if ( por_ack && !vcaon_pok_por_src ) begin
    poks_por_n <= 1'b1;
  end
end
...
always_ff @( posedge clk_aon_n, negedge rst_poks_n ) begin
  if ( !rst_poks_n ) begin
    ...
    por_req_n <= poks_por_n;
  end
end

logic por_enable;    // (rgls_sm == CLDPU) || ((rgls_sm == VCMON) || (rgls_sm == VCAON)

always_ff @( posedge clk_aon, negedge rst_poks_n ) begin
  if ( !rst_poks_n ) begin
    vcaon_pok_por_src <= 1'b0;
  end else begin
    vcaon_pok_por_src <= por_req_n || !por_enable;
  end
end
