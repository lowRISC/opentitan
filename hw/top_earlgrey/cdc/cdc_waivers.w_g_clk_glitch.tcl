...
set_rule_status -rule {W_G_CLK_GLITCH} -status {Waived} -expression             \
  ...
  (GatedClock=~ "top_earlgrey.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.*req.tck")} -comment {W_G_CLK_GLITCH issues in JTAG}

set_rule_status -rule {W_G_CLK_GLITCH} -status {Waived} -expression             \
  {(GatedClockInput=~"top_earlgrey.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.*q_o*") && \
  (GatedClock=~ "top_earlgrey.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.*req.tck")} -comment {W_G_CLK_GLITCH issues in JTAG}

set_rule_status -rule {W_G_CLK_GLITCH} -status {Waived} -expression             \
  {(GatedClockInput=~"top_earlgrey.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.tap_strap_q*") && \
  (GatedClock=~ "top_earlgrey.u_pinmux_aon.gen_hw_strap_sampling.u_pinmux_strap_sampling.*req.tck")} -comment {W_G_CLK_GLITCH issues in JTAG}
