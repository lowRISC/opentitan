...
set_rule_status -rule {W_DATA} -expression {(Signal == "top_earlgrey.u_spi_device.u_fwmode.u_*x_fifo.storage[7:0][7:0]") && (ReceivingFlop == "top_earlgrey.u_spi_device.u_fwmode.u_*x_fifo.rdata_o[7:0]") && (MultiClockDomains == "IO_DIV2_CLK,IO_DIV4_CLK,SPI_DEV_IN_CLK,SPI_DEV_PASSTHRU_IN_CLK*::IO_DIV2_CLK,IO_DIV4_CLK,SPI_DEV_IN_CLK,SPI_DEV_PASSTHRU_IN_CLK*") && (Association == "None")} -status {Waived} -comment {fwmode uses a special sequence to switch clock}
set_rule_status -rule {W_DATA} -expression {(Signal == "top_earlgrey.u_spi_device.u_fwmode.u_*xf_ctrl.*ptr[12:0]") && (ReceivingFlop == "top_earlgrey.u_spi_device.u_reg.u_intr_state_generic_tx_watermark.q[0]") && (MultiClockDomains == "IO_DIV2_CLK,IO_DIV4_CLK,SPI_DEV_IN_CLK,SPI_DEV_PASSTHRU_IN_CLK::IO_DIV4_CLK") && (Association == "None")} -status {Waived} -comment {tool doesn't recognize multiple spi clocks on the same domain}
