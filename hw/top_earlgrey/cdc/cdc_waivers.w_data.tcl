...
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "u_ast.rng_en_i*") && (Signal =~ "top_earlgrey.u_entropy_src.*") && (Association =~ "None")} -status {Deferred}   -comment {Deferred to Post-M2 while waiting for Nuvoton to clarify : https://github.com/lowRISC/opentitan/issues/16440}
set_rule_status -rule {W_DATA} -expression {(ReceivingFlop =~ "u_ast.rng_fips_i*") && (Signal =~ "top_earlgrey.u_entropy_src.*") && (Association =~ "None")} -status {Deferred}   -comment {Deferred to Post-M2 while waiting for Nuvoton to clarify : https://github.com/lowRISC/opentitan/issues/16440}
