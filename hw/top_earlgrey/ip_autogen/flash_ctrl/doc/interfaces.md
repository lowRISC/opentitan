...
<!-- BEGIN CMDGEN util/regtool.py --interfaces ./hw/top_earlgrey/ip_autogen/flash_ctrl/data/flash_ctrl.hjson -->
...
| FLASH_CTRL.MEM.ADDR_INFECTION              | On reads, the address of the request is XORed with the data inside the read pipeline. The request address is removed from the data before returning the data over TL-UL. A mismatch triggers a data integrity error.                               |
