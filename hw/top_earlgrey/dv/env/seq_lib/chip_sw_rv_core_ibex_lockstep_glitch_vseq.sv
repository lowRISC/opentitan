...
class chip_sw_rv_core_ibex_lockstep_glitch_vseq extends chip_sw_base_vseq;
  ...
  task automatic inject_glitches();
    ...
    // Pick one bit to glitch in the 4-bit MuBi encoded enable_cmp_q signal.
    bit_idx_cmp = $urandom_range(3);

    // Invert the bit selected for glitching.
    glitch_mask_cmp = 1 << bit_idx_cmp;
    glitched_val_cmp = orig_val_cmp ^ glitch_mask_cmp;
    ...
    for (int i = 0; i < lockstep_offset; i++) begin
      ...
      if (enable_cmp == 4'b0101) begin
        break;
      end else begin
        cfg.chip_vif.cpu_clk_rst_if.wait_n_clks(1);
      end
    end
    `DV_CHECK_NE_FATAL(enable_cmp, 4'b1010, "Lockstep comparison disabled, which is illegal.")
