// Copyright lowRISC contributors.
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0
//
// tb__alert_handler_connect.sv is auto-generated by `topgen.py` tool

assign alert_if[0].alert_tx = `CHIP_HIER.u_aes.alert_tx_o[0];
assign alert_if[1].alert_tx = `CHIP_HIER.u_aes.alert_tx_o[1];
assign alert_if[2].alert_tx = `CHIP_HIER.u_hmac.alert_tx_o[0];
assign alert_if[3].alert_tx = `CHIP_HIER.u_otbn.alert_tx_o[0];
assign alert_if[4].alert_tx = `CHIP_HIER.u_otbn.alert_tx_o[1];
assign alert_if[5].alert_tx = `CHIP_HIER.u_otbn.alert_tx_o[2];
assign alert_if[6].alert_tx = `CHIP_HIER.u_sensor_ctrl.alert_tx_o[0];
assign alert_if[7].alert_tx = `CHIP_HIER.u_keymgr.alert_tx_o[0];
