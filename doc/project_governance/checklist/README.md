...
In case the IP supports multiple parameterizations (e.g. instantiation parameters or memory maps), the configurations that are planned to be verified and signed off as part of the V* stages must be documented in a section called "Supported Parameter Sets".
...
Any integration deltas with respect to [DOC_INTEGRATION_D2]({{< relref "#doc_integration_d2" >}}) have been captured.

Note that if the IP is instantiated with a non-supported parameterization at a later point, D3 and V3 needs to be re-assessed for that configuration.
...
The testbench is setup to independently verify and close coverage on all parameter sets supported by the design (if applicable, this would have been documented under "Supported Parameter Sets" section in the design specification).
