 ...
 * accelerator computes (h + S1(e) + Ch) in the background.
 *
...
sha256_expand_w_word:
  ...
  bn.xor   w3,  w15, w16 >> 224, FG1
  bn.xor   w3,  w3,  w10 >> 224, FG1

  /* Collect (sigma1 + W[t+9]) and immediately start + sigma0(W[t+1]) */
  jal      x1, sec_add_next

  /* Prepare W[t] in (w2, w3) */
  bn.mov   w2,  w21
  bn.xor   w31, w31, w31
  bn.mov   w3,  w11

  /* Collect (sigma1 + W[t+9] + sigma0), start + W[t], and tail-call sec_add_finish */
  jal      x1, sec_add_next
  jal      x0, sec_add_finish

/**
 * Process a single 512-bit message block using fused sliding-window schedule
 * and 8-lane SIMD MAI state updates.
 */
sha256_process_block_masked:
  ...
  loopi    8, 86    /* SCA_TEST_REPLACE: loopi 1, 86 */
    bn.lid   x22, 0(x18++)
    ...
    loopi    8, 83
      ...
      /* Compute masked Ch(e, f, g) into (w26, w16) */
      jal      x1, sha256_ch
      ...
      /* Collect (h + S1(e)) and immediately start + Ch */
      jal      x1, sec_add_next

      /* While MAI computes (h + S1(e) + Ch), compute masked Maj(a, b, c) into (w26, w16) */
      jal      x1, sha256_maj

      /* Prepare masked K[t] in (w2, w3) */
      bn.wsrr  w4,  URND
      bn.xor   w2,  w19, w4
      bn.xor   w31, w31, w31
      bn.mov   w3,  w4
      ...
      /* Collect (h + S1(e) + Ch) and immediately start + K[t] */
      jal      x1, sec_add_next

      /* While MAI computes + K[t], shift K and prepare W[t] in (w2, w3) */
      bn.rshi  w19, w31, w19 >> 32
      bn.mov   w2,  w21
      bn.xor   w31, w31, w31
      bn.mov   w3,  w11

      /* Collect (+ K[t]) and immediately start + W[t] -> T1 */
      jal      x1, sec_add_next

      /* Prepare S0(a) in (w2, w3) while T1 finishes */
      bn.mov   w2,  w27
      bn.xor   w31, w31, w31
      bn.mov   w3,  w17

      /* Collect T1 into (w0, w1), save in (w25, w15), and start T2 = Maj + S0(a) */
      ...
      /* While MAI computes T2, prepare IN1 = (T1 << 224) | (T1 << 96) in (w2, w3) */
      bn.rshi  w2,  w25, w31 >> 32
      bn.or    w2,  w2,  w25 << 96
      bn.xor   w31, w31, w31
      bn.rshi  w3,  w15, w31 >> 32
      bn.or    w3,  w3,  w15 << 96, FG1

      /* Collect T2 into (w0, w1) and prepare IN0 = (T2, a, b, c, d, e, f, g) */
      jal      x1, sec_add_finish
      bn.rshi  w0,  w0,  w23 >> 32
      bn.xor   w31, w31, w31
      bn.rshi  w1,  w1,  w13 >> 32

      /* Start 8-lane SIMD state update in MAI and collect result */
      jal      x1, sec_add_start
      jal      x1, sec_add_finish
      bn.mov   w23, w0
      bn.xor   w31, w31, w31
