...
.globl sm2_scalar_mult
.globl sm2_base_mult
.globl sm2_generate_k
.globl sm2_generate_random_key
.globl sm2_key_from_seed
...
//射影坐标下的p-256曲线点加法,这里需要对算法进行修改,将算法四改成算法一
/**
 ...
 */
proj_add:
  /* mapping of parameters to symbols of [2] (Algorithm 1):
  ...
  /* 9: w19 = t4 <= X1 + Z1 = w8 + w10 */
  bn.addm   w19, w8, w10

  /* 10: w20 = t5 <= X2 + Z2 = w11 + w13 */
  bn.addm   w20, w11, w13

  /* 11: w18 = t4 <= t4 * t5 = w19 * w20 */
  bn.mov    w24, w20
  bn.mov    w25, w19
  jal       x1, mod_mul_256x256
  bn.mov    w18, w19
  ...
  /* 13: w18 = t4 <= t4 - t5 = w18 - w19 */
  bn.subm   w18, w18, w20

  /* 14: w20 = t5 <= Y1 + Z1 = w9 + w10 */
  bn.addm   w20, w9, w10

  /* 15: w11 = X3 <= Y2 + Z2 = w12 + w13 */
  bn.addm   w11, w12, w13

  /* 16: w20 = t5 <= t5 * X3 = w18 * w11 */
  bn.mov    w24, w11
  bn.mov    w25, w18
  jal       x1, mod_mul_256x256
  bn.mov    w18, w20
  ...
  /* 26: w15 = t1 <= t1 + t0 = w15 + w16 */
  bn.addm   w15, w16, w16

  /* 27: w16 = t2 <= a * t2 = w30 * w16 */
  bn.mov    w24, w30
  bn.mov    w25, w16
  jal       x1, mod_mul_256x256
  bn.mov    w16, w19
  ...
  /* 36: w11 = X3 <= t3 * X3 = w17 * w11 */
  bn.mov    w24, w17
  bn.mov    w25, w11
  jal       x1, mod_mul_256x256
  bn.mov    w11, w19

  /* 37: w11 = X3 <= X3 - t0 =  w11 - w14 */
  bn.subm   w11, w11, w14

  /* 38: w14 = t0 <= t3 * t1 = w17 * w15 */
  bn.mov    w24, w17
  bn.mov    w25, w15
  jal       x1, mod_mul_256x256
  bn.mov    w14, w19
  ...
  /* 40: w13 = Z3 <= Z3 + t0 = w13 + w14 */
  bn.addm   w13, w13, w14

  ret

//将曲线点的射影坐标转换为仿射坐标: P = (x_a, y_a) = (x/z mod p, y/z mod p) z^-1 = z^(p-2) mod p 为了进行指数运算,使用了27步加法链
/**
...
//用于计算变量时间模数乘法逆元的函数,计算给定的整数x在模m下的乘法逆元x_inv = x^(m-2) mod m, 满足x * x_inv = 1 mod m
/**
...
//从dmem中获取曲线点,并随机化z坐标。函数返回值 P = (x, y, z)，其中 P 是 P-256 曲线上的一个有效点,采用投影坐标表示,x_a 和 y_a 分别是从 dmem 中获取的仿射坐标,而 z 则是随机化处理过的 z 坐标。
/**
...
//在仿射空间中执行 P-256 标量点乘法，返回 R = k*P = k*(x_p, y_p)(侧信道有关)
/**
...
//使用基点 G 进行 P-256 标量乘法，返回 R = d*G = d*(x_g, y_g)
/**
...
//用于 P-256 标量点乘法(scalar_mult_int)的外部可调用包装器， 返回 R = k*P = k*(x_a, y_a)
/**
...
p256_scalar_mult:
  ...
  ret
// 在标量域中生成非零的随机值。返回 t,一个随机值,满足 t mod n 非零，以两份份额形式返回。
/**
...
/* 国密SM2 domain parameter p (modulus) */
.globl sm2_p
.balign 32
sm2_p:
  .word 0xffffffff
