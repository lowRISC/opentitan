...
/**
 * Blind a scalar with a multiple of the point order 8 * L or the base point order L.
 *
 * Given a scalar s, this routine adds a random multiple of the point order
 * k * 8 * L or of the base point order k * L to the scalar such that s + k * ( 8 * ) L.
 ...
 * that the blinded scalar is at most 385 bits.
 *
 ...
 *
 * @param[in]  x5: Blinding mode flag (0 for Ed25519, 1 for X25519).
...
sc_blind:
  ...
  /* Check blinding mode: if x5 == 0, skip the multiplication by 8 */
  beq x5, x0, .L_sc_blind_no_shift

  /* X25519 mode: Multiply k * L by 8 */
  /* w17 = (w17 << 3) | (w16 >> 253) */
...
.L_sc_blind_no_shift:
  /* Add the blinding value to the 253-bit scalar resulting in a
     blinded scalar avoiding any overflow.
       [w17:w16] <= [w17:w16] + w20. */
  bn.add w16, w16, w20
  bn.addc w17, w17, w31
