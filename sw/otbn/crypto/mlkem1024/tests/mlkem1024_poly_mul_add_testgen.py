...
def poly_mul_add_hw(
    a: list[int], b: list[int], c_in: list[int], g_mont: list[int] = gamma_mont
) -> list[int]:
    p_mul = poly_mul_hw(a, b, g_mont)
    return [(x + y) % Q for x, y in zip(c_in, p_mul)]
...
@testcase
def gen_poly_mul_add_test(seed: Optional[int] = None):
    if seed is not None:
        random.seed(seed)

    poly_a = [random.randint(0, Q - 1) for _ in range(N)]
    poly_b = [random.randint(0, Q - 1) for _ in range(N)]
    poly_c_in = [random.randint(0, Q - 1) for _ in range(N)]
    expected = poly_mul_add_hw(poly_a, poly_b, poly_c_in)
    ...
    return {
        ...
        "input": {
            "dmem": {
                ...
                "_poly_c_in": itoa_dmem(words_to_bytes(poly_c_in)),
            }
        },
        "output": {"dmem": {"_poly_out": itoa_dmem(words_to_bytes(expected))}},
    }


if __name__ == "__main__":
    gen_poly_mul_add_test()
