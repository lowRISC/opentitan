 ...
 * Fixed-point division by 3329 uses M = round(2^32 / 3329) = 1290168 with mu = 0 in `bn.mulvml.8s`.
 *
...
compress_11:
  ...
  /* Vectorized loop: process 256 coefficients in 32 iterations (8 per iteration).
     Computes Compress_11(x) = round(x * 2^11 / 3329) mod 2^11 in-place at x2.
     Division by 3329 is computed via fixed-point high-32-bit multiplication by M = round(2^32 / 3329) = 1290168.
     With MOD CSR set to Q = M = 1290168 and mu = 0, `bn.mulvml.8s` returns the upper 32 bits of
     ((x * 2048 + 1664) * 1290168) >> 32, which equals floor((x * 2048 + 1664) / 3329) exactly. */
  loopi 32, 6
    bn.lid x4, 0(x2)
    bn.shv.8s w1, w1 << 11
  ...
  /* Reset input pointer x2 back to start of buffer before calling encode_11.
     Output address x3 remains in x3. */
  addi x2, x2, -1024
  jal x1, encode_11
...
compress_5:
  ...
  /* Vectorized loop: process 256 coefficients in 32 iterations (8 per iteration).
     Computes Compress_5(x) = round(x * 2^5 / 3329) mod 2^5 in-place at x2 (see compress_11 for fixed-point division details). */
  loopi 32, 6
    bn.lid x4, 0(x2)
    bn.shv.8s w1, w1 << 5
    bn.addv.8s w1, w1, w30
  ...
  /* Reset input pointer x2 back to start of buffer before calling encode_5.
     Output address x3 remains in x3. */
  addi x2, x2, -1024
  jal x1, encode_5
