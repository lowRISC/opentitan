 ...
 * Variable-time modular multiplicative inverse computation
 *
 * returns x_inv = x^-1 mod p
 *
 * This routine computes the modular multiplicative inverse for any x < p in
 * the finite field GF(p) where p is prime.
 *
 * This is an analogue to `mod_inv_n_p384` for the P-384 finite-field prime p.

 * Note: This routine runs in variable-time w.r.t. the modulus. It should only
 * be used with a non-secret modulus.
 *
 * @param[in]  [w13, w12]: p, 384 bit modulus
 * @param[in]  [w30, w29]: x, 384 bit operand
 * @param[in]  w31, all-zero
 * @param[out] [w17, w16]: x_inv, modular multiplicative inverse
 *
...
mod_inv_p_p384:
  ...
  loopi     384, 12
    ...
    csrrs     x2, 0x7c0, x0
    andi      x2, x2, 1
    beq       x2, x0, mod_inv_p_p384_nomul
