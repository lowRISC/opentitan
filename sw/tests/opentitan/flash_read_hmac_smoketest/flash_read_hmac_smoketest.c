...
//
//
#include "../flash_hmac_smoketest/bazel-out/flash_hmac_smoketest_signed_clear32.h"
...
int main(int argc, char **argv) {
  ...
  for(i = 0; i < buffer_size_clear; i=i+2){
    ...
    read = *flash_addr;
    exp = FLASH_HMAC_SMOKETEST_SIGNED_CLEAR[i+1];
    if(read==exp)
    ...
    read = *flash_addr;
    exp = FLASH_HMAC_SMOKETEST_SIGNED_CLEAR[i];
    if(read==exp)
      printf("Correct! Addr: %x, Exptected Data: %x, Read Data: %x\r\n",flash_addr,exp,read);
    else{
      printf("Error! Addr: %x, Exptected Data: %x, Read Data: %x\r\n",flash_addr,exp, read);
