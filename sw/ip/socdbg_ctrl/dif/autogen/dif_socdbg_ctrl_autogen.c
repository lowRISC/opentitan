...
dif_result_t dif_socdbg_ctrl_init(mmio_region_t base_addr,
  ...
  socdbg_ctrl->base_addr = base_addr;
...
dif_result_t dif_socdbg_ctrl_alert_force(const dif_socdbg_ctrl_t *socdbg_ctrl,
  ...
  switch (alert) {
    case kDifSocdbgCtrlAlertFatalFault:
      alert_idx = SOCDBG_CTRL_ALERT_TEST_FATAL_FAULT_BIT;
      break;
    default:
      return kDifBadArg;
  }
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_ALERT_TEST_REG_OFFSET,
                      alert_test_reg);
 ...
 */
static bool socdbg_ctrl_get_irq_bit_index(dif_socdbg_ctrl_irq_t irq,
                                          bitfield_bit32_index_t *index_out) {
  switch (irq) {
    case kDifSocdbgCtrlIrqDebugAttention:
      *index_out = SOCDBG_CTRL_INTR_COMMON_DEBUG_ATTENTION_BIT;
      break;
    default:
      return false;
  }
...
dif_result_t dif_socdbg_ctrl_irq_get_type(const dif_socdbg_ctrl_t *socdbg_ctrl,
                                          ...
                                          dif_irq_type_t *type) {
  if (socdbg_ctrl == NULL || type == NULL ||
      irq == kDifSocdbgCtrlIrqDebugAttention + 1) {
    return kDifBadArg;
  }
...
dif_result_t dif_socdbg_ctrl_irq_get_state(
  ...
  if (socdbg_ctrl == NULL || snapshot == NULL) {
    return kDifBadArg;
  }

  *snapshot = mmio_region_read32(socdbg_ctrl->base_addr,
                                 (ptrdiff_t)SOCDBG_CTRL_INTR_STATE_REG_OFFSET);
...
dif_result_t dif_socdbg_ctrl_irq_acknowledge_state(
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_STATE_REG_OFFSET, snapshot);
...
dif_result_t dif_socdbg_ctrl_irq_is_pending(
    ...
    bool *is_pending) {
  if (socdbg_ctrl == NULL || is_pending == NULL) {
    return kDifBadArg;
  }

  bitfield_bit32_index_t index;
  if (!socdbg_ctrl_get_irq_bit_index(irq, &index)) {
    return kDifBadArg;
  }

  uint32_t intr_state_reg = mmio_region_read32(
      socdbg_ctrl->base_addr, (ptrdiff_t)SOCDBG_CTRL_INTR_STATE_REG_OFFSET);
...
dif_result_t dif_socdbg_ctrl_irq_acknowledge_all(
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_STATE_REG_OFFSET, UINT32_MAX);
...
dif_result_t dif_socdbg_ctrl_irq_force(const dif_socdbg_ctrl_t *socdbg_ctrl,
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_TEST_REG_OFFSET,
                      intr_test_reg);
...
dif_result_t dif_socdbg_ctrl_irq_get_enabled(
    const dif_socdbg_ctrl_t *socdbg_ctrl, dif_socdbg_ctrl_irq_t irq,
    dif_toggle_t *state) {
  if (socdbg_ctrl == NULL || state == NULL) {
    return kDifBadArg;
  }
  ...
  uint32_t intr_enable_reg = mmio_region_read32(
      socdbg_ctrl->base_addr, (ptrdiff_t)SOCDBG_CTRL_INTR_ENABLE_REG_OFFSET);
...
dif_result_t dif_socdbg_ctrl_irq_set_enabled(
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_ENABLE_REG_OFFSET,
                      intr_enable_reg);
...
dif_result_t dif_socdbg_ctrl_irq_disable_all(
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_ENABLE_REG_OFFSET, 0u);
...
dif_result_t dif_socdbg_ctrl_irq_restore_all(
  ...
  mmio_region_write32(socdbg_ctrl->base_addr,
                      (ptrdiff_t)SOCDBG_CTRL_INTR_ENABLE_REG_OFFSET, *snapshot);
