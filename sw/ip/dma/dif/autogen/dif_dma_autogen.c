...
dif_result_t dif_dma_alert_force(const dif_dma_t *dma, dif_dma_alert_t alert) {
  ...
  switch (alert) {
    case kDifDmaAlertFatalFault:
      alert_idx = DMA_ALERT_TEST_FATAL_FAULT_BIT;
      break;
    default:
      return kDifBadArg;
  }
  ...
  mmio_region_write32(dma->base_addr, (ptrdiff_t)DMA_ALERT_TEST_REG_OFFSET,
                      alert_test_reg);
...
dif_result_t dif_dma_irq_get_state(const dif_dma_t *dma,
  ...
  *snapshot =
      mmio_region_read32(dma->base_addr, (ptrdiff_t)DMA_INTR_STATE_REG_OFFSET);
...
dif_result_t dif_dma_irq_acknowledge_state(
  ...
  mmio_region_write32(dma->base_addr, (ptrdiff_t)DMA_INTR_STATE_REG_OFFSET,
                      snapshot);
...
dif_result_t dif_dma_irq_force(const dif_dma_t *dma, dif_dma_irq_t irq,
  ...
  mmio_region_write32(dma->base_addr, (ptrdiff_t)DMA_INTR_TEST_REG_OFFSET,
                      intr_test_reg);
...
dif_result_t dif_dma_irq_get_enabled(const dif_dma_t *dma, dif_dma_irq_t irq,
  ...
  uint32_t intr_enable_reg =
      mmio_region_read32(dma->base_addr, (ptrdiff_t)DMA_INTR_ENABLE_REG_OFFSET);
...
dif_result_t dif_dma_irq_set_enabled(const dif_dma_t *dma, dif_dma_irq_t irq,
  ...
  mmio_region_write32(dma->base_addr, (ptrdiff_t)DMA_INTR_ENABLE_REG_OFFSET,
                      intr_enable_reg);
...
dif_result_t dif_dma_irq_disable_all(const dif_dma_t *dma,
  ...
  if (snapshot != NULL) {
    *snapshot = mmio_region_read32(dma->base_addr,
                                   (ptrdiff_t)DMA_INTR_ENABLE_REG_OFFSET);
  }
