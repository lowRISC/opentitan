...
def _prepare_release_files(ctx):
    ...
    for name, target in zip(ctx.attr.variants_keys, ctx.attr.variants_values):
        ...
        elf = get_one_binary_file(target, field = "elf", providers = [SiliconBinaryInfo])
        files.extend([
            bin,
            elf,
            get_one_binary_file(target, field = "mapfile", providers = [SiliconBinaryInfo]),
        ])
        build_contents.append(_RELEASE_BUILD_TARGET.format(name = name, filename = elf.basename))
...
def _exportability_test_impl(ctx):
    ...
    symbol_flags = " ".join(["--symbol \"{}\"".format(sym) for sym in ctx.attr.exports])
    test_script = ctx.actions.declare_file(ctx.attr.name + "_test.sh")
    script_content = """
        exec "{test_tool}" "{object}" --objdump "{objdump}" {symbol_flags} {expect_fail_flag}
    """.format(
