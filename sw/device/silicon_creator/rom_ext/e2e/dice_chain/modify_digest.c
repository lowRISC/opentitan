...
static status_t clear_digest_page(const nvm_ctrl_info_page_t *info_page) {
  ...
  TRY(nvm_ctrl_info_erase(info_page, kNvmCtrlEraseTypePage));
...
static status_t modify_digest_test(void) {
  ...
  TRY(clear_digest_page(&kNvmCtrlInfoPageFactoryCerts));
  TRY(clear_digest_page(&kNvmCtrlInfoPageDiceCerts));
  return OK_STATUS();
}
