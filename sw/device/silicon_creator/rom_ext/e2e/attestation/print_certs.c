...
static status_t print_certs(void) {
  ...
  if (kDeviceType == kDeviceSilicon) {
    TRY(print_cert(buf, &kNvmCtrlInfoPageFactoryCerts));
  }
  TRY(print_cert(buf, &kNvmCtrlInfoPageDiceCerts));

  // Print owner information.
  TRY(print_owner_block(buf, &kNvmCtrlInfoPageOwnerSlot0));
  LOG_INFO("OWNER_PAGE_0: %s", buf);

  TRY(print_owner_block(buf, &kNvmCtrlInfoPageOwnerSlot1));
