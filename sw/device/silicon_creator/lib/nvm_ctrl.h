 ...
 *
 * For RRAM, this stops before `kRramCtrlEmulPageBaseB`: everything from
 * there onward is Region B's whole reserved window (the emulated info-page
 * region -- OwnerSlot0/1, DiceCerts, BootData0/1, ..., only accessible
 * through `nvm_ctrl_info_*()`, not plain data reads/writes -- plus any
 * currently-unused pages in that window, plus, right at the end, a small
 * tail reserved for OTP that's read/write protected in hardware and must
 * never be touched by software at all). For flash, there is no such
 * reserved tail, so this is just the full data partition size.
 */
#if defined(USE_RRAM)
#define NVM_USABLE_DATA_SIZE_BYTES (kRramCtrlEmulPageBaseB * NVM_BYTES_PER_PAGE)
#else
#define NVM_USABLE_DATA_SIZE_BYTES NVM_DATA_SIZE_BYTES
#endif  // USE_RRAM

/**
 * Exclusive upper bound, in bytes, of Slot A's own portion of the NVM data
 * partition actually usable for generic firmware reads/writes/erases.
 *
 * `NVM_USABLE_DATA_SIZE_BYTES` above excludes Region B's reserved window
 * (Slot B's tail), but says nothing about Region A (Slot A's tail):
 * bootstrap/rescue's generic erase/program paths operate on raw NVM byte
 * offsets, not firmware images, so they need this separate bound to avoid
 * writing into Region A's window too -- used on RRAM for
 * `CreatorReserved0`'s dedicated region, plus any currently-unused pages
 * alongside it (see `kRramCtrlEmulRegionA` in rram_ctrl.h). Equal to
 * `NVM_BYTES_PER_SLOT` for flash, which has no such reservation.
 */
#if defined(USE_RRAM)
#define NVM_SLOT_A_USABLE_SIZE_BYTES \
  (kRramCtrlEmulPageBaseA * NVM_BYTES_PER_PAGE)
#else
#define NVM_SLOT_A_USABLE_SIZE_BYTES NVM_BYTES_PER_SLOT
#endif  // USE_RRAM
