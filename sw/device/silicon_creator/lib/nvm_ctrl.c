...
void nvm_ctrl_init(void) {
  ...
  // Two calls above, so 4 writes total.
  SEC_MMIO_WRITE_INCREMENT(4);
}
...
rom_error_t nvm_ctrl_chip_erase(void) {
  ...
  //
  // Slot A has its own separate reserved tail (see
  // `NVM_SLOT_A_USABLE_SIZE_BYTES`), used for `CreatorReserved0`'s dedicated
  // region; skip it for the same reason.
  rram_ctrl_data_default_perms_set((rram_ctrl_perms_t){
      .read = kMultiBitBool4True,
      .write = kMultiBitBool4True,
  });
  ...
  for (uint32_t addr = 0; err == kErrorOk && addr < NVM_USABLE_DATA_SIZE_BYTES;
    ...
    if (addr >= NVM_SLOT_A_USABLE_SIZE_BYTES && addr < NVM_BYTES_PER_SLOT) {
      continue;
    }
