...
rom_error_t nvm_firmware_block(rescue_state_t *state) {
  ...
  // `NVM_SLOT_A_USABLE_SIZE_BYTES`), used on RRAM for `CreatorReserved0`'s
  // dedicated region. The clamp above doesn't cover it, since it's entirely
  // below `NVM_USABLE_DATA_SIZE_BYTES` and only applies when `bank_offset`
  // selects Slot A.
  if (bank_offset == 0 && nvm_limit > NVM_SLOT_A_USABLE_SIZE_BYTES) {
    nvm_limit = NVM_SLOT_A_USABLE_SIZE_BYTES;
  }
