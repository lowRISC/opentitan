...
void rescue_state_init(rescue_state_t *state, boot_data_t *bootdata,
  ...
  if (launder32((hardened_bool_t)config) == kHardenedBoolFalse) {
    ...
    uint32_t size = bitfield_field32_read(config->size, RESCUE_REGION_SIZE) *
                    kFlashPageSize;
    state->flash_limit = state->flash_start + size;
    state->erase_both_slots =
        bitfield_bit32_read(config->size, RESCUE_ERASE_BOTH_SLOTS_BIT);
    uint32_t timeout =
