...
rom_error_t dice_cdi_0_cert_build(hmac_digest_t *rom_ext_measurement,
  ...
  ASSERT_SIZE_EQ(sizeof(rom_ext_hash.digest), kCdi0ExactRomExtHashSizeBytes);
  ASSERT_SIZE_GE(sizeof(key_ids->cert->digest),
...
rom_error_t dice_cdi_1_cert_build(hmac_digest_t *owner_measurement,
  ...
  ASSERT_SIZE_EQ(sizeof(owner_hash.digest), kCdi1ExactOwnerHashSizeBytes);
  ASSERT_SIZE_EQ(sizeof(owner_manifest_hash.digest),
