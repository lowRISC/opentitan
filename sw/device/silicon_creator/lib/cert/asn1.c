...
rom_error_t asn1_start_bitstring(asn1_state_t *state, asn1_bitstring_t *out_bitstring) {
  ...
  // Push a single byte that will hold the unused bit count (it will be updated in
  // asn1_finish_bitstring.
  RETURN_IF_ERROR(asn1_push_byte(state, 0));
  return kErrorOk;
}
...
rom_error_t asn1_finish_bitstring(asn1_bitstring_t *bitstring) {
  ...
  if (bitstring->used_bits != 0) {
    ...
    bitstring->state->buffer[bitstring->unused_bits_offset] = 8 - bitstring->used_bits;
  }
  // Hardening: clear out the tag structure to prevent accidental reuse.
