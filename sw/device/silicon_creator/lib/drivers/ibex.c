...
static bool remap_verify(uint32_t reg_matching, uint32_t reg_remap,
  ...
  uint32_t reg_matching_size = 1 << __builtin_ctz(~reg_matching);
  uint32_t reg_matching_addr = reg_matching & ~(reg_matching_size - 1);

  /* Check that matching range within current remap */
  if (matching_addr < reg_matching_addr ||
      matching_addr + size > reg_matching_addr + reg_matching_size) {
    return false;
  }
