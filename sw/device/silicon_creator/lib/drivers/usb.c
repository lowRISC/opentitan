...
void handle_in(void) {
  ...
  for (uint8_t ep = 0; sent && ep < USBDEV_PARAM_N_ENDPOINTS; ++ep) {
    ...
    if (bitfield_bit32_read(reg, USBDEV_CONFIGIN_0_PEND_0_BIT)) {
      ...
      error = kUsbTransactionFlagsError;
    } else {
    ...
    if (error == 0 && (endpoint->transfer.flags & kUsbTransactionFlagsZlp)) {
      // If this is a control transfer, we need to turn around with a
      // zero-length OUT packet.
      usb_ep_transfer(ep, NULL, 0, kUsbTransactionFlagsZlpAck);
      continue;
    }
    ...
    if (endpoint->transfer.flags & kUsbTransactionFlagsZlpAck) {
      endpoint->transfer.flags = 0;
      endpoint = out_endpoints + ep;
    }
    endpoint->transfer.flags |= kUsbTransactionFlagsDone | error;
...
void handle_out(void) {
  ...
  while (!rx_fifo_empty()) {
    ...
    uint32_t error =
        size > endpoint->transfer.len ? kUsbTransactionFlagsError : 0;
    size_t chunk =
    ...
    if (error || endpoint->transfer.len == 0 ||
      ...
      if (error == 0 && endpoint->transfer.flags & kUsbTransactionFlagsZlp) {
        ...
        usb_ep_transfer(kUsbDirIn | ep, NULL, 0, kUsbTransactionFlagsZlpAck);
        continue;
      }
      // Complete the transfer.
      if (endpoint->transfer.flags & kUsbTransactionFlagsZlpAck) {
