...
static void uart_reset(void) {
  abs_mmio_write32(uart_reg_base() + UART_CTRL_REG_OFFSET, 0u);
  ...
  abs_mmio_write32(uart_reg_base() + UART_FIFO_CTRL_REG_OFFSET, reg);

  abs_mmio_write32(uart_reg_base() + UART_OVRD_REG_OFFSET, 0u);
  abs_mmio_write32(uart_reg_base() + UART_TIMEOUT_CTRL_REG_OFFSET, 0u);
  abs_mmio_write32(uart_reg_base() + UART_INTR_ENABLE_REG_OFFSET, 0u);
  abs_mmio_write32(uart_reg_base() + UART_INTR_STATE_REG_OFFSET, UINT32_MAX);
}
...
void uart_init(uint32_t precalculated_nco) {
  ...
  abs_mmio_write32(uart_reg_base() + UART_CTRL_REG_OFFSET, reg);
...
void uart_enable_receiver(void) {
  uint32_t reg = abs_mmio_read32(uart_reg_base() + UART_CTRL_REG_OFFSET);
...
static bool uart_tx_full(void) {
  uint32_t reg = abs_mmio_read32(uart_reg_base() + UART_STATUS_REG_OFFSET);
...
size_t uart_read(uint8_t *data, size_t len, uint32_t timeout_ms) {
  ...
  for (n = 0; n < len; ++n) {
    ...
    uint32_t reg = abs_mmio_read32(uart_reg_base() + UART_RDATA_REG_OFFSET);
    *data++ = (uint8_t)reg;
  }
  return n;
}
...
hardened_bool_t uart_break_detect(uint32_t timeout_us) {
  ...
  while (time < deadline) {
    uint32_t val = abs_mmio_read32(uart_reg_base() + UART_VAL_REG_OFFSET);
    if (val)
