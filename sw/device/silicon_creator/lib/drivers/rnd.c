...
enum {
  kBase48 = TOP_EARLGREY_RV_CORE_IBEX_CFG_BASE_ADDR,
};
...
uint32_t rnd_uint32(void) {
  if (otp_read32(OTP_CTRL_PARAM_CREATOR_SW_CFG_RNG_EN_OFFSET) ==
    ...
    while (!abs_mmio_read32(kBase48 + RV_CORE_IBEX_RND_STATUS_REG_OFFSET)) {
    }
  }
  uint32_t mcycle;
  CSR_READ(CSR_REG_MCYCLE, &mcycle);
  return mcycle + abs_mmio_read32(kBase48 + RV_CORE_IBEX_RND_DATA_REG_OFFSET);
}
