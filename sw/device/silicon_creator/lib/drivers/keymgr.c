...
enum {
  kBase2 = TOP_EARLGREY_KEYMGR_BASE_ADDR,
};
...
static rom_error_t expected_state_check(uint32_t expected_state) {
  ...
  do {
    op_status = abs_mmio_read32(kBase2 + KEYMGR_OP_STATUS_REG_OFFSET);
    abs_mmio_write32(kBase2 + KEYMGR_OP_STATUS_REG_OFFSET, op_status);
    op_status_field =
  ...
  uint32_t error_code = abs_mmio_read32(kBase2 + KEYMGR_ERR_CODE_REG_OFFSET);
  abs_mmio_write32(kBase2 + KEYMGR_ERR_CODE_REG_OFFSET, error_code);

  // Read the working state with sec_mmio so that we can check the expected
  // value periodically.
  uint32_t got_state = sec_mmio_read32(kBase2 + KEYMGR_WORKING_STATE_REG_OFFSET);
...
rom_error_t keymgr_init(uint16_t entropy_reseed_interval) {
  ...
  sec_mmio_write32_shadowed(kBase2 + KEYMGR_RESEED_INTERVAL_SHADOWED_REG_OFFSET,
                            reg);
  return kErrorOk;
}
...
void keymgr_sw_binding_set(
  ...
  for (size_t i = 0; i < ARRAYSIZE(binding_value_sealing->data); ++i) {
    sec_mmio_write32(
        kBase2 + KEYMGR_SEALING_SW_BINDING_0_REG_OFFSET + i * sizeof(uint32_t),
        binding_value_sealing->data[i]);
  }
  for (size_t i = 0; i < ARRAYSIZE(binding_value_attestation->data); ++i) {
    sec_mmio_write32(
        kBase2 + KEYMGR_ATTEST_SW_BINDING_0_REG_OFFSET + i * sizeof(uint32_t),
        binding_value_attestation->data[i]);
  }
  sec_mmio_write32(kBase2 + KEYMGR_SW_BINDING_REGWEN_REG_OFFSET, 0);
}

void keymgr_sw_binding_unlock_wait(void) {
  while (!abs_mmio_read32(kBase2 + KEYMGR_SW_BINDING_REGWEN_REG_OFFSET)) {
  }
  sec_mmio_read32(kBase2 + KEYMGR_SW_BINDING_REGWEN_REG_OFFSET);
}
...
void keymgr_creator_max_ver_set(uint32_t max_key_ver) {
  ...
  sec_mmio_write32_shadowed(
      kBase2 + KEYMGR_MAX_CREATOR_KEY_VER_SHADOWED_REG_OFFSET, max_key_ver);
  sec_mmio_write32(kBase2 + KEYMGR_MAX_CREATOR_KEY_VER_REGWEN_REG_OFFSET, 0);
}
...
void keymgr_owner_int_max_ver_set(uint32_t max_key_ver) {
  ...
  // Write and lock (rw0c) the max key version.
  sec_mmio_write32_shadowed(
      kBase2 + KEYMGR_MAX_OWNER_INT_KEY_VER_SHADOWED_REG_OFFSET, max_key_ver);
  sec_mmio_write32(kBase2 + KEYMGR_MAX_OWNER_INT_KEY_VER_REGWEN_REG_OFFSET, 0);
}
...
void keymgr_advance_state(void) {
  ...
  abs_mmio_write32_shadowed(kBase2 + KEYMGR_CONTROL_SHADOWED_REG_OFFSET, reg);

  abs_mmio_write32(kBase2 + KEYMGR_START_REG_OFFSET, 1);
}
