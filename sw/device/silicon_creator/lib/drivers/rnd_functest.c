...
static uint16_t random_low_threshold(dif_entropy_src_test_t test_id,
                                     uint16_t rnd_threshold) {
  uint16_t result;
  switch (test_id) {
...
static void configure_random_health_checks(void) {
  ...
  for (uint32_t test_id = kDifEntropySrcTestRepetitionCount;
    ...
    uint16_t meaningless_threshold =
        (uint16_t)rand_testutils_gen32_range(/*min=*/0, /*max=*/16);
    configs[test_id] = (dif_entropy_src_health_test_config_t){
        .test_type = (dif_entropy_src_test_t)test_id,
        .high_threshold = 0xFF00 ^ meaningless_threshold,
  ...
  // Enable entropy_src in continous mode.
  CHECK_STATUS_OK(entropy_testutils_auto_mode_init());
}

// Configures the entropy source health check and alert thresholds with the
// FIPS configuration stored in OTP.
static void configure_fips_health_checks_from_otp(void) {
  CHECK_STATUS_OK(entropy_testutils_stop_all());
  ...
  dif_entropy_src_config_t entropy_src_config = {
      ...
      .alert_threshold = (uint16_t)otp_read32(
          OTP_CTRL_PARAM_CREATOR_SW_CFG_RNG_FIPS_ALERT_THRESHOLD_OFFSET),
  };
  CHECK_DIF_OK(dif_entropy_src_configure(&entropy_src, entropy_src_config,
                                         kDifToggleEnabled));
}

// Checks that the boot-time / bypass mode CRC value stored in OTP is consistent
// with what ROM is expecting.
rom_error_t test_boot_otp_crc_check(void) {
  uint32_t ctx;
  crc32_init(&ctx);
  ...
  for (size_t i = 0; i < ARRAYSIZE(otp_in_order_offsets); ++i) {
    crc32_add32(&ctx, otp_read32(otp_in_order_offsets[i]) & 0xFFFF);
  }
  crc32_add32(
      &ctx, otp_read32(
                OTP_CTRL_PARAM_CREATOR_SW_CFG_RNG_BOOT_ALERT_THRESHOLD_OFFSET));
  uint32_t crc = crc32_finish(&ctx);
  uint32_t expected = crc ^ kErrorOk;
  uint32_t got =
      otp_read32(OTP_CTRL_PARAM_CREATOR_SW_CFG_RNG_BOOT_CONFIG_DIGEST_OFFSET);
...
rom_error_t test_rnd_fips_health_config_check(void) {
  ...
  configure_fips_health_checks_from_otp();
  for (size_t i = 0; i < ARRAYSIZE(lc_expect_check); ++i) {
    rom_error_t res =
        rnd_health_config_check(lc_expect_check[i], kHardenedBoolFalse);
  ...
  for (size_t i = 0; i < ARRAYSIZE(lc_expect_check_skip); ++i) {
    CHECK(rnd_health_config_check(lc_expect_check_skip[i],
                                  kHardenedBoolFalse) == kErrorOk);
  }
...
bool test_main(void) {
  ...
  EXECUTE_TEST(result, test_boot_otp_crc_check);
  EXECUTE_TEST(result, test_rnd_fips_health_config_check);
