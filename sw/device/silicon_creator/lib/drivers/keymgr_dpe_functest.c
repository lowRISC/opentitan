 ...
 */
static const keymgr_dpe_binding_value_t kBindingValueRom = {
 ...
 */
static const keymgr_dpe_binding_value_t kBindingValueRomExt = {
 ...
 */
static const keymgr_dpe_binding_value_t kBindingValueBl0Attestation = {
    .data = {0xe4987b39, 0x3f83d390, 0xc2f3bbaf, 0x3195dbfa, 0x23fb480c,
             0xb012ae5e, 0xf1394d28, 0x1940ceeb},
};
static const keymgr_dpe_binding_value_t kBindingValueBl0Sealing = {
 ...
 */
static const sc_keymgr_dpe_policies_t kDefaultPolicy = {
...
rom_error_t keymgr_dpe_rom_test(void) {
  sc_keymgr_dpe_advance_data_t attestation_key = {
      ...
      .sel_src_slot = kAttestationSlot,
      .sel_dst_slot = kAttestationSlot,
      .version = kMaxVerRom};
  sc_keymgr_dpe_advance_data_t sealing_key = {
      .binding_value = &kBindingValueRom,
      .policy = kDefaultPolicy,
      .sel_src_slot = kSealingSlot,
  ...
  RETURN_IF_ERROR(
      sc_keymgr_dpe_advance_owner_int(sealing_key, attestation_key));
...
rom_error_t keymgr_dpe_rom_ext_test(void) {
  ...
  RETURN_IF_ERROR(sc_keymgr_dpe_advance_owner(sealing_key, attestation_key));
...
rom_error_t keymgr_dpe_generate_sw_keys(void) {
  ...
  RETURN_IF_ERROR(sc_keymgr_dpe_generate_key(kScKeymgrDPEDestNone, key_data));
  RETURN_IF_ERROR(
      sc_keymgr_dpe_read_key(sealing_derived_key[0], sealing_derived_key[1]));

  // Generate the key from the attestation context
  key_data.sel_src_slot = kAttestationSlot;
  RETURN_IF_ERROR(sc_keymgr_dpe_generate_key(kScKeymgrDPEDestNone, key_data));
  RETURN_IF_ERROR(sc_keymgr_dpe_read_key(attestation_derived_key[0],
...
rom_error_t keymgr_dpe_disable_test(void) {
  RETURN_IF_ERROR(sc_keymgr_dpe_disable());
