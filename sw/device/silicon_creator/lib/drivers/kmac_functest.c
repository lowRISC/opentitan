...
#define SHORT_BUFFER_LEN 13
#define LONG_BUFFER_LEN 26 * 4
#define IS_ALIGNED(x, a) (((uintptr_t)(x) & ((a)-1)) == 0)

typedef struct __attribute__((packed)) {
  char unused;
  char short_payload[SHORT_BUFFER_LEN];
  char long_payload[LONG_BUFFER_LEN];
  size_t short_payload_len;
  size_t long_payload_len;
  uint32_t short_digest[SHORT_DIGEST_LEN];
  uint32_t long_digest[LONG_DIGEST_LEN];
} UnalignedHash_t;

static UnalignedHash_t kKMAC_test_data = {
    .unused = 0xFF,
    .short_payload = "Test message!",
    .short_payload_len = sizeof(kKMAC_test_data.short_payload),
    .long_payload = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuv"
    "wxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",
    .long_payload_len = sizeof(kKMAC_test_data.long_payload),
    .short_digest = {
...
void pretest_setup(void) {
  ...
  LOG_INFO("Pretest setup: Disabling all entropy sources.");
  CHECK_STATUS_OK(entropy_testutils_stop_all());

  LOG_INFO("Pretest setup: checking alignment");
  LOG_INFO("Aligned short msg addr: %p", &short_msg[0]);
  LOG_INFO("Unaligned short msg addr: %p", &kKMAC_test_data.short_payload[0]);
  LOG_INFO("Aligned long msg addr: %p", &long_msg[0]);
  LOG_INFO("Unaligned long msg addr: %p", &kKMAC_test_data.long_payload[0]);
  LOG_INFO("Is short_msg aligned? %s",
           (IS_ALIGNED(&short_msg[0], sizeof(uint32_t)))
               ? "Yes"
               : "No");
  LOG_INFO("Is kKMAC_test_data.short_payload aligned? %s",
           (IS_ALIGNED(&kKMAC_test_data.short_payload[0], sizeof(uint32_t)))
               ? "Yes"
               : "No");
  LOG_INFO("Is long_msg aligned? %s",
           (IS_ALIGNED(&long_msg[0], sizeof(uint32_t)))
               ? "Yes"
               : "No");
  LOG_INFO("Is kKMAC_test_data.long_payload aligned? %s",
           (IS_ALIGNED(&kKMAC_test_data.long_payload[0], sizeof(uint32_t)))
               ? "Yes"
               : "No");
}
...
rom_error_t kmac_shake256_test(void) {
  ...
  // Simple test.
  RETURN_IF_ERROR(shake256_test(kKMAC_test_data.short_payload_len, kKMAC_test_data.short_payload,
                                ARRAYSIZE(kKMAC_test_data.short_digest), kKMAC_test_data.short_digest));

  // Test with long input, short output.
  RETURN_IF_ERROR(shake256_test(kKMAC_test_data.long_payload_len, kKMAC_test_data.long_payload, 1, kKMAC_test_data.long_digest));

  // Test with long input, long output.
  RETURN_IF_ERROR(shake256_test(kKMAC_test_data.long_payload_len, kKMAC_test_data.long_payload,
                                ARRAYSIZE(kKMAC_test_data.long_digest), kKMAC_test_data.long_digest));
