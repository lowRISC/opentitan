...
rom_error_t boot_data_write(const boot_data_t *boot_data) {
  ...
  if (active_page.has_valid_entry == kHardenedBoolTrue) {
    ...
    active_page.page = kPages[0];
  }

  // Write to the inactive page first, and then active page.
  if (active_page.page == kPages[1]) {
    boot_data_digest_compute(&new_entry, &new_entry.digest);
    RETURN_IF_ERROR(
        boot_data_entry_write(kPages[0], 0, &new_entry, kHardenedBoolTrue));
    ...
    new_entry.counter += 1;
    boot_data_digest_compute(&new_entry, &new_entry.digest);
    return boot_data_entry_write(kPages[1], 0, &new_entry, kHardenedBoolTrue);
  } else {
    // Make `Page 1 counter` to be `Page 0 counter + 1`.
    new_entry.counter += 1;
    boot_data_digest_compute(&new_entry, &new_entry.digest);
    RETURN_IF_ERROR(
        boot_data_entry_write(kPages[1], 0, &new_entry, kHardenedBoolTrue));

    new_entry.counter -= 1;
    boot_data_digest_compute(&new_entry, &new_entry.digest);
    return boot_data_entry_write(kPages[0], 0, &new_entry, kHardenedBoolTrue);
  }
}
...
rom_error_t boot_data_redundancy_check(void) {
  ...
  // Both records must be stored at index 0.
  if (page_info_0.last_valid_index != 0 || page_info_1.last_valid_index != 0) {
    return kErrorBootDataInvalid;
  }
