...
typedef struct hybrid_ecdsa_spx_key {
  ecdsa_p256_public_key_t ecdsa;
  sigverify_spx_key_t spx;
} hybrid_ecdsa_spx_key_t;
...
typedef union owner_key {
  ...
  hybrid_ecdsa_spx_key_t hybrid_ecdsa_spx;
...
typedef enum ownership_key_alg {
  ...
  kOwnershipKeyAlgHybridEcdsaSpxPure = 0x75502b48,
  /** Key algorithm Hybrid P256 & SPX+ Prehashed SHA256: `H+S2` */
  kOwnershipKeyAlgHybridEcdsaSpxPrehash = 0x32532b48,
...
enum {
  ...
  kTlvLenApplicationKeyHybrid =
      offsetof(owner_application_key_t, data) + sizeof(hybrid_ecdsa_spx_key_t),
};
