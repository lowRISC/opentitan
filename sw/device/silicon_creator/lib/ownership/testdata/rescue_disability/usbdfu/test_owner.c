...
#include "sw/device/silicon_creator/lib/ownership/testdata/rescue_disability/usbdfu/test_owner.h"
...
rom_error_t sku_creator_owner_init(boot_data_t *bootdata,
  ...
  const owner_keydata_t *owner =
      &((const owner_block_t *)test_owner)->owner_key;
  uint32_t config_version = ((const owner_block_t *)test_owner)->config_version;
  ...
  memcpy(&owner_page[0], test_owner, sizeof(test_owner));
