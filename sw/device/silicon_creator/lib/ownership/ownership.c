...
static owner_page_status_t check_owner_page_validity(size_t page) {
  size_t sig_len = offsetof(owner_block_t, signature);

  if (ownership_seal_check(page) == kErrorOk) {
    return kOwnerPageStatusSealed;
  }
  hardened_bool_t result = ownership_key_validate(
      /*page=*/page, kOwnershipKeyOwner, &owner_page[page].signature,
  ...
  return result == kHardenedBoolTrue ? kOwnerPageStatusSigned
                                     : kOwnerPageStatusInvalid;
}
