...
/*
 *  Mock the validate and hashing functions in ownership_unlock.c
 */
namespace internal {
class MockUnlockImpl : public ::global_mock::GlobalMock<MockUnlockImpl> {
 public:
  MOCK_METHOD(hardened_bool_t, unlock_key_validate,
              (size_t, ownership_key_t, const owner_signature_t *, const void *,
               size_t));
  MOCK_METHOD(void, unlock_sha256, (const void *, size_t, hmac_digest_t *));
};
}  // namespace internal
using MockUnlock = testing::StrictMock<internal::MockUnlockImpl>;

extern "C" {
hardened_bool_t unlock_key_validate(size_t page, ownership_key_t key,
                                    const owner_signature_t *signature,
                                    const void *message, size_t len) {
  return MockUnlock::Instance().unlock_key_validate(page, key, signature,
                                                    message, len);
}

void unlock_sh256(const void *data, size_t len, hmac_digest_t *digest) {
  return MockUnlock::Instance().unlock_sha256(data, len, digest);
}
}  // extern "C"
...
TEST_F(OwnershipUnlockTest, UnlockAny) {
  ...
  EXPECT_CALL(unlock_, unlock_key_validate(
                           0,
...
TEST_F(OwnershipUnlockTest, UnlockEndorsed) {
  ...
  EXPECT_CALL(unlock_, unlock_key_validate(
                           ...
                           _, _, _))
      .WillOnce(Return(kHardenedBoolTrue));
  EXPECT_CALL(unlock_, unlock_sha256(_, _, _))
...
TEST_F(OwnershipUnlockTest, UnlockUpdate) {
  ...
  EXPECT_CALL(unlock_, unlock_key_validate(
                           0, static_cast<ownership_key_t>(kOwnershipKeyUnlock),
                           _, _, _))
      .WillOnce(Return(kHardenedBoolTrue));
  EXPECT_CALL(rnd_, Uint32()).WillRepeatedly(Return(5));
  EXPECT_CALL(hdr_, Finalize(_, _, _));
