...
rom_error_t owner_block_flash_apply(const owner_flash_config_t *flash,
  ...
  for (size_t i = 0; i < len; ++i, ++config, crypt += 0x11111111) {
    if (config->start >= start && config->start + config->size <= end) {
      ...
      // region.
      if (((kRomExtAStart >= region_start && kRomExtAStart < region_end) &&
           (kRomExtAEnd > region_start && kRomExtAEnd <= region_end)) ||
          ((kRomExtBStart >= region_start && kRomExtBStart < region_end) &&
           (kRomExtBEnd > region_start && kRomExtBEnd <= region_end))) {
