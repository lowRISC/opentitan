...
enum {
  ...
  // The number of bits in the strike mask is fixed to 256. Each bit in the
  // strike mask corresponds to a `uint32_t` word.
  kExpectedStrikeBitCount = 256,
...
rom_error_t isfb_boot_request_process(const manifest_ext_isfb_t *ext,
  ...
  *checks_performed_count = UINT32_MAX;
  ...
  // There are in total 256 bits in the strike mask. Each bit corresponds to a
  // `uint32_t` word.
  ...
  uint32_t *strike_word = (uint32_t *)data;
  size_t i;
  for (i = 0; i < ARRAYSIZE(data); ++i, ++strike_word) {
  ...
  static_assert(sizeof(data) == KProductExprMaxCountBytes,
