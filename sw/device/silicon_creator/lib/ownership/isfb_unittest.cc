...
class IsfbTest : public rom_test::RomTest {
 protected:
  ...
  void ExpectFlashConfiguration() {
    EXPECT_CALL(flash_ctrl_, InfoType0ParamsBuild(0, 0, _))
        .WillOnce(Return(kErrorOk));
    EXPECT_CALL(flash_ctrl_, InfoPermsSet(_, FlashPerms(kMultiBitBool4True,
                                                        kMultiBitBool4True,
                                                        kMultiBitBool4False)))
        .Times(1);
    EXPECT_CALL(flash_ctrl_,
                InfoCfgSet(_, FlashCfg(kMultiBitBool4False, kMultiBitBool4True,
                                       kMultiBitBool4False)))
        .Times(1);
  }

  void ExpectInfoRead(const std::vector<uint32_t> &data, rom_error_t error) {
    EXPECT_CALL(flash_ctrl_, InfoRead(_, /*offset=*/0, data.size(), _))
  ...
  void StrikeFlashWords(uint32_t strike_count, std::vector<uint32_t> &words) {
    EXPECT_LE(strike_count, kStrikeCntMax);
    std::fill_n(words.begin(), strike_count, 0);
  }

  void ProductExpressionInit(const manifest_ext_isfb_t &ext,
                             std::vector<uint32_t> &words) {
    EXPECT_LE(ext.product_expr_count, kProductExprCntMax);

    enum {
      // Copied  from `sw/device/silicon_creator/lib/ownership/isfb.c`.
      kIsfbWordOffsetStartProductExpr = 256,
    };
    for (size_t i = 0, j = kIsfbWordOffsetStartProductExpr;
         i < ext.product_expr_count; ++i, ++j) {
      words[j] = ext.product_expr[i].value;
    }
  }

  std::vector<uint32_t> IsfbInfoPageBuild(uint32_t strike_count,
                                          const manifest_ext_isfb_t &ext) {
    std::vector<uint32_t> words(kFlashPageWords,
                                std::numeric_limits<uint32_t>::max());
    StrikeFlashWords(strike_count, words);
    ProductExpressionInit(ext, words);
    return words;
  }
...
TEST_F(IsfbTest, AntiRollbackBootFailure) {
  ...
  // failure whic is mapped to the `kErrorOwnershipISFBStrikeMask` error code.
  ExpectInfoRead(IsfbInfoPageBuild(kExpectedStrikeCount, kManifestExtGood),
                 kErrorOk);
...
TEST_F(IsfbTest, ProductAssociationError) {
  ...
  ExpectInfoRead(IsfbInfoPageBuild(/*strike_count=*/1, kManifestExtBadProdExpr),
                 kErrorOk);
