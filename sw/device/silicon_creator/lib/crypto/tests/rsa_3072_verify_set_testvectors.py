...
def rsa_3072_int_to_words(x):
    '''Convert a 3072-bit integer to a list of 32-bit integers (big-endian).'''
    out = []
    for i in range(RSA_3072_NUMWORDS):
        j = RSA_3072_NUMWORDS - 1 - i
        out.append((x >> (j * 32)) & (2**32 - 1))
    return out
...
def stringify_test_vectors(testvecs):
    ...
    lines.append(
        'static const size_t RSA_3072_VERIFY_NUM_TESTS = {:d};'.format(
            len(testvecs)))
    lines.append('')
    lines.append('static const rsa_3072_verify_test_vector_t')
    lines.append('    rsa_3072_verify_tests[RSA_3072_VERIFY_NUM_TESTS] = {')
    for tv in testvecs:
