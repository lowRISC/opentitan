---
title: "Reference ROM: ${module} Module"
---

<p style="text-align: right">
Contributors(s):
  <a href="https://github.com/lenary">Sam Elliott</a>,
  <a href="https://github.com/gkelly">Garret Kelly</a>
</p>

<p style="color: red; text-align: right;">
  Status: Draft
</p>

This should be read in conjunction with the [Reference ROM Secure Boot Description][rom-description] and the [Secure Boot][csm-secure-boot] specification.
References to those documents are included.

<!-- Use # for Sections, and ## for Sub-sections. A ToC is generated automatically. -->
${content}

<!-- Links -->
[csm-secure-boot]: {{< relref "doc/security/specs/secure_boot" >}}
[rom-description]: {{< relref "sw/device/silicon_creator/rom/docs" >}}
