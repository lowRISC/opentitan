...
dif_result_t dif_otp_ctrl_get_status(const dif_otp_ctrl_t *otp,
  ...
  // Partition status bits are now in separate PARTITION_STATUS_0 register
...
#error "dif_otp_ctrl does not support this top"
  ...
  // Non-partition status bits remain in main STATUS register
  ...
  if (bitfield_bit32_read(status_code, OTP_CTRL_STATUS_PARTITION_ERROR_BIT)) {
    uint32_t partition_status_code = mmio_region_read32(
        otp->base_addr, OTP_CTRL_PARTITION_STATUS_0_REG_OFFSET);

    // Process partition status bits from PARTITION_STATUS_0 register
    for (int i = 0; i < ARRAYSIZE(kPartitionIndices); ++i) {
      // If the error is not present at all, we clear its cause bit if relevant,
      // and bail immediately.
      if (!bitfield_bit32_read(partition_status_code, kPartitionIndices[i])) {
  ...
  // Process non-partition status bits from main STATUS register
  for (int i = 0; i < ARRAYSIZE(kStatusIndices); ++i) {
