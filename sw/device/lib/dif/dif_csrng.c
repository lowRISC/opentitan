...
dif_csrng_result_t dif_csrng_get_internal_state(
  ...
  reg = bitfield_bit32_write(0, CSRNG_HALT_MAIN_SM_HALT_MAIN_SM_BIT, true);
  mmio_region_write32(csrng->params.base_addr, CSRNG_HALT_MAIN_SM_REG_OFFSET,
                      reg);

  do {
    reg = mmio_region_read32(csrng->params.base_addr,
                             CSRNG_MAIN_SM_STS_REG_OFFSET);
  } while (!bitfield_bit32_read(reg, CSRNG_MAIN_SM_STS_MAIN_SM_STS_BIT));
  ...
  // After reading the state, clear the state machine halt bit, and wait for the
  // status to resume.
  reg = bitfield_bit32_write(0, CSRNG_HALT_MAIN_SM_HALT_MAIN_SM_BIT, false);
  mmio_region_write32(csrng->params.base_addr, CSRNG_HALT_MAIN_SM_REG_OFFSET,
                      reg);
