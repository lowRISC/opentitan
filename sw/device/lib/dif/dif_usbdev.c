...
dif_result_t dif_usbdev_configure(const dif_usbdev_t *usbdev,
  ...
  if (!is_valid_toggle(config.have_differential_receiver) ||
      !is_valid_toggle(config.single_ended_tx) ||
  ...
  if (config.have_differential_receiver == kDifToggleEnabled) {
    phy_config_val = bitfield_field32_write(
        ...
        (bitfield_field32_t){
            .mask = 1,
            .index = USBDEV_PHY_CONFIG_RX_SINGLE_ENDED_BIT,
        },
        1);
  }

  if (config.single_ended_tx == kDifToggleEnabled) {
    phy_config_val = bitfield_field32_write(
        phy_config_val,
        (bitfield_field32_t){
            .mask = 1,
            .index = USBDEV_PHY_CONFIG_TX_SINGLE_ENDED_BIT,
        },
        1);
  }
