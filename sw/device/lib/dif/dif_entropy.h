...
typedef enum dif_entropy_mode {
  ...
  /**
   * The linear feedback shift register mode.
   *
   * This mode is digital, and as such is only pseudo-random
   */
  kDifEntropyModeLfsr,
} dif_entropy_mode_t;
...
/**
 * An entropy source health test mode.
 *
 * Each mode has a separate set of registers for configuring how tests are
 * performed and their failure criteria.
 */
typedef enum dif_entropy_test_mode {
  /**
   * A FIPS-compliant mode.
   */
  kDifEntropyTestModeFips,

  /**
   * A bypass mode to ignore any certification-specific settings.
   */
  kDifEntropyTestModeBypass,
} dif_entropy_test_mode_t;
...
typedef struct dif_entropy_config {
  ...
  /**
   * The mode to perform health tests in.
   *
   * Among other things, this sets the type of conditioning performed on data,
   * and which registers the health test parameters below are written to.
   */
  // Open Question: Maybe make this and the u16 fields below a separate
  // configuration??
  dif_entropy_test_mode_t test_mode;
 ...
 */
// Open Question: I'm not excited by this struct, but I really don't want to
// have one field per datum...
typedef struct dif_entropy_test_stats {
   ...
   * first array element will be the number of fails, and the second will be
   * zeroed.
   */
...
typedef enum dif_entropy_irq {
  ...
  /**
   * Indicates that an internal error occured in the FIFO.
   */
  kDifEntropyIrqFifoError,
} dif_entropy_irq_t;
 ...
 * will not resume until `dif_entropy_override_reset()` is called.
 *
 * @param entropy An entropy source handle.
 ...
 */
DIF_WARN_UNUSED_RESULT
dif_entropy_result_t dif_entropy_override_reconnect(
