...
dif_result_t dif_clkmgr_gateable_clock_get_enabled(
  ...
  OT_UNREACHABLE();
#else
  // NOTE: the code below fails to compile on tops with no gateable clocks,
  //  where CLKMGR_CLK_ENABLES_REG_OFFSET is not defined.
...
dif_result_t dif_clkmgr_hintable_clock_get_enabled(
  ...
  OT_UNREACHABLE();
#else
  // NOTE: the code below fails to compile on tops with no hintable clocks,
  //  where CLKMGR_CLK_HINTS_STATUS_REG_OFFSET is not defined.
...
#else
  // NOTE: the code below fails to compile on tops with no hintable clocks,
  //  where CLKMGR_CLK_HINTS_REG_OFFSET is not defined.
