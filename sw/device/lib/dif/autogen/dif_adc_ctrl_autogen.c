...
dif_result_t dif_adc_ctrl_init_from_dt(const dt_adc_ctrl_t *dt,
                                       dif_adc_ctrl_t *adc_ctrl) {
  if (adc_ctrl == NULL || dt == NULL) {
    return kDifBadArg;
  }

  adc_ctrl->base_addr = mmio_region_from_addr(
      dt_adc_ctrl_reg_block(dt, kDtAdcCtrlRegBlockDefault));
...
dif_result_t dif_adc_ctrl_alert_force(const dif_adc_ctrl_t *adc_ctrl,
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_ALERT_TEST_REG_OFFSET,
                      alert_test_reg);
...
static bool adc_ctrl_get_irq_bit_index(dif_adc_ctrl_irq_t irq,
  ...
  switch (irq) {
    case kDtAdcCtrlIrqMatchPending:
...
dif_result_t dif_adc_ctrl_irq_get_type(const dif_adc_ctrl_t *adc_ctrl,
                                       ...
                                       dif_irq_type_t *type) {
  if (adc_ctrl == NULL || type == NULL || irq == kDtAdcCtrlIrqCount) {
    return kDifBadArg;
  }
...
dif_result_t dif_adc_ctrl_irq_get_state(
  ...
  *snapshot = mmio_region_read32(adc_ctrl->base_addr,
                                 (ptrdiff_t)ADC_CTRL_INTR_STATE_REG_OFFSET);
...
dif_result_t dif_adc_ctrl_irq_acknowledge_state(
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_STATE_REG_OFFSET, snapshot);
...
dif_result_t dif_adc_ctrl_irq_is_pending(const dif_adc_ctrl_t *adc_ctrl,
  ...
  uint32_t intr_state_reg = mmio_region_read32(
      adc_ctrl->base_addr, (ptrdiff_t)ADC_CTRL_INTR_STATE_REG_OFFSET);
...
dif_result_t dif_adc_ctrl_irq_acknowledge_all(const dif_adc_ctrl_t *adc_ctrl) {
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_STATE_REG_OFFSET, UINT32_MAX);
...
dif_result_t dif_adc_ctrl_irq_force(const dif_adc_ctrl_t *adc_ctrl,
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_TEST_REG_OFFSET, intr_test_reg);
...
dif_result_t dif_adc_ctrl_irq_get_enabled(const dif_adc_ctrl_t *adc_ctrl,
  ...
  uint32_t intr_enable_reg = mmio_region_read32(
      adc_ctrl->base_addr, (ptrdiff_t)ADC_CTRL_INTR_ENABLE_REG_OFFSET);
...
dif_result_t dif_adc_ctrl_irq_set_enabled(const dif_adc_ctrl_t *adc_ctrl,
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_ENABLE_REG_OFFSET,
                      intr_enable_reg);
...
dif_result_t dif_adc_ctrl_irq_disable_all(
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_ENABLE_REG_OFFSET, 0u);
...
dif_result_t dif_adc_ctrl_irq_restore_all(
  ...
  mmio_region_write32(adc_ctrl->base_addr,
                      (ptrdiff_t)ADC_CTRL_INTR_ENABLE_REG_OFFSET, *snapshot);
