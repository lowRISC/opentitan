...
void pinmux_init(void) {
  ...
  for (uint32_t i = 0; i < NUM_UART; ++i) {
    ...
    uint32_t reg_value = MIO_32_IOR2_IDX + PINMUX_MIO_PERIPH_INSEL_IDX_OFFSET + 2 * i;
    uint32_t reg_offset = (UART_RX_PERIPH_IDX + i) << 2;
    ...
    reg_value = UART_TX_PERIPH_IDX + PINMUX_PERIPH_OUTSEL_IDX_OFFSET + i;
    reg_offset = (MIO_33_IOR3_IDX + 2 * i) << 2;
