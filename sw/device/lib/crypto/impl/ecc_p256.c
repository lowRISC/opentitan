...
static status_t internal_p256_keygen_finalize(
  ...
  if (private_key->config.hw_backed == kHardenedBoolTrue) {
    ...
    // Verify the checksum going out.
    HARDENED_CHECK_EQ(
        p256_masked_scalar_integrity_checksum_check(&private_scalar),
        kHardenedBoolTrue);
  } else {
    return OTCRYPTO_BAD_ARGS;
  }
...
otcrypto_status_t otcrypto_ecdsa_p256_sign_async_start(
  ...
  if (private_key->config.hw_backed == kHardenedBoolFalse) {
    ...
    private_scalar->checksum =
        p256_masked_scalar_integrity_checksum(private_scalar);
...
otcrypto_status_t otcrypto_ecdh_p256_async_finalize(
  ...
  HARDENED_CHECK_EQ(p256_ecdh_shared_key_integrity_checksum_check(&ss),
                    kHardenedBoolTrue);
