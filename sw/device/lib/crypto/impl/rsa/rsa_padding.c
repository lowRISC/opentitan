 ...
 * @param lhash_data A encoded message hash.
...
static status_t oaep_check_padding_and_find_message_start(
    uint32_t *db, uint32_t *lhash_data, uint32_t digest_wordlen,
    size_t digest_bytelen, unsigned char *encoded_message_bytes,
    size_t db_bytelen, uint32_t *message_start_idx) {
...
status_t rsa_padding_oaep_decode(const otcrypto_hash_mode_t hash_mode,
  ...
  *message_bytelen = 0;
  ...
  // Locate the start of the message in DB = lhash || 0x00..0x00 || 0x01 || M
  // by searching for the 0x01 byte in constant time.
  uint32_t message_start_idx0 = 0;
  uint32_t message_start_idx1 = 0;
  HARDENED_TRY(oaep_check_padding_and_find_message_start(
      db, lhash_data, digest_wordlen, digest_bytelen, encoded_message_bytes,
