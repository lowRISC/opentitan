...
#include "sw/device/lib/crypto/impl/sm2/ecdsa_sm2.h"
...
static const otbn_addr_t kOtbnVarEcdsaM
ode = OTBN_ADDR_T_INIT(sm2_ecdsa, mode);
...
status_t ecdsa_sm2_keygen_start(void) {
...
status_t ecdsa_sm2_sideload_keygen_start(void) {
  // Load the ECDSA/P-256 app. Fails if OTBN is non-idle.
  HARDENED_TRY(otbn_load_app(kOtbnAppEcdsa));
...
status_t ecdsa_sm2_keygen_finalize(sm2_masked_scalar_t *private_key,
                                    sm2_point_t *public_key) {
  // Spin here waiting for OTBN to complete.
  HARDENED_TRY(otbn_busy_wait_for_done());

  // Read the masked private key from OTBN dmem.
  HARDENED_TRY(otbn_dmem_read(ksm2MaskedScalarShareWords, kOtbnVarEcdsaD0,
                              private_key->share0));
  HARDENED_TRY(otbn_dmem_read(ksm2MaskedScalarShareWords, kOtbnVarEcdsaD1,
                              private_key->share1));

  // Read the public key from OTBN dmem.
  HARDENED_TRY(otbn_dmem_read(ksm2CoordWords, kOtbnVarEcdsaX, public_key->x));
  HARDENED_TRY(otbn_dmem_read(ksm2CoordWords, kOtbnVarEcdsaY, public_key->y));
...
status_t ecdsa_sm2_sideload_keygen_finalize(sm2_point_t *public_key) {
  // Spin here waiting for OTBN to complete.
  HARDENED_TRY(otbn_busy_wait_for_done());
 ...
 * @return OK or error.
 */
static status_t set_message_digest(const uint32_t digest[ksm2ScalarWords]) {
  ...
  // 186-5, section B.2.1).
  uint32_t digest_little_endian[ksm2ScalarWords];
  for (size_t i = 0; i < ksm2ScalarWords; i++) {
    digest_little_endian[i] =
        __builtin_bswap32(digest[ksm2ScalarWords - 1 - i]);
  }
  return otbn_dmem_write(ksm2ScalarWords, digest_little_endian,
                         kOtbnVarEcdsaMsg);
}

status_t ecdsa_sm2_sign_start(const uint32_t digest[ksm2ScalarWords],
                               const sm2_masked_scalar_t *private_key) {
  // Load the ECDSA/P-256 app. Fails if OTBN is non-idle.
  HARDENED_TRY(otbn_load_app(kOtbnAppEcdsa));
  ...
  HARDENED_TRY(
      sm2_masked_scalar_write(private_key, kOtbnVarEcdsaD0, kOtbnVarEcdsaD1));

  // Start the OTBN routine.
  return otbn_execute();
}

status_t ecdsa_sm2_sideload_sign_start(
    const uint32_t digest[ksm2ScalarWords]) {
  // Load the ECDSA/P-256 app. Fails if OTBN is non-idle.
  HARDENED_TRY(otbn_load_app(kOtbnAppEcdsa));
...
status_t ecdsa_sm2_sign_finalize(ecdsa_sm2_signature_t *result) {
  // Spin here waiting for OTBN to complete.
  HARDENED_TRY(otbn_busy_wait_for_done());

  // Read signature R out of OTBN dmem.
  HARDENED_TRY(otbn_dmem_read(ksm2ScalarWords, kOtbnVarEcdsaR, result->r));

  // Read signature S out of OTBN dmem.
  HARDENED_TRY(otbn_dmem_read(ksm2ScalarWords, kOtbnVarEcdsaS, result->s));
...
status_t ecdsa_sm2_verify_start(const ecdsa_sm2_signature_t *signature,
                                 const uint32_t digest[ksm2ScalarWords],
  ...
  // Set the signature R.
  HARDENED_TRY(otbn_dmem_write(ksm2ScalarWords, signature->r, kOtbnVarEcdsaR));

  // Set the signature S.
  HARDENED_TRY(otbn_dmem_write(ksm2ScalarWords, signature->s, kOtbnVarEcdsaS));

  // Set the public key x coordinate.
  HARDENED_TRY(otbn_dmem_write(ksm2CoordWords, public_key->x, kOtbnVarEcdsaX));

  // Set the public key y coordinate.
  HARDENED_TRY(otbn_dmem_write(ksm2CoordWords, public_key->y, kOtbnVarEcdsaY));

  // Start the OTBN routine.
  return otbn_execute();
}

status_t ecdsa_sm2_verify_finalize(const ecdsa_sm2_signature_t *signature,
                                    hardened_bool_t *result) {
  // Spin here waiting for OTBN to complete.
  HARDENED_TRY(otbn_busy_wait_for_done());

  // Read x_r (recovered R) out of OTBN dmem.
  uint32_t x_r[ksm2ScalarWords];
  HARDENED_TRY(otbn_dmem_read(ksm2ScalarWords, kOtbnVarEcdsaXr, x_r));

  *result = hardened_memeq(x_r, signature->r, ksm2ScalarWords);
