...
static void context_restore(hmac_ctx_t *ctx) {
  ...
  // `key->key_len` is set to 0 during `ctx` initialization.
  key_write(&ctx->key);
...
static status_t hmac_context_wipe(hmac_ctx_t *ctx) {
  ...
  hardened_memshred(ctx->key.key_block, kHmacMaxBlockWords);
