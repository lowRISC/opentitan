...
static void hmac_halt(void) {
  ...
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_CFG_REG_OFFSET, 0);
  ...
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_WIPE_SECRET_REG_OFFSET,
                   0xffffffff);

  // Disable and clear interrupts. INTR_STATE register is rw1c.
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_INTR_ENABLE_REG_OFFSET,
                   0);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_INTR_STATE_REG_OFFSET,
                   UINT32_MAX);
}
...
static void hmac_init(hardened_bool_t enable_hmac) {
  ...
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_CFG_REG_OFFSET, cfg);

  uint32_t cmd = bitfield_bit32_write(0, HMAC_CMD_HASH_START_BIT, true);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_CMD_REG_OFFSET, cmd);
}
...
void hmac_hmac_init(const hmac_key_t *key) {
  ...
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_0_REG_OFFSET,
                   key->key[0]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_1_REG_OFFSET,
                   key->key[1]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_2_REG_OFFSET,
                   key->key[2]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_3_REG_OFFSET,
                   key->key[3]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_4_REG_OFFSET,
                   key->key[4]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_5_REG_OFFSET,
                   key->key[5]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_6_REG_OFFSET,
                   key->key[6]);
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_KEY_7_REG_OFFSET,
                   key->key[7]);
...
void hmac_update(const uint8_t *data, size_t len) {
  ...
  for (; len != 0 && (uintptr_t)data & 3; --len) {
    abs_mmio_write8(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_MSG_FIFO_REG_OFFSET,
                    *data++);
  }

  for (; len >= sizeof(uint32_t); len -= sizeof(uint32_t)) {
    uint32_t data_aligned = read_32(data);
    abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_MSG_FIFO_REG_OFFSET,
                     data_aligned);
...
void hmac_final(hmac_digest_t *digest) {
  ...
  abs_mmio_write32(TOP_DARJEELING_HMAC_BASE_ADDR + HMAC_CMD_REG_OFFSET, reg);

  do {
    reg = abs_mmio_read32(TOP_DARJEELING_HMAC_BASE_ADDR +
  ...
  for (size_t i = 0; i < ARRAYSIZE(digest->digest); ++i) {
    digest->digest[i] =
        abs_mmio_read32(TOP_DARJEELING_HMAC_BASE_ADDR +
