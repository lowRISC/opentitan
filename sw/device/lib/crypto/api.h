 ...
 * operation
 */
crypto_status_t otcrypto_gcm_ghash(
    const crypto_blinded_key_t *hash_subkey,
 ...
 */
crypto_status_t otcrypto_aes_kwp_decrypt(
    crypto_const_uint8_buf_t wrapped_key,
    const crypto_blinded_key_t *key_kek,
 ...
 *
 * The update operation process the `input_message` using the selected 
 ...
 */
crypto_status_t otcrypto_hash_update(
    hash_context_t *const ctx,
 ...
 */
crypto_status_t otcrypto_sha256_init(hash_context_t *const ctx);
 ...
 */
crypto_status_t otcrypto_sha256_update(
    hash_context_t *const ctx,
    crypto_const_uint8_buf_t input_message);
 ...
 *
 * The final function process the remaining data and copies the final 
 * hash to the `digest` parameter.
 *
 * This API is internal to the generic hash API and should not be 
 * called directly.
 *
 ...
 */
crypto_status_t otcrypto_sha256_final(hash_context_t *const ctx,
						                          crypto_uint8_buf_t *digest);
 ...
 */
crypto_status_t otcrypto_sha384_init(hash_context_t *const ctx);
 ...
 */
crypto_status_t otcrypto_sha384_update(
    hash_context_t *const ctx,
    crypto_const_uint8_buf_t input_message);
 ...
 */
crypto_status_t otcrypto_sha384_final(hash_context_t *const ctx,
						                          crypto_uint8_buf_t *digest);
 ...
 */
crypto_status_t otcrypto_sha512_init(hash_context_t *const ctx);
 ...
 */
crypto_status_t otcrypto_sha512_update(
    hash_context_t *const ctx,
    crypto_const_uint8_buf_t input_message);
 ...
 */
crypto_status_t otcrypto_sha512_final(hash_context_t *const ctx,
						                          crypto_uint8_buf_t *digest);
...
/**
 * Performs the INIT operation for a HMAC function.
 *
...
/**
 * Performs the UPDATE operation for generic HMAC.
 *
 * The update operation process the `input_message` using the selected 
 ...
 */
crypto_status_t otcrypto_hmac_update(
    hmac_context_t *const ctx,
    crypto_const_uint8_buf_t input_message);
 ...
 */
crypto_status_t otcrypto_ecdsa_verify (ecc_public_key_t *public_key,
						                           crypto_const_uint8_buf_t input_message,
						                           ecc_signature_t *signature,
						                           ecc_curve_t *elliptic_curve,
						                           verification_status_t *verification_result);
...
/**
 * Performs the key generation for Ed25519 operation.
 *
...
/**
 * Performs EdDSA digital signature verification for Ed25519.
 *
 * @param public_key Pointer to the unblinded public key struct
 * @param input_message Input message to be signed for verification
 * @param sign_mode Parameter for EdDSA or Hash EdDSA sign mode
...
/**
 * Performs the Key generation for X25519 operation.
 *
...
/**
 * Starts the asynchronous key generation for Ed25519 operation.
 *
 * Initializes OTBN and starts the OTBN routine to compute the private
...
/**
 * Finalizes the asynchronous key generation for Ed25519 operation.
 *
 ...
 *
 * Initializes OTBN and starts the OTBN routine to verify teh 
...
/**
 * Starts the asynchronous key generation for X25519 operation.
 *
 * Initializes OTBN and starts the OTBN routine to compute the private
 * exponent (d) and public key (Q) based on Curve25519. The DRBG state
 * is passed as an input parameter.
...
/**
 * Finalizes the asynchronous key generation for X25519 operation.
 *
 * Returns `kCryptoStatusOK` and copies private key (d) and public key 
 * (Q), if the OTBN status is done, or `kCryptoStatusAsyncIncomplete`
 * if the OTBN is busy or `kCryptoStatusInternalError` if there is an
...
crypto_status_t otcrypto_build_unblinded_key(
    crypto_const_uint8_buf_t plain_key,
    key_mode_t key_mode,
 ...
 */
crypto_status_t otcrypto_build_blinded_key(
    crypto_const_uint8_buf_t plain_key,
    key_mode_t key_mode,
 ...
 * removed the masking and generates an unblinded key struct as output
 * 
...
crypto_status_t otcrypto_unblinded_to_blinded_key(
    const crypto_unblinded_key unblinded_key,
    crypto_blinded_key_t blinded_key);
