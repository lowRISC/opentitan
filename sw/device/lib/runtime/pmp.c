...
// TODO - explanation to why macro magic is necessary (csr must be const expr).
...
// TODO - X macros as suggested by @lenary.
 ...
 * @param address PMP address to be set.
 * @return `pmp_region_configure_result_t`.
 */
pmp_region_configure_result_t pmp_csr_address_write(
    pmp_region_index_t region, pmp_region_address_t address) {
  pmp_region_address_t address_shifted = address >> PMP_ADDRESS_SHIFT;
...
/**
 * Constructs a NAPOT address from the requested conventional address and size.
 *
 ...
 * @return `true` on success, `false` on failure.
 */
static bool pmp_napot_address_construct(pmp_region_address_t address,
                                        uint32_t size,
                                        pmp_region_address_t *napot_address) {
  ...
  // Check if the `size` if Power Of Two.
  ...
  uint32_t size_mask_inverted = ~size_mask;
  if (address == (address & size_mask_inverted)) {
    return kPmpRegionConfigureNapotBadAddress;
  }

  // `size` is Power Of Two, so the invert can be used to clear the size bit of
  // the NAPOT address.
  uint32_t size_inverted = ~size;

  *napot_address = address | size_inverted;
  *napot_address &= size_mask;
...
pmp_region_configure_na4_result_t pmp_region_configure_na4(
  ...
  if (config == NULL) {
    return kPmpRegionConfigureNa4BadArg;
  }
...
pmp_region_configure_napot_result_t pmp_region_configure_napot(
    pmp_region_index_t region, const pmp_region_config_t *config,
    pmp_region_address_t address, uint32_t size) {
  if (config == NULL) {
    return kPmpRegionConfigureNapotBadArg;
  }
...
pmp_region_configure_result_t pmp_region_configure_tor(
  ...
  if (region == 0 && address_start > 0) {
    return kPmpRegionConfigureBadAddress;
  }

  if (region > 0 && !pmp_address_aligned(address_start)) {
    return kPmpRegionConfigureBadAddress;
  }
  ...
  if (region != 0) {
    pmp_region_configure_result_t result =
        pmp_csr_address_write(region - 1, address_start);
    if (result != kPmpRegionConfigureOk) {
      return result;
    }
  }
  ...
  pmp_region_configure_result_t result =
      pmp_csr_address_write(region, address_end);
  if (result != kPmpRegionConfigureOk) {
    return result;
  }
...
pmp_region_configure_result_t pmp_cfg_mode_lock_status_get(
  ...
  if (bitfield_bit32_read(field_value, PMP_CFG_CSR_L)) {
