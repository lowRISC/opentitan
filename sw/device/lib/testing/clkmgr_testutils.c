...
enum {
  // We do not know the exact number of measurablement clocks but
...
dif_result_t initialize_expected_counts(const dif_clkmgr_t *clkmgr) {
  ...
  // Go through all measurablement clocks.
...
status_t clkmgr_testutils_enable_clock_counts_with_expected_thresholds(
  ...
  for (int clk = 0; clk < dt_clkmgr_measurable_clock_count(clkmgr_dt); ++clk) {
