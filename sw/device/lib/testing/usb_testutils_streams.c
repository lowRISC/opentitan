...
static status_t buffer_data_check(usbdev_stream_t *s, const uint8_t *data,
                                  size_t offset, size_t len) {
  uint8_t rxtx_lfsr = s->rxtx_lfsr;
  uint8_t rx_lfsr = s->rx_lfsr;

  const uint8_t *esp = &data[len];
  ...
  s->rx_bytes += len - offset;

  return OK_STATUS();
}

static status_t buffer_ctl_check(usbdev_stream_t *s,
                                 dif_usbdev_rx_packet_info_t packet_info,
                                 const uint8_t *data) {
  if (packet_info.is_setup) {
    // SETUP packets have a fixed length...
    TRY_CHECK(packet_info.length == 8,
              "S%u: Invalid SETUP packet length %u bytes", s->id,
              packet_info.length);
    // ... and specify the direction of the transfer. The Endpoint
    // number should be consistent with that in the SETUP token packet
    // because of the way our host-side code behaves.
    uint8_t bmRequestType = data[0];
    if (bmRequestType & 0x80U) {
      // This is a request for IN data
      // TODO: decide whether the host should specify the transfer length, in
      // time...
      s->tx.ctl_state = kStreamCtlSendData;
    } else {
      // OUT data
      s->tx.ctl_state = kStreamCtlWaitData;
    }
  } else {
    switch (s->tx.ctl_state) {
      case kStreamCtlWaitAck:
        // An acknowledgement should be a ZLP
        TRY_CHECK(!packet_info.length);
        s->tx.ctl_state = kStreamCtlIdle;
        break;

      case kStreamCtlWaitData:
        // Treat this as a normal Rx data packet
        TRY(buffer_data_check(s, data, 0U, packet_info.length));

        // We must send a ZLP to acknowledge receipt of this data packet
        // TODO: more generally we must know how much data we are expecting to
        // receive from the host!
        s->tx.ctl_state = kStreamCtlSendAck;
        break;

      default:
        TRY_CHECK(false, "Unexpected RX data packet");
        break;
    }
  }
  ...
  if (len > 0) {
    ...
    switch (s->xfr_type) {
      case kUsbTransferTypeControl:
        TRY(buffer_ctl_check(s, packet_info, data));
        break;
      ...
      case kUsbTransferTypeBulk: {
        TRY(buffer_data_check(s, data, offset, bytes_read));
...
static status_t buffer_send(usb_testutils_streams_ctx_t *ctx,
                            usbdev_stream_t *s, uint8_t ep,
                            dif_usbdev_buffer_t *buf) {
  usb_testutils_ctx_t *usbdev = ctx->usbdev;

  TRY(dif_usbdev_send(usbdev->dev, ep, buf));

  if (s->xfr_type == kUsbTransferTypeControl) {
    switch (s->tx.ctl_state) {
      case kStreamCtlSendData:
        s->tx.ctl_state = kStreamCtlQueuedData;
        break;
      case kStreamCtlSendAck:
        s->tx.ctl_state = kStreamCtlQueuedAck;
        break;
      default:
        break;
    }
...
static status_t strm_tx_done(void *cb_v, usb_testutils_xfr_result_t result) {
  ...
  // Note: since buffer transmission and completion signaling both occur within
  ...
  // Control Endpoints: advance to next state
  if (s->xfr_type == kUsbTransferTypeControl) {
    switch (s->tx.ctl_state) {
      case kStreamCtlQueuedData:
        s->tx.ctl_state = kStreamCtlWaitAck;
        break;
      case kStreamCtlQueuedAck:
        s->tx.ctl_state = kStreamCtlIdle;
        break;
      default:
        LOG_INFO("S%u: Unexpected ctl_state 0x%x", s->id, s->tx.ctl_state);
        return INTERNAL();
    }
  }
  ...
  if (nqueued) {
    TRY(buffer_send(ctx, s, tx_ep, &ctx->tx_bufs[tx_ep][0u].buf));
  }
...
static status_t strm_rx(void *cb_v, dif_usbdev_rx_packet_info_t packet_info,
  ...
  if (s->xfr_type == kUsbTransferTypeControl) {
    TRY(dif_usbdev_endpoint_out_enable(usbdev->dev, s->rx_ep,
                                       kDifToggleEnabled));
  } else {
...
bool usb_testutils_stream_completed(const usb_testutils_streams_ctx_t *ctx,
  ...
  return (s->tx.bytes >= s->transfer_bytes) &&
         ...
         // Control Endpoints must wait for the final OUT packet to be
         // acknowledged which obviously occurs after all bytes have been
         // successfully counted in both direcions
         (s->tx.ctl_state == kStreamCtlIdle);
}
...
status_t usb_testutils_stream_init(usb_testutils_streams_ctx_t *ctx, uint8_t id,
  ...
  s->xfr_type = xfr_type;
  // Initialize the state of Control streams
  s->tx.ctl_state = kStreamCtlIdle;
  ...
  usb_testutils_out_transfer_mode_t out_mode = kUsbdevOutStream;
  if (s->xfr_type == kUsbTransferTypeControl) {
    out_mode = kUsbdevOutMessage;
  }

  // Set up the endpoint for OUT transfers (FROM host)
  CHECK_STATUS_OK(usb_testutils_out_endpoint_setup(
      ctx->usbdev, ep_out, xfr_type, out_mode, cb, strm_rx, NULL));
...
status_t usb_testutils_stream_service(usb_testutils_streams_ctx_t *ctx,
  ...
  bool tx_more = (s->tx.bytes < s->transfer_bytes);
  switch (s->xfr_type) {
    case kUsbTransferTypeIsochronous:
      // Isochronous streams never cease transmission because packets are
      // expected to get dropped; let the reception side decided test completion
      tx_more = true;
      break;
    case kUsbTransferTypeControl:
      // Attempt to transmit only if we're in the appropriate Control Transfer
      // state...we should either be trying to send IN data or trying to ACK/NAK
      // the reception of OUT data
      switch (s->tx.ctl_state) {
        case kStreamCtlSendAck:
          // We need to be able to send ACK to received OUT packets even when
          // have already sent all of the data to be transmitted
          tx_more = true;
          break;
        case kStreamCtlSendData:
          break;
      ...
      break;
    default:
      // Other streams require no special handling
      break;
  }
  ...
  if (tx_more &&                              // More bytes to transfer?
    ...
    if (dif_result == kDifOk) {
      ...
      if (s->tx.ctl_state == kStreamCtlSendAck) {
        s->tx.ctl_state = kStreamCtlQueuedAck;
      } else {
      ...
      if (nqueued <= 1U) {
        TRY(buffer_send(ctx, s, tx_ep, &buf));
      }
