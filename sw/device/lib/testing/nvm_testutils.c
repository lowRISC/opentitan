...
status_t nvm_testutils_info_page_setup(nvm_info_page_t page,
  ...
  if (p->emulated) {
    ...
    TRY(rram_relocated_region_enable(&rram, p, cfg));
    return OK_STATUS();
  }
...
status_t nvm_testutils_info_page_clear(nvm_info_page_t page,
                                       ...
                                       nvm_page_cfg_t cfg) {
  const rram_ctrl_info_page_t *p = nvm_ctrl_rram_page_info(page);
  // `CreatorReserved0` doesn't actually share its region with anything else
  // (see the comment on `kRramCtrlEmulRegionA` in rram_ctrl.h), but this
  // check still conservatively refuses every emulated page uniformly.
  TRY_CHECK(!p->emulated,
            "page %d is in a shared/relocated mp region (%d); clearing "
            "permissions there would affect other pages sharing it too",
            page,
            p->page_id >= kRramCtrlEmulPageBaseA ? kRramCtrlEmulRegionA
