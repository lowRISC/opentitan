...
// Specify the current file for function point reporting
#define USBUTILS_FUNCPT_FILE USBUTILS_FUNCPT_FILE_USB_TESTUTILS

#undef USBUTILS_FUNCPT
#define USBUTILS_FUNCPT(a, b)
...
void usb_testutils_poll(usb_testutils_ctx_t *ctx) {
  ...
  if ((istate & (1u << kDifUsbdevIrqFrame))) {
    ...
    USBUTILS_FUNCPT(0xF7AE, ctx->frame);
    ctx->got_frame = true;
  }
  ...
  if (istate & (1u << kDifUsbdevIrqPktSent)) {
    ...
    for (unsigned ep = 0; ep < USBDEV_NUM_ENDPOINTS; ep++) {
      if (sentep & (1 << ep)) {
        ...
        // TODO - supply another buffer immediately from a queue, to lessen the
        // latency in responding to 'packet sent?'
  ...
  if (istate & (1u << kDifUsbdevIrqPktReceived)) {
    ...
    while (true) {
      ...
      //      LOG_INFO("recv ep %u callback %p buffer id %u - %u byte(s)",
      //      endpoint,
      //               ctx->rx_callback[endpoint], buffer.id,
      //               packet_info.length);
  ...
  USBUTILS_FUNCPT(0xC1EA, istate);

  // Clear the interrupts that we have accepted
  // TODO - we need a DIF call to clear a set of interrupts efficiently,
  //        in order to avoid race hazards with newly-arriving interrupts
  ...
  // mmio_region_write32(ctx->dev->base_addr, USBDEV_INTR_STATE_REG_OFFSET,
  //                    istate);

  USBUTILS_FUNCPT(0xC1ED, istate);
