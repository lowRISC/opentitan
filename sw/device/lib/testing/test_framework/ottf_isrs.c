...
void ottf_generic_fault_print(uint32_t *exc_info, const char *reason,
  ...
  if (exc_info) {
    ...
    base_printf("\n");
    uint32_t ram_base_addr = dt_sram_ctrl_reg_block(
        &kDtSramCtrl[kDtIndexSramCtrlMain], kDtSramCtrlRegBlockRam);
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  dt_device_id_t devid = dt_plic_id_to_device_id(plic_irq_id);
  if (devid == dt_uart_device_id(&kDtUart[0]) &&
