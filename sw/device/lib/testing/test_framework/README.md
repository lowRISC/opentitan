...
On the _host_ side, two main tools are used to initiate tests on the device. For the DV simulation target, the [dvsim](https://github.com/lowRISC/dvsim) tool is used, while for Verilator and FPGA targets, Bazel (and `opentitantool`) is used.
