 ...
 * ```
 */
#define OTTF_BACKDOOR_VAR(type, name, val) \
  OT_USED OT_SECTION(".data") volatile type name = val;

/**
 * Same as `OTTF_BACKDOOR_VAR` but for DV environments.
 * ```
 */
#define OTTF_BACKDOOR_VAR_DV(type, name, val) \
  OT_USED OT_SECTION(".rodata") volatile type name = val;
