...
// DIF handles for OTTF console communication.
dif_gpio_t ottf_console_gpio;
dif_pinmux_t ottf_console_pinmux;

// Default console.
static ottf_console_t default_console;
// Pointer to main console
...
void ottf_console_init(void) {
  ...
  // Configure TX indicator before UART/SPI.
  ottf_console_spi_setup_tx_indicator(
      &default_console, kOttfTestConfig.console_tx_indicator.enable,
  ...
  ottf_console_setup(&default_console, kOttfTestConfig.console.type,
                     kOttfTestConfig.console.base_addr);
  // For the SPI console, we use the global SPI buffer.
  ottf_console_set_buffering(&default_console,
                             kOttfTestConfig.console.putbuf_buffered,
                             main_spi_buf, kSpiDeviceMaxFramePayloadSizeBytes);
  ottf_console_set_main(&default_console);

  // Initialize/Configure console flow control (if requested).
  if (kOttfTestConfig.enable_uart_flow_control) {
    ottf_console_flow_control_enable(main_console);
  }
}

void ottf_console_set_main(ottf_console_t *console) {
  main_console = console;
  base_set_stdout(ottf_console_buffer_sink(console));
}
...
void ottf_console_setup(ottf_console_t *console, ottf_console_type_t type,
  ...
  switch (type) {
    case kOttfConsoleUart:
      ...
      ottf_console_configure_uart(console, base_addr);
      break;
    case kOttfConsoleSpiDevice:
      if (base_addr == 0) {
        base_addr = TOP_EARLGREY_SPI_DEVICE_BASE_ADDR;
      }
      ottf_console_configure_spi_device(console, base_addr);
      break;
    default:
...
void ottf_console_flow_control_enable(ottf_console_t *console) {
  CHECK(console->type == kOttfConsoleUart,
        "only UART console supports flow control");

  ottf_console_uart_flow_control_enable(console, &ottf_plic);
...
bool ottf_console_flow_control_isr(uint32_t *exc_info) {
  flow_control_irqs += 1;
  return ottf_console_uart_flow_control_isr(main_console);
}
...
status_t ottf_console_flow_control(ottf_console_t *console,
  ...
  CHECK(main_console->type == kOttfConsoleUart,
        "only UART console supports flow control");
...
size_t ottf_console_write(ottf_console_t *console, const char *buf,
                          size_t len) {
  if (console->buffered) {
