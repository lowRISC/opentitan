  ...
  switch (kOttfTestConfig.console.type) {
    case kOttfConsoleUart:
      ...
      if (base_addr == 0) {
        ...
        base_addr = dt_uart_reg_block(&kDtUart[0], kDtUartRegBlockCore);
      }
...
static uint32_t get_flow_control_watermark_plic_id(void) {
  for (size_t i = 0; i < kDtUartCount; i++) {
    const dt_uart_t *uart = &kDtUart[i];
    if (kOttfTestConfig.console.base_addr ==
        dt_uart_reg_block(uart, kDtUartRegBlockCore)) {
      return dt_uart_irq_to_plic_id(uart, kDtUartIrqRxWatermark);
    }
  }
  return dt_uart_irq_to_plic_id(&kDtUart[0], kDtUartIrqRxWatermark);
}

void ottf_console_flow_control_enable(void) {
  CHECK_DIF_OK(dif_rv_plic_init_from_dt(&kDtRvPlic[0], &ottf_plic));
