...
static size_t spi_device_send_frame(ottf_console_t *console, const char *buf,
  ...
  // host there is data in the egress buffer, and then block until the host
  ...
  if (console->data.spi.tx_ready_gpio != kOttfSpiNoTxGpio) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      // i < 4, as there are 4 CSB-toggle events in two standard SPI transfers.
      ...
      // transfers, so we can already de-assert the Tx-Ready indicator GPIO.
      if (i == 0) {
        OT_DISCARD(dif_gpio_write(&ottf_console_gpio,
