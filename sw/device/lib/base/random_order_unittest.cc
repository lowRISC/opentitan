...
static uint32_t rnd(void) { return kRandomWord; }
static uint32_t zero(void) { return 0; }
...
TEST(RandomOrder, InitRndZeroTest) {
  ...
  size_t len = 10;
  random_order_t ctx;
  random_order_init(zero, &ctx, len);
  ctx_check(&ctx, len);
}
