...
static void mmio_region_memcpy32(mmio_region_t base, uint32_t offset,
  ...
  if (misalignment != 0) {
    ...
    size_t realignment = sizeof(uint32_t) - (size_t)misalignment;
    ...
    ptrdiff_t current_word_offset = (ptrdiff_t)offset - misalignment;
  ...
  while (len > 0) {
    ...
    if (from_mmio || bytes_to_copy != sizeof(uint32_t)) {
      ...
      // `offset`.
      current_word = mmio_region_read32(base, (ptrdiff_t)offset);
    }
    ...
    if (from_mmio) {
      ...
      mmio_region_write32(base, (ptrdiff_t)offset, current_word);
    }
