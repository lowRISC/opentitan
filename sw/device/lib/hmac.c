...
void hmac_init(hmac_cfg_t hmac_cfg) {
  ...
  for (int i = 0; i < 8; i++) {
    REG32(HMAC_KEY0(0) + i * sizeof(uint32)) = hmac_cfg.keys[i];
  }
...
void hmac_update(const void *data, usize size_in_bytes) {
  const uint32 *wp = (const uint32 *)data;
  uint32 bytes_per_word = sizeof(uint32) / sizeof(uint8);
  uint32 bytes_left_over = (size_in_bytes % bytes_per_word);
  usize words_remaining = size_in_bytes / bytes_per_word;
...
void hmac_done(uint32 *digest) {
  ...
  for (uint32 i = 0; i < 8; i++) {
    *digest++ = REG32(HMAC_DIGEST0(0) + i * sizeof(uintptr));
  }
}
