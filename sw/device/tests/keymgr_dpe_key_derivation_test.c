...
bool test_main(void) {
  ...
  CHECK_STATUS_OK(keymgr_dpe_testutils_check_state(&keymgr_dpe, 
  ...
  keymgr_dpe_advance_params.slot_policy=0xFFFFFFFF;

  CHECK_STATUS_OK(keymgr_dpe_testutils_advance_state(&keymgr_dpe, 
                  &keymgr_dpe_advance_params));
  CHECK_STATUS_OK(keymgr_dpe_testutils_check_state(&keymgr_dpe, 
                  kDifKeymgrDpeStateAvailable));
  LOG_INFO("KeymgrDpe entered slot 2 State");
