...
// Include OTTF (OpenTitan Test Framework) configuration
...
status_t configure_loose_health_tests(void) {
  ...
  if (is_locked) {
    LOG_ERROR("Entropy source is locked. Cannot configure health tests");
    return INTERNAL();
  }
  ...
  LOG_INFO("Loose health test thresholds configured");
  return OK_STATUS();
}
...
status_t enable_loose_entropy_src_bypass_mode(void) {
  ...
  TRY(dif_entropy_src_is_locked(&entropy_src, &is_locked));
  if (is_locked) {
    LOG_ERROR("Entropy source is locked. Cannot configure ENTROPY_SRC");
    return INTERNAL();
  }
  ...
  // DEBUG
  if (result != kDifOk) {
    LOG_ERROR("dif_entropy_src_configure failed: %d", result);
    return INTERNAL();
  }

  LOG_INFO("ENTROPY_SRC is now enabled in bypass mode");
...
status_t entropy_testutils_error_check_b4_proceeding(void) {
  ...
  TRY(entropy_testutils_error_check(&entropy_src, &csrng, &edn0, &edn1));
  return OK_STATUS();
}
...
status_t start_otbn_program(void) {
  ...
  otbn_randomness_test_start(&otbn, 1);  // iters = 1
...
status_t verify_otbn_hang(void) {
  ...
  const uint32_t kOtbnTimeoutCycles = 1000000;
  bool otbn_busy = true;
  uint32_t timeout_cycles = kOtbnTimeoutCycles;

  dif_otbn_status_t
      otbn_status;  // Declare otbn_status here so it's in scope later.
  ...
  while (timeout_cycles > 0) {
    ...
    if (!otbn_busy) {
      LOG_ERROR("OTBN program completed unexpectedly; expected it to hang");
      return INTERNAL();
    }
    timeout_cycles--;
  }
...
status_t configure_stringent_health_tests(void) {
  ...
  dif_result_t result =
      dif_entropy_src_health_test_configure(&entropy_src, repcnt_test_config);
  if (result != kDifOk) {
    LOG_ERROR("Failed to configure Repetition Count Test: %d", result);
    return INTERNAL();
  }
  ...
  result =
      dif_entropy_src_health_test_configure(&entropy_src, adaptp_test_config);
  if (result != kDifOk) {
    LOG_ERROR("Failed to configure Adaptive Proportion Test: %d", result);
    return INTERNAL();
  }
  ...
  result =
      dif_entropy_src_health_test_configure(&entropy_src, bucket_test_config);
  if (result != kDifOk) {
    LOG_ERROR("Failed to configure Bucket Test: %d", result);
    return INTERNAL();
  }
  ...
  result =
      dif_entropy_src_health_test_configure(&entropy_src, markov_test_config);
  if (result != kDifOk) {
    LOG_ERROR("Failed to configure Markov Test: %d", result);
    return INTERNAL();
  }
...
status_t verify_recoverable_alert(void) {
  ...
  if (result != kDifOk) {
    LOG_ERROR("Failed to get recoverable alerts: %d", result);
    return INTERNAL();
  }
  ...
  if (alerts != 0) {
    ...
    if (alerts & kEsThreshCfgAlertBit) {
      LOG_ERROR("ES_THRESH_CFG_ALERT is asserted");
      return INTERNAL();
    } else {
    ...
    if (alerts & kEsMainSmAlertBit) {
      LOG_INFO("ES_MAIN_SM_ALERT is asserted as expected");
    } else {
      LOG_ERROR("ES_MAIN_SM_ALERT is not asserted when expected");
      return INTERNAL();
    }

    // Clear the recoverable alerts.
    result = dif_entropy_src_clear_recoverable_alerts(&entropy_src, alerts);
    if (result != kDifOk) {
      LOG_ERROR("Failed to clear recoverable alerts: %d", result);
      return INTERNAL();
    }

    // Verify that the alerts have been cleared.
    result = dif_entropy_src_get_recoverable_alerts(&entropy_src, &alerts);
    if (result != kDifOk) {
      LOG_ERROR("Failed to get recoverable alerts after clearing: %d", result);
      return INTERNAL();
    }
    ...
    if (alerts == 0) {
      ...
      LOG_ERROR("Recoverable alerts not cleared. Alerts: 0x%x", alerts);
      return INTERNAL();
    }

    // Acknowledge the Health Test Failed interrupt.
    result = dif_entropy_src_irq_acknowledge(
        &entropy_src, kDifEntropySrcIrqEsHealthTestFailed);
    if (result != kDifOk) {
      LOG_ERROR("Failed to acknowledge IRQ: %d", result);
      return INTERNAL();
    }
...
status_t execute_test(void) {
  ...
  // entropy (e.g. AES) to test EDN0 Step 7: Verify the entropy consuming
