...
status_t cryptolib_sca_ed25519_sign_impl(
  ...
  // Set up the private key with the input scalar as seed.
  uint32_t private_key_data[ED25519_CMD_SCALAR_BYTES / sizeof(uint32_t)];
  memset(private_key_data, 0, sizeof(private_key_data));
  memcpy(private_key_data, uj_input.scalar, ED25519_CMD_SCALAR_BYTES);
