...
status_t handle_aes_sca_key_set(ujson_t *uj) {
  penetrationtest_aes_sca_key_t uj_key_data;
  TRY(ujson_deserialize_penetrationtest_aes_sca_key_t(uj, &uj_key_data));
...
static status_t aes_send_ciphertext(bool only_first_word, ujson_t *uj) {
  ...
  penetrationtest_aes_sca_ciphertext_t uj_output;
  ...
  RESP_OK(ujson_serialize_penetrationtest_aes_sca_ciphertext_t, uj, &uj_output);
  return OK_STATUS(0);
}
...
status_t handle_aes_sca_single_encrypt(ujson_t *uj) {
  penetrationtest_aes_sca_text_t uj_data;
  TRY(ujson_deserialize_penetrationtest_aes_sca_text_t(uj, &uj_data));
...
status_t handle_aes_sca_batch_encrypt(ujson_t *uj) {
  penetrationtest_aes_sca_data_t uj_data;
  TRY(ujson_deserialize_penetrationtest_aes_sca_data_t(uj, &uj_data));
 ...
 * @param uj The received uJSON data.
 */
status_t aes_sca_fvsr_key_batch_generate(
    penetrationtest_aes_sca_data_t uj_data) {
  uint32_t num_encryptions = 0;
  num_encryptions = read_32(uj_data.data);
...
status_t handle_aes_sca_seed_lfsr(ujson_t *uj) {
  penetrationtest_aes_sca_lfsr_t uj_lfsr_data;
  TRY(ujson_deserialize_penetrationtest_aes_sca_lfsr_t(uj, &uj_lfsr_data));
...
status_t handle_aes_sca_init(ujson_t *uj) {
  ...
  penetrationtest_aes_sca_fpga_mode_t uj_data;
  TRY(ujson_deserialize_penetrationtest_aes_sca_fpga_mode_t(uj, &uj_data));
