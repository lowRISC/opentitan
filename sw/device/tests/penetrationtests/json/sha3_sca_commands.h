...
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaMasks, penetrationtest_sha3_sca_masks_off_t, SHA3_SCA_MASKS);

#define SHA3_SCA_LFSR(field, string) \
    field(seed, uint8_t, SHA3SCA_CMD_MAX_LFSR_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaLfsr, penetrationtest_sha3_sca_lfsr_t, SHA3_SCA_LFSR);

#define SHA3_SCA_DATA(field, string) \
    field(data, uint8_t, SHA3SCA_CMD_MAX_DATA_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaData, penetrationtest_sha3_sca_data_t, SHA3_SCA_DATA);

#define SHA3_SCA_FPGA_MODE(field, string) \
    field(fpga_mode, uint8_t)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaFpgaMode, penetrationtest_sha3_sca_fpga_mode_t, SHA3_SCA_FPGA_MODE);

#define SHA3_SCA_MSG(field, string) \
    field(msg, uint8_t, SHA3SCA_CMD_MAX_MSG_BYTES) \
    field(msg_length, size_t)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaMsg, penetrationtest_sha3_sca_msg_t, SHA3_SCA_MSG);

#define SHA3_SCA_STATUS(field, string) \
    field(status, uint8_t)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaStatus, penetrationtest_sha3_sca_status_t, SHA3_SCA_STATUS);

#define SHA3_SCA_BATCH_DIGEST(field, string) \
    field(batch_digest, uint8_t, SHA3SCA_CMD_MAX_BATCH_DIGEST_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestSha3ScaBatchDigest, penetrationtest_sha3_sca_batch_digest_t, SHA3_SCA_BATCH_DIGEST);
