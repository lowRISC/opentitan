...
UJSON_SERDE_STRUCT(PenetrationtestKmacScaKey, penetrationtest_kmac_sca_key_t, KMAC_SCA_KEY);

#define KMAC_SCA_LFSR(field, string) \
    field(seed, uint8_t, KMACSCA_CMD_MAX_LFSR_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestKmacScaLfsr, penetrationtest_kmac_sca_lfsr_t, KMAC_SCA_LFSR);

#define KMAC_SCA_FPGA_MODE(field, string) \
    field(fpga_mode, uint8_t)
UJSON_SERDE_STRUCT(PenetrationtestKmacScaFpgaMode, penetrationtest_kmac_sca_fpga_mode_t, KMAC_SCA_FPGA_MODE);

#define KMAC_SCA_DATA(field, string) \
    field(data, uint8_t, KMACSCA_CMD_MAX_DATA_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestKmacScaData, penetrationtest_kmac_sca_data_t, KMAC_SCA_DATA);

#define KMAC_SCA_MSG(field, string) \
    field(msg, uint8_t, KMACSCA_CMD_MAX_MSG_BYTES) \
    field(msg_length, size_t)
UJSON_SERDE_STRUCT(PenetrationtestKmacScaMsg, penetrationtest_kmac_sca_msg_t, KMAC_SCA_MSG);

#define KMAC_SCA_BATCH_DIGEST(field, string) \
    field(batch_digest, uint8_t, KMACSCA_CMD_MAX_BATCH_DIGEST_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestKmacScaBatchDigest, penetrationtest_kmac_sca_batch_digest_t, KMAC_SCA_BATCH_DIGEST);
