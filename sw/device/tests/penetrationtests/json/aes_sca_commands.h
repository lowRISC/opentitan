...
UJSON_SERDE_STRUCT(PenetrationtestAesScaKey, penetrationtest_aes_sca_key_t, AES_SCA_KEY);

#define AES_SCA_DATA(field, string) \
    field(data, uint8_t, AESSCA_CMD_MAX_DATA_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestAesScaData, penetrationtest_aes_sca_data_t, AES_SCA_DATA);

#define AES_SCA_TEXT(field, string) \
    field(text, uint8_t, AESSCA_CMD_MAX_DATA_BYTES) \
    field(text_length, size_t)
UJSON_SERDE_STRUCT(PenetrationtestAesScaText, penetrationtest_aes_sca_text_t, AES_SCA_TEXT);

#define AES_SCA_LFSR(field, string) \
    field(seed, uint8_t, AESSCA_CMD_MAX_LFSR_BYTES)
UJSON_SERDE_STRUCT(PenetrationtestAesScaLfsr, penetrationtest_aes_sca_lfsr_t, AES_SCA_LFSR);

#define AES_SCA_CIPHERTEXT(field, string) \
    field(ciphertext, uint8_t, AESSCA_CMD_MAX_MSG_BYTES) \
    field(ciphertext_length, uint32_t)
UJSON_SERDE_STRUCT(PenetrationtestAesScaCiphertext, penetrationtest_aes_sca_ciphertext_t, AES_SCA_CIPHERTEXT);

#define AES_SCA_FPGA_MODE(field, string) \
    field(fpga_mode, uint8_t)
UJSON_SERDE_STRUCT(PenetrationtestAesScaFpgaMode, penetrationtest_aes_sca_fpga_mode_t, AES_SCA_FPGA_MODE);
// clang-format on
