...
/*   dif_kmac_config_t kmac_config; */
/*   bool expect_hash_cnt_reset; */
/* } test_case_t; */
...
status_t test_kmac_config(dif_kmac_config_t *test_case) {
  ...
  CHECK(test_case->entropy_hash_threshold <= kHashCntMax,
  ...
  CHECK_DIF_OK(dif_kmac_configure(&kmac, *test_case));
  ...
  CHECK_DIF_OK(dif_kmac_get_hash_counter(&kmac, &hash_count));
  CHECK(hash_count == 0, "Failed to clear hash counter");
  ...
  for (size_t i = 0; i < kKmacIterations; i++) {
    ...
    CHECK_DIF_OK(dif_kmac_mode_kmac_start(
        &kmac, &op_state, kKmacTestVector.mode, kKmacTestVector.digest_len,
        &kKmacTestVector.key, &cust_str));

    CHECK_DIF_OK(dif_kmac_absorb(&kmac, &op_state, kKmacTestVector.message,
                                 kKmacTestVector.message_len,
                                 /* processed=*/NULL));

    // All squeeze operations in this test are expected to succeed
    uint32_t out[kKmacDigestLenMax];
    CHECK_DIF_OK(dif_kmac_squeeze(
        &kmac, &op_state, out, kKmacTestVector.digest_len, /*processed=*/NULL));
    ...
    if (irq_err_pending) {
      ...
      CHECK(!irq_err_pending, "KMAC error occurred: 0x%x", err_status);
    }
    ...
    CHECK(expected_hash_count == hash_count,
