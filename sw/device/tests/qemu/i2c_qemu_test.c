 ...
 * `kEchoDeviceAddr` on each i2c bus.
 *
...
status_t i2c_ds1338_test(dif_i2c_t *i2c) {
  uint8_t resp[56] = {0};
  uint8_t byte = kDsNvramBaseAddr;

  /* Check NVRAM initially contains all zeroes */
  TRY(i2c_testutils_write(i2c, kDsDeviceAddr, 1, &byte, true));
  TRY(i2c_testutils_read(i2c, kDsDeviceAddr, ARRAYSIZE(resp), resp, 10));
  for (size_t i = 0; i < ARRAYSIZE(resp); i++) {
    TRY_CHECK(resp[i] == 0, "NVRAM should be all zeroes");
  }

  /*
   * Write arbitrary data to NVRAM,
   * with the first byte setting the pointer to
   * the start of NVRAM.
   */
  uint8_t payload[56 + 1] = {0};
  payload[0] = byte;
...
static status_t i2c_echo_test(dif_i2c_t *i2c) {
  ...
  TRY(dif_i2c_acquire_byte(i2c, &byte, &sig));
  TRY_CHECK((byte >> 1) == kI2cDeviceTargetAddr, "Unexpected address");
  TRY_CHECK((byte & 1) == 0, "Expected write command, got read");
  TRY_CHECK(sig == kDifI2cSignalStart, "Expected START signal");

  TRY(dif_i2c_acquire_byte(i2c, &byte, &sig));
  TRY_CHECK(byte == kPayloadData1, "Unexpected payload");

  TRY(dif_i2c_acquire_byte(i2c, &byte, &sig));
  TRY_CHECK(byte == kPayloadData2, "Unexpected payload");
