...
/**
 * Compare `num_words` of `got` against `expected` words.
 *
 * Prints differences between `got` and `expected` before generating a test
 * assertion if a mismatch is found.
 *
 * @param got Input buffer.
 * @param expected Buffer containing expected values.
 * @param num_words Number of words to compare.
 */
static void check_buffer(const uint32_t *got, const uint32_t *expected,
                         size_t num_words) {
  if (memcmp(got, expected, num_words * sizeof(uint32_t)) == 0) {
    return;
  }

  for (uint32_t i = 0; i < num_words; ++i) {
    LOG_INFO("[%d] got = 0x%x; expected = 0x%x", i, got[i], expected[i]);
  }
  CHECK(false, "Detected buffer mismatch.");
}
