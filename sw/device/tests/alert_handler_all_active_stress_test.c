...
static void cleanup_after_alert(const alert_config_t *config,
  ...
  if (current_test_phase == TEST_PHASE_ALL_ACTIVE_IRQ) {
    ...
    // In the IRQ phase, trigger a software reset to move to the next alert.
    ...
    LOG_INFO(
        "Saving next alert ID %d to retention SRAM. Triggering software "
        "reset...",
        next_alert_id_to_save);
...
static void check_alert_all_active_reset(uint32_t alert_id) {
  ...
  if (config->skip_test) {
    ...
    do {
      next_alert_id_to_save++;
             ...
             kAlertConfigs[next_alert_id_to_save].name == NULL);
    retention_sram_get()->creator.reserved[2] = next_alert_id_to_save;
    return;
  }
  ...
  // Find the next valid alert ID to test, and save it to retention SRAM
  // before we trigger the reset.
  uint32_t next_alert_id_to_save = alert_id;
  do {
    next_alert_id_to_save++;
  } while (next_alert_id_to_save < ALERT_HANDLER_PARAM_N_ALERTS &&
...
bool test_main(void) {
  ...
  LOG_INFO(">> TEST_MAIN: Read state_id=0x%x", state_id);

  // Recommendation for Option 1: Check retention SRAM scrambling status.
  dif_sram_ctrl_t ret_sram_ctrl;
  CHECK_DIF_OK(dif_sram_ctrl_init(
      mmio_region_from_addr(TOP_EARLGREY_SRAM_CTRL_RET_AON_REGS_BASE_ADDR),
      &ret_sram_ctrl));
  dif_sram_ctrl_status_bitfield_t sram_status;
  CHECK_DIF_OK(dif_sram_ctrl_get_status(&ret_sram_ctrl, &sram_status));
  LOG_INFO(">> TEST_MAIN: Retention SRAM status=0x%x (ScrKeyValid=%d)",
           sram_status, (sram_status & kDifSramCtrlStatusScrKeyValid) != 0);
  ...
  if (state_id != TEST_PHASE_ALL_ACTIVE_RESET) {
    ...
    LOG_INFO(">> TEST_MAIN: ALL Active IRQ Phase (AAIP) complete.");
    LOG_INFO(
        ">> Transitioning to Phase 2 All Active Reset Phase (AARP) (Reset "
        "Phase).");
    CHECK_STATUS_OK(ret_sram_testutils_counter_set(
    ...
    CHECK_STATUS_OK(
        ret_sram_testutils_counter_set(RET_SRAM_SLOT_ALL_ACTIVE_IRQ_IDX, 0));
    CHECK_STATUS_OK(
        ret_sram_testutils_counter_set(RET_SRAM_SLOT_ALL_ACTIVE_RESET_IDX, 0));
    CHECK_DIF_OK(dif_rstmgr_software_device_reset(&rstmgr));
    busy_spin_micros(1000);
    return true;
  } else {
    ...
    // Handle subsequent HW Request or initial SW Reset
    if (reset_info & kDifRstmgrResetInfoHwReq) {
    ...
    } else if (reset_info & kDifRstmgrResetInfoSw) {
      LOG_INFO(
