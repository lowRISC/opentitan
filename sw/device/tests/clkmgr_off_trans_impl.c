...
static const dt_clkmgr_t kTestClkmgr = (dt_clkmgr_t)0;
...
static const dt_pwrmgr_t kTestPwrmgr = (dt_pwrmgr_t)0;
static const dt_rstmgr_t kTestRstmgr = (dt_rstmgr_t)0;
...
bool execute_off_trans_test(test_trans_block_t block) {
  ...
  CHECK_DIF_OK(dif_pwrmgr_init_from_dt(kTestPwrmgr, &pwrmgr));

  // Initialize aon timer.
  CHECK_DIF_OK(dif_aon_timer_init_from_dt(kTestAonTimer, &aon_timer));
  ...
  for (test_trans_block_t trans = kTestTransFirst; trans < kTestTransCount;
    ...
    switch (trans) {
      ...
      case kTestTransKmac:
        ...
        info[trans].csr_offset = addr_as_offset(kmac.base_addr, KMAC_STATUS_REG_OFFSET);
      ...
      case kTestTransOtbn:
        ...
        info[trans].csr_offset = addr_as_offset(otbn.base_addr, OTBN_ERR_BITS_REG_OFFSET);
    ...
    CHECK_DIF_OK(dif_clkmgr_find_hintable_clock(&clkmgr, inst, &info[trans].clock));
  }
