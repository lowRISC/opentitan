...
static void handle_uart_isr(const dif_rv_plic_irq_id_t interrupt_id) {
  ...
  switch (interrupt_id) {
    case kTopDarjeelingPlicIrqIdUart0RxOverflow:
      ...
      break;
    case kTopDarjeelingPlicIrqIdUart0TxEmpty:
...
void ottf_external_isr(void) {
  ...
  top_darjeeling_plic_peripheral_t peripheral_id =
      top_darjeeling_plic_interrupt_for_peripheral[interrupt_id];
  CHECK(peripheral_id == kTopDarjeelingPlicPeripheralUart0,
...
static void plic_configure_irqs(dif_rv_plic_t *plic) {
  ...
  CHECK_DIF_OK(dif_rv_plic_irq_set_priority(
      plic, kTopDarjeelingPlicIrqIdUart0RxOverflow, kDifRvPlicMaxPriority));

  CHECK_DIF_OK(dif_rv_plic_irq_set_priority(
      plic, kTopDarjeelingPlicIrqIdUart0TxEmpty, kDifRvPlicMaxPriority));
  ...
  // Enable IRQs in PLIC
  CHECK_DIF_OK(
      dif_rv_plic_irq_set_enabled(plic, kTopDarjeelingPlicIrqIdUart0RxOverflow,
...
bool test_main(void) {
  ...
  mmio_region_t uart_base_addr =
      mmio_region_from_addr(TOP_DARJEELING_UART0_BASE_ADDR);
