 ...
 */
static bool test_kmac_with_sideloaded_key(void) {
  ...
  // DV SYNC MESSAGE
  LOG_INFO("KeymgrDpe generated HW output for Kmac from the CreatorRootKey");
  ...
  CHECK_STATUS_OK(
      keymgr_dpe_testutils_generate_key(&keymgr_dpe, &sideload_params));
  LOG_INFO("KeymgrDpe regenerated HW output for Kmac from the CreatorRootKey");
...
bool test_main(void) {
  ...
  LOG_INFO("KeymgrDpe is ready for the KMAC test!");

  // Run the AES test.
  return test_kmac_with_sideloaded_key();
}
