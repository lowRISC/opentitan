...
//  ... Bus Reset ->
// Power On Reset -> Configuration ->
//           [from prev phase] ... -> DPI drops SOF -> Suspend -> Actvivate AON
...
////////////////////////////////////////////////////////////////////////////////
static void sense_report(usbdev_suspend_ctx_t *ctx) {
  while (true) {
  ...
  switch (ctx->test_state) {
    case kSuspendStateActivatedAON:
      ...
      if (ctx->wake_status.active) {
        ...
        // Enter low power mode; note on FPGA at present the only wakeup
        // condition that we can produce is a disconnection of the USB; we thus
        // skip over the sleep state for other Resume|Reset phases and exercise
        // only the USB AON Wake module.
        ...
        switch (ctx->test_phase) {
          ...
          case kSuspendPhaseDeepDisconnect:
            if (!host_disconnects) {
              LOG_INFO("auto-skipping WFI (host does not support Disconnects");
              can_awaken = false;
            }
            break;
          ...
          default:
            ...
            if (!host_resumes) {
              LOG_INFO(
                  "auto-skipping WFI (host does not support Resume Signaling");
              can_awaken = false;
            }
            break;
        }
        ...
        if (can_awaken) {
          if (ctx->test_phase == kSuspendPhaseDeepResume ||
            ...
            // becausse otherwise when we return to the active state, the
...
static status_t phase_run(usbdev_suspend_ctx_t *ctx) {
  ...
  // Advance to next phase, unless this we have just completed the final phase
  // or the test has failed.
  switch (ctx->test_state) {
    case kSuspendStatePowerOnReset:
    case kSuspendStateBusReset:
    case kSuspendStateNextPhase: {
