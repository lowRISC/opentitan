 ...
 */
#define TIMEOUT_RESUME_MISSED 40U * 1000U

#define TIMEOUT_RESET_MISSED 60U * 1000U

#define TIMEOUT_WAKEUP_RESUME 30000u
#define TIMEOUT_FINISH_MISSED 2000u

#define TIMEOUT_AON_RESPONSE 5000u

#define TIMEOUT_SLEEP_FAILED 5000u
...
#define TIMEOUT_PHYS_CONFIG 10U * 1000U * 1000U
...
#define FRAMES_CONFIG_DELAY 2000u

#define FRAMES_SUSPEND_MISSED 2000u
...
#define USB_EVENT_REPORT(s) (void)0
...
static status_t timeout_handle(usbdev_suspend_ctx_t *ctx) {
  ...
  switch (ctx->test_state) {
    ...
    case kSuspendStateWaitSuspendTimeout:
      ...
      timeout_frames_set(ctx, TIMEOUT_AON_RESPONSE);
      failed = false;
      break;
...
static status_t phase_start_resume(usbdev_suspend_ctx_t *ctx) {
  ...
  switch (ctx->test_state) {
    ...
    case kSuspendStateBusReset:
      ...
      // has gone wrong.
      if (physical_timings()) {
        timeout_set(ctx, TIMEOUT_PHYS_CONFIG);
      } else {
      ...
      if (ctx->wake_status.active) {
        ...
        if (can_awaken) {
          if (ctx->test_phase == kSuspendPhaseDeepResume ||
            ...
            timeout_frames_set(ctx, TIMEOUT_SLEEP_FAILED);
          } else {
