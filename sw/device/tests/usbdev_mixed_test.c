...
static uint8_t config_descriptors[CFG_DSCR_TOTAL_LEN] = {
    USB_CFG_DSCR_HEAD(CFG_DSCR_TOTAL_LEN, USBUTILS_STREAMS_MAX)
...
bool test_main(void) {
  ...
  CHECK(kDeviceType == kDeviceSimDV || kDeviceType == kDeviceSimVerilator,
        "This test is not expected to run on FPGA platforms at present. It "
        "needs logic on the host side to support non-Bulk Transfer streams.");
