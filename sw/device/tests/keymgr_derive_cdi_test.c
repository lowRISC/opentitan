...
typedef struct keymgr_output {
  ...
  // This is only a test, thus unmasking the key manager output has no security
  // implications, but of course, this must *never* be done in any production
  // code.
  // Why is this necessary?
  // The testplan calls for a verification that the same outputs are generated
  // when the device configuration and inputs are have not changed. This is done
  // via a software reset. However, the masking entropy might not be the same
  // before and after a reset, hence checking whether a output has not changed
  // after a reset, the output must be unmasked.
 ...
 */
static void derive_identity(const char *state_name, keymgr_output_t *identity) {
 ...
 */
static void derive_sw_key(const char *state_name, keymgr_output_t *key) {
...
static void derive_keys(const char *state_name,
  ...
  if (prev_outputs) {
    CHECK_ARRAYS_NE(prev_outputs->identity.unmasked,
                    next_outputs->identity.unmasked, kKeymgrOutputSizeWords);
    CHECK_ARRAYS_NE(prev_outputs->sw_key.unmasked,
                    next_outputs->sw_key.unmasked, kKeymgrOutputSizeWords);
  ...
  if (write) {
    ...
    CHECK_ARRAYS_EQ(scratch.identity.unmasked, next_outputs->identity.unmasked,
                    kKeymgrOutputSizeWords);
    CHECK_ARRAYS_EQ(scratch.sw_key.unmasked, next_outputs->sw_key.unmasked,
                    kKeymgrOutputSizeWords);
 ...
 *   `manuf_ft_provision_rma_token_and_personalization` testpoint, otherwise
 *   the keymgr will fail to advance into operational states.
 *
 * The provisioning of the device secrets in the OTP is handled by the keymgr
 * testutils (see `keymgr_testutils_init_nvm_then_reset`).
 *
