...
void _ottf_main(void) {
  ...
  CHECK(bad_bss_index == -1,
        "found non-zero .bss byte at *(0x%08x - 1) == 0x%02x", bss_start_addr,
        (uint32_t)bss[bad_bss_index]);
  CHECK(bad_data_index == -1,
        "found bad .data byte at *(0x%08x - 1) == 0x%02x, expected 0x%02x",
        data_start_addr, (uint32_t)data_init[bad_data_index]);
