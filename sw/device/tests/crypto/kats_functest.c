...
status_t test_stateful_kat_execution(void) {
  ...
  // Verify that some KAT bit was successfully set in the OTBN scratch state
  crypto_state_t current_state = {0};
  TRY(read_state(&current_state));
  TRY_CHECK(memcmp(&initial_state, &current_state, sizeof(crypto_state_t)) != 0,
