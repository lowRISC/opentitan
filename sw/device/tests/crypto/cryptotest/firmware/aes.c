...
status_t handle_aes_gcm_encryption(ujson_t *uj,
                                   ...
                                   otcrypto_const_byte_buf_t aad) {
  // Create buffer to store the input. For encryption, it is the plaintext.
  uint8_t input_buf[uj_data.input_length];
  ...
  size_t ciphertext_blocks = ceil_div(uj_data.input_length, kAesBlockNumBytes);
  uint8_t ciphertext_data[ciphertext_blocks * kAesBlockNumBytes];
  ...
  otcrypto_aes_gcm_tag_len_t tag_len = get_tag_length(tag_num_words);
  ...
  uj_output.output_length = sizeof(ciphertext);
  memcpy(uj_output.oneshot_output, ciphertext.data, uj_output.output_length);

  // Zero out `ciphertext` to mitigate chance of a false positive in the
  // stepwise test
  memset(&ciphertext, 0, sizeof(otcrypto_byte_buf_t));
...
status_t handle_aes_gcm_decryption(ujson_t *uj,
  ...
  // Create buffer to store the input. For decryption, it is the ciphertext.
  uint8_t input_buf[uj_data.input_length];
  memcpy(input_buf, uj_data.input, uj_data.input_length);
  ...
  // Set the plaintext field
  size_t plaintext_blocks = ceil_div(uj_data.input_length, kAesBlockNumBytes);
  uint8_t plaintext_data[plaintext_blocks * kAesBlockNumBytes];
  ...
  // Zero out `plaintext` to mitigate chance of a false positive in the
  // stepwise test
  memset(&plaintext, 0, sizeof(otcrypto_byte_buf_t));
...
status_t handle_aes_gcm(ujson_t *uj) {
  ...
  otcrypto_key_config_t config = {
      ...
      .key_length = uj_data.key_length * sizeof(uint32_t),  // length in bytes
