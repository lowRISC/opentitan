...
enum {
  ...
  /**
   * Bytes in one share of an ECDH/P-256 shared secret (affine point, x || y).
   */
  kP256SharedSecretBytes = 64,
  /**
 ...
 *
 * The caller should ensure at least kP256CoordinateBytes of space are
 * allocated at `ss`.
 *
...
static status_t ecdh_p256(cryptotest_ecdh_private_key_t d,
  ...
  if (priv_status.value == kOtcryptoStatusValueBadArgs) {
    *valid = false;
    memset(ss, 0, kP256CoordinateBytes);
    return OK_STATUS();
  }
  TRY(priv_status);
...
static status_t ecdh_p384(cryptotest_ecdh_private_key_t d,
  ...
  TRY(otcrypto_ecc_p384_point_on_curve(&shared_point_key, &on_curve));
  if (on_curve != kHardenedBoolTrue) {
    LOG_ERROR("ECDH/P-384 shared point is not on the curve");
    return INTERNAL();
  }
...
status_t handle_ecdh(ujson_t *uj) {
  ...
  switch (uj_curve) {
    case kCryptotestEcdhCurveP256: {
      uint32_t shared_secret_words[kP256CoordinateBytes / sizeof(uint32_t)];
      ...
      uj_output.shared_secret_len = kP256CoordinateBytes;
      break;
    }
