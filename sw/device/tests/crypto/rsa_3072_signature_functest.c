...
status_t pkcs1v15_sign_test(void) {
  ...
  TRY(run_rsa_3072_sign(kTestMessage, kTestMessageLen, kOtcryptoRsaPaddingPkcs, sig));
...
status_t pss_sign_test(void) {
  ...
  uint32_t sig[kRsa3072NumWords];
  TRY(run_rsa_3072_sign(kTestMessage, kTestMessageLen, kOtcryptoRsaPaddingPss, sig));

  // Try to verify the signature.
  hardened_bool_t verification_result;
  TRY(run_rsa_3072_verify(kTestMessage, kTestMessageLen, sig, kOtcryptoRsaPaddingPss,
                          &verification_result));
