...
static status_t run_mlkem1024_testvector_decaps_test(void) {
  ...
  memcpy(sk_data + 2 * kSkSWords, kAcvpPkT, sizeof(kAcvpPkT));
  memcpy(sk_data + 2 * kSkSWords + kPkTWords, kAcvpPkRho, sizeof(kAcvpPkRho));
  memcpy(sk_data + 2 * kSkSWords + kPkTWords + kPkRhoWords, kAcvpSkHpk,
         sizeof(kAcvpSkHpk));
  memcpy(sk_data + 2 * kSkSWords + kPkTWords + kPkRhoWords + kSkHpkWords,
         kAcvpSkZ, sizeof(kAcvpSkZ));
