...
def parse_test(raw_data, n, e, t):
    ...
    test["comment"] = "wycheproof test with tcId={:d}, notes={}".format(t["tcId"], ", ".join(notes))
...
def parse_test_vectors(raw_data):
    if raw_data["algorithm"] != "RSASSA-PKCS1-v1_5":
        raise RuntimeError(
            "Unexpected algorithm: {}, expected {}".format(
    ...
    for group in raw_data["testGroups"]:
        if group["sha"] != "SHA-256":
            raise RuntimeError(
                "Unexpected hash function: {}, expected {}".format(group["sha"], "SHA-256")
            )
