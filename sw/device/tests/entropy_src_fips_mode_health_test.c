...
#define OTBN_UNEXPECTED_COMPLETE_VALUE 0x7000A
...
static inline status_t otbn_unexpected_complete_status(void) {
  status_t st;
  st.value = OTBN_UNEXPECTED_COMPLETE_VALUE;
  return st;
}
...
static dif_result_t my_check_locked(const dif_csrng_t *csrng) {
...
dif_result_t my_dif_csrng_configure(const dif_csrng_t *csrng) {
  LOG_INFO("Starting my_dif_csrng_configure");
  if (csrng == NULL) {
    return kDifBadArg;
  }
  DIF_RETURN_IF_ERROR(my_check_locked(csrng));
  ...
  LOG_INFO("Done my_dif_csrng_configure");
  return kDifOk;
}

static dif_entropy_src_config_t my_entropy_testutils_config_default(void) {
...
// Configures the entropy_src using the above default config, then logs it.
static status_t my_setup_entropy_src(const dif_entropy_src_t *entropy_src) {
  // 1) Get the default config
  dif_entropy_src_config_t config = my_entropy_testutils_config_default();

  // 2) Configure ENTROPY_SRC with that config
  CHECK_DIF_OK(
  ...
  LOG_INFO("  bypass_conditioner: %d", config.bypass_conditioner);
  LOG_INFO("  single_bit_mode: %d", config.single_bit_mode);
  LOG_INFO("  health_test_window_size: %u", config.health_test_window_size);
  LOG_INFO("  alert_threshold: %u", config.alert_threshold);
...
status_t my_entropy_testutils_auto_mode_init(void) {
  ...
  busy_spin_micros(250000);

  print_entropy_src_state(&entropy_src);
  // re-enable entropy src and csrng
  my_setup_entropy_src(&entropy_src);
  print_entropy_src_state(&entropy_src);
  busy_spin_micros(250000);
  LOG_INFO("Configuring csrng:");
  TRY(my_dif_csrng_configure(&csrng));
  print_entropy_src_state(&entropy_src);
...
status_t execute_test(void) {
  ...
  for (uint16_t iter = 0; iter < 1; iter++) {
    ...
    for (uint16_t i = 0; i < 500; i++) {
      LOG_INFO("Step 17:loop i =%d, iter = %d", i, iter);
      CHECK_STATUS_OK(test_and_verify_aes_operation());
    }
    CHECK_STATUS_OK(print_entropy_src_state(&entropy_src));
  }
