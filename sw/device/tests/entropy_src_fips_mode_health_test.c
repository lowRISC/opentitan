...
status_t entropy_testutils_error_check_before_proceeding(void) {
  LOG_INFO("Debugging error checks before proceeding...");
...
status_t execute_test(void) {
  ...
  for (uint16_t i = 0; i < ARRAYSIZE(window_sizes); i++) {
    ...
    CHECK_STATUS_OK(entropy_testutils_error_check_before_proceeding());
  ...
  for (uint16_t iter = 0; iter < 1; iter++) {
    ...
    const uint32_t kEntropySrcStartupWaitMicros = 250000;
