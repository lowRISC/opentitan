...
static volatile uint8_t parity = 0xff;
...
static status_t execute_test(void) {
  ...
  CHECK_ARRAYS_EQ(recv_buf, kUartData, ARRAYSIZE(kUartData));
...
bool test_main(void) {
  ...
  OTTF_WAIT_FOR(uart_idx != 0xff && parity != 0xff, kCommandTimeoutMicros);
