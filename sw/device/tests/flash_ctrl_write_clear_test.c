...
static void flash_word_write_verify(uintptr_t address,
  ...
  // Change the array so that if the read function does not touch the array, the
  // test will not pass.
  tmp[0] ^= 1;
  CHECK_STATUS_OK(
      flash_ctrl_testutils_read(&flash, address, kUnusedDataPartitionParam, tmp,
                                kDifFlashCtrlPartitionTypeData, kWordSize,
                                /*delay=*/1));
  uint64_t got_value;
  memcpy(&got_value, tmp, sizeof(uint64_t));
