...
fn asn1time_to_string(time: &Asn1TimeRef) -> Result<Value<String>> {
    // SAFETY: OpenSSL guarantees that an ASN1_TIME is in fact just a typedef for ASN1_STRING
    ...
    // SAFETY: the above pointer is guaranteed to be valid since `time` is valid reference.
    // From the OpenSSL documentation: ASN1_STRING_get0_data() returns an internal pointer to
    // the data of x. Since this is an internal pointer it should not be freed or modified in any way.
    let time_str = unsafe {
        let len = openssl_sys::ASN1_STRING_length(time_str);
        let data = openssl_sys::ASN1_STRING_get0_data(time_str);
