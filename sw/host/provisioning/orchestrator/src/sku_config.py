...
class SkuConfig:
    ...
    def validate(self) -> None:
        ...
        if self.dice_mldsa_certs_from_device:
            if not self.dice_mldsa_ca:
                raise ValueError("Expect ML-DSA CA configuration to be present "
                                 "when ML-DSA TBS certificates are expected "
                                 "from device")
