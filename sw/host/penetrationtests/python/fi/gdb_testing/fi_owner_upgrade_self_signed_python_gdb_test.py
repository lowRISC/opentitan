...
def re_initialize(gdb, jump_address, print_output=False):
    ...
    while "owner_page_1 erased and written" not in response:
        gdb = GDBController(
...
class FiSimOwnerUpgradeSelfSigned(unittest.TestCase):
    def test_owner_upgrade_self_signed(self):
        print("Starting the owner upgrade self signed test")

        # Directory for the trace log files
        pc_trace_file = os.path.join(log_dir, "owner_upgrade_self_signed_pc_trace.log")
        # Directory for the the log of the campaign
        campaign_file = os.path.join(
            log_dir, "owner_upgrade_self_signed_test_campaign.log"
        )
        ...
        with open(campaign_file, "w", buffering=1) as campaign:
            ...
            try:
                ...
                # We perform the tracing over the ownership_init function
                trace_start_address = rom_ext_parser.get_function_start_address(
                    "ownership_init"
                )
                trace_end_address = rom_ext_parser.get_function_end_address(
                    "ownership_init"
                )

                print(
                    "Start and stop addresses for the owner upgrade: ",
                    trace_start_address,
                    trace_end_address,
                    flush=True,
                )
                            ...
                            with IterationTimeout(seconds=180):
                                ...
                                if "instruction skip applied" in gdb_response:
                                    ...
                                    if "owner_page_0: 2" in response:
                                        ...
                                        print(
                                            "Seeing a flash change, reflashing",
                                            flush=True,
                                        )
                                        gdb = re_initialize(gdb, jump_address)
                                    else:
