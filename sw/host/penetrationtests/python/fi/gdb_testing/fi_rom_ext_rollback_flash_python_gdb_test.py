...
high_target = None
low_target = None
low_firmware_path = None
high_firmware_path = None
bitstream_path = None
...
def re_initialize(gdb, jump_address, print_output=False):
    # Close everything
    gdb.close_gdb()
    high_target.close_openocd()
    high_target.clear_bitstream()

    # Initialize the high security version
    high_target.initialize_target(print_output=print_output)

    # Let the flashing propagate
    gdb = GDBController(gdb_path=GDB_PATH, gdb_port=GDB_PORT, elf_file=rom_ext_elf_path)
    gdb.send_command(f"set $pc={jump_address}")
    ...
    # Remove the high security version firmware
    high_target.target.zeroize_target()

    high_target.target.flash_target(low_firmware_path, print_output=True)
    high_target.start_openocd()
    gdb = GDBController(gdb_path=GDB_PATH, gdb_port=GDB_PORT, elf_file=rom_ext_elf_path)
    gdb.send_command(f"set $pc={jump_address}")
    high_target.dump_all()
    return gdb
    ...
    def test_rom_ext_rollback_flash(self):
        ...
        with open(campaign_file, "w") as campaign:
            ...
            try:
                ...
                # upsert_register_address = rom_ext_parser.get_function_start_address("upsert_register")
                # hmac_sha256_update_address = rom_ext_parser.get_function_start_address("hmac_sha256_update")
                ...
                # trace_start_address = "0x20005f42"
                # # trace_start_address = rom_ext_parser.get_function_start_address("rom_ext_verify")
                ...
                #     trace_start_address,
                #     trace_end_address,
                #     skip_addrs=[upsert_register_address, hmac_sha256_update_address, dbg_printf_address],
                # )
                # gdb.send_command("c", check_response=False)
...
if __name__ == "__main__":
    ...
    high_firmware_path = r.Rlocation(
        "lowrisc_opentitan/sw/device/tests/penetrationtests/fi_rom_ext_rollback_flash_high_python_gdb_test_fpga_cw340_rom_ext.img"
    )
    low_firmware_path = r.Rlocation(
        "lowrisc_opentitan/sw/device/tests/penetrationtests/fi_rom_ext_rollback_flash_low_python_gdb_test_fpga_cw340_rom_ext.img"
    )
    # Get the rom path.
    ...
    high_target_cfg = targets.TargetConfig(
        target_type=target_type,
        interface_type="hyperdebug",
        fw_bin=high_firmware_path,
    ...
    low_target_cfg = targets.TargetConfig(
        target_type=target_type,
        interface_type="hyperdebug",
        fw_bin=low_firmware_path,
        opentitantool=opentitantool_path,
        bitstream=bitstream_path,
        tool_args=config_args,
        openocd=openocd_path,
    ...
    high_target = targets.Target(high_target_cfg)
    low_target = targets.Target(low_target_cfg)
    ...
    print("Low security firmware", low_firmware_path)
    print("High security firmware", high_firmware_path)
