...
import serial
from sw.host.penetrationtests.python.util import targets, utils
...
# How many times we want to skip an instruction inside a loop
MAX_SKIPS_PER_LOOP = 1
...
class IterationTimeout:
    def __init__(self, seconds=60, error_message="Iteration timed out"):
...
def reset_target_and_gdb(gdb, jump_address, print_output=False):
    # Close GDB and restart OpenOCD if needed
    if gdb:
        gdb.close_gdb()
...
def re_initialize(gdb, jump_address, print_output=False):
    ...
    while "boot_data min_sec_ver_bl0 upgraded to 2" not in response:
        gdb = GDBController(
            gdb_path=GDB_PATH, gdb_port=GDB_PORT, elf_file=rom_ext_elf_path
        )
        gdb.reset_target()
...
class FiSimRomExtFlashRollback(unittest.TestCase):
    def test_rom_ext_flash_rollback(self):
        print("Starting the rom_ext flash rollback test")

        # Directory for the trace log files
        pc_trace_file = os.path.join(
            log_dir, "rom_ext_flash_rollback_pc_trace.log"
        )
        # Directory for the log of the campaign
        campaign_file = os.path.join(
            log_dir, "rom_ext_flash_rollback_test_campaign.log"
        )
        ...
        with open(campaign_file, "w", buffering=1) as campaign:
            ...
            try:
                ...
                jump_address = rom_parser.get_function_start_address(
                    "kRomStartRmaSpinSkip"
                )

                # Functions to skip tracing
                upsert_register_address = rom_ext_parser.get_function_start_address(
                    "upsert_register"
                )

                # Reset and let the flashing continue until min_sec_ver_bl0 is upgraded
                response = ""
                timeout = 2
                ...
                # Perform PC tracing over boot_svc_handler and BL0 slot rollback verification
                trace_start_address = (
                    rom_ext_parser.get_function_start_address("boot_svc_handler") or
                    rom_ext_parser.get_inlined_function_address("boot_svc_handler")
                )
                trace_end_address = (
                    rom_ext_parser.get_function_start_address("hmac_sha256_init") or
                    rom_ext_parser.get_inlined_function_address("hmac_sha256_init")
                )

                print(
                    "Start and stop addresses for rom_ext flash rollback: ",
                    trace_start_address,
                    trace_end_address,
                    flush=True,
                )
                        ...
                        try:
                            ...
                            with IterationTimeout(seconds=60):
                                gdb.apply_instruction_skip(
                                    pc,
                                    rom_ext_parser.parse_next_instruction(pc),
                                    i_count,
                                )
                                gdb.send_command("c", check_response=False)
                                ...
                                if "instruction skip applied" in gdb_response:
                                    ...
                                    if "BL0 Slot B (sec_ver=0) booted!" in response:
                                        successful_faults += 1
                                        print("-" * 80)
                                        ...
                                        try:
                                            gdb = reset_target_and_gdb(
                                                gdb, jump_address
                                            )
                                        except TimeoutError:
                                            print("Timeout, reflashing", flush=True)
                                            gdb = re_initialize(gdb, jump_address)
                                    elif "boot_data min_sec_ver_bl0 upgraded to 2" in response:
                                        try:
                                            gdb = reset_target_and_gdb(
                                                gdb, jump_address
                                            )
...
if __name__ == "__main__":
    ...
    # Get the disassembly path.
    rom_ext_dis_path = rom_ext_path.replace(
        ".prod_key_0.prod_key_0.signed.bin", ".dis"
    ).replace(".prod_key_0.signed.bin", ".dis")
    # And the path for the elf.
    rom_ext_elf_path = rom_ext_path.replace(
        ".prod_key_0.prod_key_0.signed.bin", ".elf"
    ).replace(".prod_key_0.signed.bin", ".elf")
