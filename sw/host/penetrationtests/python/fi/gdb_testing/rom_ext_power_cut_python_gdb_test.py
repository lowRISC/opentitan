...
class RomExtPowerCutFiSim(unittest.TestCase):
    def test_rom_ext_power_cut(self):
        print("Starting the rom_ext power cut test")

        # Directory for the trace log files
        pc_trace_file = os.path.join(log_dir, "rom_ext_power_cut_pc_trace.log")
        # Directory for the the log of the campaign
        campaign_file = os.path.join(log_dir, "rom_ext_power_cut_test_campaign.log")
        ...
        with open(campaign_file, "w") as campaign:
            ...
            try:
                ...
                rnd_uint32_address = rom_ext_parser.get_function_start_address("rnd_uint32")
                hardened_memeq_address = rom_ext_parser.get_function_start_address("hardened_memeq")
                ...
                # dbg_print_epmp is a function close to the end of the rom_ext
                trace_end_address = rom_ext_parser.get_inlined_function_address("dbg_print_epmp")
                    ...
                    for i_count in range(min(MAX_SKIPS_PER_LOOP, count)):
                        ...
                        try:
                            ...
                            with IterationTimeout(seconds=60):
                                gdb.apply_power_cut(pc, i_count)
                ...
                print("-" * 80)
                print(f"Total cuts {total_attacks}, successful cuts {successful_faults}")
