...
class FiSimOwnerRollback(unittest.TestCase):
    def test_owner_rollback(self):
        print("Starting the owner rollback test")

        # Directory for the trace log files
        pc_trace_file = os.path.join(log_dir, "owner_rollback_pc_trace.log")
        # Directory for the log of the campaign
        campaign_file = os.path.join(
            log_dir, "owner_rollback_test_campaign.log"
        )
        ...
        with open(campaign_file, "w", buffering=1) as campaign:
            ...
            try:
                ...
                # We perform unbiased PC tracing over the full ownership_init function range
                ...
                print(
                    "Start and stop addresses for the owner rollback: ",
                    trace_start_address,
                    trace_end_address,
                    flush=True,
                )
                        ...
                        try:
                            ...
                            with IterationTimeout(seconds=180):
                                ...
                                if "instruction skip applied" in gdb_response:
                                    ...
                                    if (
                                        "owner_page_0: 1" in response and "saved" not in response
                                    ):
                                        successful_faults += 1
                                        print("-" * 80)
