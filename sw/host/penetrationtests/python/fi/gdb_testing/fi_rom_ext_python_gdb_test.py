...
class RomExtFiSim(unittest.TestCase):
    def test_rom_ext_secure_boot(self):
        print("Starting the rom_ext secure boot test")

        # Directory for the trace log files
        pc_trace_file = os.path.join(log_dir, "rom_ext_secure_boot_pc_trace.log")
        # Directory for the the log of the campaign
        campaign_file = os.path.join(log_dir, "rom_ext_secure_boot_test_campaign.log")
        ...
        with open(campaign_file, "w") as campaign:
            ...
            try:
                ...
                # We perform the tracing over the rom_ext_try_next_stage function
                trace_start_address = rom_ext_parser.get_inlined_function_address(
                    "rom_ext_try_next_stage"
                )
