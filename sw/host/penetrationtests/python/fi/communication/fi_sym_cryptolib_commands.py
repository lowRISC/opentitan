...
class OTFISymCrypto:
    ...
    def _ujson_sym_crypto_fi_cmd(self) -> None:
        self.target.write(json.dumps("CryptoLibFiSym").encode("ascii"))
        time.sleep(0.003)
    ...
    def handle_aes(
        self, data, data_len, key, key_len, iv, padding, mode, op_enc, cfg, trigger
    ) -> None:
    ...
    def handle_cmac(self, data, data_len, key, key_len, iv, cfg, trigger) -> None:
        ...
        """
        self._ujson_sym_crypto_fi_cmd()
        self.target.write(json.dumps("Cmac").encode("ascii"))
        input_data = {
            "data": data,
            "data_len": data_len,
            "key": key,
    ...
    def handle_gcm(
        self, data, data_len, key, key_len, aad, aad_len, tag, tag_len, iv, cfg, trigger
    ) -> None:
        ...
        Args:
            ...
            tag: Array of max 64 bytes of tag data.
            tag_len: Input tag length.
            iv: Array of 16 bytes for the iv.
            cfg: Integer for configuration.
            trigger: Integer specifying which triggers to set.
        """
        self._ujson_sym_crypto_fi_cmd()
        self.target.write(json.dumps("Gcm").encode("ascii"))
        input_data = {
            "data": data,
            "data_len": data_len,
            "key": key,
    ...
    def handle_drbg_generate(
        ...
        """
        self._ujson_sym_crypto_fi_cmd()
        self.target.write(json.dumps("DrbgGenerate").encode("ascii"))
        input_data = {
            "nonce": nonce,
            "nonce_len": nonce_len,
            "data_len": data_len,
    ...
    def handle_trng_init(self, mode, cfg, trigger) -> None:
        ...
        Args:
            mode: Integer for the mode.
            cfg: Integer for configuration.
            trigger: Integer specifying which triggers to set.
        """
        self._ujson_sym_crypto_fi_cmd()
        self.target.write(json.dumps("TrngInit").encode("ascii"))
        input_data = {
            "mode": mode,
            "cfg": cfg,
            "trigger": trigger,
    ...
    def handle_trng_generate(self, cfg, trigger) -> None:
        ...
        """
        self._ujson_sym_crypto_fi_cmd()
        self.target.write(json.dumps("TrngGenerate").encode("ascii"))
        input_data = {
            "cfg": cfg,
            "trigger": trigger,
        }
