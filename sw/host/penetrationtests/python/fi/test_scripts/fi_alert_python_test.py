...
# Note that these tests were only tried on silicon GB A1/2 chips.
class AlertFiTest(unittest.TestCase):
    ...
    def test_char_alert_trigger(self):
        ...
        # On the FPGA, anything to do with flash alerts causes the FPGA to crash
        # We exclude those from the test
        if is_fpga:
            exception_list.extend([35, 36, 37, 38, 39])
