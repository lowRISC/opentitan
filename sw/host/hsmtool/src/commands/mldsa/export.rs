...
use crate::util::key::mldsa::{MldsaPrivateKey, MldsaPublicKey, save_private_key, save_public_key};
...
pub struct Export {
    ...
    #[arg(long)]
    private: bool,
    /// Wrap the exported key in a wrapping key.
    #[arg(long)]
    wrap: Option<String>,
...
impl Dispatch for Export {
    fn run(
        ...
        if self.wrap.is_some() {
            ...
            let sk = MldsaPrivateKey::try_from(&map)?;
            save_private_key(&self.filename, &sk, self.format)?;
        } else {
            let map = AttributeMap::from_object(session, object)?;
            let pk = MldsaPublicKey::try_from(&map)?;
            save_public_key(&self.filename, &pk, self.format)?;
        }
