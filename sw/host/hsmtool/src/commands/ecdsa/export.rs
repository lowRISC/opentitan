...
impl Export {
    fn export(&self, session: &Session, object: ObjectHandle) -> Result<()> {
        ...
        if self.private {
            match map.get(&AttributeType::Value) {
                None => return Err(anyhow!("Key does not contain a private value")),
                Some(AttrData::None) => return Err(anyhow!("Private key is not exportable")),
                _ => {}
            };
            let key = SigningKey::try_from(&map)?;
            save_private_key(&self.filename, &key, self.format)?
        } else {
            let key = VerifyingKey::try_from(&map)?;
    ...
    fn wrap_key(&self, session: &Session, object: ObjectHandle) -> Result<()> {
        let wrapper: Wrap = self
            .wrap_mechanism
            .context("wrap_mechanism is required when wrap is specified")?
            .into();
...
#[typetag::serde(name = "ecdsa-export")]
