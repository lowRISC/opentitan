...
use pentest_commands::fi_sym_cryptolib_commands::CryptoLibFiSymSubcommand;
...
struct Opts {
    ...
    #[arg(long, num_args = 1..)]
    fi_sym_cryptolib_json: Vec<String>,
}

#[derive(Debug, Deserialize)]
struct FiCryptoLibTestCase {
    test_case_id: usize,
    command: String,
...
fn run_fi_sym_cryptolib_testcase(
    ...
    PenetrationtestCommand::CryptoLibFiSym.send(uart)?;

    // Send test subcommand.
    CryptoLibFiSymSubcommand::from_str(test_case.command.as_str())
...
fn test_fi_sym_cryptolib(opts: &Opts, transport: &TransportWrapper) -> Result<()> {
    ...
    let test_vector_files = &opts.fi_sym_cryptolib_json;
    for file in test_vector_files {
        let raw_json = fs::read_to_string(file)?;
        let fi_sym_cryptolib_tests: Vec<FiCryptoLibTestCase> = serde_json::from_str(&raw_json)?;
        for fi_sym_cryptolib_test in &fi_sym_cryptolib_tests {
            test_counter += 1;
            log::info!("Test counter: {}", test_counter);
            run_fi_sym_cryptolib_testcase(fi_sym_cryptolib_test, opts, &*uart, &mut fail_counter)?;
        }
    }
    assert_eq!(
        0, fail_counter,
...
fn main() -> Result<()> {
    ...
    execute_test!(test_fi_sym_cryptolib, &opts, &transport);
    Ok(())
}
