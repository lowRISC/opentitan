...
use opentitanlib::util::hexdump::hexdump_string;
    ...
    F: Fn(&mut OwnerBlock),
{
    ...
    if cfg & CFG_CORRUPT != 0 {
        detached_sig.ecdsa.as_mut().map(|val| val.r[0] ^= 1);
        detached_sig.spx.as_mut().map(|val| val[0] ^= 1);
    }
    ...
    log::info!("Owner Block\n\n{}", hexdump_string(&owner_config)?);
    rescue.enter(transport, /*reset=*/ true)?;
    rescue.wait()?;
    if key_alg.is_detached() {
