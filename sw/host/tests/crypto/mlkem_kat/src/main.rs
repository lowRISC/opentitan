...
const KNOWN_DECAPS_FAILURES: &[usize] = &[];
...
fn run_mlkem_testcase(
    ...
    match test_case.operation.as_str() {
        "encaps" => {
            ...
            let ct_matches = ct_res.ciphertext_len == test_case.ciphertext.len()
                && ct_res.ciphertext[..ct_res.ciphertext_len] == test_case.ciphertext[..];
            let ss_matches = ss_res.shared_secret_len == test_case.shared_secret.len()
                && ss_res.shared_secret[..ss_res.shared_secret_len] == test_case.shared_secret[..];

            let test_passed = if !test_case.result {
                ct_res.ciphertext_len == 0 || ss_res.shared_secret_len == 0
            } else {
                ct_matches && ss_matches
            };
        ...
        "decaps" => {
            ...
            let ss_res =
                CryptotestMlkemSharedSecret::recv(spi_console, opts.timeout, false, false)?;
            let ss_matches = if test_case.shared_secret.is_empty() {
                if !test_case.result {
                    ss_res.shared_secret_len == 32 || ss_res.shared_secret_len == 0
                } else {
                    ss_res.shared_secret_len == 0
                }
            } else {
                ss_res.shared_secret_len == test_case.shared_secret.len()
                    && ss_res.shared_secret[..ss_res.shared_secret_len]
