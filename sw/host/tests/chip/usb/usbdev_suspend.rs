...
fn reset(
    device: &mut rusb::DeviceHandle<rusb::GlobalContext>,
    opts: &Opts,
    hub: &UsbHub,
    port: u8,
) -> Result<()> {
    ...
    if false {
        ...
        log::info!("Attempting device reset");
...
fn usbdev_suspend(
    ...
    // TODO: but I WANT the device handle, thank you!
    let mut devices = opts.usb.wait_for_device(opts.timeout)?;
        ...
        loop {
            ...
            match phase {
                ...
                SuspendPhase::Suspend => {
                    resume(&hub, port)?;
                    delay_millis(3);
                    // Device expects a deliberate Bus Reset to complete this phase.
                    reset(&mut device, &opts, &hub, port)?;
