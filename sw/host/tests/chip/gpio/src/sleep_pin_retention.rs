...
fn read_pins(transport: &TransportWrapper, pins: &[&str]) -> Result<u8> {
    ...
    for (i, pin) in pins.iter().enumerate() {
        values |= (transport.gpio_pin(pin)?.read()? as u8) << i;
    }
    Ok(values)
}
...
fn sleep_pin_retention_test(opts: &Opts, transport: &TransportWrapper) -> Result<()> {
    ...
    log::info!("Doing {num_rounds:?} rounds of testing");
    ...
    for _ in 0..num_rounds {
        ...
        log::info!("Chosen GPIO value 0x{gpio_awake_value:x}");
        ...
        log::info!("Read GPIO value 0x{pin_values:x}");
        ensure!(
            pin_values == gpio_sleep_value,
            "GPIO pin value 0x{pin_values:x} does not match the expected sleep value 0x{gpio_sleep_value:x}",
        );
        ...
        let pin_values = read_pins(transport, GPIO_PINS)?;
        log::info!("Read GPIO value 0x{pin_values:x} expected 0x{gpio_awake_value:x}",);
        ensure!(
            pin_values == gpio_awake_value,
            "GPIO pin value 0x{pin_values:x} does not match the expected awake value 0x{gpio_awake_value:x}",
        );
