...
fn test_opendrain(transport: &TransportWrapper, config: &Config) -> Result<()> {
    let uart = transport.uart("console")?;
    ...
    // Get all pads that support opendrain and configure the attribute.
    let opendrain_outputs: HashMap<_, _> = config
        .output
        .clone()
        .into_iter()
        .filter(|&(mio, _)| SUPPORTS_OD.iter().any(|&o| o == mio))
        .collect();
...
fn test_virtual_opendrain(transport: &TransportWrapper, config: &Config) -> Result<()> {
    let uart = transport.uart("console")?;
...
fn main() -> Result<()> {
    ...
    if opts.init.backend_opts.interface == "teacup" {
        execute_test!(test_opendrain, &transport, &config);
    }
    execute_test!(test_virtual_opendrain, &transport, &config);
