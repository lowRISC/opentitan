      ...
      case kSigStateCheckTail: {
        ...
        if (match == *sp) {
          if (sig_cnt_ >= sizeof(usbdev_stream_sig_t) - 1U) {
            ...
            uint8_t stream = sig->stream & kUsbdevStreamFlagID;
...
void USBDevStream::SigProcess(const usbdev_stream_sig_t &sig) {
  // Stream IDentified and flags
  uint8_t stream = sig.stream & kUsbdevStreamFlagID;
  bool retrieve = ((sig.stream & kUsbdevStreamFlagRetrieve) != 0U);
  bool check = ((sig.stream & kUsbdevStreamFlagCheck) != 0U);
  bool send = ((sig.stream & kUsbdevStreamFlagSend) != 0U);
...
uint32_t USBDevStream::SpaceAvailable(uint8_t **space, bool contig) {
  uint32_t space_bytes = kBufferSize - buf_.wr_idx;
  uint8_t *start = &buf_.data[buf_.wr_idx];

  if (buf_.rd_idx > buf_.wr_idx) {
    // All space is contiguous; may return start pointer to the caller
    space_bytes = (buf_.rd_idx - 1U) - buf_.wr_idx;
  } else if (buf_.rd_idx > 0U) {
    if (!contig) {
      space_bytes += buf_.rd_idx - 1U;
      // Not all data is contiguous, but the caller wants to know about all of
      // the free space.
      start = nullptr;
    }
  } else {
...
bool USBDevStream::AddData(const uint8_t *data, uint32_t len) {
  ...
  uint32_t space_bytes = SpaceAvailable(nullptr, false);
  if (space_bytes < len) {
    return false;
  }
  ...
  if (kBufferSize - buf_.wr_idx <= chunk) {
    // A second chunk to do?
    uint32_t left = len - chunk;
    if (chunk < len && data) {
      // Add data into the buffer
      memcpy(buf_.data, data, left);
    }
    buf_.wr_idx = left;
  } else {
    buf_.wr_idx += chunk;
...
uint32_t USBDevStream::DataAvailable(uint8_t **data, bool contig) {
  uint32_t data_bytes = kBufferSize - buf_.rd_idx;
  uint8_t *start = &buf_.data[buf_.rd_idx];

  if (buf_.wr_idx >= buf_.rd_idx) {
    data_bytes = buf_.wr_idx - buf_.rd_idx;
  } else if (buf_.wr_idx > 0U && !contig) {
    data_bytes += buf_.wr_idx;
    // Not all data is contiguous, but the caller wants to know about all of the
    // data available.
    start = nullptr;
  }
  if (data) {
...
bool USBDevStream::ConsumeData(uint32_t len) {
  ...
  uint32_t data_bytes = DataAvailable(nullptr, false);
  if (data_bytes < len) {
    return false;
  }

  // Size of first contiguous chunk
  uint32_t chunk = kBufferSize - buf_.rd_idx;
  if (len < chunk) {
    buf_.rd_idx += len;
  } else {
...
void USBDevStream::ClearBuffer() {
  // TODO: introduce validity checks here.
