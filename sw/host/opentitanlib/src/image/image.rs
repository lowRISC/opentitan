...
impl Image {
    ...
    pub fn borrow_manifest(&self) -> Result<&Manifest> {
        ...
        let manifest_layout: zerocopy::Ref<&[u8], Manifest> =
    ...
    pub fn borrow_manifest_mut(&mut self) -> Result<&mut Manifest> {
        ...
        let manifest_layout: zerocopy::Ref<&mut [u8], Manifest> =
            zerocopy::Ref::new(&mut *manifest_slice).ok_or(ImageError::Parse)?;
