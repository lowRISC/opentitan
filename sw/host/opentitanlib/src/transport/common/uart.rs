...
/// Represents a file-backed UART connection.
pub struct FileUart {
    baudrate: Cell<u32>,
    flow_control: Cell<FlowControl>,
    file: RefCell<File>,
...
impl FileUart {
    pub fn open<P: AsRef<Path>>(path: P, baudrate: u32) -> Result<Self> {
        let path = path.as_ref();
        Ok(Self {
            baudrate: Cell::new(baudrate),
            flow_control: Cell::new(FlowControl::None),
            file: RefCell::new(
                OpenOptions::new()
                    .read(true)
                    ...
                    .open(path)
                    .map_err(|e| {
                        TransportError::OpenError(path.display().to_string(), e.to_string())
                    })?,
            ),
            rxbuf: RefCell::default(),
        })
