...
impl QemuGpioPin {
    ...
    /// Whether the pin is being actively driven by the host.
    fn driven(&self) -> bool {
        match self.mode.get() {
            PinMode::Input => false,
...
impl GpioPin for QemuGpioPin {
    ...
    fn write(&self, value: bool) -> anyhow::Result<()> {
        ...
        gpio.send_frame(QEMU_GPIO_INPUT, value)?;
    ...
    fn set_mode(&self, mode: PinMode) -> anyhow::Result<()> {
        ...
        let output_ignore = !gpio.host_output_enable;
        gpio.send_frame(QEMU_GPIO_MASK, output_ignore)?;
