...
impl RomPatchMatchDescriptor {
    fn from_hjson(remap_base: u32, region: &RomPatchRegionHjson) -> Result<Self> {
        ...
        let locked = if region.locked {
            1 << PATCH_LOCKED_FIELD_SHIFT
        } else {
            0
        };

        let match_address = match match_addr {
            addr if addr <= PATCH_MATCH_ADDR_FIELD_MASK => addr & PATCH_MATCH_ADDR_FIELD_MASK,
            _ => bail!("Invalid ROM patch match address {match_addr}"),
        };
...
#[derive(AsBytes, Copy, Clone, Debug, Default)]
#[repr(packed, C)]
struct RomPatchMatchTable {
    table: [RomPatchMatchDescriptor; N_MATCH_DESCRIPTORS],
}

impl RomPatchMatchTable {
    fn from_hjson(base_addr: u32, regions: &[RomPatchRegionHjson]) -> Result<Self> {
        let mut table: [RomPatchMatchDescriptor; N_MATCH_DESCRIPTORS] = Default::default();
...
impl RomPatchCode {
    fn from_hjson(regions: &Vec<RomPatchRegionHjson>) -> Result<Self> {
        ...
        for region in regions {
            if let Some(sub_routines_hjson) = &region.sub_routines {
                for s in sub_routines_hjson {
                    code_size_words =
                        std::cmp::max(code_size_words, (s.offset >> 2) + s.code.len());
                }
            }
        }
        ...
        for region in regions {
            if let Some(sub_routines_hjson) = &region.sub_routines {
                for s in sub_routines_hjson {
                    for (index, insn) in s.code.iter().enumerate() {
                        let insn_hex = u32::from_str(insn.trim_end_matches(','))?;
                        let offset_words = s.offset >> 2;
...
struct RomPatch {
    ...
    match_table: RomPatchMatchTable,
    code: RomPatchCode,
    signature: RomPatchSig,
}

impl ToWriter for RomPatch {
...
#[derive(Debug, Clone)]
pub struct RomPatchPartition {
    patches: Vec<RomPatch>,
}

impl RomPatchPartition {
...
impl ToWriter for RomPatchPartition {
    fn to_writer(&self, w: &mut impl Write) -> Result<()> {
        for patch in &self.patches {
            patch.to_writer(w)?;
        }
...
mod tests {
    ...
    fn rom_test_patch(config: &RomPatchConfig) -> Result<RomPatch> {
    ...
    fn rom_test_patch_partition(
        first_config: &RomPatchConfig,
        second_config: &RomPatchConfig,
    ) -> Result<RomPatchPartition> {
        let key = EcdsaPrivateKey::load(testdata!("key_ecdsa.der"))?;
    ...
    fn rom_patch_test_deser(patch: &RomPatch, config: &RomPatchConfig) -> Result<()> {
    ...
    fn rom_patch_test_write(patch: &RomPatch, config: &RomPatchConfig) -> Result<()> {
    ...
    fn rom_patch_partition_test_write(
        partition: &RomPatchPartition,
        first_config: &RomPatchConfig,
        second_config: &RomPatchConfig,
    ) -> Result<()> {
    ...
    fn test_rom_patch_deser() -> Result<()> {
        rom_patch_test_deser(&rom_test_patch(&FIRST_CONFIG)?, &FIRST_CONFIG)
    }

    #[test]
    fn test_rom_patch_write() -> Result<()> {
        rom_patch_test_write(&rom_test_patch(&FIRST_CONFIG)?, &FIRST_CONFIG)
    }

    #[test]
    fn test_rom_patch_partition_deser() -> Result<()> {
        let patch_partition = rom_test_patch_partition(&FIRST_CONFIG, &SECOND_CONFIG)?;
    ...
    fn test_rom_patch_partition_write() -> Result<()> {
        rom_patch_partition_test_write(
            &rom_test_patch_partition(&FIRST_CONFIG, &SECOND_CONFIG)?,
            &FIRST_CONFIG,
            &SECOND_CONFIG,
        )
    }
