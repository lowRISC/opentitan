...
///
pub fn clean_pem_bytes_for_parsing(pem_bytes: &[u8]) -> Result<Vec<u8>> {
    let pem_content_str = std::str::from_utf8(pem_bytes)
        .map_err(|e| anyhow!("Input PEM data is not valid UTF-8: {}", e))?;
    ...
    if let Some(begin_line_str) = lines_iter.next() {
        ...
        return Err(anyhow!(
            "Input PEM data is empty or contains no line breaks"
        ));
    }
    ...
    for line_str in lines_iter {
        if !processing_base64_payload && line_str.len() == pem_rfc7468::BASE64_WRAP_WIDTH {
