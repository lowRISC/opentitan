...
use crate::io::jtag::{Jtag, JtagChain, JtagParams, JtagTap, RiscvReg, RiscvGpr};
...
use probe_rs::{Permissions, Session, RegisterId, RegisterRole, CoreRegisters, MemoryInterface};
        ...
        */
        ...
        for gpr in RiscvGpr::iter() {
            ...
            for r in regs.core_registers(){
                ...
                for role in r.roles{
                    match role{
                        RegisterRole::Core(name) | RegisterRole::Argument(name) | RegisterRole::Return(name) | RegisterRole::Other(name) => {
                            if *name == lname{
                                reg_map.insert(gpr, r.id());
                            }
                        },
...
impl JtagChain for ProbeJtagChain {
    ...
    fn connect(mut self: Box<Self>, tap: JtagTap) -> Result<Box<dyn Jtag>> {
        /*
          ...
          eventually the probe-rs implementation should probably do the same
        */
        ...
        if probes.len() != 1 {
            // TODO better handling of this error case, and handle options to disambiguate multiply probes
            bail!("Did not find exactly one probe during probe-rs init")
        }
        // until the above TODO is fixed, always use the first probe found.
        let mut probe = probes[0].open();
        if probe.is_err(){
            // could be that another session just ended, and we need to wait a bit
            let timeout = Instant::now() + Duration::from_secs(1);
            while probe.is_err() && Instant::now() < timeout{
        ...
        // Attach to a chip.
        let mut session = probe.attach("riscv", Permissions::default())?;
...
impl ProbeJtagTap {
    fn reg_id(&self, reg: &RiscvGpr) -> Result<RegisterId>{
        ...
        match id{
            None => { bail!("Unable to determine probe-rs register ID for register {:?}", reg); }
            Some(id) => { Ok(*id) }
        }
    }
}
...
impl Jtag for ProbeJtagTap {
    ...
    fn disconnect(self: Box<Self>) -> Result<()> {
        ...
        // HOWEVER: disconnecting probe-rs appears to have some other unintended
        // effects on the target which don't happen with OpenOCD. For now, to make
        // some of the tests pass, we just leak the instance, so that it does not
        // do whatever it is that's causing a problem during disconnect. This
        // means that any tests that wants to disconnect and reconnect mid-test
        // is going to hang when it tries to reconnect. This is a big problem
        // that will need solving before switching to probe-rs.
        
        // for now we have this line, but it will be removed later
        Box::<ProbeJtagTap>::leak(self);
        Ok(())
    }
