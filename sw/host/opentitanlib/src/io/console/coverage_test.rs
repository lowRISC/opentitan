...
mod tests {
    ...
    use crate::io::console::CoverageConsole;
    ...
    impl crate::io::uart::Uart for MockConsole {
    ...
    fn read_to_eof(device: &impl ConsoleDevice) -> Result<Vec<u8>> {
    ...
    fn get_xprofraw_files(dir: &std::path::Path) -> Result<Vec<std::path::PathBuf>> {
        let mut files: Vec<_> = fs::read_dir(dir)?
            .filter_map(|e| e.ok())
            .map(|e| e.path())
            .filter(|p| p.extension().is_some_and(|ext| ext == "xprofraw"))
            .collect();
        files.sort();
        Ok(files)
    }
    ...
    fn test_coverage_full_flow() -> Result<()> {
        ...
        let files = get_xprofraw_files(tmp_dir.path())?;
    ...
    fn test_coverage_corrupted_crc() -> Result<()> {
        ...
        let device =
            CoverageMiddleware::new_with_template(mock, profraw_path.to_str().unwrap().to_owned());
        let _ = read_to_eof(&device)?;

        // No xprofraw file should be created because of CRC failure.
        let files_count = get_xprofraw_files(tmp_dir.path())?.len();
    ...
    fn test_coverage_invalid_hex() -> Result<()> {
        ...
        data.extend(b"012345"); // This will be treated as normal text
        ...
        assert!(output_str.contains("X012345"));
    ...
    fn test_wait_for_coverage() -> Result<()> {
        ...
        let device = CoverageMiddleware::new_with_template(mock, "%p.profraw".to_string());
    ...
    #[test]
    fn test_coverage_clear_rx_buffer() -> Result<()> {
        let mut data = VecDeque::new();
        data.extend(b"== COVERAGE PROFILE SKIP ==\r\n");
        ...
        let device = CoverageMiddleware::new(mock);

        device.wait_for_coverage(Duration::from_secs(1))?;

        assert_eq!(device.coverage_blocks_processed(), 1);

        use crate::io::middleware::UartMiddleware;
        device.clear_rx_buffer_impl()?;

        assert_eq!(device.coverage_blocks_processed(), 0);
