...
impl<T> CoverageMiddleware<T> {
    ...
    fn process_coverage_data(&self, coverage_buf: &str) -> Result<()> {
        ...
        // By default, Bazel 8 uses `%h-%p-%m.profraw`.
        // %h, %p and %m are standard LLVM profile file placeholders:
        //   - %h: hostname hardcoded as "test.on.device"
        //   - %p: process ID replaced with a random number
        //   - %m: merge pool hardcoded as "0"
        // Since we are running on a device and saving the profile on the host,
        // we don't have a meaningful values for those placeholders.
    ...
    fn flush_match_buf(&self, state: &mut CoverageState, len: usize) {
        for _ in 0..len {
            let ch = state.match_buf.pop_front().unwrap();
            if state.mode == BufferMode::Normal {
                state.output_buf.push_back(ch);
            } else if ch.is_ascii_hexdigit() {
                state.coverage_buf.push(ch as char);
            } else {
                log::warn!(
        ...
        loop {
            ...
            if !state.match_buf.is_empty() {
                ...
                if let Some(anchor) = Self::starts_with_any_anchor(match_buf_flat) {
                    if let Some(p) = self.handle_anchor(&mut state, anchor, cx) {
                        return p;
                    }
                    continue;
                }
                ...
                if safe_len > 0 {
                    self.flush_match_buf(&mut state, safe_len);
                    continue;
                }
            ...
            match res {
                Ok(0) => {
                    // EOF. Flush match_buf.
                    if !state.match_buf.is_empty() {
                        let safe_len = state.match_buf.len();
                        self.flush_match_buf(&mut state, safe_len);
                    }
                    return Poll::Ready(Ok(0));
                }
