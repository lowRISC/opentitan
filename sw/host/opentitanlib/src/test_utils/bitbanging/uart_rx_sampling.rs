...
impl<const RX: u8> UartRxMonitoringDecoder<RX> {
    ...
    /// - next_sample_time: The current time at which the next sample should be
    ///   taken. A sample (/ several samples) will be taken if the edge occurs
    ///   after this time, and the time will then be correspondingly updated.
    ...
    pub fn decode_response(
        ...
        self.decode_waveform(response.events, read_end_timestamp, &sampling_period)
    }
}
...
mod test {
    ...
    fn sample_and_decode<const RX: u8>(
        decoder: UartBitbangDecoder<RX>,
        start_idle: bool,
        ...
        let mut response_decoder =
            UartRxMonitoringDecoder::<RX>::new(decoder, CLOCK_NATURE, start_response)?;
        let decoded = response_decoder.decode_response(read_response, baud_rate)?;
    ...
    fn smoke() -> Result<()> {
        ...
        let decoder = UartBitbangDecoder::<0>::new(config);
        sample_and_decode(
            decoder,
            true,
    ...
    fn bit_indexes() -> Result<()> {
        ...
        let config = UartBitbangConfig::new(8, UartStopBits::Stop1, 2, Parity::None)?;
        let decoder_1 = UartBitbangDecoder::<1>::new(config.clone());
        let decoder_5 = UartBitbangDecoder::<5>::new(config);
        sample_and_decode(
            decoder_1,
            true,
            START_RESPONSE.clone(),
    ...
    fn baud_rates() -> Result<()> {
        ...
        for divider in 1..=10u32 {
            ...
            let decoder = UartBitbangDecoder::<0>::new(config.clone());
            sample_and_decode(
                decoder,
                true,
                START_RESPONSE.clone(),
    ...
    fn partial_responses() -> Result<()> {
        ...
        let events = EVENT_TIMESTAMPS
            ...
            .collect::<Vec<_>>();
        let mut response_decoder =
            UartRxMonitoringDecoder::<0>::new(decoder, CLOCK_NATURE, START_RESPONSE.clone())?;
        ...
        for partial_edges in events[..60].chunks(3) {
            ...
            decoded.extend(response_decoder.decode_response(read_response, BAUD_RATE)?);
        }
