...
Currently, bitbanging utilities are available for [UART](../src/test_utils/bitbanging/uart.rs), [SPI](../src/test_utils/bitbanging/spi.rs), [I2C](../src/test_utils/bitbanging/i2c.rs) and [PWM](../src/test_utils/bitbanging/pwm.rs).
...
let mut monitor_decoder = UartRxMonitoringDecoder::<0>::new(decoder, clock_nature, start)?;

// After some time has passed, read the monitoring response and decode
let read_response = gpio_monitoring.monitoring_read(&[rx_pin.borrow()], true)?;
let decoded = monitor_decoder.decode_response(read_response, baud_rate)?;
```
...
Alternatively, by using the `VcdParser` and `VcdWriter` implementations directly, you can write or read to a file (or any other writable/readable context).
...
Further examples can be seen in the unit test in [`vcd.rs`](../src/util/vcd.rs).
