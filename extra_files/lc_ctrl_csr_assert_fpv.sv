...
`else
  `define REGWEN_PATH lc_ctrl.u_reg
`endif

// Block: lc_ctrl
module lc_ctrl_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  assign regwen[0] = 1;
  assign regwen[1] = 1;
  assign regwen[2] = `REGWEN_PATH.transition_regwen_qs;
  assign regwen[3] = 1;
  ...
  fpv_reg_access_e access_policy [4];
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:136]})
           ) begin
  ...
  `ASSERT(claim_transition_if_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h8 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[1] & 'h1))
