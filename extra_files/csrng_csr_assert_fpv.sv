...
`else
  `define REGWEN_PATH csrng.u_reg
`endif

// Block: csrng
module csrng_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  assign regwen[5] = 1;
  assign regwen[6] = 1;
  assign regwen[7] = `REGWEN_PATH.int_state_read_enable_regwen_qs;
  assign regwen[8] = 1;
  assign regwen[9] = 1;
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:92]})
           ) begin
  ...
  `ASSERT(reseed_interval_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h1c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[6] & 'hffffffff))

  `ASSERT(int_state_read_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h38 |->
         d2h.d_error ||
         (d2h.d_data & 'h7) == (exp_vals[7] & 'h7))

  `ASSERT(int_state_read_enable_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h3c |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[8] & 'h1))

  `ASSERT(int_state_num_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h40 |->
         d2h.d_error ||
         (d2h.d_data & 'hf) == (exp_vals[9] & 'hf))

  `ASSERT(fips_force_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h48 |->
         d2h.d_error ||
         (d2h.d_data & 'h7) == (exp_vals[10] & 'h7))
