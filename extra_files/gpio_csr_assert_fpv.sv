...
`ifndef SYNTHESIS
  ...
  fpv_reg_access_e access_policy [9];
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:60]})
           ) begin
  ...
  `ASSERT(intr_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         ...
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[0] & 'hffffffff))

  `ASSERT(intr_ctrl_en_rising_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h2c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[3] & 'hffffffff))

  `ASSERT(intr_ctrl_en_falling_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h30 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[4] & 'hffffffff))

  `ASSERT(intr_ctrl_en_lvlhigh_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h34 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[5] & 'hffffffff))

  `ASSERT(intr_ctrl_en_lvllow_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h38 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[6] & 'hffffffff))

  `ASSERT(ctrl_en_input_filter_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h3c |->
         d2h.d_error ||
