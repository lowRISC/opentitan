...
`else
  `define REGWEN_PATH entropy_src.u_reg
`endif

// Block: entropy_src
module entropy_src_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  assign regwen[4] = 1;
  assign regwen[5] = 1;
  assign regwen[6] = `REGWEN_PATH.me_regwen_qs;
  assign regwen[7] = `REGWEN_PATH.regwen_qs;
  assign regwen[8] = `REGWEN_PATH.regwen_qs;
  assign regwen[9] = `REGWEN_PATH.regwen_qs;
  assign regwen[10] = `REGWEN_PATH.regwen_qs;
  assign regwen[11] = `REGWEN_PATH.regwen_qs;
  assign regwen[12] = 1;
  assign regwen[13] = 1;
  assign regwen[14] = `REGWEN_PATH.regwen_qs;
  assign regwen[15] = 1;
  assign regwen[16] = 1;
  ...
  fpv_reg_access_e access_policy [17];
  ...
  `ASSERT(intr_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         ...
         d2h.d_error ||
         (d2h.d_data & 'hf) == (exp_vals[0] & 'hf))

  `ASSERT(me_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h10 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[3] & 'h1))

  `ASSERT(sw_regupd_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h14 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[4] & 'h1))

  `ASSERT(module_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h20 |->
         d2h.d_error ||
         (d2h.d_data & 'hf) == (exp_vals[6] & 'hf))

  `ASSERT(conf_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h24 |->
         d2h.d_error ||
         (d2h.d_data & 'h3ffffff) == (exp_vals[7] & 'h3ffffff))

  `ASSERT(entropy_control_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h28 |->
         d2h.d_error ||
         (d2h.d_data & 'hff) == (exp_vals[8] & 'hff))

  `ASSERT(health_test_windows_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'h30 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[9] & 'hffffffff))

  `ASSERT(alert_threshold_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'ha0 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[10] & 'hffffffff))

  `ASSERT(fw_ov_control_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'hb0 |->
         d2h.d_error ||
         (d2h.d_data & 'hff) == (exp_vals[11] & 'hff))

  `ASSERT(fw_ov_sha3_start_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'hb4 |->
         d2h.d_error ||
         (d2h.d_data & 'hf) == (exp_vals[12] & 'hf))

  `ASSERT(observe_fifo_thresh_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'hc8 |->
         d2h.d_error ||
         (d2h.d_data & 'h3f) == (exp_vals[14] & 'h3f))

  `ASSERT(err_code_test_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 8'hdc |->
         d2h.d_error ||
         (d2h.d_data & 'h1f) == (exp_vals[15] & 'h1f))
