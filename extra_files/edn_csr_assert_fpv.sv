...
`else
  `define REGWEN_PATH edn.u_reg
`endif

// Block: edn
module edn_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  assign regwen[2] = 1;
  assign regwen[3] = 1;
  assign regwen[4] = `REGWEN_PATH.regwen_qs;
  assign regwen[5] = 1;
  assign regwen[6] = 1;
  assign regwen[7] = 1;
  assign regwen[8] = 1;
  ...
  fpv_reg_access_e access_policy [13];
  ...
  `ASSERT(intr_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         ...
         d2h.d_error ||
         (d2h.d_data & 'h3) == (exp_vals[0] & 'h3))

  `ASSERT(regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h10 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[3] & 'h1))

  `ASSERT(ctrl_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h14 |->
         d2h.d_error ||
         (d2h.d_data & 'hffff) == (exp_vals[4] & 'hffff))

  `ASSERT(boot_ins_cmd_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h18 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[5] & 'hffffffff))

  `ASSERT(boot_gen_cmd_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h1c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[6] & 'hffffffff))

  `ASSERT(max_num_reqs_between_reseeds_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h34 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[10] & 'hffffffff))

  `ASSERT(err_code_test_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h40 |->
         d2h.d_error ||
         (d2h.d_data & 'h1f) == (exp_vals[11] & 'h1f))
