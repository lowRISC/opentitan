...
`else
  `define REGWEN_PATH sram_ctrl.u_reg_regs
`endif

// Block: sram_ctrl
module sram_ctrl_regs_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  assign regwen[0] = 1;
  assign regwen[1] = 1;
  assign regwen[2] = `REGWEN_PATH.exec_regwen_qs;
  assign regwen[3] = 1;
  assign regwen[4] = `REGWEN_PATH.ctrl_regwen_qs;
  assign regwen[5] = 1;
  assign regwen[6] = `REGWEN_PATH.readback_regwen_qs;
  assign regwen[7] = 1;
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:32]})
           ) begin
  ...
  `ASSERT(exec_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h8 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[1] & 'h1))

  `ASSERT(exec_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'hc |->
         d2h.d_error ||
         (d2h.d_data & 'hf) == (exp_vals[2] & 'hf))

  `ASSERT(ctrl_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h10 |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[3] & 'h1))

  `ASSERT(readback_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 6'h1c |->
         d2h.d_error ||
         (d2h.d_data & 'h1) == (exp_vals[5] & 'h1))

  `ASSERT(readback_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
