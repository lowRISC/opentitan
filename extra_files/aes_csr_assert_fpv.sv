...
`else
  `define REGWEN_PATH aes.u_reg
`endif

// Block: aes
module aes_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
  ...
  fpv_reg_access_e access_policy [3];
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:132]})
           ) begin
  ...
  `ASSERT(ctrl_aux_regwen_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
